...
const Signin = () => {
  ...
  useEffect(() => {
    ...
    if (tokenFromUrl) {
      ...
      // handleCheckLogin(tokenFromUrl); // Automatically check login if token exists
    }
  }, []);

  const handleCheckLogin = async (tokenToCheck) => {
    console.log(tokenToCheck, 'tokenToCheck')
    try {
      console.log(AUTH.LOGINCHECKREDIRECTION);
      const response = await axios.post(AUTH.LOGINCHECKREDIRECTION, {
        token: tokenToCheck,
      });
      ...
      setResponse(response.data); // Set the response from the backend
      setError(""); // Clear any previous error
    } catch (err) {
      console.log(err, 'url Weeroror')
  ...
  const handleLogin = async (response, loginusershift, notificationSound) => {
    // console.log(9)
    localStorage.setItem("APIToken", response?.data?.token);
    localStorage.setItem("LoginUserId", response?.data?.result?._id);
    localStorage.setItem("LoginUserrole", response?.data?.result?.role);
    ...
    handleNotificationPreview(notificationSound)
    let ansswer = controlpanel?.data?.overallsettings;

    console.log(notificationSound, 'notificationSoundPreview')
    setButtonStyles({
      btncancel: {
        backgroundColor:
