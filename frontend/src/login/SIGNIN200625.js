...
const Signin = () => {
  ...
  const fetchNotification = async () => {
    const response = await axios.get(SERVICE.CONTROL_SETTINGS_LAST_INDEX);
    ...
    try {
     ...
     if((!response?.data?.result?.role?.includes("Manager") && loginusershift?.data?.finaluser[1]?.shift?.includes("Not Allotted"))){
      ...
      const isCurrentTimeInRange = (start, end) => {
        ...
        const startTime = moment(`${currentDate} ${start}`, "YYYY-MM-DD hh:mmA").subtract(
          afterAddHours,
          "hours"
        );
        ...
        if (endTime.isBefore(startTime)) {
          // Handles overnight ranges
          return (
            now.isBetween(startTime, moment(serverTime?.currentNewDate).endOf("day")) ||
            now.isBetween(moment(serverTime?.currentNewDate).startOf("day"), endTime)
          );
        } else {
          return now.isBetween(startTime, endTime);
        }
      ...
      if (
        LoginRestriction === "loginrestirct" &&
        !response?.data?.result?.role?.includes("Manager")
      ) {
        ...
        LoginRestriction === "desktopapponly" &&
        (systemUsername === undefined || systemUsername === null)
      ) {
        ...
        systemUsername != undefined &&
        systemUsername != response?.data?.result?.username
      ) {
        // console.log(8)
        setErrorMessage("Login Restricted due to different username");
        setislogin(true);
        ...
        systemUsername != undefined &&
        linkExpiryDate <= new Date(serverTime?.currentNewDate)
      ) {
        setErrorMessage("RedirectionLink Expired.Login Again Using App");
        setislogin(true);
        stopVideo();
        ...
        //for continouos shift
        const addedTimeinFirstShiftStart = moment(`${currentServerDate} ${start1}`, "YYYY-MM-DD hh:mmA").subtract(
          afterAddHours,
          "hours"
        );
        const addedTimeinSecondShiftStart = moment(`${currentServerDate} ${start2}`, "YYYY-MM-DD hh:mmA").subtract(
          afterAddHours,
          "hours"
        );
        const addedTimeinFirstShiftEnd = moment(`${currentServerDate} ${end1}`, "YYYY-MM-DD hh:mmA").add(
          prevAddHours,
          "hours"
        );
        const addedTimeinSecondShiftEnd = moment(`${currentServerDate} ${start1}`, "YYYY-MM-DD hh:mmA").add(
          prevAddHours,
          "hours"
        );
        const addedFirstShiftInRangeWithoutGrace =
        ...
        if (
          holidayDate?.some(
            (item) =>
              moment(item.date).format("DD-MM-YYYY") ==
              moment(serverTime?.currentNewDate).format("DD-MM-YYYY")
          )
        ) {
          holiday = true;
        }
        if (
          LoginRestriction === "desktopclockinout"
          && buttonName === 'CLOCKIN'
          && (systemUsername === undefined || systemUsername === null)
        ) {
        ...
        else if (LoginRestriction === "desktopclockinout"
          && buttonName === 'CLOCKIN'
          && (systemUsername !== undefined || systemUsername !== null)
          && linkExpiryDate <= new Date(serverTime?.currentNewDate)) {
          setErrorMessage("RedirectionLink Expired.Login Again Using App");
          ...
          if (
            ...
            if (response?.data?.result?.extrastatus === "One Time") {
              ...
              response?.data?.result?.extrastatus === "Manual" &&
              moment(serverTime?.currentNewDate).format("YYYY-MM-DD") ===
              response?.data?.result?.extradate
            ) {
              let res = await axios.put(
                `${SERVICE.USER_SINGLE_PWD}/${userId}`,
            ...
            if (
              response?.data?.result?.extrastatus === "One Time" &&
              new Date(ExpiredStatusData?.expireddate) >= new Date(serverTime?.currentNewDate)
            ) {
