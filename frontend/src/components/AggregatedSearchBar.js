import { Box, FormControl, IconButton, InputAdornment, OutlinedInput, Popover, Tooltip } from "@mui/material";
...
function AggregatedSearchBar({ columnDataTable, setItems, addSerialNumber, setPage, maindatas, setSearchedString, searchQuery,
    ...
    const applyAdvancedFilter = (filters, logicOperator) => {
        ...
        const filtered = totalDatas?.filter((item) => {
        ...
        <>
            ...
            >
                <Box sx={{ padding: '10px' }}>
                    <AdvancedSearchBar
                        setPage={setPage}
                        columns={columnDataTable?.filter(data => data.field !== 'checkbox' && data.field !== "actions")} onSearch={applyAdvancedFilter} initialSearchValue={searchQuery} handleCloseSearch={handleCloseSearch}
