...
function TrainingUserCompletedStatus({ com }) {
  ...
  let exportColumnNames = ['Training Status', 'Training Date', 'Training Time', 'Priority', 'Training Name', 'Training Details', 'Frequency', 'Schedule', 'Duration', 'Required'];
  let exportRowValues = ['taskstatus', 'taskassigneddate', 'tasktime', 'priority', 'trainingdetails', 'taskdetails', 'frequency', 'schedule', 'duration', 'required'];
    ...
    try {
      ...
      const answer =
        res_task?.data?.trainingforuser?.length > 0
          ? res_task?.data?.trainingforuser
              ?.filter((data) => data.username === isUserRoleAccess?.companyname && ['Completed', 'Finished By Others', 'Not Applicable to Me']?.includes(data?.taskstatus))
              ?.map((item, index) => ({
