...
function TrainingForUserCompleted({ com }) {
  ...
  const ListPageLoadDataCompleted = async () => {
    ...
    try {
      ...
      const final = res_task?.data?.trainingforuser?.length > 0 ? res_task?.data?.trainingforuser?.filter((data) => data.username === isUserRoleAccess?.companyname && ['Completed', 'Finished By Others', 'Not Applicable to Me']?.includes(data?.taskstatus)) : [];
      const answer =
        final?.length > 0
