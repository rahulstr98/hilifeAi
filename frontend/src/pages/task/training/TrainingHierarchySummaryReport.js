...
function TrainingHierarchySummaryReport({ com }) {
  ...
  let exportColumnNames = ['Company', 'Branch', 'Unit', 'Team', 'Department', 'User Name', 'Completed Training', 'Assigned Training', 'Pending Training', 'Paused Training', 'Postponed Training', 'Not Applicable Training', 'Finished By Others Training'];
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String('Training/Training Hierarchy Summary Reports'),
  ...
  let listpageaccessby = listPageAccessMode?.find((data) => data.modulename === 'Training' && data.submodulename === 'Training Hierarchy Summary Reports' && data.mainpagename === '' && data.subpagename === '' && data.subsubpagename === '')?.listpageaccessmode || 'Overall';
  ...
  const handleCaptureImagePopUp = () => {
    if (componentRefPopUp.current) {
      html2canvas(componentRefPopUp.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, 'TrainingUserPanelSummaryReports.png');
        });
      });
    }
  };
  // Excel
  const fileName = 'TrainingUserPanelSummaryReports';
  //print...
  const componentRefPopUp = useRef();
  const handleprintPopUp = useReactToPrint({
    content: () => componentRefPopUp.current,
    documentTitle: 'Training User Panel Summary Reports',
    pageStyle: 'print',
  });
  ...
  const getexcelDatas = () => {
    var data = AttendanceList?.[AttendanceList?.column]?.map((t, index) => ({
      ...
      'Training Date': t.taskassigneddate,
      'User Name': t.username,
      'Training Name': t.trainingdetails,
      'Training Details': t.taskdetails,
      Frequency: t.frequency,
      Schedule: t.schedule,
      'Training Status': t.taskstatus,
    }));
    setApplyData(data);
  };
  useEffect(() => {
  ...
  const downloadPdf = () => {
    ...
    const itemsWithSerial = AttendanceList?.[AttendanceList?.column]?.map((t, index) => ({
      ...
      username: t.username,
      trainingdetails: t.trainingdetails,
    ...
    doc.save('Training User Panel Summary Reports.pdf');
  };
  ...
  const columnDataTable = [
    ...
    },
    {
      ...
      headerName: 'Not Applicable Training',
      flex: 0,
      width: 150,
    ...
    },
    {
      ...
      headerName: 'Finished By Others Training',
      flex: 0,
      width: 150,
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage
        ...
        .then((blob) => {
          saveAs(blob, 'Users Training Hierarchy Summary Reports.png');
        })
        .catch((error) => {
  ...
  const ListPageLoadDataOnprogress = async () => {
    ...
    try {
      setQueueCheck(false);
      let res_task = await axios.post(SERVICE.ALL_TRAINING_HIERARCHY_SUMMARY_REPORTS, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        ...
        pagename: 'menutraininghierarchysummaryreports',
        listpageaccessmode: listpageaccessby,
        role: isUserRoleAccess?.role

      });
      console.log(res_task?.data, 'data');
  ...
  return (
    <Box>
      ...
      <PageHeading title="Training Hierarchy Summary Reports" modulename="Training" submodulename="Training Hierarchy Summary Reports" mainpagename="" subpagename="" subsubpagename="" />
      {isUserRoleCompare?.includes('ltraininghierarchysummaryreports') && (
        <>
        ...
        <>
          <Box sx={userStyle.selectcontainer}>
            ...
            <Grid container spacing={2} sx={{ justifyContent: 'center' }}>
              ...
              >
                {isUserRoleCompare?.includes('exceltraininghierarchysummaryreports') && (
                  <>
                    <Button
                      onClick={(e) => {
                ...
                {isUserRoleCompare?.includes('csvtraininghierarchysummaryreports') && (
                  <>
                    <Button
                      onClick={(e) => {
                        setIsFilterOpen(true);
                ...
                {isUserRoleCompare?.includes('printtraininghierarchysummaryreports') && (
                  <>
                ...
                {isUserRoleCompare?.includes('pdftraininghierarchysummaryreports') && (
                  <>
                    <Button
                      sx={userStyle.buttongrp}
                      onClick={() => {
                ...
                {isUserRoleCompare?.includes('imagetraininghierarchysummaryreports') && (
      ...
      <Dialog open={isAttendanceList} onClose={handleCloseAttendanceList} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="lg" fullWidth={true} sx={{ marginTop: '80px' }}>
        <Box sx={{ padding: '20px 50px' }}>
          <>
            <Typography sx={userStyle.HeaderText}>Training User Panel Summary</Typography>
            <br /> <br />
            <Grid container spacing={2}>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
              ...
              <Grid item md={12} xs={12} sm={12}>
                <Typography variant="h6">Training Summary</Typography>
                <br />
                  ...
                  <Table aria-label="customized table" id="usertable" ref={componentRefPopUp}>
                    <TableHead>
                      <StyledTableRow>
                        ...
                        <StyledTableCell style={tableHeadCellStyle}>{'Training Date'}</StyledTableCell>
                        <StyledTableCell style={tableHeadCellStyle}>{'User Name'}</StyledTableCell>
                        <StyledTableCell style={tableHeadCellStyle}>{'Training Name'}</StyledTableCell>
                        <StyledTableCell style={tableHeadCellStyle}>{'Training Details'}</StyledTableCell>
                        ...
                        <StyledTableCell style={tableHeadCellStyle}> {'Training Status'}</StyledTableCell>
                      </StyledTableRow>
                    </TableHead>
                    <TableBody>
                      {AttendanceList?.[AttendanceList?.column]?.map((item, i) => (
                        <StyledTableRow>
                          ...
                          <StyledTableCell style={tableBodyCellStyle}> {item.trainingdetails}</StyledTableCell>
      ...
      <ExportData
        ...
        filename={'Users Training Hierarchy Summary Reports'}
...
export default TrainingHierarchySummaryReport;
