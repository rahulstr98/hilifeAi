...
function TaskUserPanelView() {
    ...
    const handleOnCloseTicket = async () => {
        ...
        try {
            ...
            else if (singleDoc?.schedule === "Any Time" && (new Date() < endTimeCompleted)) {
                setPopupContentMalert(`Should Match the duration to complete the task - ${addHours > 0 ? `${addHours} Hours and ` : ""}${addMinutes > 0 ? `${addMinutes} mins` : ""}`);
                setPopupSeverityMalert("info");
                handleClickOpenPopupMalert();
                handleCloseDelete();
            }
            else if (singleDoc?.schedule === "Fixed" && (new Date() < todoDate)) {
                setPopupContentMalert(`Complete the task after the Scheduled Time ${singleDoc?.timetodo[0]?.hour}:${singleDoc?.timetodo[0]?.min} ${singleDoc?.timetodo[0]?.timetype}`);
                setPopupSeverityMalert("info");
                handleClickOpenPopupMalert();
                handleCloseDelete();
            }
                                                                            ...
                                                                            return (
                                                                                <StyledTableRow id={indexs}>
                                                                                    ...
                                                                                    <StyledTableCell>{
                                                                                        ...
                                                                                        />}</StyledTableCell>
                                                                                    <StyledTableCell>{
                                                                                        (item?.reason && !item?.status) && <Button sx={buttonStyles.buttonsubmit} variant="contained" onClick={() => handleUpdatesubmitTable(indexs, item?.files)}>Add</Button>}</StyledTableCell>
