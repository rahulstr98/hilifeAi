...
function UsersTaskAllocation() {
  ...
  let exportColumnNames = ['Task Details', 'Priority', 'Frequency', 'Schedule', 'Task', 'Sub Task', 'Duration', 'Required', 'Break Up'];
  let exportRowValues = ['taskdetails', 'priority', 'frequency', 'schedule', 'category', 'subcategory', 'duration', 'required', 'breakup'];
  ...
  const [dateManual, setDateManual] = useState(serverTime);
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (holidayDate?.some((item) => moment(item.date).format('DD-MM-YYYY') == moment(serverTime).format('DD-MM-YYYY'))) {
  ...
  const fetchAllRaisedTickets = async (date) => {
    ...
    try {
      if (date === '') {
        ...
        let uniqueElements = updatedAns1?.filter(
          (obj1) =>
            !userCheck?.data?.taskforuser?.some(
              (obj2) =>
                obj1.category === obj2.category && obj1.subcategory === obj2.subcategory && obj1.frequency === obj2.frequency && obj1.schedule === obj2.schedule && obj2.username === isUserRoleAccess.companyname && new Date(date) <= new Date(obj2.shiftEndTime) && obj2?.taskdetails === 'schedule'
            )
        );
        ...
        const prioritySchedule =
          priorityCheck?.length > 0
            ? priorityCheck?.map((data) => {
                return { ...data, taskdetails: 'schedule' };
              })
            : [];

        const prioritynonschedule =
          answerUserNonSchedule?.length > 0
            ? answerUserNonSchedule?.map((data) => {
                return { ...data, taskdetails: 'nonschedule' };
              })
            : [];

        const priorityanswerUserNonSchedule =
          nonschedule?.length > 0
            ? nonschedule
                ?.filter((obj1) => !prioritynonschedule?.some((obj2) => obj1.category === obj2.category && obj1.subcategory === obj2.subcategory && obj1.schedule === obj2.schedule && obj1.date === obj2.taskdate && obj2.username === isUserRoleAccess?.companyname))
                .map((data) => {
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="User Task Panel Allocations" modulename="Task" submodulename="Tasks Allocation" mainpagename="" subpagename="" subsubpagename="" />
