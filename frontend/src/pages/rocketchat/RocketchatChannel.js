...
function RocketchatChannel() {
  ...
  let exportColumnNames = ["Channel Name", "Team Name", "Type", "CreatedAt", "CreatedBy", "UpdatedAt", "Users Count", "Messages Count"];
  let exportRowValues = [
    "name",
    "teamName",
    "type",
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Create Connections Channel"),
  ...
  const deleteTeam = async () => {
    ...
    try {
      await axios.delete(
        `${SERVICE.DELETE_ROCKETCHAT_CHANNEL}/?channelid=${deleteTeamId?.id}&type=${deleteTeamId?.type}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const sendRequest = async () => {
    ...
    try {
      let createTeam = await axios.post(
        SERVICE.CREATE_ROCKETCHAT_CHANNEL,
        {
          name: String(assetVariant.name?.trim()),
          type: assetVariant.type,
          teamid: assetVariant.teamid
        },
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
  ...
  const sendEditRequest = async () => {
    ...
    try {
      let res = await axios.put(
        `${SERVICE.UPDATE_ROCKETCHAT_CHANNEL}`,
        {
          name: String(assetVariantEdit.name),
          type: assetVariantEdit.type,
  ...
  const fetchRockeChatChannels = async () => {
    ...
    try {
      ...
      setTeamsArray(response?.data?.rocketchatChannels.sort((a, b) => new Date(a.ts) - new Date(b.ts)));
      setLoader(false);
    } catch (err) {
      setLoader(false);
      console.log(err);
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Connections Channel.png");
        });
      });
    }
  };
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "Connections Channels",
    pageStyle: "print",
  });
  ...
  const addSerialNumber = () => {
    const itemsWithSerialNumber = teamsArray?.map((data, index) => ({
      ...
      name: data?.name,
      type: data?.type,
      createdby: data?.u?.name,
      createdat: moment(data?.ts).format("DD-MM-YYYY hh:mm A"),
      updatedat: moment(data?._updatedAt).format("DD-MM-YYYY hh:mm A"),
      teamId: data?.teamId,
  ...
  const columnDataTable = [
...

    {
      ...
      flex: 0,
      width: 100,
      hide: !columnVisibility.userscount,
      headerClassName: "bold-header",
    },
    {
      field: "msgs",
  ...
  return (
    <Box>
      <Headtitle title={"CREATE CONNECTIONS CHANNEL"} />
      {/* ****** Header Content ****** */}
      <PageHeading
        title="Connections Channel"
        modulename="Connections"
        submodulename="Create Connections Channel"
        mainpagename=""
        subpagename=""
        subsubpagename=""
      />
            ...
            <>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    <Typography>
                      Teams <b style={{ color: "red" }}>*</b>
                    </Typography>
                    <Selects
                      maxMenuHeight={300}
                      options={teamsOptions}
                      value={{
                        label: assetVariant.team || "Please Select Team",
                        value: assetVariant.team || "Please Select Team",
                      }}
                      onChange={(e) => {
                        setAssetVariant({
                          ...assetVariant,
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>{singleRow.teamName}</Typography>
                </FormControl>
              </Grid>
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
              ...
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>{singleRow.userscount}</Typography>
                </FormControl>
              </Grid>
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  <Typography variant="h6">Messages Count</Typography>
                  <Typography>{singleRow.msgs}</Typography>
                </FormControl>
              </Grid>
            </Grid>
            <br /> <br /> <br />
...
export default RocketchatChannel;