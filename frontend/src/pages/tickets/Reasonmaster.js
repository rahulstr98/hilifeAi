...
function Reasonmaster() {
  ...
  let exportRowValues = ['categoryreason', 'subcategoryreason', 'subsubcategoryreason', 'typereason', 'namereason'];
  ...
  const getOverallEditSectionUpdate = async () => {
    ...
    try {
      ...
      sendEditRequestOverall(res?.data?.selfcheck, res?.data?.priority, res?.data?.duedate, res?.data?.check, res?.data?.raiseticket);
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  ...
  const sendEditRequest = async () => {
    setPageName(!pageName);
     const NewDatetime = await getCurrentServerTime();
    try {
      let res = await axios.put(`${SERVICE.REASONMASTER_SINGLE}/${subprojectsid}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  return (
    <Box>
      ...
      <PageHeading title="Reason Master" modulename="Tickets" submodulename="Master" mainpagename="Reason Master" subpagename="" subsubpagename="" />
          ...
          <Box sx={userStyle.dialogbox}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={8}>
                  <Typography sx={userStyle.importheadtext}>Add Reason Master</Typography>
                </Grid>
              </Grid>
              <br />
              <Grid container spacing={2}>
              ...
              <Grid container spacing={2}>
                <Grid item md={2.5} xs={12} sm={6}>
                  <LoadingButton loading={isBtn} variant="contained" color="primary" onClick={handleSubmit} sx={buttonStyles.buttonsubmit}>
                    Submit
                  </LoadingButton>
                </Grid>
                <Grid item md={2.5} xs={12} sm={6}>
              ...
              <form onSubmit={editSubmit}>
                ...
                <Grid container spacing={2}>
                  <Grid item md={12} xs={12} sm={12}>
                    <Typography sx={userStyle.HeaderText}>Edit Reason Master</Typography>
                  </Grid>
                </Grid>
                <br />
                <Grid container spacing={2}>
      ...
      {isUserRoleCompare?.includes('lreasonmaster') && (
        <>
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>Reason Master List</Typography>
            </Grid>
            <br />
      ...
      <Dialog open={openview} onClose={handleClickOpenview} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="md">
        <Box sx={{ padding: '20px 50px' }}>
          <>
            <Typography sx={userStyle.HeaderText}> View Reason Master</Typography>
            <br /> <br />
            <Grid container spacing={2}>
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
          ...
          <DialogActions>
            {selectedRowsCount > 0 ? (
              <>
                ...
                <Button variant="contained" color="error" onClick={(e) => delReasoncheckbox(e)} sx={buttonStyles.buttonsubmit}>
                  {' '}
                  OK{' '}
                </Button>
              </>
      ...
      />
      {/* INFO */}
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Reason Master Info" addedby={addedby} updateby={updateby} />
    </Box>
  );
}
