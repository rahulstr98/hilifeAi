...
function Checkpointticket() {
  ...
  let exportColumnNames = ['Category', 'Subcategory', 'Sub Subcategory', 'Type', 'Reason', 'Checkpoint Group'];
  ...
  const sendRequest = async () => {
    ...
    try {
      valueReason.forEach((data, index) => {
        axios.post(SERVICE.CHECKPOINTTICKET_CREATE, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
          category: [...valueCat],
  ...
  const handleSubmit = (e) => {
    ...
    const isNameMatch = checkpointticketmastersList?.some(
      (item) =>
        ...
        item.type.toLowerCase() === String(checkpointticketmaster?.type.toLowerCase()) &&
  ...
  const getCode = async (e, name) => {
    ...
    try {
      ...
      setSubCategoryValueCateEdit(res?.data?.scheckpointticketmaster?.subcategory);
      setSelectedSubCategoryOptionsCateEdit([
      ...
      setSubSubCategoryValueCateEdit(res?.data?.scheckpointticketmaster?.subsubcategory);
      setSelectedSubSubCategoryOptionsCateEdit([
  ...
  const sendEditRequest = async () => {
    setPageName(!pageName);
     const NewDatetime = await getCurrentServerTime();
    try {
      let res = await axios.put(`${SERVICE.CHECKPOINTTICKET_SINGLE}/${subprojectsid}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  const fetchCheckpointticketmaster = async () => {
    ...
    try {
      ...
      const answer =
        res_vendor?.data?.checkpointticketmasters?.length > 0
          ? res_vendor?.data?.checkpointticketmasters?.map((item, index) => ({
  ...
  const fetchCheckpointticketAll = async () => {
    ...
    try {
      ...
      return res_check?.data?.checkpointticketmasters.filter((item) => item._id !== checkpointticketmasterEdit._id);
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Checkpoint Ticket Master" modulename="Tickets" submodulename="Master" mainpagename="Checkpoint Ticket Master" subpagename="" subsubpagename="" />
        ...
        <Box sx={userStyle.dialogbox}>
          <>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                <Typography sx={userStyle.importheadtext}>Add Checkpoint Ticket Master</Typography>
              </Grid>
            </Grid>
            <br />
            <Grid container spacing={2}>
                ...
                <>
                  ...
                  <Grid container spacing={1}>
                    ...
                    <Grid item md={2.5} xs={12} sm={6}>
                      <Button variant="contained" color="primary" onClick={handleSubmit} disabled={btnSubmit} sx={buttonStyles.buttonsubmit}>
                        Submit
                      </Button>{' '}
                      &nbsp;
      ...
      <Box>
        {/* Edit DIALOG */}
        <Dialog open={isEditOpen} onClose={handleCloseModEdit} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" fullWidth={true} maxWidth="md" sx={{ marginTop: '40px' }}>
          <Box sx={{ padding: '20px' }}>
            <>
              <form onSubmit={editSubmit}>
                <Grid container spacing={2}>
                  <Grid item md={12} xs={12} sm={12}>
                    <Typography sx={userStyle.HeaderText}>Edit Checkpoint Ticket Master</Typography>
                  </Grid>
                </Grid>
                <br />
                <Grid container spacing={2}>
      ...
      {isUserRoleCompare?.includes('lcheckpointticketmaster') && (
        <>
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>CheckPoint Ticket Master List</Typography>
            </Grid>
            <br />
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              >
                <Box>
                  {isUserRoleCompare?.includes('excelcheckpointticketmaster') && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes('printcheckpointticketmaster') && (
                    <>
                  ...
                  {isUserRoleCompare?.includes('imagecheckpointticketmaster') && (
      ...
      <Box>
        ...
        <Dialog open={isDeleteOpen} onClose={handleCloseMod} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
          ...
          <DialogActions>
            ...
            <Button autoFocus variant="contained" color="error" onClick={(e) => delCheckpointticket(Checkpointticketsid)}>
              {' '}
              OK{' '}
            </Button>
          </DialogActions>
      ...
      <Dialog open={openview} onClose={handleClickOpenview} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="lg">
        <Box sx={{ width: '750px', padding: '20px 50px' }}>
          <>
            <Typography sx={userStyle.HeaderText}> View Checkpoint Ticket Master</Typography>
            <br /> <br />
            <Grid container spacing={2}>
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
      ...
      <Box>
        <Dialog open={isDeleteOpencheckbox} onClose={handleCloseModcheckbox} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
          ...
          <DialogActions>
            ...
            <Button autoFocus variant="contained" color="error" onClick={(e) => delCheckpointticketcheckbox(e)}>
              {' '}
              OK{' '}
            </Button>
          </DialogActions>
      ...
      />
      {/* INFO */}
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Checkpoint Master Info" addedby={addedby} updateby={updateby} />
    </Box>
  );
}
