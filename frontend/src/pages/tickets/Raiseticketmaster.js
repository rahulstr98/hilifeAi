...
function RaiseticketMaster() {
  ...
  const [serverTime, setServerTime] = useState(moment().format('YYYY-MM-DD'));
  const [serverTimeTwoDays, setServerTimeTwoDays] = useState(moment().subtract(3, 'days').format('YYYY-MM-DD'));
  useEffect(() => {
  ...
  const fetchworkStationNames = async () => {
    ...
    try {
      ...
      let res_queue = await axios.post(SERVICE.WORKSTATION_SHORTNAME_TICKET, {
        ...
        allnames: selectedOptionsEmployees?.map(data => data?.username)
      });
  ...
  const fetchResolverDetails = async (e, access) => {
    ...
    try {
      ...
      let CatgeorySubCatgeoryOnly = res_due?.data?.teamgroupings?.filter((data) => selectedOptionsEmployeesValues?.some((item) => data?.employeenamefrom.includes(item)) && data?.categoryfrom.includes(raiseTicketMaster.category) && data?.subcategoryfrom.includes(e));
      let CatgeorySubSubCatgeoryOnly = res_due?.data?.teamgroupings?.filter(
        (data) => selectedOptionsEmployeesValues?.some((item) => data?.employeenamefrom.includes(item)) && data?.categoryfrom.includes(raiseTicketMaster.category) && data?.subcategoryfrom.includes(raiseTicketMaster.subcategory) && data.subsubcategoryfrom.includes(raiseTicketMaster.subsubcategory)
      );
        ...
        <>
          <Box sx={userStyle.selectcontainer}>
            ...
            <>
                      ...
                      <FormControl fullWidth size="small">
                        ...
                        {/* <Selects
                          ...
                          formatOptionLabel={(data, { context }) =>
                            context === 'menu' ? (
                              <div>
                                {data.labelParts?.main}{' '}
                                <span style={{ color: 'green' }}>{data.labelParts?.highlight}</span>
                              </div>
                            ) : data.value
                          }
                          getOptionLabel={(data) => data.value}
                          value={
                            workStationOptions.find(
                              (opt) => opt.value === raiseTicketMaster.workstation
                            ) || null
                          }
                          onChange={(e) => {
                            setRaiseTicketMaster({
              ...
              <Grid container spacing={2}>
                ...
                <Grid item md={12} sm={12} xs={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <ReactQuillAdvanced agenda={textSumm}
                      setAgenda={setTextSummary}
                      disabled={false}
              ...
              <br />
              {isUserRoleCompare.includes('lassignraiseticketuser') && raiseTicketMaster.access === 'Manual' ? (
                <>
                  <Grid container spacing={2}>
