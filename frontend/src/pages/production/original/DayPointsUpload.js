...
function DayPointsUpload() {
  ...
  let exportRowValuesViewAll = ['name', 'empcode', 'company', 'branch', 'unit', 'unit', 'dateval', 'user', 'unitid', 'filename', 'category', 'exper', 'target', 'points', 'avg', 'processcode', 'aprocess', 'sprocess', 'contarget', 'conpoints', 'conavg'];
  ...
  const addSerialNumberviewAll = () => {
    ...
    const itemsWithSerialNumber = productionoriginalviewAll?.map((item, index) => {
      ...
      return {
        ...
        production: Number(Number(item.production).toFixed(2)),
        manual: Number(Number(item.manual).toFixed(2)),
        ...
        allowancepoint: item.type === 'Created' ? (item.shiftsts === 'Enable' ? Number(Number(item.conshiftpoints).toFixed(2)) : 0) : Number(item.allowancepoint),
        nonallowancepoint: diff < 1e-1 ? 0 : Number(diff) > 0 ? Number(Number(diff).toFixed(2)) : 0,
      };
    });
