...
function ProductionUnmatchUnitList() {
  ...
  let exportColumnNamesFive = ["Date", "Project", "Vendor", "Category", "Sub Category", "Indentifier Name", "Flag Count", "Section", "Count"];
  let exportRowValuesFive = ["date", "project", "vendor1", "category1", "subcategory", "unitid", "flag", "section", "count"];

  let exportColumnNamesAllList = ["Date", "Project", "Vendor", "Category", "Sub Category", "Flag Count", "Unit", "Count"];
  let exportRowValuesAllList = ["date", "project", "vendor", "category", "subcategory", "flag", "unit", "count"];

  let exportColumnNamesUpdatedList = ["Login ID", "Date", "Vendor", "Category", "Sub Category", "Identity Name", "UnitRate", "Flag Count", "Section", "A UnitRate", "A Flag Count", "A Section"];
  let exportRowValuesUpdatedList = ["user", "dateval", "vendor", "filename", "category", "unitid", "unitrate", "flag", "section", "updatedunitrate", "updatedflag", "updatedsection"];
              ...
              if (validItems.length > 0) {
                updateDataInBulk(validItems)
                  .then(() => {
                    ...
                    setPopupContent(validItems.length === filteredselectedTwoIds.length ? "Alloted" : "Present unitrate value only updated");
          ...
          if (filteredselectedThreeIds.filter((item) => item.mismatchmode.includes("Unit + Flag")).length > 0) {
            if (filteredselectedThreeIds.some((item) => item.mrate != undefined && item.mrate != 0 && item.unitflag != undefined && item.unitflag != 0)) {
              ...
              if (validItems.length > 0) {
                updateDataInBulk(validItems)
                  .then(() => {
                    setPopupContent(validItems.length === filteredselectedThreeIds.length ? "Alloted" : "Present unitrate value only updated");
                    setPopupSeverity("success");
                    handleClickOpenPopup();
          ...
          if (filteredselectedFourIds.filter((item) => item.mismatchmode.includes("Unit + Flag")).length > 0) {
            if (filteredselectedFourIds.some((item) => item.mrate != undefined && item.mrate != 0 && item.unitflag != undefined && item.unitflag != 0)) {
              ...
              if (validItems.length > 0) {
                updateDataInBulk(validItems)
                  .then(() => {
                    setPopupContent(validItems.length === filteredselectedFourIds.length ? "Alloted" : "Present unitrate value only updated");
                    setPopupSeverity("success");
                    handleClickOpenPopup();
          ...
          if (filteredselectedFiveIds.filter((item) => item.mismatchmode.includes("Unit + Flag")).length > 0) {
            if (filteredselectedFiveIds.some((item) => item.mrate != undefined && item.mrate != 0 && item.unitflag != undefined && item.unitflag != 0)) {
              ...
              if (validItems.length > 0) {
                updateDataInBulk(validItems)
                  .then(() => {
                    setPopupContent(validItems.length === filteredselectedFiveIds.length ? "Alloted" : "Present unitrate value only updated");
                    setPopupSeverity("success");
                    handleClickOpenPopup();
                    setSelectAllCheckedFive(false);
                    setSelectedRowsFive([]);
  ...
  const bulkUndoUpdatedList = async () => {
    ...
    try {
      let update = await axios.post(`${SERVICE.BULK_UPDATE_UNDO_FIELDNAME}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
            ...
            if (buttonSelectValuesOnly.length > 0) {
              updateDataInBulk(buttonSelectValuesOnly)
                .then(() => {
                  ...
                  // setPopupSeverityMalert("info");
                  // handleClickOpenPopupMalert();
                  setPopupContent(validItems.length === ids.length ? "Alloted" : "Present unitrate value only updated");
                  setPopupSeverity("success");
                  handleClickOpenPopup();
                })
                .catch((err) => {
        ...
        if (ids.some((item) => item.mrate != undefined && item.mrate != 0)) {
          ...
          if (validItems.length > 0) {
            ...
            if (buttonSelectValuesOnly.length > 0) {
              updateDataInBulk(buttonSelectValuesOnly)
                .then(() => {
                  setPopupContent(validItems.length === ids.length ? "Unit Updated" : "Present unitrate value only updated");
                  setPopupSeverity("success");
                  handleClickOpenPopup();
                  setIsUnitone(false);
                })
        ...
        if (ids.some((item) => item.unitflag != undefined && item.unitflag != 0)) {
          ...
          if (validItems.length > 0) {
            ...
            if (buttonSelectValuesOnly.filter((d) => d.mismatchmode.includes("Flag")).length > 0) {
              updateDataInBulk(buttonSelectValuesOnly)
                .then(() => {
                  setPopupContent(validItems.length === ids.length ? "Flag Updated" : "Present unitrate value only updated");
                  setPopupSeverity("success");
                  handleClickOpenPopup();
                  setIsFlagtwo(false);
                })
  ...
  return (
    <Box>
      ...
      <PageHeading title=" Production Unmatch Unit" modulename="Production" submodulename="Upload" mainpagename="Original" subpagename="Original Mismatch" subsubpagename="" />
                        ...
                        <>
                          <Box sx={userStyle.container}>
                            ...
                            <Grid container spacing={1}>
                              ...
                              <Grid item md={2} xs={2} sm={2}>
                                <LoadingButton onClick={(e) => handleBulkUpdateFour()} loading={isBulkUpdateFour} color="primary" loadingPosition="end" variant="contained">
                                  Bulk Update
                                </LoadingButton>
                              </Grid>
                            </Grid>
