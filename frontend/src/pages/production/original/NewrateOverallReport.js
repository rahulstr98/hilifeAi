...
function ZeroUnitNewRateOverallReport() {
...
'Flag Count',
    ...
    let exportRowValues =
        [ 'formatteddate','vendor', 'vendornew',
             'filenameupdated', 
             'category',
            'matchcount',
            'orate',
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, 'New Rate Overall Report.png');
                })
                .catch((error) => {
    ...
    const fetchBatchFilter = async () => {
        ...
        try {
            let res_employee = await axios.post(SERVICE.PRODUCTION_UPLOAD_NEWRATE_OVERALL_REPORT,{
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
            ...
            const itemsWithSerialNumber = res_employee.data.prodresult?.map((item, index) => {
                ...
                return {
                    ...
                    oldtotal:parseFloat(Math.round(item.oldtotal * 100000) / 100000),
                    newtotal:parseFloat(Math.round(item.newtotal * 100000) / 100000),
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "Zero Unit NewRate Overall Report",
        pageStyle: "print",
    });
    ...
    const columnDataTable =
        [
            ...
            //     headerClassName: "bold-header",
            // },
            { field: "vendor", headerName: "Project", flex: 0, width: 140, hide: !columnVisibility.vendor, headerClassName: "bold-header",wrapHeaderText: true, autoHeaderHeight: true,  },
            { field: "vendornew", headerName: "Vendor", flex: 0, width: 95, hide: !columnVisibility.vendornew, headerClassName: "bold-header",wrapHeaderText: true, autoHeaderHeight: true,  },
            { field: "filenameupdated", headerName: "Category", flex: 0, width: 300, hide: !columnVisibility.filenameupdated, headerClassName: "bold-header",wrapHeaderText: true, autoHeaderHeight: true,  },
            { field: "category", headerName: "SubCategory", flex: 0, width: 400, hide: !columnVisibility.category, headerClassName: "bold-header",wrapHeaderText: true, autoHeaderHeight: true,  },
            { field: "matchcount", headerName: "Count", flex: 0, width: 100, hide: !columnVisibility.matchcount, headerClassName: "bold-header",wrapHeaderText: true, autoHeaderHeight: true,  },
            { field: "orate", headerName: "Orate", flex: 0, width: 100, hide: !columnVisibility.orate, headerClassName: "bold-header" ,wrapHeaderText: true, autoHeaderHeight: true, },
            { field: "flagcount", headerName: "Flag Count", flex: 0, width: 100, hide: !columnVisibility.flagcount, headerClassName: "bold-header",wrapHeaderText: true, autoHeaderHeight: true,  },
            { field: "oldtotal", headerName: "Old Total", flex: 0, width: 100, hide: !columnVisibility.oldtotal, headerClassName: "bold-header",wrapHeaderText: true, autoHeaderHeight: true,  },
            { field: "newrate", headerName: "New Rate", flex: 0, width: 100, hide: !columnVisibility.newrate, headerClassName: "bold-header",wrapHeaderText: true, autoHeaderHeight: true,  },
            { field: "newtotal", headerName: "New Total", flex: 0, width: 100, hide: !columnVisibility.newtotal, headerClassName: "bold-header" ,wrapHeaderText: true, autoHeaderHeight: true, },
    ...
    const fetchVendorsDropDown = async (e) => {
        ...
        try {
            ...
            const projFilt = res_vendor?.data?.vendormaster?.filter((item) => e.includes(item.projectname))
            setVendormasterOpt(
                projFilt?.map((t) => ({
                    ...t,
                    label: t.name,
    ...
    const fetchAllSubCategory = async (project, category) => {
        try {
            ...
            let res_module = await axios.post(SERVICE.SUBCATEGORYPROD_LIMITED_UNALLOT, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            ...
            let resall = [...new Set(res_module?.data?.subcategoryprod.map((item) => item.name))].map((d) => ({
              ...d,
              label: d,
              value: d,
            }));
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Zero Unit NewRate Overall Report"),
    ...
    return (
        <Box>
            <Headtitle title={"Zero Unit NewRate Overall Report"} />
            {/* ****** Header Content ****** */}
            <PageHeading
                title="Zero Unit NewRate Overall Report"
                modulename="Production"
                submodulename="SetUp"
                mainpagename="Zero Unit NewRate Overall Report"
                subpagename=""
                subsubpagename=""
            />
            <>
            <Box sx={userStyle.selectcontainer}>
                <Grid item xs={8}>
                    <Typography sx={userStyle.importheadtext}>Zero Unit NewRate Overall Report</Typography>
                </Grid>
                <>
            ...
            <br />
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("lzerounitnewrateoverallreport") && (
                <>
                    <Box sx={userStyle.container}>
                        {/* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>Zero Unit NewRate Overall Report List</Typography>
                        </Grid>
                        <Grid container spacing={2} style={userStyle.dataTablestyle}>
                            ...
                            <Grid item md={8} xs={12} sm={12} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                                <Box>
                                    {isUserRoleCompare?.includes("excelzerounitnewrateoverallreport") && (
                                        <>
                                            <Button onClick={(e) => {
                                    ...
                                    {isUserRoleCompare?.includes("csvzerounitnewrateoverallreport") && (
                                        <>
                                            <Button onClick={(e) => {
                                                setIsFilterOpen(true)
                                                setFormat("csv")
                                    ...
                                    {isUserRoleCompare?.includes("printzerounitnewrateoverallreport") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("pdfzerounitnewrateoverallreport") && (
                                        <>
                                            <Button
                                    ...
                                    {isUserRoleCompare?.includes("imagezerounitnewrateoverallreport") && (
                                        <>
                        ...
                        {loaderList ? (
                            ...
                            <>
                                <AggridTable
                                    ...
                                    pagenamecheck={"Zero Unit NewRate Overall Report"}
                                    paginated={false}
            ...
            <ExportData
                ...
                itemsTwo={projmaster ?? []}
                filename={"Zero Unit NewRate Overall Report"}
...
export default ZeroUnitNewRateOverallReport;