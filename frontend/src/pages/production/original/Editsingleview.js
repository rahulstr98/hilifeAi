...
function EditSingleViewList() {
  ...
  const getviewCodealledit = async () => {
    try {
      let res = await axios.get(`${SERVICE.PRODUCTION_UPLOAD_SINGLE}/${id}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  const cancelvalues = (e) => {
    backPage("/production/productionupload")
  }
  ...
  return (
    <Box>
      <Headtitle title={"Production Details"} />
      {/* ****** Header Content ****** */}
      {isUserRoleCompare?.includes("eproductionoriginalupload") && (
        <>
          <Typography sx={userStyle.HeaderText}>Production Details</Typography>
          <Box sx={userStyle.dialogbox}>
            <Typography sx={userStyle.SubHeaderText}>Edit Production Details</Typography>
            <br />
            <br />
