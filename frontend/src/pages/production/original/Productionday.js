...
function ProductionDay() {
  ...
  // let exportColumnNames = ['Date', 'Company Name', 'Created Date', 'Day Point Status'];
  // let exportRowValues = ['date', 'companyname', 'createddate', 'status'];
  // //Rowdatatable view
  // let exportColumnNamesView = ['Mode', 'Company', 'Branch', 'Unit', 'Team', 'Name', 'Emp Code', 'Department', 'Date', 'Project-Vendor', 'User', 'Category', 'Process Code', 'Exp', 'Shift', 'Target', 'Points', 'Avg Point', 'P Process', 'S Process', 'Con Tar', 'Con Points', 'Con Avg'];
  // let exportRowValuesView = ['mode', 'company', 'branch', 'unit', 'team', 'empname', 'empcode', 'department', 'date', 'vendor', 'user', 'filename', 'processcode', 'experience', 'weekoff', 'target', 'points', 'avgpoint', 'aprocess', 'sprocess', 'contarget', 'conpoints', 'conavg'];

  // //Rowdatatable Category view
  // let exportColumnNamesCategoryView = ['Mode', 'Company', 'Branch', 'Unit', 'Team', 'Name','Exp','Emp Code', 'Department', 'Date', 'Project-Vendor', 'User', 'Category', 'Process Code',  'Target', 'Points', 'Avg Point', 'P Process', 'S Process', 'Con Tar', 'Con Points', 'Con Avg'];
  // let exportRowValuesCategoryView = ['mode', 'company', 'branch', 'unit', 'team', 'empname', 'experience', 'empcode', 'department', 'dateval', 'vendor', 'user', 'filename', 'processcode', 'target', 'points', 'avgpoint', 'aprocess', 'sprocess', 'contarget', 'conpoints', 'conavg'];
  ...
  const handleClickOpenDelBeforeDayPoint = (date, filestatusupload) => {
    setBeforeDaypointDel({ open: true, date: date, filestatusupload });
  };
  ...
  const handleClickOpenProdDayCreate = (date, filestatusupload) => {
    setbeforeProdDayCreate({ open: true, date: date, filestatusupload });
  };
  ...
  const handleClickOpenProdDayRerun = (date, filestatusupload) => {
    setbeforeProdDayRerun({ open: true, date: date, filestatusupload });
  };
    ...
    try {
      ...
      if (res_Day.statusText === "OK") {
        if (res_Day.data.countDay > 0) {
          handleClickOpenProdDayRerun(date, filestatusCheckFileupload);
        } else {
          handleClickOpenProdDayCreate(date, filestatusCheckFileupload);
        }
      }
  ...
  const handleCurrDayPointDel = async (date, type, uniqueid, beforeProdDay, beforeDayPoint, filestatus, currDaypoint, filestatusCheckFileupload) => {
    try {
      ...
      handleClickOpenPopup();
      await fetchProductionDay();
      await handleRerunRow(date, type, uniqueid, beforeProdDay, beforeDayPoint, filestatus, currDaypoint, filestatusCheckFileupload);
    } catch (err) {
      handleCurrentDaypointDelPopClose();
  ...
  const fetchProductionDayLastData = async (selcDate) => {
    ...
    try {
      if (selcDate != "") {
        ...
        //  setCheckFileStatusSelectedDate(res_FileStatus.data.productionupload);
  ...
  const handleRerun = async (selcDate, uniqueid, id) => {
    ...
    try {
      ...
      if (isPayrunGenerated) {
        ...
        let filestatus = res_FileStatus.data.productionupload;
        let filestatusCheckFileupload = res_FileStatus.data.productiontempcheckfileupload;
  ...
  const handleRerunRow = async (selcDate, type, uniqueid, beforeProdDay, beforeDayPoint, filestatus, currDaypoint, filestatusupload) => {
    setRerunLoad(id);
    handleCurrentDaypointDelPopClose();
    setDeldayPointLoadPop(true);
    let alertval = "";
            ...
            try {
              if (allData === -1) {
                ...
                if (datavalue.length > 0) {
                  ...
                  let filestatusfinal = filestatusupload === 0 ? "Pending Upload" : "Final";
  ...
  const handleFilterSaveRerun = async (selcDate, type, filestatusupload) => {
    ...
    try {
      ...
      if (selcDate === "") {
        ...
        try {
          // Fetch all necessary data
          let [CHECK_ZERO, ResChevkCount] = await Promise.all([
            axios.post(SERVICE.CHECK_ZERO_MISMATCH_PRESENT, {
              headers: {
                Authorization: `Bearer ${auth.APIToken}`,
              },
  ...
  const handleFilterSave = async (selcDate, type, beforeday, beforedaypoint, filestatus, filestatusupload) => {
    try {
      let alertval = "";
      handleProgressUpdate(alertval, "Checking...");
      handleLoaderDialogOpen();
                  ...
                  if (allData === -1) {
                    ...
                    if (datavalue.length > 0) {
                      ...
                      if (finalRemovedUserData.length > 0) {
                        ...
                        handleProgressUpdate(9, "Creating...");
                        let resCreate;
                        console.log(filestatusupload, "filestatusupload");
                        let filestatusfinal = filestatusupload == 0 ? "Pending Upload" : "Final";
                        ...
                        let [res_LastDayPoint, res_Day, res_FileStatus] = await Promise.all([
                          axios.post(SERVICE.CHECK_DAYPOINT_ORG_BYDATE, {
                            headers: {
                              Authorization: `Bearer ${auth.APIToken}`,
                            },
                        ...
                        // setProdLastDayPoint(res_LastDayPoint.data.count);
                        // setProdLastDay(res_Day.data.count);

                        if (beforeday > 0 && beforedaypoint > 0) {
                          handleClickOpenDelBeforeDayPoint(newDateOneMinus, filestatusCheckFileupload);
                        } else if (beforeday > 0) {
                          handleClickOpenProdDayRerun(newDateOneMinus, filestatusCheckFileupload);
                        } else if (beforeday === 0) {
                          handleClickOpenProdDayCreate(newDateOneMinus, filestatusCheckFileupload);
                        }
                        setSelectedDate("");
                        setIsCheckUnAllotMismatch(false);
                      } else {
    ...
    <Box>
      ...
      <Dialog open={beforeProdDayCreate.open} onClose={handleClickCloseProdDayCreate} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
        ...
        <DialogActions>
          ...
          <LoadingButton autoFocus variant="contained" loading={rerunLoader} color="error" onClick={(e) => handleFilterSaveRerun(beforeProdDayCreate.date, "create", beforeProdDayCreate.filestatusupload)}>
            {" "}
            OK
          </LoadingButton>
        </DialogActions>
      ...
      <Dialog open={beforeProdDayRerun.open} onClose={handleClickCloseProdDayRerun} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
        ...
        <DialogActions>
          ...
          <LoadingButton autoFocus variant="contained" loading={rerunLoader} color="error" onClick={(e) => handleFilterSaveRerun(beforeProdDayRerun.date, "rerun", beforeProdDayRerun.filestatusupload)}>
            {" "}
            OK{" "}
          </LoadingButton>
        </DialogActions>
