...
import { Box, Typography, OutlinedInput, TextareaAutosize, TableBody, TableRow, Chip, TableCell, Select, Paper, MenuItem, Dialog, DialogTitle, DialogContent, DialogActions, FormControl, Grid, Table, TableHead, TableContainer, Button, List, ListItem, ListItemText, Popover, Checkbox, TextField, IconButton } from "@mui/material";
...
function Onprogressmanualentrylist() {
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, 'OnProgress Manual Entry List.png');
                })
                .catch((error) => {
    ...
    const fetchProductionIndividual = async () => {
        ...
        try {
            ...
            let res_project = await axios.post(SERVICE.PRODUCTION_INDIVIDUAL_ONPROGRESS, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            ...
            const itemsWithSerialNumber = ans?.map((item, index) => {
                ...
                return {
                    ...
                    startpage: item.startpage === "Please Select Start Page" ? "" : item.startpage, startdate: moment(item.startdate).format("DD/MM/YYYY"),
    ...
    const downloadPdf = (isfilter) => {
        ...
        axios.post(SERVICE.PRODUCTION_INDIVIDUALMANUAL_EXCEL_OVERALL, {
            ...
            .then(response => {
                ...
                doc.save("OnProgress Manual Entry List.pdf");
            })
            .catch(error => {
    ...
    const fileName = "OnProgress Manual Entry List";
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "OnProgress Manual Entry List",
        pageStyle: "print",
    });
        ...
        <Box>
            ...
            {isUserRoleCompare?.includes("lproductionmanualentrylist") && (
                <>
                    <Box sx={userStyle.container}>
                        {/* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>OnProgress Manual Entry List</Typography>
                        </Grid>
                        ...
                        {projectCheck ? (
                            ...
                            <>
                                ...
                                <AggridTable
                                    ...
                                    pagenamecheck={"On Progress Manual Entry List"}
            ...
            >
                <Box sx={{ padding: '20px 50px' }}>
                    <>
                        <Typography sx={userStyle.HeaderText}> View OnProgress Manual Entry</Typography>
                        <br />
            ...
            <ExportData
                ...
                itemsTwo={projmaster ?? []}
                filename={"OnProgress Individual List"}
...
export default Onprogressmanualentrylist;