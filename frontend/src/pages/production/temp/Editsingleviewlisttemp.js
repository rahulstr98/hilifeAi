...
function EditSingleViewList() {
  ...
  const getviewCodealledit = async () => {
    try {
      let res = await axios.get(`${SERVICE.PRODUCTION_TEMP_UPLOAD_SINGLE}/${id}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });

      setSingleViewEdit(res?.data?.sproductiontempuploadall);
    } catch (err) { handleApiError(err, setShowAlert, handleClickOpenerr); }
  };
  ...
  const cancelvalues = (e) => {
    backPage("/production/productiontempuploadall")
  }
  ...
  const editSubmit = async () => {
    try {
      ...
      backPage("/production/productiontempuploadall")
      //   setSingleViewEdit({ ...singleViewEdit, vendor: "" });
    } catch (err) { handleApiError(err, setShowAlert, handleClickOpenerr); }
  };
  ...
  return (
    <Box>
      <Headtitle title={"Production Temp Details"} />
      {/* ****** Header Content ****** */}
      {isUserRoleCompare?.includes("eproductionoriginalupload") && (
        <>
          <Typography sx={userStyle.HeaderText}>Production Temp Details</Typography>
          <Box sx={userStyle.dialogbox}>
            <Typography sx={userStyle.SubHeaderText}>Edit Production Temp Details</Typography>
            <br />
            <br />
...
export default EditSingleViewList;