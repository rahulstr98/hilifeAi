...
function ProductionUnAllotTemp() {
  ...
  // let exportColumnNames = ["Date", "Project Name", "Category", "SubCategory", "Production", "Mode"];
  ...
  const delupload = async () => {
    try {
      ...
      async function updateDataInBatches(items) {
        ...
        try {
          await axios.post(
            SERVICE.BULK_DELETE_UNITRATE_UNALLOT_TEMP,
            {
              updates: updatePayload,
            },
            {
  ...
  const editupload = async () => {
    try {
      ...
      async function updateDataInBatches(items) {
        ...
        try {
          await axios.post(
            SERVICE.PRODUCTION_TEMP_UNITRATE_BULK_UPDATECATSUBCATEGORY,
            {
              updates: updatePayload,
            },
            {
    ...
    try {
      if (selectedRows.length > 0) {
        ...
        setShowAlert(
          <>
            <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: "orange" }} />
            <p style={{ fontSize: "18px", fontWeight: 900 }}>Please Select any Row</p>
          </>
        );
        handleClickOpenerrinfo();
      }
        ...
        updateDataInBulk(validItems)
          .then(() => {
            setShowAlert(
              <>
                <CheckCircleOutline sx={{ fontSize: "100px", color: "#007F73" }} />
                <p style={{ fontSize: "20px", fontWeight: 900 }}>{validItems.length === data.length ? "Alloted" : "Present unitrate value only updated"}</p>
              </>
            );
            setUpdatedIdsDisabled((prev) => [...prev, ...validItems]);
            setBulkloader(false);
  ...
  return (
    <Box>
      ...
      <PageHeading title="Zero Unit Rate UnAllot Temp" modulename="Production" submodulename="Upload" mainpagename="Temp" subpagename="Zero Unit Rate Unallot Temp" subsubpagename="" />
          ...
          <Box sx={userStyle.dialogbox}>
            <>
              <Grid container spacing={2}>
                ...
                <Grid item md={4} sm={12} xs={12} marginTop={3}>
      ...
      {isUserRoleCompare?.includes("lzerounitrateunallottemp") && (
        <>
          <Box sx={userStyle.container}>
            ...
            &ensp;
            <LoadingButton onClick={(e) => handleBulkUpdate(e)} loading={bulkloader} color="primary" loadingPosition="end" variant="contained">
              {" "}
              Bulk Allot
            </LoadingButton>
            &ensp;
            <LoadingButton onClick={(e) => handleBulkSection(e)} loading={bulkloaderSec} color="primary" loadingPosition="end" variant="contained">
              {" "}
              Bulk Section
            </LoadingButton>
            {/* Manage Column */}
