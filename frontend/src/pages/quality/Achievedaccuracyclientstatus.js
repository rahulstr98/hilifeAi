...
function AcheivedAccurayClientStatus() {
    ...
    const [acheivedAccuracyDetails, setAcheivedAccuracyDetails] = useState({
        ...
        date: "", acheivedaccuracy: "", clientstatus: "", 
    });
    ...
    const fetchAccuracyMaster = async () => {
        try {
            let res_freq = await axios.get(SERVICE.ACHEIVEDACCURACYCLIENTGETALL, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
    ...
    const rowData = async (id) => {
        try {
            let res = await axios.get(`${SERVICE.ACHEIVEDACCURACYCLIENT_SINGLE}/${id}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
    ...
    const delBrand = async () => {
        try {
            await axios.delete(`${SERVICE.ACHEIVEDACCURACYCLIENT_SINGLE}/${brandid}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
    ...
    const sendRequest = async () => {
        ...
        try {
            await axios.post(SERVICE.ACHEIVEDACCURACYCLIENT_CREATE, {
    ...
    const handleSubmit = (e) => {
        ...
        if (acheivedAccuracyDetails.date === "" || acheivedAccuracyDetails.date === undefined) {
            ...
            setShowAlert(
                <>
                    <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: "orange" }} />
                    <p style={{ fontSize: "20px", fontWeight: 900 }}>{"Client Status is Must! "}</p>
                </>
            );
            handleClickOpenerr();
        }
    ...
    const getCode = async (e) => {
        try {
            let res = await axios.get(`${SERVICE.ACHEIVEDACCURACYCLIENT_SINGLE}/${e}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
    ...
    const sendEditRequest = async () => {
        // handleCloseModSubmit()       
        try {
            let res = await axios.put(`${SERVICE.ACHEIVEDACCURACYCLIENT_SINGLE}/${frequencyId}`, {
    ...
    const editSubmit = (e) => {
        ...
        else if ((acheivedAccuracyDetailsEdit.clientstatus === "" || acheivedAccuracyDetailsEdit.clientstatus === undefined)) {
            setShowAlert(
                <>
                    <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: "orange" }} />
                    <p style={{ fontSize: "20px", fontWeight: 900 }}>{"Client Status is Must! "}</p>
    ...
    const delAreagrpcheckbox = async () => {
        try {
            const deletePromises = selectedRows?.map((item) => {
                return axios.delete(`${SERVICE.ACHEIVEDACCURACYCLIENT_SINGLE}/${item}`, {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
                });
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "Achieved Accuracy Client Status.png");
                });
            });
        }
    };
    ...
    const downloadPdf = () => {
        ...
        doc.save("Achieved Accuracy Client Status.pdf");
    };
    // Excel
    const fileName = "Achieved Accuracy Client Status";
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "Achieved Accuracy Client Status",
        pageStyle: "print",
    });
        ...
        // },
        {
            ...
            renderCell: (params) => (
                <Grid sx={{ display: "flex" }}>
                    {isUserRoleCompare?.includes("eacheivedaccuracyclientstatus") && (
                        <Button
                    ...
                    {isUserRoleCompare?.includes("dacheivedaccuracyclientstatus") && (
                        <Button
                    ...
                    {isUserRoleCompare?.includes("vacheivedaccuracyclientstatus") && (
                        <Button
                            sx={userStyle.buttonedit}
                            onClick={() => {
                    ...
                    {isUserRoleCompare?.includes("iacheivedaccuracyclientstatus") && (
                        <Button
                            sx={userStyle.buttonedit}
                            onClick={() => {
                                handleClickOpeninfo();
    ...
    return (
        <Box>
            <Headtitle title={"ACHIEVED ACCURACY CLIENT STATUS"} />
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Achieved Accuracy Client Status</Typography>
            <>
                {isUserRoleCompare?.includes("aacheivedaccuracyclientstatus") && (
                    <Box sx={userStyle.selectcontainer}>
                        <>
                            <Grid container spacing={2}>
                                <Grid item xs={8}>
                                    <Typography sx={userStyle.importheadtext}>Add Achieved Accuracy Client Status</Typography>
                                </Grid>
                            </Grid>
                            <br />
                            <Grid container spacing={2}>
            ...
            {/* ****** Table Start ****** */}
            {
                isUserRoleCompare?.includes("lacheivedaccuracyclientstatus") && (
                    <>
                        <Box sx={userStyle.container}>
                            {/* ******************************************************EXPORT Buttons****************************************************** */}
                            <Grid item xs={8}>
                                <Typography sx={userStyle.importheadtext}>List Achieved Accuracy Client Status</Typography>
                            </Grid>
                            <Grid container spacing={2} style={userStyle.dataTablestyle}>
                                ...
                                >
                                    <Box>
                                        {isUserRoleCompare?.includes("excelacheivedaccuracyclientstatus") && (
                                            <>
                                        ...
                                        {isUserRoleCompare?.includes("csvacheivedaccuracyclientstatus") && (
                                            <>
                                        ...
                                        {isUserRoleCompare?.includes("printacheivedaccuracyclientstatus") && (
                                            <>
                                        ...
                                        {isUserRoleCompare?.includes("pdfacheivedaccuracyclientstatus") && (
                                            <>
                                        ...
                                        {isUserRoleCompare?.includes("imageacheivedaccuracyclientstatus") && (
                                            <>
                            ...
                            &ensp;
                            {isUserRoleCompare?.includes("bdacheivedaccuracyclientstatus") && (
                                <>
            ...
            <Dialog open={openInfo} onClose={handleCloseinfo} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                <Box sx={{ width: "550px", padding: "20px 50px" }}>
                    <>
                        <Typography sx={userStyle.HeaderText}>Achieved Accuracy Client Status Info</Typography>
                        <br />
                        <br />
                        <Grid container spacing={2}>
            ...
            <Box sx={{ width: "550px", padding: "20px 50px", overflowX: "hidden" }}>
                    <>
                        <Typography sx={userStyle.HeaderText}> View Achieved Accuracy Client Status</Typography>
                        <br /> <br />
                        <Grid container spacing={2}>
            ...
            <Box>
                <Dialog open={isEditOpen} onClose={handleCloseModEdit} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="md" fullWidth={true} sx={{
                    ...
                    <Box sx={{ padding: "20px 50px" }}>
                        <>
                            <Grid container spacing={2}>
                                <Typography sx={userStyle.HeaderText}>Edit Achieved Accuracy Client Status</Typography>
                            </Grid>
                            <br />
                            <Grid container spacing={2}>
...
export default AcheivedAccurayClientStatus;
