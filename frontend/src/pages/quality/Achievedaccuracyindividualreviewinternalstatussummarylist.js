...
function AchievedAccuracyIndividualReviewInternalstatusSummaryList() {
    ...
    const handleSubmission = async () => {
        ...
        try {
            ...
            let withComp = Array.isArray(result) ? result : []
            const itemsWithSerialNumber = [...withComp]?.map((item, index) => ({
                ...item, serialNumber: index + 1, date: `${moment(filterUser.fromdate).format('DD-MM-YYYY')} - ${moment(filterUser.todate).format('DD-MM-YYYY')}`,
