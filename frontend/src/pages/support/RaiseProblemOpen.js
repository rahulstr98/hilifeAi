...
function RaiseProblemOpen() {
  ...
  const exportallData = async () => {
    ...
    try {
      ...
      setAllDatta(data.map((item, index) => {
        return {
          id: item._id,
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("RasieProblemOpen"),
...

    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("vraiseproblemopen") && (
            <Link
              to={`/production/raiseproblemview/${params.data.id}/statusupdate/open`}
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage.toBlob(gridRefTableImg.current)
        .then((blob) => {
          saveAs(blob, "Raise Problem Open.png");
        })
        .catch((error) => {
