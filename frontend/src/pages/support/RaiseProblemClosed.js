...
function RaiseProblemClosed() {
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("RaiseProblemClosed"),
...

    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("vraiseproblemclosed") && (
            <Link
              to={`/production/raiseproblemview/${params.data.id}/statusupdate/closed`}
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "Raise Problem Closed",
    pageStyle: "print",
  });
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage.toBlob(gridRefTableImg.current)
        .then((blob) => {
          saveAs(blob, "Raise Problem Closed.png");
        })
        .catch((error) => {
