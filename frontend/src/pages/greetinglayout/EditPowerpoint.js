...
const EditPowerpoint = () => {
    ...
    const updateSlidesWithImagePositions = (slides) => {
        if (!Array.isArray(slides)) return [];

        return slides.map((slide, slideIndex) => {
            const imagess = slide?.images || [];

            const images = imagess.map((image, imageIndex) => ({
                x: image.xdrag,
                y: image.ydrag,
                width: image.w,
                height: image.h,
            ...
            const imagesdrag = imagess.map((_, imageIndex) => ({
                x: 751,
                y: 314,
            }));
    ...
    const rowData = async () => {
        ...
        try {
            let res = await axios.get(`${SERVICE.POWERPOINT_SINGLE}/${ids}`, {
                headers: {
            ...
            setHeaderfooter({
                isheadertext: res?.data?.spowerpoint?.isheadertext,
                isfootertext: res?.data?.spowerpoint?.isfootertext,
                isSlidenumber: res?.data?.spowerpoint?.isSlidenumber,
                headertext: res?.data?.spowerpoint?.headertext,
                footertext: res?.data?.spowerpoint?.footertext,
                isheaderfootercolor: res?.data?.spowerpoint?.isheaderfootercolor,
                headerfootercolor: res?.data?.spowerpoint?.headerfootercolor,
            })
            setBdayCompanyLogo(res?.data?.spowerpoint?.companylogo)
            const slides = res?.data?.spowerpoint?.slides || [];
            const imagesPut = updateSlidesWithImagePositions(slides);

            setImagePositions(imagesPut.map(slide => ({
                images: slide.images,
            })));
            ...
            setDimensions(imagesPut.map(slide => ({
                images: slide.images,
            })));

            setFileName(res?.data?.spowerpoint?.documentname)
            setSlides(res?.data?.spowerpoint?.slides);
            setPowerpoint(res?.data?.spowerpoint)
    ...
    const fetchCategoryAll = async () => {
        ...
        try {
            ...
            fetchSubcategoryBasedEdit(powerpoint.categoryname);
    ...
    const getCategoryList = async () => {
        ...
        try {
            ...
            setCategoryList(response?.data?.powerpoint?.filter((item, index) => item._id !== ids));
    ...
    const [selectedSlide, setSelectedSlide] = useState(0); // Assume selectedSlide is managed in state
    ...
    const handleSelectLayout = (layout) => {
        ...
        setSlides((prevSlides) => {
            ...
            // console.log(selectedSlideData, "selectedSlideData")
    ...
    //         setSelectedSlide(Math.max(selectedSlideIndex - 1, 0));
    ...
    //         setSlides(updatedSlides);
    //         setSelectedSlide(Math.max(slideid - 1, 0));
    //         slideRefs.current = updatedSlides.map((_, i) => slideRefs.current[i] || React.createRef());
    //     }
    // };
    ...
    const downloadPPT = () => {
        ...
        slides?.forEach((slide, slideIndex) => {
            ...
            //             x: 0.8,
            //             y: 1,
            //             fontSize: slide.titleFontSize || 16,
            ...
            //             x: 0.8,
            //             y: 3,
            //             fontSize: slide.subtitleFontSize || 16,
            ...
            //             x: 5,
            //             y: 2,
            //             fontSize: slide.subtitleFontSizetwo || 16,
            ...
            slide.images?.forEach((image, index) => {
                ...
                if (imgSrc && (imgSrc.startsWith("data:image") || imgSrc.startsWith("http"))) {
                    ...
                    const imageYInches = toInches(imageY) + 0.5; // Vertical position adjustment
        ...
        fileInput.onchange = (event) => {
            ...
            if (file) {
                ...
                reader.onload = (e) => {
                    ...
                    setPowerpoint((prevPowerpoint) => ({ ...prevPowerpoint }));
    ...
    const updateSlidesWithImagePositionsput = (slides, imagePositions) => {
    ...
    const sendRequest = async () => {
        setPageName(!pageName);
        try {
            const updatedSlides = updateSlidesWithImagePositionsput(slides, dimensions);

            let projectscreate = await axios?.put(`${SERVICE.POWERPOINT_SINGLE}/${ids}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
    ...
    const onMouseDown = (e, ImageIndexForD, slideIndexForD) => {

        const newX = e.clientX - imagePositions[slideIndexForD]?.images[ImageIndexForD]?.x;
        const newY = e.clientY - imagePositions[slideIndexForD]?.images[ImageIndexForD]?.y;


        const initialWidth = 200;
        const initialHeight = 200;
        const initialX = newX || e.clientX;
        const initialY = newY || e.clientY;
        ...
        const onMouseMove = (moveEvent) => {
            ...
            setImagePositions((prevPositions) => {
                ...
                if (!updatedPositions[slideIndexForD]) {
                    updatedPositions[slideIndexForD] = { images: [] };
                }

                updatedPositions[slideIndexForD].images[ImageIndexForD] = {
                    ...updatedPositions[slideIndexForD].images[ImageIndexForD],
                    w: Math.max(100, newWidth),
                    h: Math.max(100, newHeight),
                };

                if (slides[slideIndexForD] && slides[slideIndexForD].images[ImageIndexForD]) {
                    slides[slideIndexForD].images[ImageIndexForD] = {
                        ...slides[slideIndexForD].images[ImageIndexForD],  // Preserve the other properties
                        w: Math.max(100, newWidth),
                        h: Math.max(100, newHeight),
                    };
                }
    ...
    //                 istitlecursorposition: currentCaretPosition,
    //             };
    ...
    //                 issubtitlecursorposition: currentCaretPosition,
    //             };
    ...
    //                 issubtitlecursorpositiontwo: currentCaretPosition,
    //             };
    //         }
    //         return updatedSlides;
    //     });
    ...
    const handleRemoveImage = (imgIndex, slideIndex) => {
        ...
        // Handle setDragStart
        setDragStart((prevPositions) => {
            const updatedPositions = [...prevPositions];

            // Check if the slide exists and has an images array
            if (updatedPositions[slideIndex] && Array.isArray(updatedPositions[slideIndex].images)) {
                const updatedImages = [...updatedPositions[slideIndex].images]; // Copy the images array
                updatedImages.splice(imgIndex, 1); // Remove the image at imgIndex

                // Update the images array in the slide
                updatedPositions[slideIndex].images = updatedImages;

                // If images array is empty, clear it
                if (updatedImages.length === 0) {
                    updatedPositions[slideIndex].images = [];
                }
            }
    ...
    //                     headerfooter?.isSlidenumber ||
    //                     headerfooter?.headertext !== "" ||
    //                     headerfooter?.footertext !== "" ||
    //                     headerfooter?.isheaderfootercolor ||
    //                     headerfooter?.headerfootercolor !== "" ? "28px" : "38px",
    //                 minHeight: "194px",
    //                 fontSize: slides[index]?.titleFontSize || 18,
        ...
        switch (layout) {
            case "Title Slide": // Centered title with smaller subtitle
                return {
                    width: "750px",
                    minHeight: "140px",
                    fontSize: slides[index]?.subtitleFontSize || 16,
                    padding: "10px",
        ...
        switch (layout) {
            case "Title Slide": // Centered title with smaller subtitle
                return {
                    width: "750px",
                    minHeight: "140px",
                    fontSize: slides[index]?.subtitleFontSizetwo || 16,
                    padding: "10px",
                ...
                <Grid container sx={{
                    ...
                    <AppBar position="static" sx={{ marginBottom: "20px" }}>
                        ...
                        <Toolbar sx={{ display: "flex", justifyContent: "space-between" }}>
                            ...
                            <Box>
                                <Button startIcon={<SaveAsIcon />} color="inherit" onClick={handleEditOpen}>Update</Button>
                            </Box>
                        </Toolbar>
                    </AppBar>
                    <Grid container >
                                ...
                                <>
                                    <Box
                                        ...
                                        style={{
                                            ...
                                            overflow: "visible", // Prevent child overflow from affecting the Box
                                        }}
                                    ...
                                    >
                                            ...
                                            >
                                                <img
                                                    ...
                                                    style={{
                                                        ...
                                                        // Adjusts its position from the top of the div
                                        ...
                                        <div
                                            ...
                                            style={{
                                                ...getStylesByLayout(slides[index]?.layout, index, "title"),
                ...
                <DialogContent>
                    <FormGroup>
                        <FormControlLabel control={<Checkbox
                            ...
                            value={headerfooterForChoose.isheadertext}
                            onChange={(e) =>
                                setHeaderfooterForChoose({
                                    ...headerfooterForChoose,
                                    isheadertext: !headerfooterForChoose.isheadertext,
                                })
                            }
                        />} label="Header" />
                        <TextField
                    ...
                    <FormGroup>
                        <FormControlLabel control={<Checkbox
                            ...
                            value={headerfooterForChoose.isfootertext}
                            onChange={(e) =>
                                setHeaderfooterForChoose({
                                    ...headerfooterForChoose,
                                    isfootertext: !headerfooterForChoose.isfootertext,
                                })
                            }
                        />} label="Footer" />
                        <TextField
                    ...
                    <FormGroup>
                        <FormControlLabel control={<Checkbox
                            ...
                            value={headerfooterForChoose.isSlidenumber}
                            onChange={(e) =>
                                setHeaderfooterForChoose({
                                    ...headerfooterForChoose,
                                    isSlidenumber: !headerfooterForChoose.isSlidenumber,
                                })
                            }
                        />} label="Slide Number" />
                    </FormGroup>
                    <FormGroup>
                        <FormControlLabel control={<Checkbox
                            ...
                            value={headerfooterForChoose.isheaderfootercolor}
                            onChange={(e) =>
                                setHeaderfooterForChoose({
                                    ...headerfooterForChoose,
                                    isheaderfootercolor: !headerfooterForChoose.isheaderfootercolor,
                                })
                            }
                        />} label="Color" />
                    </FormGroup>
                    ...
                    <input type="color" style={{ width: "50%" }}
                        ...
                        value={headerfooterForChoose?.headerfootercolor}
                        onChange={(e) =>
                            setHeaderfooterForChoose({
                                ...headerfooterForChoose,
