...
function BiometricDevicesPairing() {
  ...
  const [biometricDevicePairingCreate, setBiometricDevicePairingCreate] =
    useState({
      mode: "New",
    ...
    try {
      ...
      const answer =
        response?.data?.devicespairing?.length > 0
          ? response?.data?.devicespairing?.map((item, index) => ({
              serialNumber: index + 1,
              pairdevicestable: item?.pairdevices
  ...
  const fetchDeviceNamesBasedOnArea = async (biometric, area) => {
    ...
    try {
      const response = await axios.post(
        SERVICE.ALL_BIOMETRICDEVICES_BASED_ON_AREA,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const getOverallEditSectionOverallDelete = async (ids) => {
    ...
    try {
      let res = await axios.post(
        SERVICE.OVERALL_BULK_BIO_DEVICE_PAIRING_DELETE,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const rowData = async (id, name) => {
    ...
    try {
      let res = await axios.get(
        `${SERVICE.SINGLE_BIOMETRICDEVICESPAIRING}/${id}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const delSource = async () => {
    ...
    try {
      const deletePromises = await axios.delete(
        `${SERVICE.SINGLE_BIOMETRICDEVICESPAIRING}/${Sourcesid}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const delSourcecheckbox = async () => {
    ...
    try {
      const deletePromises = selectedRows?.map((item) => {
        return axios.delete(
          `${SERVICE.SINGLE_BIOMETRICDEVICESPAIRING}/${item}`,
          {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
  ...
  const handleSubmit = async (e) => {
    ...
    const pairedDeviceData = pairedDevicesGroupingArray?.some(
      (data) =>
        valuePairedDevices?.includes(data?.paireddeviceone) ||
        valuePairedDevices?.includes(data?.paireddevicetwo)
    );
    console.log(isNameMatch, valuePairedDevices, "isNameMatch");
    if (
      biometricDevicePairingCreate.company === "Please Select Company" ||
      !biometricDevicePairingCreate.company
    ) {
      setPopupContentMalert("Please Select Company");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      biometricDevicePairingCreate.branch === "Please Select Branch" ||
      !biometricDevicePairingCreate.branch
    ) {
      setPopupContentMalert("Please Select Branch");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      biometricDevicePairingCreate.unit === "Please Select Unit" ||
      !biometricDevicePairingCreate.unit
    ) {
      setPopupContentMalert("Please Select Unit");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      biometricDevicePairingCreate.floor === "Please Select Floor" ||
      !biometricDevicePairingCreate.floor
    ) {
      ...
      setPopupContentMalert(
        "Can't pair. One device is already added as single."
      );
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else {
  ...
  const getCode = async (e, name) => {
    ...
    try {
      let res = await axios.get(
        `${SERVICE.SINGLE_BIOMETRICDEVICESPAIRING}/${e}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      fetchDeviceNamesBasedOnAreaEdit(
        res?.data?.sdevicespairing,
        res?.data?.sdevicespairing?.area
      );
      setSelectedPairedDeviceOptionsEdit(
        res?.data?.sdevicespairing?.pairdevices?.map((data) => ({
          label: data,
          value: data,
        }))
      );
  ...
  const sendEditRequest = async () => {
    ...
    try {
      let res = await axios.put(
        `${SERVICE.SINGLE_BIOMETRICDEVICESPAIRING}/${subprojectsid}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const editSubmit = async (e) => {
    ...
    const isNameMatch = acpointCalculationArrayEdit.some(
      (item) =>
        ...
        item?.pairdevices?.some((data) =>
          valuePairedDevicesEdit?.includes(data)
        )
    );
    const pairedDeviceData = pairedDevicesGroupingArray?.some(
      (data) =>
        valuePairedDevicesEdit?.includes(data?.paireddeviceone) ||
        valuePairedDevicesEdit?.includes(data?.paireddevicetwo)
    );
    ...
    if (
      ...
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (valuePairedDevicesEdit?.length === 1) {
      setPopupContentMalert("Atlease Choose two devices as Paired Devices");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (valuePairedDevicesEdit?.length > 2) {
      setPopupContentMalert("Paired Devices Must be of two");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (isNameMatch) {
  ...
  const columnDataTable = [
    ...
    {
      field: "pairdevicestable",
      headerName: "Pair Devies",
      flex: 0,
      width: 150,
      hide: !columnVisibility.pairdevicestable,
      headerClassName: "bold-header",
    },
    {
      field: "actions",
  ...
  return (
    <Box>
      ...
      <PageHeading
        ...
        subpagename="Biometric Devices Pairing"
        subsubpagename=""
      />
                    ...
                    <FormControl fullWidth size="small">
                      ...
                      <Selects
                        options={allfloor
                          ?.filter(
                            (u) =>
                              biometricDevicePairingCreate?.branch === u.branch
                          )
                          .map((u) => ({
                            ...u,
                            label: u.name,
                ...
                <Grid container spacing={2}>
                  <Grid item md={12} xs={12} sm={12}>
                    <Typography sx={userStyle.HeaderText}>
                      Edit Biometric Devices Pairing
                    </Typography>
                  </Grid>
                </Grid>
                <br />
                      ...
                      <FormControl fullWidth size="small">
                        ...
                        <Selects
                          options={allfloor
                            ?.filter(
                              (u) =>
                                biometricDevicePairingEdit?.branch === u.branch
                            )
                            .map((u) => ({
                              ...u,
                              label: u.name,
      ...
      {isUserRoleCompare?.includes("lbiometricdevicespairing") && (
        <>
          <Box sx={userStyle.container}>
            ...
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>
                Biometric Devices Pairing List
              </Typography>
            </Grid>
            <br />
      ...
      >
        <Box sx={{ padding: "20px 50px" }}>
          <>
            <Typography sx={userStyle.HeaderText}>
              {" "}
              View Biometric Devices Pairing
            </Typography>
            <br /> <br />
            <Grid container spacing={2}>
      ...
      <InfoPopup
        ...
        heading="Biometric Devices Pairing Info"
        addedby={addedby}
        updateby={updateby}
      />
