...
import { Box, Button, Dialog, Backdrop, DialogTitle, DialogActions, DialogContent, FormControl, LinearProgress, FormHelperText, Grid, IconButton, List, ListItem, ListItemText, MenuItem, Popover, Select, TextField, Tooltip, Typography, OutlinedInput, InputAdornment, Stack } from '@mui/material';
...
function DomainUsersList() {
  ...
  let exportColumnNames = ['Status', 'Account Type', 'Employment Type', 'Name', 'Username', 'Password', 'SN', 'CN', 'OU', 'Company', 'Branch', 'Unit', 'Team', 'Department', 'Designation', 'Employee code'];
  let exportRowValues = ['matchstatus', 'lockedstatus', 'workmode', 'companyname', 'username', 'originalpassword', 'sn', 'cn', 'ou', 'company', 'branch', 'unit', 'team', 'department', 'designation', 'empcode'];
    ...
    return (
      <Tooltip title={who} arrow>
        ...
        >
          ...
          >
            {who === 'Employee' && status === 'Matched' ? who : who === 'Employee' && status === 'Mismatched' ? 'Not Mentioned' : who}
          </Typography>
        </Button>
      </Tooltip>
    );
  ...
  const columnDataTable = [
    ...
    },
    {
      ...
      cellRenderer: (params) => renderWho(params?.data?.employeementtype, params?.data?.matchstatus),
  ...
  const editSubmitReset = (e) => {
    ...
    if (!editDetailsReset.password) {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (editDetailsReset.password !== '' && !/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#])[A-Za-z\d@$!%*?&#]{16,}$/.test(editDetailsReset.password)) {
      setPopupContentMalert('Password must be at least 16 characters long and include uppercase, lowercase, number, and special character.');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else {
  ...
  const handleSubmit = (e) => {
    ...
    if (!addDetails.cn) {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (addDetails.password !== '' && !/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#])[A-Za-z\d@$!%*?&#]{16,}$/.test(addDetails.password)) {
      setPopupContentMalert('Password must be at least 16 characters long and include uppercase, lowercase, number, and special character.');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      let res = await axios.post(
        ...
        {
          cn: addDetails.cn + ' ' + addDetails.sn,
          sn: addDetails.sn,
  ...
  let [valueStatusCatNew, setValueStatusCatNew] = useState(['Matched', 'Mismatched']);
  ...
  const customValueRendererStatusNew = (valueStatusCatNew, _categoryname) => {
    return valueStatusCatNew?.length ? valueStatusCatNew?.map(({ label }) => label)?.join(', ') : 'Please Select Status';
  };
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Domain Users List" modulename="LDAP" submodulename="Domain Users List" mainpagename="" subpagename="" subsubpagename="" />
          ...
          <Box sx={userStyle.dialogbox}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={12}>
                  <Typography sx={userStyle.importheadtext}>Filter Domain Users List</Typography>
                </Grid>
          ...
          {isUserRoleCompare?.includes('ldomainuserslist') && (
            <>
              <Box sx={userStyle.container}>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={10}>
                    <Typography sx={userStyle.SubHeaderText}>Domain Users List</Typography>
                  </Grid>
                  <Grid item xs={2}>
                    {isUserRoleCompare?.includes('adomainuserslist') && (
                      <>
      ...
      <Dialog open={openview} onClose={handleClickOpenview} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" fullWidth={true} maxWidth="lg" sx={{ marginTop: '50px' }}>
        <Box sx={{ padding: '20px 50px' }}>
          <>
            <Typography sx={userStyle.HeaderText}> View Domain User Details</Typography>
            <br />
            <Grid container spacing={2}>
      ...
      <Box>
        ...
        >
          <Box sx={{ padding: '20px 50px' }}>
            <>
              <Grid container spacing={2}>
                <Typography sx={userStyle.HeaderText}>Edit Domain User Details</Typography>
              </Grid>
              <br />
              <Grid container spacing={2}>
                <Grid item md={4} xs={12} sm={12}>
            ...
            <>
              <Grid container spacing={2}>
                <Typography sx={userStyle.HeaderText}>
                  Reset Password <span style={{ color: 'red', fontSize: '0.8rem' }}>(Length: At least 12-16 characters, Mix of Characters: Uppercase, lowercase, numbers, and special characters)</span>
                </Typography>
              </Grid>
              <br />
              <Grid container spacing={2}>
              ...
              >
                <Grid item md={3} xs={12} sm={12}>
                  <Button variant="contained" onClick={editSubmitReset} sx={buttonStyles.buttonsubmit}>
                    {' '}
                    Update
                  </Button>
                </Grid>
                <br />
                <Grid item md={3} xs={12} sm={12}>
                  <Button sx={buttonStyles.btncancel} onClick={handleCloseModEditReset}>
                    {' '}
                    Cancel{' '}
                  </Button>
                </Grid>
      ...
      <Box>
        ...
        >
          <Box sx={{ padding: '20px 50px' }}>
            <>
              <Grid container spacing={2}>
                <Typography sx={userStyle.HeaderText}>Create New Domain User</Typography>
              </Grid>
              <br />
              <Grid container spacing={2}>
                <Grid item md={4} xs={12} sm={12}>
      ...
      <DeleteConfirmation open={isDeleteOpen} onClose={handleClickCloseDelete} onConfirm={deleteFunction} title="Are you sure? Do You Want to Delete this User?" confirmButtonText="Yes" cancelButtonText="Cancel" />
    </Box>
  );
}
