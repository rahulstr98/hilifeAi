...
function EBReadingDetails() {
    ...
    const [isEbserviceMasterall, setIsEbServiceMasterall] = useState([]);
    ...
    const [futureupdate, setFutureUpdate] = useState({
        id: "",
        openkwh: "",
        kvah: "",
        kwhunit: "",
    ...
    const fetchEbServiceMaster = async (servicenumber, floor, area, e) => {
        let result = isEbserviceMasterall.find(
            (d) =>
    ...
    const fetchServiceNumber = async (e) => {
        let data_set = isEbserviceMasterall.filter((data) => e.value === data.unit);
        const branchall = [
            ...data_set.map((d) => ({
                ...d,
    ...
    const fetchServiceNumberall = async () => {
        setServicemaster(isEbserviceMasterall);
    };

    const fetchServiceNumberFilter = async (e) => {
        let data_set = isEbserviceMasterall.filter(
    ...
    const fetchServiceNumberEdit = async (e) => {
        let data_set = isEbserviceMasterall.filter((data) => e === data.unit);
        const branchall = [
            ...data_set.map((d) => ({
                ...d,
                label: d.servicenumber,
    ...
    const fetchEbservicemasterAll = async () => {
        try {
            let res_freq = await axios.get(SERVICE.EBSERVICEMASTER, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });

            setIsEbServiceMasterall(res_freq?.data?.ebservicemasters);
        } catch (err) {
    ...
    const fetchFloor = async (e) => {
        let data_set = isEbserviceMasterall.find(
            (data) => e.value === data.servicenumber
        );
    ...
    const fetchFloorEdit = async (e) => {
        ...
        fetchEbServiceMasterEdit(
            ebreadingdetailEdit.servicenumber,
            branchall[0]?.value,
            branchallarea[0]?.value,
            readingmodeEdit
        );
    ...
    useEffect(() => {
        ...
        fetchEbservicemasterAll();
    }, []);

    const [deleteEbreading, setDeleteEbreading] = useState("");

    const [futureupdatedelbefore, setFutureUpdatedelbefore] = useState({
        id: "",
        openkwh: "",
        kvah: "",
        kwhunit: "",
    ...
    const [futureupdatedelafter, setFutureUpdatedelafter] = useState({
        id: "",
        openkwh: "",
        kvah: "",
        kwhunit: "",
    ...
    const rowData = async (id) => {
        ...
        try {
            ...
            if (res_read?.data?.ebreadingfiltertimefuture.length > 1) {
                setPopupContentMalert("More Than two Dates After This Date !");
    ...
    const delEbreading = async () => {
        try {
            if (Ebreadingsid) {
                await axios.delete(
                    `${SERVICE.EBREADINGDETAIL_SINGLE}/${Ebreadingsid}`,
                    {
                        headers: {
                            Authorization: `Bearer ${auth.APIToken}`,
                        },
            ...
            if (
                futureupdatedelafter.length >= 1 &&
                futureUpdatedelbeforearr.length >= 1
            ) {
                if (futureupdatedelafter && futureupdatedelafter?._id) {
                    let futureid = futureupdatedelafter?._id;
                    let pfdiffkwh =
                        Number(futureupdatedelafter.openkwh) -
                        Number(futureupdatedelbefore.openkwh);
                    let pfdiffkvh =
                        Number(futureupdatedelafter.kvah) -
                        Number(futureupdatedelbefore.kvah);
                    let pfdiff = pfdiffkwh / pfdiffkvh;
                    let res = await axios.put(
                        `${SERVICE.EBREADINGDETAIL_SINGLE}/${futureid}`,
                        {
                            headers: {
                                Authorization: `Bearer ${auth.APIToken}`,
                            },
                ...
                if (isLastDayOfMonth) {
                    ...
                    if (result) {
                        ...
                        let res = await axios.delete(
                            `${SERVICE.EBREADINGDETAIL_SINGLE}/${resultid}`,
                            {
                                headers: {
                                    Authorization: `Bearer ${auth.APIToken}`,
                                },
            ...
            if (futureupdatearr.length === 1) {
                try {
                    ...
                    let subprojectscreate = await axios.post(
                        SERVICE.EBREADINGDETAIL_CREATE,
                        {
                            headers: {
                                Authorization: `Bearer ${auth.APIToken}`,
                            },
                    ...
                    if (futureupdate && futureupdate?._id) {
                        ...
                        let pfdiffkwh =
                            Number(futureupdate.openkwh) - Number(ebreadingdetail.openkwh);
                        let pfdiffkvh =
                            Number(futureupdate.kvah) - Number(ebreadingdetail.kvah);
                        let pfdiff = pfdiffkwh / pfdiffkvh;
                        let res = await axios.put(
                            `${SERVICE.EBREADINGDETAIL_SINGLE}/${futureid}`,
                            {
                    ...
                    if (readingmode == "Daily Closing") {
                        ...
                        if (isLastDayOfMonth) {
                            ...
                            const resultcheck = ebreadingdetails.filter(
                                (item) =>
                                    item.readingmode === "Month Closing" &&
                            ...
                            if (lastvalue && resultcheck.length === 0) {
                                let subprojectscreate = axios.post(
                                    SERVICE.EBREADINGDETAIL_CREATE,
                                    {
                                        headers: {
                                            Authorization: `Bearer ${auth.APIToken}`,
                                        ...
                                        kvah: String(ebreadingdetail.kvah),
                                        kwhunit:
                                            Number(ebreadingdetail.openkwh) - Number(lastvalue.openkwh),
                                        kvahunit:
                                            Number(ebreadingdetail.kvah) - Number(lastvalue.kvah),
                                        pf: pfdifference,
                                        md: String(ebreadingdetail.md),
                                        pfrphase: String(ebreadingdetail.pfrphase),
                                        pfyphase: String(ebreadingdetail.pfyphase),
    ...
    const handleSubmit = (e) => {
        ...
        e.preventDefault();
        const isNameMatchDaily = ebreadingdetails.some(
            (item) =>
                readingmode === "Daily Closing" &&
        ...
        const isNameMatchBillbefore = ebreadingdetails.some((item) => {
            return (
                ...
                item.floor === String(ebreadingdetail.floor) &&
                item.area === String(ebreadingdetail.area) &&
                ebreadingdetail.date <= item.date
            );
        });
    ...
    const fetchFilteredDatas = async () => {
        try {
            let subprojectscreate = await axios.post(SERVICE.CHECK_EBREADINGDETAIL, {
                ...
                branch:
                    String(ebreadingdetailFilter.branch) === "Please Select Branch"
                        ? ""
                        : ebreadingdetailFilter.branch,
                floor:
                    String(ebreadingdetailFilter.floor) === "Please Select Floor"
                        ? ""
                        : ebreadingdetailFilter.floor,
                servicenumber:
                    String(ebreadingdetailFilter.servicenumber) === "Please Select Service"
                        ? ""
                        : ebreadingdetailFilter.servicenumber
            });
    ...
    const sendEditRequest = async () => {
        ...
        //     setPopupContentMalert(
        //       `You have Already Added two more dates before this Date ${ebreadingdetailEdit.date}`
        //     );
        //     setPopupSeverityMalert("info");
        //     handleClickOpenPopupMalert();
        //   } else if (futureupdatedelafterarr.length === 1) {
        //     let res = await axios.put(
        //       `${SERVICE.EBREADINGDETAIL_SINGLE}/${subprojectsid}`,
        //       {
        //         headers: {
        //           Authorization: `Bearer ${auth.APIToken}`,
        //         },
        ...
        //             ? ebreadingdetailEdit.description
        //             : ""
        //         ),
        ...
        //         currentusageunit:
        //           Number(ebservicesEdit) - Number(readingFilterEdit),
        ...
        //       let pfdiffkwh =
        //         Number(futureupdatedelafter.openkwh) -
        //         Number(ebreadingdetailEdit.openkwh);
        //       let pfdiffkvh =
        //         Number(futureupdatedelafter.kvah) -
        //         Number(ebreadingdetailEdit.kvah);
        //       let pfdiff = pfdiffkwh / pfdiffkvh;
        //       let res = await axios.put(
        //         `${SERVICE.EBREADINGDETAIL_SINGLE}/${futureid}`,
        //         {
        ...
        //             item.date.includes(formattedPreviousMonth)
        //         );
        //         const resultcheck = ebreadingdetails.find(
        //           (item) =>
        //             item.readingmode === "Month Closing" &&
        ...
        //           let res = await axios.put(
        //             `${SERVICE.EBREADINGDETAIL_SINGLE}/${resultcheck._id}`,
        //             {
        //               headers: {
        //                 Authorization: `Bearer ${auth.APIToken}`,
        //               },
        ...
        //               time: String(ebreadingdetailEdit.time),
        //               openkwh:
        //                 String(ebreadingdetailEdit.openkwh) -
        //                 Number(lastvalue.openkwh),
        //               kvah:
        //                 String(ebreadingdetailEdit.kvah) - Number(lastvalue.kvah),
        //               kwhunit: readingFilterEdit,
        //               kvahunit: readingFilterKvhEdit,
        //               pf: pfdifference,
        //               md: String(ebreadingdetailEdit.md),
        ...
        //                 Number(lastvalue.openkwh),
        //               kvahunit:
        //                 Number(ebreadingdetailEdit.kvah) - Number(lastvalue.kvah),
        //               pf: pfdifference,
        ...
        //     handleClickOpenPopup();
        //   } else if (
        //     futureupdatedelafterarr.length === 0 ||
        //     (futureupdatedelafterarr.length > 1 &&
        //       ebreadingdetailEdit.date == olddate)
        //   ) {
        //     let res = await axios.put(
        //       `${SERVICE.EBREADINGDETAIL_SINGLE}/${subprojectsid}`,
        //       {
        ...
        try {
            ...
            if (futureupdatebeforearr.length > 1) {
                ...
                await fetchEbreadingdetails(); fetchEbSort();
                handleCloseModEdit();
                setPopupContent("Updated Successfully");
                setPopupSeverity("success");
                handleClickOpenPopup();
    ...
    const editSubmit = async (e) => {
        ...
        // let resdata = await fetchEbreadingdetailsAll();
        const isNameMatchDaily = allEbreadingdetailedit.some(
            (item) =>
                readingmodeEdit === "Daily Closing" &&
        ...
        const isNameMatchBillbefore = allEbreadingdetailedit.some((item) => {
            return (
                ...
                item.floor === String(ebreadingdetailEdit.floor) &&
                item.area === String(ebreadingdetailEdit.area) &&
                ebreadingdetailEdit.date <= item.date
            );
        });
    ...
    const fetchEbreadingdetails = async () => {
        try {
            let res_vendor = await axios.get(SERVICE.EBREADINGDETAIL, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
            ...
            // setEbreadingdetails((prevDetails) => [
            //   res_vendor?.data?.ebreadingdetails,
            //   ...prevDetails,
            // ]);
        } catch (err) {
            setEbreadingdetailcheck(true);
    ...
    const fetchEbreadingdetailsFilterDate = async (date) => {
        ...
        try {
            ...
            if (res_read?.data?.ebreadingfiltertime.length > 0) {
                const mostRecentDate = varfilter.reduce((prev, current) =>
                    new Date(current.date) > new Date(prev.date) ? current : prev
                );

                if (ebreadingdetail.openkwh > 0 && mostRecentDate.openkwh > 0) {
                    let differecnce =
                        Number(ebreadingdetail.openkwh) - Number(mostRecentDate.openkwh);
            ...
            if (res_read?.data?.ebreadingfiltertime.length > 0) {
                ...
                if (ebreadingdetail.kvah > 0 && mostRecentDate.kvah > 0) {
                    let differecnce =
                        Number(ebreadingdetail.kvah) - Number(mostRecentDate.kvah);
    ...
    const fetchEbreadingdetailsFilterDateEdit = async (date) => {
        ...
        try {
            ...
            if (res_read?.data?.ebreadingfiltertime.length > 0) {
                ...
                let differecnce =
                    Number(ebreadingdetailEdit.openkwh) - Number(mostRecentDate.openkwh);
            ...
            if (res_read?.data?.ebreadingfiltertime.length > 0) {
                ...
                if (ebreadingdetailEdit.kvah > 0 && mostRecentDate.kvah > 0) {
                    let differecnce =
                        Number(ebreadingdetailEdit.kvah) - Number(mostRecentDate.kvah);
            ...
            if (
                res_read?.data?.ebreadingfiltertime &&
                res_read?.data?.ebreadingfiltertime.length > 0
            ) {
    ...
    const fetchEbreadingdetailsFilterKVHEdit = async (kvahval) => {
        ...
        try {
            ...
            if (
                res_read?.data?.ebreadingfiltertimefuture &&
                res_read?.data?.ebreadingfiltertimefuture.length > 0
            ) {
                setFutureUpdatedelafter(res_read?.data?.ebreadingfiltertimefuture[0]);
                setFutureUpdatedelafterarr(res_read?.data?.ebreadingfiltertimefuture);
            }
    ...
    const fetchEbreadingdetailsAll = async () => {
        try {
            let res_meet = await axios.get(SERVICE.EBREADINGDETAIL, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
            setAllEbreadingdetailedit(
                res_meet?.data?.ebreadingdetails.filter(
                    (item) => item._id !== ebreadingdetailEdit._id
                )
            );
        } catch (err) {
            handleApiError(err, setShowAlert, handleClickOpenerr);
    ...
    useEffect(() => {
        fetchEbreadingdetailsAll();
    }, [isEditOpen, ebreadingdetailEdit, allEbreadingdetailedit],);
    ...
    const addSerialNumber = () => {
        const itemsWithSerialNumber = ebreadingdetails?.map((item, index) => ({
            ...item,
...

        {
            ...
            renderCell: (params) => (
                <Grid sx={{ display: "flex" }}>
                    {isUserRoleCompare?.includes("eebreadingdetails") && (
                        <Button
                    ...
                    {isUserRoleCompare?.includes("debreadingdetails") && (
                        <Button
                    ...
                    {isUserRoleCompare?.includes("vebreadingdetails") && (
                        <Button
                            sx={userStyle.buttonedit}
                            onClick={() => {
                                handleClickOpenview();
                    ...
                    {isUserRoleCompare?.includes("iebreadingdetails") && (
                        <Button
                            sx={userStyle.buttonedit}
                            onClick={() => {
                                handleClickOpeninfo();
    ...
    const fetchEbSort = async () => {
        try {
            let res_employee = await axios.post(SERVICE.OVERALLSORT_EBREADING, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                page: Number(page),
    ...
    return (
        <Box>
                                ...
                                <Grid item md={3} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <Selects
                                            ...
                                            styles={colourStyles}
                                            value={{
                                                label: ebreadingdetail.company,
                                                value: ebreadingdetail.company,
                                            }}
                                            onChange={(e) => {
                                                setEbreadingdetail({
                                                    ...ebreadingdetail,
                                ...
                                <Grid item md={3} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <Selects
                                            options={isAssignBranch ? isAssignBranch
                                                ?.filter(
                                                    (comp) => ebreadingdetail.company === comp.company
                                                )
                                                ?.map((data) => ({
                                                    label: data.branch,
                                                    value: data.branch,
                                            ...
                                            styles={colourStyles}
                                            value={{
                                                label: ebreadingdetail.branch,
                                                value: ebreadingdetail.branch,
                                            }}
                                            onChange={(e) => {
                                                setEbreadingdetail({
                                                    ...ebreadingdetail,
                                ...
                                <Grid item md={3} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <Selects
                                            options={isAssignBranch ? isAssignBranch
                                                ?.filter(
                                                    (comp) =>
                                                        ...
                                                        ebreadingdetail.branch === comp.branch
                                                )
                                                ?.map((data) => ({
                                                    label: data.unit,
                                                    value: data.unit,
                                            ...
                                            styles={colourStyles}
                                            value={{
                                                label: ebreadingdetail.unit,
                                                value: ebreadingdetail.unit,
                                            }}
                                            onChange={(e) => {
                                                setEbreadingdetail({
                                                    ...ebreadingdetail,
                                ...
                                <Grid item md={3} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <Selects
                                            ...
                                            styles={colourStyles}
                                            value={{
                                                label: ebreadingdetail.servicenumber,
                                                value: ebreadingdetail.servicenumber,
                                            }}
                                            onChange={(e) => {
                                                fetchFloor(e);
                                            }}
                                ...
                                <Grid item md={3} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <Selects
                                            ...
                                            styles={colourStyles}
                                            value={{
                                                label: ebreadingdetail.floor,
                                                value: ebreadingdetail.floor,
                                            }}
                                            onChange={(e) => {
                                                setEbreadingdetail({
                                                    ...ebreadingdetail,
                                ...
                                <Grid item md={3} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <Selects
                                            ...
                                            styles={colourStyles}
                                            value={{
                                                label: ebreadingdetail.area,
                                                value: ebreadingdetail.area,
                                            }}
                                            onChange={(e) => {
                                                setEbreadingdetail({
                                                    ...ebreadingdetail,
                                    ...
                                    <FormControl fullWidth size="small">
                                        ...
                                        <Select
                                            ...
                                            onChange={(e) => {
                                                ...
                                                fetchEbServiceMaster(
                                                    ebreadingdetail.servicenumber,
                                                    ebreadingdetail.floor,
                                                    ebreadingdetail.area,
                                                    e.target.value
                                                );
                                            }}
                                            displayEmpty
                                ...
                                <Grid item md={3} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <OutlinedInput
                                            ...
                                            type="time"
                                            placeholder="HH:MM"
                                            value={ebreadingdetail.time}
                                            onChange={(e) => {
                                                setEbreadingdetail({
                                                    ...ebreadingdetail,
                                                    time: e.target.value,
                                                });
                                                fetchEbreadingdetailsFilterTime(e.target.value);
                                            }}
                                        />
                                    </FormControl>
                                </Grid>
                                    ...
                                    <Grid item md={3} xs={12} sm={12}>
                                        <FormControl fullWidth size="small">
                                            ...
                                            <Selects
                                                ...
                                                styles={colourStyles}
                                                value={{
                                                    label: ebreadingdetailEdit.company,
                                                    value: ebreadingdetailEdit.company,
                                                }}
                                                onChange={(e) => {
                                                    setEbreadingdetailEdit({
                                                        ...ebreadingdetailEdit,
                                    ...
                                    <Grid item md={3} xs={12} sm={12}>
                                        <FormControl fullWidth size="small">
                                            ...
                                            <Selects
                                                ...
                                                styles={colourStyles}
                                                value={{
                                                    label: ebreadingdetailEdit.branch,
                                                    value: ebreadingdetailEdit.branch,
                                                }}
                                                onChange={(e) => {
                                                    setEbreadingdetailEdit({
                                                        ...ebreadingdetailEdit,
                                            ...
                                            <Selects
                                                options={isAssignBranch ? isAssignBranch
                                                    ?.filter(
                                                        (comp) =>
                                                            ebreadingdetailEdit.company === comp.company &&
                                                            ebreadingdetailEdit.branch === comp.branch
                                                    )
                                                    ?.map((data) => ({
                                                        label: data.unit,
                                                        value: data.unit,
                                                ...
                                                styles={colourStyles}
                                                value={{
                                                    label: ebreadingdetailEdit.unit,
                                                    value: ebreadingdetailEdit.unit,
                                                }}
                                                onChange={(e) => {
                                                    setEbreadingdetailEdit({
                                                        ...ebreadingdetailEdit,
                                    ...
                                    <Grid item md={3} xs={12} sm={12}>
                                        <FormControl fullWidth size="small">
                                            ...
                                            <Selects
                                                ...
                                                styles={colourStyles}
                                                value={{
                                                    label: ebreadingdetailEdit.servicenumber,
                                                    value: ebreadingdetailEdit.servicenumber,
                                                }}
                                                onChange={(e) => {
                                    ...
                                    <Grid item md={3} xs={12} sm={12}>
                                        <FormControl fullWidth size="small">
                                            ...
                                            <Selects
                                                ...
                                                styles={colourStyles}
                                                value={{
                                                    label: ebreadingdetailEdit.floor,
                                                    value: ebreadingdetailEdit.floor,
                                                }}
                                                onChange={(e) => {
                                                    setEbreadingdetailEdit({
                                                        ...ebreadingdetailEdit,
                                    ...
                                    <Grid item md={3} xs={12} sm={12}>
                                        <FormControl fullWidth size="small">
                                            ...
                                            <Selects
                                                ...
                                                styles={colourStyles}
                                                value={{
                                                    label: ebreadingdetailEdit.area,
                                                    value: ebreadingdetailEdit.area,
                                                }}
                                                onChange={(e) => {
                                                    setEbreadingdetailEdit({
                                                        ...ebreadingdetailEdit,
                                        ...
                                        <FormControl fullWidth size="small">
                                            ...
                                            <Select
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    fetchEbreadingdetailsFilterReadingEdit(
                                                        e.target.value
                                                    );
                                                    fetchEbServiceMasterEdit(
                                                        ebreadingdetailEdit.servicenumber,
                                                        ebreadingdetailEdit.floor,
                                                        ebreadingdetailEdit.area,
                                                        e.target.value
                                                    );
                                                }}
                                                displayEmpty
            ...
            <br />
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("lebreadingdetails") && (
                <>
                    <Box sx={userStyle.container}>
                        {/* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>
                                EB Reading Details List
                            </Typography>
                        </Grid>
                        <br />
                        <Grid container spacing={2}>
                            ...
                            <Grid item md={2.4} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    ...
                                    <Selects
                                        options={isAssignBranch ? isAssignBranch
                                            ?.filter(
                                                (comp) => ebreadingdetailFilter.company === comp.company
                                            )
                                            ?.map((data) => ({
                                                label: data.branch,
                                                value: data.branch,
                        ...
                        <Grid container spacing={2} style={userStyle.dataTablestyle}>
                            ...
                            >
                                <Box>
                                    {isUserRoleCompare?.includes("excelebreadingdetails") && (
                                        <>
                                            <Button
                                                onClick={(e) => {
                                    ...
                                    {isUserRoleCompare?.includes("csvebreadingdetails") && (
                                        <>
                                            <Button
                                                onClick={(e) => {
                                                    setIsFilterOpen(true);
                                    ...
                                    {isUserRoleCompare?.includes("printebreadingdetails") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("pdfebreadingdetails") && (
                                        <>
                                            <Button
                                                sx={userStyle.buttongrp}
                                                onClick={() => {
                                    ...
                                    {isUserRoleCompare?.includes("imageebreadingdetails") && (
                                        <Button
                                            sx={userStyle.buttongrp}
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item md={3} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    ...
                                    <Typography>
                                        {moment(ebreadingdetailEdit.date).format("DD/MM/YYYY")}
                                    </Typography>
                                </FormControl>
                            </Grid>
                            <Grid item md={3} xs={12} sm={12}>
                            ...
                            {serviceNumberEdit ? (
                                <>
                                    ...
                                    <Grid item md={3} xs={12} sm={12}>
                                        <FormControl fullWidth size="small">
                                            <Typography variant="h6">P.F</Typography>
