...
function OrganizationDocumentCategory() {
  ...
  const { isUserRoleCompare, buttonStyles, isUserRoleAccess, pageName, setPageName } = useContext(
    UserRoleAccessContext
  );
  ...
  const getcode = async (popuptype, id, categoryname, subcategory) => {
    ...
    try {
      ...
      rowDataEdit(res.data.sorgcategory.categoryname, res.data.sorgcategory.subcategoryname)
      if (popuptype == "view") {
        handleViewOpen();
  ...
  const rowDataEdit = async (categoryname, subcat) => {
    setPageName(!pageName)
    try {

      let res = await axios.post(SERVICE.ORGDOCUMENT_OVERALLDELETE, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  const rowData = async (id, categoryname, subcat) => {
    ...
    try {
      const [res, resdev] = await Promise.all([
        axios.get(`${SERVICE.ORGCATEGORYDOCUMENT_SINGLE}/${id}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        }),
        axios.post(SERVICE.ORGCATEGORYDOCUMENT_OVERALLEDIRT, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
          oldname: String(categoryname),
      ...
      if (resdev?.data?.count > 0) {
        setPopupContentMalert(
          <span style={{ fontWeight: "700", color: "#777" }}>
            {`${categoryname} & ${subcat?.map((item, index) =>
              `${index + 1}.) ${item}`).join("\n")}`}
            <span style={{ fontWeight: "bold", color: "black" }}> was linked</span>
          </span>
        );
  ...
  const handleSubmit = () => {
    ...
    if (isNameMatch) {
      ...
      setPopupContentMalert("Already Added ! Please Enter Another Sub Categories");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (subcategory !== "") {
  ...
  const handleSubmitEdit = () => {
    ...
    if (isNameMatch) {
      ...
      (singleCategory.categoryname != ovProj ||
        !editTodo?.every((item) => ovProjsub?.includes(item))) && ovProjCount > 0
    ) {
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "OrganizationCategoryDocument.png");
        });
      });
    }
  };
  return (
    <Box>
      ...
      <InfoPopup
        ...
        heading="Oraganization Category Document Info "
        addedby={addedby}
        updateby={updateby}
      />
    </Box>
