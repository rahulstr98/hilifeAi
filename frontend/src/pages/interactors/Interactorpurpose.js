...
function InteractorPurpose() {
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Interactorpurpose"),
  ...
  const overallBulkdelete = async (ids) => {
    ...
    try {
      let overallcheck = await axios.post(
        `${SERVICE.OVERALL_INTERACTORPURPOSE_BULKDELETE}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const rowData = async (id, name) => {
    ...
    try {
      ...
      const [res, resuser] = await Promise.all([
        await axios.get(`${SERVICE.SINGLE_INTERACTORPURPOSE}/${id}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        }),
        axios.post(SERVICE.OVERALL_INTERACTORPURPOSE, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
          oldname: String(name),
      ...
      if (resuser?.data?.count > 0) {
        ...
        setPopupContentMalert(
          <span style={{ fontWeight: "700", color: "#777" }}>
            {`${res?.data?.sinteractorpurpose?.name} `}
  ...
  const getOverallEditSection = async (e) => {
    try {
      let res = await axios.post(SERVICE.OVERALL_INTERACTORPURPOSE, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        oldname: e,
  ...
  const editSubmit = async (e) => {
    ...
    if (interactorPurposeEdit.name === "") {
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (interactorPurposeEdit.name != ovProj && ovProjCount > 0) {
  ...
  return (
    <Box>
      ...
      <InfoPopup
        ...
        heading="Interactor Purpose Info"
        addedby={addedby}
        updateby={updateby}
      />
      <DeleteConfirmation
