...
import { Backdrop, Box, Button, Dialog, DialogActions, DialogContent, DialogTitle, FormGroup, FormControlLabel, Checkbox, FormControl, Grid, MenuItem, OutlinedInput, Paper, Select, Table, TableBody, TableContainer, TableHead, TextareaAutosize, Typography } from '@mui/material';
...
cbuildingapartmentname:"",
  ...
  const { isUserRoleAccess, isAssignBranch, allUsersData, allTeam, allareagrouping, allfloor, alldepartment, pageName, setPageName, buttonStyles } = useContext(UserRoleAccessContext);
...
cbuildingapartmentname: !vendor.samesprmnt ? String(vendor?.cbuildingapartmentname || '') : String(vendor?.pbuildingapartmentname || ''),
  ...
  const handleSubmit = async (e, type) => {
    ...
    // }
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendor.meetingpersonemployeename === 'Please Select Employee Name') {
      setPopupContentMalert('Please Select Employee Name');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendor.enquiredbranchnumber === 'Please Select Number') {
      setPopupContentMalert('Please Select Enquired Branch Number');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendor.enquiredbranchemail === 'Please Select Email') {
  ...
  const handleAutoSelect = async () => {
    ...
    try {
      ...
      setEnquiredBranchNumber(
        setPhoneEmailOpt?.phone?.length > 0
          ? setPhoneEmailOpt?.phone?.map((item) => ({
              label: item,
              value: item,
            }))
          : []
      ...
      setEnquiredBranchEmail(
        setPhoneEmailOpt?.email?.map((item) => ({
          label: item,
          value: item,
        }))
      );
...
cbuildingapartmentname: e?.cbuildingapartmentname || "",
  ...
  return (
    <Box>
      ...
      <PageHeading title="Visitors Enquiry" modulename="Interactors" submodulename="Visitor" mainpagename="Visitor Enquiry" subpagename="" subsubpagename="" />
      <>
        <Box sx={userStyle.dialogbox}>
          <>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                {' '}
                <Typography sx={{ fontWeight: 'bold' }}>Add Visitor Enquiry</Typography>{' '}
              </Grid>
            </Grid>
            <br />
            <Grid container spacing={2}>
...
paddressthree:vendor?.paddressthree || "",
              ...
              {/* {vendor.meetingdetails && (
                <> */}
              <Grid item md={12} xs={12} sm={12}>
                <Typography sx={{ fontWeight: 'bold' }}>Enquiry Person</Typography>
              </Grid>
