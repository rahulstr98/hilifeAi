      ...
      renderCell: (params) => (
        <Grid sx={{ display: 'flex', alignItems: 'center' }}>
          {params?.row?.modelName === 'Candidate' && params?.row?.lastInterviewRound?.roundname ? (
            // <Link to={`/interviewrounds/${params.row.jobopeningsid}`} target="_blank">
            <Button
              onClick={() => {
                navigate(`/interviewrounds/${params.row.jobopeningsid}`, {
                  state: { withoutrole: true },
                });
              }}
            >
          ...
          params?.row?.status === 'Employee' || params?.row?.status === 'Visitor' ? (
            <Button sx={{ ...userStyle.buttonedit, opacity: 0.5, pointerEvents: 'none' }} disabled></Button>
          ) : (
            <Button sx={{ ...userStyle.buttonedit, opacity: 0.5, pointerEvents: 'none' }} disabled>
              No Interview Round
            </Button>
          )}
        </Grid>
          ...
          <>
            <Button
              ...
              onClick={() => {
                ...
                console.log(params.row.alldata);
              }}
            >
              {' '}
              <ContentCopyIcon />
          ...
          {params?.row?.modelName === 'Candidate' ? (
            <Link to={`/recruitment/viewresume/${params.row.id}/unassigned`} target="_blank">
              <Button
                sx={userStyle.buttonedit}
                // onClick={() => {
                //   navigate(`/recruitment/viewresume/${params.row.id}/unassigned`, {
                //     state: { withoutrole: true },
                //   })
                // }}
              >
