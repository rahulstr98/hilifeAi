...
function VisitorFollowupFilter() {
  ...
  let exportColumnNames = ['Company', 'Branch', 'Unit', 'Followup Date', "Visitor's ID", 'Visitor Name', 'Visitor Type', 'Visitor Mode', 'Visitor Purpose', 'Visitor Contact No', 'IN Time', 'OUT Time'];
  let exportRowValues = ['company', 'branch', 'unit', 'followupdate', 'visitorid', 'visitorname', 'visitortype', 'visitormode', 'visitorpurpose', 'visitorcontactnumber', 'intime', 'outtime'];
  ...
  const { isUserRoleCompare, pageName, isUserRoleAccess, setPageName, buttonStyles, isAssignBranch } = useContext(UserRoleAccessContext);
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: 'flex' }}>
          {isUserRoleCompare?.includes('vvisitorsfollowupfilter') && (
            <Link to={`/interactor/master/viewvisitors/${params.data.id}/followupfilter`} style={{ textDecoration: 'none', color: '#fff', minWidth: '0px' }}>
  ...
  const fetchVendor = async () => {
    ...
    try {
      ...
      const filterfollowup = res_vendor?.data?.visitors.filter((item) => item.followupaction === 'Required');
  ...
  const filterData = async () => {
    if (filterUser.fromdate === '') {
      ...
      const filtered = allDatas?.filter((item) => {
        ...
        return followupDate >= new Date(filterUser.fromdate) && followupDate <= new Date(filterUser.todate);
      });
  ...
  return (
    <Box>
      ...
      <PageHeading title="Visitors Followup Filter" modulename="Interactors" submodulename="Visitor" mainpagename="Visitors Followup Filter" subpagename="" subsubpagename="" />
      <br />
      {/* ****** Table Start ****** */}
        ...
        <>
          <Box sx={userStyle.container}>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                <Typography sx={userStyle.SubHeaderText}>Visitors Followup Filter List</Typography>
              </Grid>
            </Grid>
            <br />
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              >
                <Box>
                  {isUserRoleCompare?.includes('excelvisitorsfollowupfilter') && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes('printvisitorsfollowupfilter') && (
                    <>
                  ...
                  {isUserRoleCompare?.includes('imagevisitorsfollowupfilter') && (
                    <>
            ...
            <Grid container spacing={1}>
              ...
              <Grid item md={1} xs={12} sm={12}>
                <Typography>&nbsp;</Typography>
                <Button sx={buttonStyles.btncancel} onClick={handleClear} variant="contained">
                  Clear
                </Button>
              </Grid>
            </Grid>
