...
cbuildingapartmentname:singleCandidate?.cbuildingapartmentname || "",
      ...
      console.log(countryp, statep, cityp);
      console.log(State.getStatesOfCountry(countryp?.isoCode), 'State.getStatesOfCountry(countryp?.isoCode)');
      ...
      if (res?.data?.svisitors?.visitorid) {
        ...
        console.log(resVisitor?.data?.svisitordetailslog, 'resVisitor?.data?.svisitordetailslog');

        // Assign the files and use them
        file = resVisitor?.data?.svisitordetailslog?.slice(-1)[0]?.files;
        setRefImage(file);
        setRefImageedit(file);
      }
      ...
      setColor((prev) => {
        ...
        let availed = Array(blobedDatas.length).fill(overllsettings?.backgroundcolour || '#ffffff');
  ...
  useEffect(() => {
    const loadModels = async () => {
      ...
      console.log('FaceRecognitionNet loaded:', faceapi.nets.faceRecognitionNet.isLoaded); // ✅ Now correct
    };
    ...
    previewBase64 = await resizeImageKeepFormat(
      ...
      overllsettings?.height || image.height
                  ...
                  setColor((prev) => {
                    ...
                    if (availed.length > 0) {
                      availed.push(overllsettings?.backgroundcolour || '#ffffff');
                    } else {
                      // If no colors are present, create a new array with default colors
                      availed = Array(newSelectedFiles.length).fill(overllsettings?.backgroundcolour || '#ffffff');
                    }
...
cbuildingapartmentname: !vendor.samesprmnt ? String(vendor?.cbuildingapartmentname || '') : String(vendor?.pbuildingapartmentname || ''),
        ...
        exemployeeid: String(vendor?.exemployeeid || ""),
        intime: String(vendor.intime),
        visitorpurpose: String(vendor.visitorpurpose),
  ...
  const handleSubmit = async (e) => {
    ...
    if (vendor.company === 'Please Select Company' || vendor.company === '' || vendor.company == 'undefined') {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendor.meetingdetails === true && (vendor.meetingpersonemployeename === 'Please Select Employee Name' || vendor.meetingpersonemployeename === '')) {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (assignurl !== 'visitorenquiry' && vendor.meetingdetails === true && (vendor.meetinglocationarea === 'Please Select Area' || vendor.meetinglocationarea === '')) {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (!(visitorsTypeOption?.filter((item) => item?.interactorstype === vendor.visitortype && item?.interactorspurpose?.includes(vendor.visitorpurpose))[0]?.requestdocument || false) && (vendor.followupaction === 'Please Select Follow Up Action' || vendor.followupaction === '')) {
        ...
        <Box sx={userStyle.dialogbox}>
          <>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                {' '}
                <Typography sx={{ fontWeight: 'bold' }}>Edit Visitor</Typography>{' '}
              </Grid>
            </Grid>
            <br />
            <Grid container spacing={2}>
...
paddressthree:vendor?.paddressthree || "",
              ...
              {assignurl !== 'visitorenquiry' && (
                <>
                  <Grid item md={3} xs={12} sm={12}>
                    <FormGroup>
                      <FormControlLabel
                        control={<Checkbox checked={Boolean(vendor.meetingdetails)} />}
                        onChange={(e) =>
                          setVendor({
                            ...vendor,
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput id="component-outlined" type="text" value={vendor.detailsaddedy} placeholder="Please Enter AddedBy" readOnly />
                </FormControl>
              </Grid>
            </Grid>
            <br /> <br />
      ...
      <Dialog open={isWebcamOpen} onClose={webcamClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="sm" fullWidth={true}>
        ...
        >
          <Webcamimage name={name} getImg={getImg} setGetImg={setGetImg} valNum={valNum} setValNum={setValNum} capturedImages={capturedImages} setCapturedImages={setCapturedImages} setRefImage={setRefImage} setRefImageDrag={setRefImageDrag} overallsettings={overllsettings}/>
        </DialogContent>
        <DialogActions>
