...
function VisitorInformationApprovalCreate() {
  ...
  const [checkContactEmailForpopup, setCheckContactEmailForpopup] =
    useState(false);
  ...
  const getinfoCode = async () => {
    ...
    try {
      ...
      let res = await axios.get(
        `${SERVICE.SINGLE_VISITORINFORMATIONS}/${ids}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      setVendor({
        ...
        pgenerateviapincode:
          Boolean(singleCandidate?.pgenerateviapincode) || false,
        pvillageorcity: singleCandidate?.pvillageorcity || "",
        pdistrict: singleCandidate?.pdistrict || "",
        cgenerateviapincode:
          Boolean(singleCandidate?.cgenerateviapincode) || false,
      ...
      //permananet addresss
      if (
        singleCandidate?.pgenerateviapincode &&
        singleCandidate?.ppincode !== ""
      ) {
      ...
      if (
        singleCandidate?.cgenerateviapincode &&
        singleCandidate?.cpincode !== ""
      ) {
      ...
      const country = Country.getAllCountries().find(
        (country) => country.name === singleCandidate.ccountry
      );
      const state = State.getStatesOfCountry(country?.isoCode).find(
        (state) => state.name === singleCandidate.cstate
      );
      const city = City.getCitiesOfState(
        state?.countryCode,
        state?.isoCode
      ).find((city) => city.name === singleCandidate?.ccity);
      ...
      const countryp = Country.getAllCountries().find(
        (country) => country.name === singleCandidate.pcountry
      );
      const statep = State.getStatesOfCountry(countryp?.isoCode).find(
        (state) => state.name === singleCandidate.pstate
      );
      const cityp = City.getCitiesOfState(
        statep?.countryCode,
        statep?.isoCode
      ).find((city) => city.name === singleCandidate?.pcity);
      setSelectedCountryp(countryp);
      setSelectedStatep(statep);
      setSelectedCityp(cityp);

      const setPhoneEmailOpt = res_branch?.data?.branch?.filter(
        (item) => item?.name === res?.data?.svisitors?.branch
      );
      ...
      if (uploadBills?.length > 0) {
        setUploadBills([]);
        let multerUploadedFiles = res?.data?.svisitors?.visitordocument?.filter(
          (data) => !data.preview
        );
        let multerUploadedRequestFiles =
          res?.data?.svisitors?.requestdocument?.filter(
            (data) => !data.preview
          );
      ...
      if (res?.data?.svisitors?.visitorid) {
        ...
        let url = `${
          SERVICE.VISITORINFORMATIONDETAILS_SINGLE_PROFILE
        }/${encodeURIComponent(res?.data?.svisitors?.visitorid)}`;
      ...
      setColor((prev) => {
        ...
        let availed = Array(blobedDatas.length).fill(
          overllsettings?.backgroundcolour || "#ffffff"
        );
      ...
      //     allfloor
      //         ?.filter((u) => res?.data?.svisitors?.meetinglocationbranch?.includes(u.branch))
      //         .map((t) => ({
      //             ...t,
      //             label: t?.name,
      //             value: t?.name,
  ...
  useEffect(() => {
    ...
    capturedImages.forEach((item) => {
      ...
      newColors.push(overllsettings?.backgroundcolour || "#ffffff");
    });
  ...
  const fetchOverAllSettings = async () => {
    try {
      ...
      let lastobj =
        res?.data?.overallsettings?.length > 0
          ? res?.data?.overallsettings?.at(-1)
          : {};
  ...
  function handleChangeImage(e) {
    try {
      ...
      if (sizeInMB > (overllsettings?.filesize || 1)) {
        setPopupContentMalert(
          `Image size exceeds ${overllsettings?.filesize || 1} MB`
        );
        setPopupSeverityMalert("info");
        handleClickOpenPopupMalert();
        setBtnUpload(false);
        ...
        image.onload = async () => {
          try {
            ...
            const detections = await faceapi
              ...
              .withFaceLandmarks()
              .withFaceDescriptors();
            console.log("Completed face detection...");
              ...
              for (let i = 0; i < files?.length; i++) {
                ...
                if (file?.type.startsWith("image/")) {
                  ...
                  reader.onload = async () => {
                    ...
                    // ✅ Resize only if settings are provided
                    if (
                      overllsettings?.dimensionswidth ||
                      overllsettings?.height
                    ) {
                      previewBase64 = await resizeImageKeepFormat(
                        reader.result,
                        overllsettings?.dimensionswidth || image.width,
                    ...
                    newSelectedFiles.push({
                      ...
                      base64: previewBase64?.split(",")[1],
                    });
                    setRefImage(newSelectedFiles);
                    console.log(newSelectedFiles);
                    const base64Data = previewBase64?.split(",")[1]; // Get base64 data (without the prefix)
                    const binaryData = atob(base64Data); // Decode base64 data
                    const arrayBuffer = new ArrayBuffer(binaryData?.length);
                    const uint8Array = new Uint8Array(arrayBuffer);
                    // Fill the array buffer with the decoded binary data
            ...
            console.log(error, "error in upload inside");
            setIsLoading(false);
  ...
  const sendRequest = async (type, ticketid) => {
    ...
    try {
      ...
      let requestCheck =
        visitorsTypeOption?.filter(
          (item) =>
            item?.interactorstype === vendor.visitortype &&
            item?.interactorspurpose?.includes(vendor.visitorpurpose)
        )[0]?.requestdocument && vendor?.raiseticket;
      const jsonData = {
        ...
        ticketstatus: requestCheck && ticketid ? "Ticket Raised" : "",
        ...
        requestvisitorfollowupdate:
          visitorsTypeOption?.filter(
            (item) =>
              item?.interactorstype === vendor.visitortype &&
              item?.interactorspurpose?.includes(vendor.visitorpurpose)
          )[0]?.requestdocument || false
            ? String(vendor.requestvisitorfollowupdate)
            : "",
        ...
        followupdate: visitorsTypeOption?.filter(
          (item) =>
            item?.interactorstype === vendor.visitortype &&
            item?.interactorspurpose?.includes(vendor.visitorpurpose)
        )[0]?.requestdocument
        ...
        followuptime: visitorsTypeOption?.filter(
          (item) =>
            item?.interactorstype === vendor.visitortype &&
            item?.interactorspurpose?.includes(vendor.visitorpurpose)
        )[0]?.requestdocument
        ...
        followupaction: visitorsTypeOption?.filter(
          (item) =>
            item?.interactorstype === vendor.visitortype &&
            item?.interactorspurpose?.includes(vendor.visitorpurpose)
        )[0]?.requestdocument
        ...
        equipmentborrowed: String(
          path === "interactor/addvisitorin" ? "" : vendor.equipmentborrowed
        ),
        outtime: String(
          path === "interactor/addvisitorin" ? "" : vendor.outtime
        ),
        remark: String(path === "interactor/addvisitorin" ? "" : vendor.remark),
        visitorsurvey: String(
          path === "interactor/addvisitorin" ? "" : vendor.visitorsurvey
        ),
        ...
        followuparray: [
          {
            ...
            ccountry: !vendor.samesprmnt
              ...
              : String(
                  selectedCountryp?.name == undefined
                    ? ""
            ...
            requestvisitorfollowaction: requestCheck
              ? String(vendor.requestvisitorfollowaction)
              : "",
            requestfollowupactionupdate: String(
              vendor.requestfollowupactionupdate
            ),
            requestfollowupactionuptime: String(
        ...
        interactorstatus:
          visitorsTypeOption?.filter(
            (item) =>
              item?.interactorstype === vendor.visitortype &&
              item?.interactorspurpose?.includes(vendor.visitorpurpose)
          )[0]?.requestdocument && vendor.followupaction === "Required"
            ? String("requestfollowupvisitor")
            : String("visitor"),
        addedby: [
          {
      ...
      let addVisitorProfileDetail = await axios.post(
        SERVICE.VISITORDETAILS_LOG_CREATE,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      statusFormData.append("jsonData", JSON.stringify(statusUpdateData));

      let updateVisitorinformations = await axios.put(
        `${SERVICE.SINGLE_VISITORINFORMATIONS}/${ids}`,
        statusFormData,
        {
          visitorname: String(vendor.visitorname),
      ...
      if (type === "save1") {
        ...
        if (path === "interactor/addvisitorin") {
          ...
          setTimeout(() => {
            ...
            backPage("/interactor/master/visitorinformationapprovallist");
          }, 1000);
        }
      }
  ...
  const handleSubmit = async (e, type) => {
      ...
      (item) =>
        ...
        (!vendor.meetingdetails || // If meetingdetails is false, skip the following conditions
          ...
          (item.meetingpersoncompany.some((data) => compopt?.includes(data)) &&
            item.meetingpersonbranch.some((data) =>
              branchopt?.includes(data)
            ) &&
    ...
    if (checkContactEmailForpopup) {
      ...
      setCheckContactEmailMessage(
        `Update Contact Number & Email?\nContact Number: ${
      ...
      vendor.visitorcontactnumber === "" ||
      vendor.visitorcontactnumber === undefined
    ) {
      ...
      vendor.requestvisitorfollowupdate === ""
    ) {
      ...
      vendor.requestvisitorfollowaction ===
        "Please Select Request Follow Up Action"
    ) {
      ...
      vendor.materialcarryinginfo === true &&
  ...
  const handleCloseModEdit = async (e, reason) => {
    ...
    setVendor({
      ...
      cgenerateviapincode: !e.samesprmnt
        ? Boolean(e?.cgenerateviapincode || false)
        : Boolean(e?.pgenerateviapincode || false),
      cvillageorcity: !e.samesprmnt
        ? String(e?.cvillageorcity || "")
        : String(e?.pvillageorcity || ""),
      cdistrict: !e.samesprmnt
        ? String(e?.cdistrict || "")
        : String(e?.pdistrict || ""),
    });
    if (
      e?.pgenerateviapincode &&
      e?.ppincode !== "" &&
      e?.pcountry === "India"
    ) {
    ...
    let multerUploadedFiles = e?.visitordocument?.filter(
      (data) => !data.preview
    );
    setBillUploadedFiles(multerUploadedFiles);
  ...
  const handleCloseModEditProfileCheck = async (e, reason) => {
    ...
    // setRefImage(newimage)
    let url = `${
      SERVICE.VISITORDETAILS_LOG_SINGLE_PROFILE
    }/${encodeURIComponent(e?.visitorid)}`;
    let resVisitor = await axios.get(url, {
      headers: {
        Authorization: `Bearer ${auth.APIToken}`,
      },
    ...
    const file =
      resVisitor?.data?.svisitordetailslog?.slice(-1)[0]?.files || [];
    if (file?.length > 0) {
      // Create a Blob from the binary data
      ...
      const prof =
        e?.files || e?.uploadedimage?.some((item) => item?.preview === "")
          ? []
          : e?.uploadedimage || newimage || [];
    ...
    setVendor({
      ...
      visitorcompnayname:
        e?.visitorcompnayname === "undefined"
          ? ""
          : e?.visitorcompnayname || "",
      documenttype: e?.documenttype || "Please Select Document Type",
      // requestvisitorfollowaction: 'Please Select Request Follow Up Action',
      requestvisitorfollowaction: "Required",
      documentnumber:
        e?.documentnumber === "undefined" ? "" : e?.documentnumber || "",
      meetingdetails: e?.meetingdetails,
      meetingpersonemployeename:
        e?.meetingpersonemployeename || "Please Select Employee Name",
      meetinglocationarea: e?.meetinglocationarea || "Please Select Area",
      escortinformation: e?.escortinformation,
      escortdetails: e?.escortdetails,
      equipmentborrowed:
        e?.equipmentborrowed === "undefined" ? "" : e?.equipmentborrowed || "",
      outtime: "",
      ...
      followuptime: "",
      visitorbadge:
        e?.visitorbadge === "undefined" ? "" : e?.visitorbadge || "",
      visitorsurvey:
        e?.visitorsurvey === "undefined" ? "" : e?.visitorsurvey || "",
      uploadedimage: null,
    ...
    setSelectedCountryp(
      Country.getAllCountries().find((country) => country.name === e?.pcountry)
    );
    setSelectedStatep(
      State.getStatesOfCountry(
        Country.getAllCountries().find(
          (country) => country.name === e?.pcountry
        )?.isoCode
      ).find((state) => state.name === e?.pstate)
    );
    setSelectedCityp(
      City.getCitiesOfState(
        State.getStatesOfCountry(
        ...
        ).find((state) => state.name === e?.pstate)?.countryCode,
        State.getStatesOfCountry(
          Country.getAllCountries().find(
            (country) => country.name === e?.pcountry
          )?.isoCode
        ).find((state) => state.name === e?.pstate)?.isoCode
    ...
    let res = await axios.get(
      `${SERVICE.VISITORDETAILS_LOG_SINGLE}/${encodeURIComponent(
        e?.visitorid
      )}`,
      {
        headers: {
  ...
  const [isGettingVisitorUserCandidate, setIsGettingVisitorUserCandidate] =
    useState(false);
  ...
  return (
    <Box>
        ...
        <Box sx={userStyle.dialogbox}>
          <>
              ...
              {visitorCheck === "Existing Visitor" ? (
                <>
                  ...
                  <Grid item md={3} xs={12} sm={12}>
                    <Typography>&nbsp;</Typography>
                    <LoadingButton
                      loading={isGettingVisitorUserCandidate}
                      onClick={handlesubmitexistingvisitorCheck}
                      sx={buttonStyles.buttonsubmit}
                    >
                      Ok
                    </LoadingButton>
                    &nbsp; &nbsp;
                    <Button
                      onClick={handlesubmitexistingvisitorClose}
                    ...
                    onChange={(e) => {
                      ...
                      if (dueDays !== "") {
                        ...
                        followUpDate.setDate(
                          currentDate.getDate() + parseInt(dueDays)
                        );
                ...
                <FormControl fullWidth size="small">
                  ...
                  {visitorsTypeOption?.filter(
                    ...
                    <>
                      <Selects
                        maxMenuHeight={300}
                        options={exitEmployee?.filter(
                          (data) =>
                            data?.company === vendor.company &&
                            data?.branch === vendor.branch &&
                            data?.unit === vendor.unit
                        )}
                ...
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput
                    ...
                    onChange={(e) => {
                      setVendor({
                        ...vendor,
                        visitoremail: e.target.value?.toLowerCase(),
                      });
                    }}
                  />
                </FormControl>
                              ...
                              <StyledTableRow key={index}>
                                ...
                                >
                                  ...
                                  <a
                                    ...
                                    onClick={() =>
                                      renderFilePreviewMulter(file?.file)
                                    }
                                  >
                                    View
                                  </a>
                                ...
                                <StyledTableCell align="center">
                                  <Button
                                    onClick={() =>
                                      handleDeleteFileDocumentEditRequestDocument(
                                        index
                                      )
                                    }
                                    variant="contained"
                    ...
                    <FormControl fullWidth size="small">
                      ...
                      <OutlinedInput
                        ...
                        onChange={(e) => {
                          setVendor({
                            ...vendor,
                            requestvisitorfollowupdate: e.target.value,
                          });
                        }}
                      />
                    </FormControl>
                        ...
                        options={[
                          ...new Set(
                            filteredAreas
                              ?.filter(
                                (item) =>
                                  valueFloorLocationCat?.includes(item.floor) &&
            ...
            >
              ...
              <Grid item md={1} sm={2} xs={12}>
                <Link
                  to="/interactor/master/visitorinformationapprovallist"
                  style={{
                    textDecoration: "none",
                    color: "white",
                    float: "right",
      ...
      >
        ...
        >
          <Webcamimage
            ...
            overallsettings={overllsettings}
          />
        </DialogContent>
        <DialogActions>
            ...
            <>
              ...
              <Grid item md={12} sm={12} xs={12}>
                ...
                >
                  <Button
                    sx={buttonStyles.btncancel}
                    onClick={handleCloseModEditBack}
                  >
                    Back
                  </Button>
                </Grid>
      ...
      <Box>
        ...
        <Dialog
          open={isErrorOpenpop}
          onClose={handleCloseModEditProfileCheck}
          aria-labelledby="alert-dialog-title"
          aria-describedby="alert-dialog-description"
          maxWidth="lg"
          fullWidth={true}
      ...
      <Box>
        {/* Edit DIALOG */}
        <Dialog
          open={isErrorOpenpopdragdrop}
          onClose={handleCloseerrpopdragdrop}
          aria-labelledby="alert-dialog-title"
          aria-describedby="alert-dialog-description"
          maxWidth="lg"
          fullWidth={true}
            ...
            <>
              ...
              <Grid item md={12} sm={12} xs={12}>
                ...
                >
                  ...
                  <Button
                    sx={buttonStyles.btncancel}
                    onClick={handleCloseerrpopdragdrop}
                  >
                    Cancel
                  </Button>
                </Grid>
        ...
        >
          <Box sx={{ width: "350px" }}>
            ...
            <DialogContent>
              ...
              >
                <InfoOutlinedIcon
                  style={{ fontSize: "3.5rem", color: "teal" }}
                />
                <Typography
      ...
      <Box>
        <Dialog
          ...
          fullWidth={true}
        >
          <RaiseTicketRequestDocument
            handleCloseTicket={handleCloseTicket}
            buttonName={buttonName}
            sendRequest={sendRequest}
            attachmentFiles={uploadBillsRequestDocument}
          />
        </Dialog>
      </Box>
      <MessageAlert
...
export default VisitorInformationApprovalCreate;
