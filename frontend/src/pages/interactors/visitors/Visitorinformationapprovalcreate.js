...
cbuildingapartmentname: !vendor.samesprmnt ? String(vendor?.cbuildingapartmentname || '') : String(vendor?.pbuildingapartmentname || ''),
                ...
                interactorstatus: visitorsTypeOption?.filter((item) => item?.interactorstype === vendor.visitortype && item?.interactorspurpose?.includes(vendor.visitorpurpose))[0]?.requestdocument && vendor.followupaction === 'Required' ? String('requestfollowupvisitor') : String('visitor'),
                addedby: [
                    {
            ...
            let updateVisitorinformations = await axios.put(
                `${SERVICE.SINGLE_VISITORINFORMATIONS}/${ids}`, statusFormData,
                {
                    visitorname: String(vendor.visitorname),
                    visitorcontactnumber: String(vendor.visitorcontactnumber),
                    visitoremail: String(vendor.visitoremail),
    ...
    const handleSubmit = async (e, type) => {
        ...
        if (checkContactEmailForpopup) {
            ...
            (vendor.followupaction === 'Please Select Follow Up Action' || vendor.followupaction === '')
        ) {
