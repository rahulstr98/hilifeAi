...
function VisitorInformationApprovalList() {
  ...
  const { isUserRoleCompare, pageName, buttonStyles, allTeam, allUsersData, setPageName, isAssignBranch, isUserRoleAccess } = useContext(UserRoleAccessContext);
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String('Visitor Information Approval List'),
  ...
  const columnDataTable = [
    ...
    // },
    {
      ...
      headerClassName: 'bold-header',
      cellRenderer: (params) => {
        const status = params.value?.toLowerCase(); // safe lowercase comparison
        ...
        return (
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <Chip
              icon={isPending ? <HourglassEmpty /> : <CheckCircleOutline />}
              ...
              sx={{
                ...
                borderWidth: '2px',
                margin: '8px',
                backgroundColor: isApproved ? '#e8f5e9' : isPending ? '#fff8e1' : '#f5f5f5',
                '& .MuiChip-icon': {
                  color: isApproved ? '#2e7d32' : isPending ? '#ff8f00' : '#9e9e9e',
                },
              }}
            />
          </div>
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: 'flex' }}>
          {isUserRoleCompare?.includes('evisitorinformationapprovallist') && params.data.visitorinformationstatus === 'Pending' && (
            <Link to={`/interactor/master/visitorinformationapprovalcreate/${params.data.id}`}>
              <Button
                variant="contained"
                ...
                // onClick={() => { rowData(params?.data?.id) }}
              >
                Approve
              </Button>
            </Link>
          ...
          &ensp; &ensp;
          {isUserRoleCompare?.includes('vvisitorinformationapprovallist') && (
            <Link to={`/interactor/master/visitorinformationapprovalview/${params.data.id}`} style={{ textDecoration: 'none', color: '#fff', minWidth: '0px' }}>
  ...
  const fetchVendor = async () => {
    ...
    try {
      let res = await axios.post(SERVICE.SKIPPED_VISITORINFORMATIONS_APPROVALS, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      ...
      // const filteredAns = ans.filter(item => Array.isArray(item.branchforwardlog) && item.branchforwardlog.length > 0);
      const filteredAns = ans;

      const itemsWithSerialNumber = filteredAns?.map((item, index) => ({
        ...item,
      ...
      setVendormaster(
        itemsWithSerialNumber.map((item, index) => {
          ...
          return {
            ...
            requestvisitorfollowupstatus: item.requestvisitorfollowupstatus,
            visitorinformationstatus: item.visitorinformationstatus,
            isBtnEnable: !mgTypePurpose?.find((itemmg) => itemmg?.interactorstype?.toLowerCase() === item?.visitortype?.toLowerCase() && itemmg?.interactorspurpose?.some((purpose) => purpose?.toLowerCase() === item?.visitorpurpose?.toLowerCase()))?.addcandidate || false,
            files: item.files,
            addvisitorin: item.addvisitorin,
          };
        })
      ...
      setTotalProjects(filteredAns?.length > 0 ? res?.data?.totalProjects : 0);
      // setTotalProjects(filteredAns.length > 0 ? filteredAns.length : 0);
      // setTotalPages(filteredAns.length > 0 ? 1 : 0);
      setTotalPages(filteredAns?.length > 0 ? res?.data?.totalPages : 0);
      setPageSize((data) => {
        return filteredAns.length > 0 ? data : 10;
      });
      setPage((data) => {
        return filteredAns.length > 0 ? data : 1;
      });
  ...
  const fetchVendorEmployee = async () => {
    ...
    try {
      let res = await axios.post(SERVICE.SKIPPED_VISITORINFORMATIONS_EXPORTS, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      ...
      const datas = itemsWithSerialNumberOverall.map((item, index) => {
        return {
          ...
          visitorinformationstatus: item?.visitorinformationstatus,
          interactorstatus: item.interactorstatus,
          isBtnEnable: !mgTypePurpose?.find((itemmg) => itemmg?.interactorstype?.toLowerCase() === item?.visitortype?.toLowerCase() && itemmg?.interactorspurpose?.some((purpose) => purpose?.toLowerCase() === item?.visitorpurpose?.toLowerCase()))?.addcandidate || false, // Default to `false` if no match is found
  ...
  //get all  vendordetails.
  const handleResetSearchh = async () => {
    setVendorcheck(true);
    setPageName(!pageName);
    ...
    try {
      let res = await axios.post(SERVICE.SKIPPED_VISITORS_BRANCHFORWARDED, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      ...
      setAllvisitorAllData(
        res?.data?.totalProjectsAllData?.map((item, index) => {
          const IsAvaile = allCandidate?.some((data) => data.firstname?.toLowerCase() + data.lastname?.toLowerCase() === String(item.visitorname?.replace(/\s+/g, '')?.toLowerCase()));
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage
        ...
        .then((blob) => {
          saveAs(blob, 'Visitor Information Approval List.png');
        })
        .catch((error) => {
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: 'Visitor Information Approval List',
    pageStyle: 'print',
  });
  ...
  return (
    <Box>
      <Headtitle title={'Visitor Information Approval List'} />
      <PageHeading title="Visitor Information Approval List" modulename="Interactors" submodulename="Visitor" mainpagename="Visitor Information Approval List" subpagename="" subsubpagename="" />
      {/* ****** Table Start ****** */}
      <Box sx={userStyle.selectcontainer}>
        <>
          <Grid item xs={8}>
            <Typography sx={userStyle.SubHeaderText}>Filter Visitors</Typography>
          </Grid>
          <br />
      ...
      <br />
      {isUserRoleCompare?.includes('lvisitorinformationapprovallist') && (
        <>
          <Box sx={userStyle.container}>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                <Typography sx={userStyle.SubHeaderText}>Visitor Information Approval List</Typography>
              </Grid>
            </Grid>
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              >
                <Box>
                  {isUserRoleCompare?.includes('excelvisitorinformationapprovallist') && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes('csvvisitorinformationapprovallist') && (
                    <>
                      <Button
                        onClick={(e) => {
                          setIsFilterOpen(true);
                  ...
                  {isUserRoleCompare?.includes('printvisitorinformationapprovallist') && (
                    <>
                  ...
                  {isUserRoleCompare?.includes('pdfvisitorinformationapprovallist') && (
                    <>
                      <Button
                        sx={userStyle.buttongrp}
                        onClick={() => {
                  ...
                  {isUserRoleCompare?.includes('imagevisitorinformationapprovallist') && (
                    <>
      ...
      <ExportData
        ...
        // itemsTwo={AllvisitorAllData ?? []}
        filename={'Visitor Information Approval List'}
        exportColumnNames={exportColumnNames}
        exportRowValues={exportRowValues}
        componentRef={componentRef}
      />
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Visitor Information Approval List Info" addedby={addedby} updateby={updateby} />
...
export default VisitorInformationApprovalList;
