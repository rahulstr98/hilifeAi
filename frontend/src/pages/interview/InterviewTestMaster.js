...
function InterviewTestMaster() {
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage.toBlob(gridRefTableImg.current)
        .then((blob) => {
          saveAs(blob, "InterviewTestMaster.png");
        })
        .catch((error) => {
    ...
    try {
      if (Checkpointticketsid) {
        ...
        if (overallcheck?.data?.mayidelete) {
          await axios.delete(
            `${SERVICE.INTERVIEWTESTMASTER_SINGLE}/${Checkpointticketsid}`,
            {
              headers: {
                Authorization: `Bearer ${auth.APIToken}`,
              },
  ...
  const fetchInterviewOrders = async () => {
    ...
    try {
      ...
      setInterviewgroupingall(res_vendor?.data?.interviewtestmasters?.map((item, index) => ({
        ...item,
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Interview Test Master"),
  ...
  return (
    <Box>
      ...
      <ExportData
        ...
        filename={"Interview Test Master"}
      ...
      <InfoPopup
        ...
        heading="Interview Test Master Info"
        addedby={addedby}
        updateby={updateby}
      />
