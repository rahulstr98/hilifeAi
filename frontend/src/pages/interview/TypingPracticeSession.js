...
function TypingPracticeSession() {
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Practice Session"),
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "PracticeSession.png");
                })
                .catch((error) => {
...

        {
            ...
            cellRenderer: (params) => (
                <Grid sx={{ display: "flex" }}>
                    {isUserRoleCompare?.includes("vpracticesession") && (
                        <Button
                    ...
                    <Button
                        ...
                        size="small"
                        onClick={() => {
                            const url = `${BASE_URL}/practicesession/${params?.data?.id}/${isUserRoleAccess?._id}`;
                    ...
                    {isUserRoleCompare?.includes("vpracticesession") && params?.data?.responseexist && (
                        <Button
                            ...
                            onClick={() => {
                                window.open(`/individualtypingresponse/${params?.data?.id}`);
                            }}
                        >
    ...
    const fetchReturnData = async () => {
        ...
        try {
            ...
            let response = await axios.post(
                SERVICE.INDIVIDUAL_TYPING_PRACTICE_SESSION,
                {
            ...
            const itemsWithSerialNumber = response?.data?.individualPracticeSession?.map((item, index) => {
    ...
    return (
        <Box>
            <NotificationContainer />
            <Headtitle title={"PRACTICE SESSION"} />
            {/* ****** Header Content ****** */}
            <PageHeading
                title="Practice Session"
                modulename="Typing Practice"
                submodulename="Practice Session"
                mainpagename=""
                subpagename=""
                subsubpagename=""
            />
            ...
            {tableLoader ? (
                ...
                <>
                    {isUserRoleCompare?.includes("lpracticesession") && (
                        <>
                            <Box sx={userStyle.container}>
                                {/* ******************************************************EXPORT Buttons****************************************************** */}
                ...
                <Box sx={{ padding: "20px 50px" }}>
                    <>
                        <Typography sx={userStyle.HeaderText}>
                            {" "}
                            View Practice Session Questions
                        </Typography>
                        <br />
                        <Grid container spacing={2}>
...
export default TypingPracticeSession;