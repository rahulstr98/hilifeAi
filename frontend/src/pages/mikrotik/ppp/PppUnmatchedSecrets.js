...
function PppUnmatchedSecrets({ filterFinal, setFilterFinal, filterDatas, setFilterDatas }) {
    ...
    const [unmatchedSecrets, setUnmatchedSecrets] = useState([]);
    ...
    const handleRemoteAddress = (value) => {
        ...
        if (ipPoolsFilter) {
            const secretPPPValues = teamsArray?.filter(data => data?.profile === value?.value)?.map(data => data["remote-address"]);
    ...
    const handleRemoteAddressEdit = (value, remote) => {
        ...
        if (ipPoolsFilter) {
            const secretPPPValues = unmatchedSecrets?.filter(data => data?.profile === value?.value)?.map(data => data["remote-address"]);
    ...
    //useEffect
    useEffect(() => {
        addSerialNumber(unmatchedSecrets);
    }, [unmatchedSecrets]);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        const isNameMatch = teamsArray?.some(
            (item) =>
        ...
        else if (assetVariant.name === "Please Select Name" || assetVariant.name === "" || assetVariant.name === undefined) {
        ...
        try {
            await axios.post(
                SERVICE.CREATE_MIKROTIK_SECRET,
                {
                    ...
                    mikrotikname: String(assetVariant.name),
                    ...
                    adminusername: String(assetVariant.adminusername),
                    adminpassword: String(assetVariant.adminpassword),
            ...
            setRemoteAddress([])
            setAutoGenPassword("")
            fetchMikroTikProfiles(assetVariant?.url,
                assetVariant?.adminusername,
                assetVariant?.adminpassword,)
            await fetchMikroTikSecrets(assetVariant?.url,
                assetVariant?.adminusername,
                assetVariant?.adminpassword)
    ...
    const sendEditRequest = async () => {
        ...
        try {
            ...
            if (assetVariantEdit.name !== filterFinal?.name) {
                await axios.post(
                    SERVICE.CREATE_MIKROTIK_SECRET,
                    {
                ...
                // Deleting the Existing Data in Secret List
                await axios.post(
                    `${SERVICE.DELETE_MIKROTIK_SECRET}`, {
                    id: assetVariantEdit?.id,
                    url: filterFinal?.url,
                    username: filterFinal?.username,
                    password: filterFinal?.password,
                },
    ...
    const editSubmit = (e) => {
        e.preventDefault();
        const isNameMatch = teamsArray?.filter((item) => item.id !== assetVariantEdit.id)?.some(
            (item) =>
    ...
    const fetchMikroTikSecrets = async (url,
        ...
        try {
            ...
            let unMatchedDatas = response?.data?.unmatchedNames;
            ...
            setTeamsArray(items);
            setUnmatchedSecrets(unMatchedDatas);
            setLoader(false);
        } catch (err) {
            setLoader(false);
            console.log(err);
    ...
    const fetchMikroTikSecretsEdit = async (url,
        username,
        password) => {
        setPageName(!pageName);
        try {
    ...
    useEffect(() => {
        if (filterFinal.company) {
            fetchMikroTikSecrets(filterFinal?.url,
                filterFinal?.username,
                filterFinal?.password)
    ...
    const fetchMikroTikIpPools = async (url,
        username,
        password) => {
        setPageName(!pageName);
        try {
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "Mikrotik Unmatched PPP Secrets.png");
                });
            });
        }
    };
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "Mikrotik Unmatched PPP Secrets",
        pageStyle: "print",
    });
                    ...
                    {isUserRoleCompare?.includes("esecrets") && (
                        <>
                            <Button
                                ...
                                onClick={() => {
                                    getCode(params.row)
                                    setEmployeeNameEdit(params.row?.employeename)
                                    setSelectedTeamOptionsEdit(params.row?.team?.map(data => ({
                                        value: data,
                                        label: data,
                                    })))
                                    setValueTeamEdit(params.row?.team)
                                    ...
                                    setAssetVariantEdit({
                                        ...
                                        unit: filterDatas?.unit,
                                        name: filterDatas?.name,
                                        password: params.row?.temppassword,
                                        livepassword: params.row?.password,
    ...
    return (
        <Box>
                            ...
                            <Grid container spacing={2}>
                                <Grid item xs={10}>
                                    <Typography sx={userStyle.importheadtext}>
                                        List Mikrotik Unmatched PPP Secrets
                                    </Typography>
                                </Grid>
                            </Grid>
                            <br />
                                ...
                                <Grid item md={2} xs={12} sm={12}>
                                    <Box>
                                        ...
                                        >
                                            ...
                                            <MenuItem value={unmatchedSecrets?.length}>
                                                All
                                            </MenuItem>
                                        </Select>
                                    </Box>
                        ...
                        <>
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item md={4} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        <Typography>
                                            Employee Name<b style={{ color: "red" }}>*</b></Typography>
                                        <FormControl fullWidth size="small">
                                                ...
                                                options={allUsersData?.filter(
                                                    (comp) =>
                                                        ...
                                                        assetVariantEdit.unit === comp.unit && selectedTeamOptionsEdit
                                                            .map((item) => item.value)
                                                            .includes(comp.team) && !teamsArray?.filter(data => data?.id !== assetVariantEdit?.id)
                                                                .map((item) => item.name)
            ...
            <ExportData
                ...
                itemsTwo={items ?? []}
                filename={"Mikrotik Unmatched PPP Secrets"}
            ...
            <InfoPopup
                ...
                heading="Mikrotik Unmatched PPP Secrets Info"
...
export default PppUnmatchedSecrets;