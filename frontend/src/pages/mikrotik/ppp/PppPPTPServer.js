...
function PppPPTPServer() {
    ...
    const handleFilter = (e) => {
        ...
        if (filterDatas.company === "Please Select Company") {
            ...
            fetchMikroTikPPTPServer(filterDatas?.url,
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Mikrotik PPP PPTP Server"),
    ...
    const sendRequest = async () => {
        ...
        try {
            ...
            await fetchMikroTikPPTPServer(filterFinal?.url,
    ...
    //get all Asset Variant name.
    const fetchMikroTikPPTPServer = async (url,
        username,
        password) => {
        setPageName(!pageName);
        try {
            setLoader(true);
            let response = await axios.post(SERVICE.GET_MIKROTIK_PPTPSERVER, {
            ...
            let datas = response?.data?.pptpserver
    ...
    return (
        <Box>
            <Headtitle title={"MIKROTIK PPP PPTP SERVER"} />
            {/* ****** Header Content ****** */}
            <PageHeading
                title="Mikrotik PPP PPTP Server"
                modulename="Mikrotik"
                submodulename="PPP"
                mainpagename="PPTP Server"
                subpagename=""
                subsubpagename=""
            />
            <>
                {isUserRoleCompare?.includes("lpptpserver") && (
                    <Box sx={userStyle.selectcontainer}>
                        <>
                            <Grid container spacing={2}>
                                <Grid item xs={8}>
                                    <Typography sx={userStyle.importheadtext}>
                                        Filter Mikrotik PPP PPTP Server
                                    </Typography>
                                </Grid>
                            </Grid>
                            <br />
