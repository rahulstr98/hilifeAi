...
function IpPoolUsedAddresses() {
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Mikrotik IP Pool Used Addresses"),
    ...
    const editSubmit = (e) => {
        ...
        const isNameMatch = items?.filter((item) => item.id !== assetVariantEdit.id)?.some(
            (item) =>
                ...
                item.localaddress === assetVariantEdit.localaddress &&
                item.remoteaddress === assetVariantEdit.remoteaddress
        );
        if (isNameMatch) {
            setPopupContentMalert("Data Already Exist!");
            setPopupSeverityMalert("info");
        ...
        else if (assetVariantEdit.profile === "") {
            setPopupContentMalert("Please Select Profile!");
            setPopupSeverityMalert("info");
            handleClickOpenPopupMalert();
        }
    ...
    const fetchMikroTikSecrets = async (url,
        ...
        try {
            setLoader(true);
            let response = await axios.post(SERVICE.GET_MIKROTIK_IPPOOL_USEDADDRESSES, {
                url, username, password
            }, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
            ...
            let datas =
                response?.data?.ippoolusedaddresses
            setTeamsArray(datas);
            setLoader(false);
        } catch (err) {
            setLoader(false);
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "Mikrotik IP Pool Used Addresses.png");
                });
            });
        }
    };
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "Mikrotik IP Pool Used Addresses",
        pageStyle: "print",
    });
...

        {
            ...
            renderCell: (params) => (
                <Grid sx={{ display: "flex" }}>
                    {/* {isUserRoleCompare?.includes("eippoolusedaddresses") && (
                        <>
                            <Button
                    ...
                    {isUserRoleCompare?.includes("dippoolusedaddresses") && (
                        <>
                            <Button
                    ...
                    {isUserRoleCompare?.includes("vippoolusedaddresses") && (
                        <Button
                            sx={userStyle.buttonedit}
                            onClick={() => {
                    ...
                    {/* {isUserRoleCompare?.includes("iippoolusedaddresses") && (params?.row?.addedby?.length > 0 || params?.row?.updatedby?.length > 0) && (
                        <Button
                            size="small"
    ...
    return (
        <Box>
            <Headtitle title={"MIKROTIK IP POOL USED ADDRESSES"} />
            {/* ****** Header Content ****** */}
            <PageHeading
                title="Mikrotik IP Pool Used Addresses"
                modulename="Mikrotik"
                submodulename="IP"
                mainpagename="Pool"
                subpagename="IP Pool Used Addresses"
                subsubpagename=""
            />
            <>
                {isUserRoleCompare?.includes("lippoolusedaddresses") && (
                    <Box sx={userStyle.selectcontainer}>
                        <>
                            <Grid container spacing={2}>
                                <Grid item xs={8}>
                                    <Typography sx={userStyle.importheadtext}>
                                        Filter Mikrotik IP Pool Used Addresses
                                    </Typography>
                                </Grid>
                            </Grid>
                            <br />
                    ...
                    {loader ? (
                        ...
                        <Box sx={userStyle.container}>
                            ...
                            <Grid container spacing={2}>
                                <Grid item xs={12}>
                                    <Typography sx={userStyle.importheadtext}>
                                        List Mikrotik IP Pool Used Addresses
                                    </Typography>
                                </Grid>
                                {/* <Grid item xs={2}>
                                    {isUserRoleCompare?.includes("aippoolusedaddresses") && filterApplied && (
                                        <>
                            ...
                            <Grid container spacing={2} style={userStyle.dataTablestyle}>
                                ...
                                >
                                    <Box>
                                        {isUserRoleCompare?.includes("excelippoolusedaddresses") && (
                                            <>
                                                <Button
                                                    onClick={(e) => {
                                        ...
                                        {isUserRoleCompare?.includes("csvippoolusedaddresses") && (
                                            <>
                                                <Button
                                                    onClick={(e) => {
                                                        setIsFilterOpen(true);
                                        ...
                                        {isUserRoleCompare?.includes("printippoolusedaddresses") && (
                                            <>
                                        ...
                                        {isUserRoleCompare?.includes("pdfippoolusedaddresses") && (
                                            <>
                                                <Button
                                                    sx={userStyle.buttongrp}
                                                    onClick={() => {
                                        ...
                                        {isUserRoleCompare?.includes("imageippoolusedaddresses") && (
                                            <Button
                                                sx={userStyle.buttongrp}
                ...
                <Box sx={{ padding: "20px 50px" }}>
                    <>
                        <Typography sx={userStyle.HeaderText}>
                            {" "}
                            View Mikrotik IP Pool Used Addresses
                        </Typography>
                        <br /> <br />
                        <Grid container spacing={2}>
                            <Grid item md={6} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    ...
                                    <Typography>{singleRow.pool}</Typography>
                                </FormControl>
                            </Grid>
                            <Grid item md={6} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                            ...
                            <Grid item md={6} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    ...
                                    <Typography>{singleRow.owner}</Typography>
                                </FormControl>
                            </Grid>
                            <Grid item md={6} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    <Typography variant="h6">Info</Typography>
                                    <Typography>{singleRow.info}</Typography>
                                </FormControl>
                            </Grid>
                ...
                >
                    <Box sx={{ padding: "20px 50px" }}>
                        <>
                            <Grid container spacing={2}>
                                <Typography sx={userStyle.HeaderText}>
                                    Add Mikrotik IP Pool Used Addresses
                                </Typography>
                            </Grid>
                            <br />
                            <Grid container spacing={2}>
            ...
            <ExportData
                ...
                itemsTwo={items ?? []}
                filename={"Mikrotik IP Pool Used Addresses"}
            ...
            <InfoPopup
                ...
                heading="Mikrotik IP Pool Used Addresses Info"
...
export default IpPoolUsedAddresses;
