...
const PaymentDueRemainderLogList = ({ ExistingVisitors, handleCloseModEdit }) => {
    ...
    let exportColumnNames = ["Mode Of Payment", "Payment Date", "Paid Amount", "Balance"];
    let exportRowValues = ['modeofpayments', 'payamountdate', 'payamount', 'balanceamount']
    ...
    const fetchTaskcategory = async () => {
        ...
        try {
            ...
            setTaskcategorys(ExistingVisitors?.map((item) => ({
                ...item,
                payamountdate: moment(item.payamountdate).format("DD-MM-YYYY")
            })));
