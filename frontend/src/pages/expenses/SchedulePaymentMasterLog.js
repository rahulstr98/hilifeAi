...
function SchedulePaymentMasterLog() {
  ...
  const { isUserRoleCompare, isUserRoleAccess, pageName, setPageName, buttonStyles, isAssignBranch } =
  ...
  const fetchAllPayments = async () => {
    ...
    try {
      ...
      setpayments(res_grp?.data?.sschedulepaymentmaster?.statuslog?.map((data, index) => ({
        ...data,
        serialNumber: index + 1,
        daywiseandweeklydays: data.daywiseandweeklydays
          ?.map((t, i) => `${i + 1 + ". "}` + t)
          .toString(),
        changedat: moment(data?.createdAt).format("DD-MM-YYYY hh:mm:ss A"),
      })))?.filter((item) =>
        accessbranch.some(
          (branch) =>
  ...
  return (
    <Box>
      <Grid container spacing={2}>
        <PageHeading
          title="Schedule Payment Master Log"
          modulename="Expenses"
