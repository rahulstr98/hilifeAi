...
import {
  Box, Typography, OutlinedInput, TableBody, TableRow, TableCell, Select, Paper, MenuItem, Dialog, DialogContent, DialogActions,
  FormControl, Grid, Table, TableHead, TableContainer, Button, List, ListItem, ListItemText, Popover, Checkbox, TextField,
  IconButton,
} from "@mui/material";
...
function Connectivity() {
  ...
  const [connectivity, setConnectivity] = useState({ name: "" });
  const [connectivityEdit, setConnectivityEdit] = useState({ name: "" });
  ...
  const rowData = async (id, name) => {
    ...
    try {
      let res = await axios.get(`${SERVICE.SINGLE_CONNECTIVITY}/${id}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setDeleteAssetType(res?.data?.sconnectivity);
      handleClickOpen();
    } catch (err) {
  ...
  const deleteFucntion = async () => {
    ...
    setPageName(!pageName)
    try {
      await axios.delete(`${SERVICE.SINGLE_CONNECTIVITY}/${delid}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  const sendRequest = async () => {
    ...
    setIsBtn(true);
    try {
      let freqCreate = await axios.post(SERVICE.CREATE_CONNECTIVITY, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      ...
      setConnectivity(freqCreate.data);
      await fetchConnectivity();
      setConnectivity({ name: "" });
  ...
  const handleSubmit = (e) => {
    ...
    const isNameMatch = connectivityArray?.some(
      (item) =>
        ...
        connectivity.name?.trim().toLowerCase()
    );
  ...
  const getCode = async (e) => {
    ...
    setPageName(!pageName)
    try {
      let res = await axios.get(`${SERVICE.SINGLE_CONNECTIVITY}/${e}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setConnectivityEdit(res?.data?.sconnectivity);
      handleClickOpenEdit();
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  ...
  const sendEditRequest = async () => {
    ...
    setPageName(!pageName)
    try {
      let res = await axios.put(`${SERVICE.SINGLE_CONNECTIVITY}/${updateid}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        name: String(connectivityEdit.name),
        updatedby: [
          ...updateby,
          {
            name: String(isUserRoleAccess.companyname),
  ...
  const editSubmit = (e) => {
    ...
    const isNameMatch = allConnectivityEdit?.some(
      (item) =>
        item.name?.trim().toLowerCase() ===
        connectivityEdit.name?.trim().toLowerCase()
    );
    if (connectivityEdit.name === "") {
      setPopupContentMalert("Please Enter Name!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (isNameMatch) {
  ...
  const fetchConnectivity = async () => {
    ...
    setPageName(!pageName)
    try {
      let res_freq = await axios.get(SERVICE.ALL_CONNECTIVITY, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setLoader(true);
      setConnectivityArray(res_freq?.data?.connectivity);
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  const bulkdeletefunction = async () => {
    ...
    try {
      const deletePromises = selectedRows?.map((item) => {
        return axios.delete(`${SERVICE.SINGLE_CONNECTIVITY}/${item}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        });
  ...
  const fetchConnectivityAll = async () => {
    ...
    try {
      ...
      setAllConnectivityEdit(
        res_freq?.data?.connectivity.filter(
          (item) => item._id !== connectivityEdit._id
        )
      );
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
  ...
  return (
    <Box>
      <Headtitle title={"CONNECTIVITY"} />
      {/* ****** Header Content ****** */}
      {/* <Typography sx={userStyle.HeaderText}>Connectivity</Typography> */}
      <PageHeading
        title="Connectivity"
        modulename="Asset"
        submodulename="Asset Specifications"
        mainpagename="Connectivity"
        subpagename=""
        subsubpagename=""
      />
            ...
            <Grid item md={2} xs={12} sm={12}>
              <Box>
                ...
                >
                  ...
                  {/* <MenuItem value={connectivityArray?.length}>
                        All
                      </MenuItem> */}
                </Select>
              </Box>
            ...
            <Grid container spacing={2}>
              <Grid item md={12} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>{connectivityEdit.name}</Typography>
                </FormControl>
              </Grid>
            </Grid>
            <br /> <br /> <br />
