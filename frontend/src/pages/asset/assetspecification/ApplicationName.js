...
function ApplicationName() {
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Application Name"),
  ...
  const [applicationNameEdit, setApplicationNameEdit] = useState({ name: "" });
  ...
  const rowData = async (id, name) => {
    ...
    try {
      let res = await axios.get(`${SERVICE.SINGLE_APPLICATIONNAME}/${id}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setDeleteApplicationName(res?.data?.sapplicationname);
      handleClickOpen();
    } catch (err) {
  ...
  const delAsset = async () => {
    setPageName(!pageName)
    try {
      await axios.delete(`${SERVICE.SINGLE_APPLICATIONNAME}/${assetid}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  const sendRequest = async () => {
    ...
    setIsBtn(true);
    try {
      let freqCreate = await axios.post(SERVICE.CREATE_APPLICATIONNAME, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  const getCode = async (e) => {
    setPageName(!pageName)
    try {
      let res = await axios.get(`${SERVICE.SINGLE_APPLICATIONNAME}/${e}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setApplicationNameEdit(res?.data?.sapplicationname);
      handleClickOpenEdit();
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  ...
  const sendEditRequest = async () => {
    ...
    try {
      let res = await axios.put(
        `${SERVICE.SINGLE_APPLICATIONNAME}/${frequencyId}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const fetchApplicationName = async () => {
    setPageName(!pageName)
    try {
      let res_freq = await axios.get(SERVICE.ALL_APPLICATIONNAME, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setLoader(true);
      setApplicationNameArray(res_freq?.data?.applicationname);
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  const bulkDeleteFunction = async () => {
    ...
    try {
      const deletePromises = selectedRows?.map((item) => {
        return axios.delete(`${SERVICE.SINGLE_APPLICATIONNAME}/${item}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        });
  ...
  const fetchApplicationNameAll = async () => {
    ...
    try {
      ...
      setAllApplicationNameEdit(
        res_freq?.data?.applicationname.filter(
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Application Name.png");
        });
      });
    }
  };
