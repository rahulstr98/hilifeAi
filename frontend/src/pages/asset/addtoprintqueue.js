...
import {
  Box, Button, Checkbox, Dialog, DialogActions, DialogContent, DialogContentText, FormControl, Grid, IconButton,
  InputLabel, List, ListItem, ListItemText, MenuItem, OutlinedInput, Popover, Select, TextField, Typography,
} from "@mui/material";
...
function AddtoPrintQueue() {
  ...
  let exportColumnNames = ["Status", "Company", "Branch", "Unit", "Floor", "Area", "Location", "WorkStation", "Asset Type", "Asset",
    "Material", "Component", "Material Code", "Countquantity", "Rate", "Warranty", "Purchasedate", "Vendor",];
  let exportRowValues = ["status", "company", "branch", "unit", "floor", "area", "location", "workstation", "assettype",
    "asset", "material", "component", "code", "countquantity", "rate", "warranty", "purchasedate", "vendor",];
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Queue Label List"),
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "AddToPrintQueue.png");
        });
      });
    }
  };
  ...
  const fetchAssetDetails = async () => {
    ...
    try {
      ...
      // let res_vendor = await axios.get(SERVICE.ADDTOPRINTQUEUE_LIMIT, {
      let res_vendor = await axios.post(SERVICE.ADDTOPRINT_QUEUE_LIMITED_ACCESS, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      ...
      // Usage
      let uniqueArray = removeDuplicates(res_vendor.data.addtoprintqueue, [
        "company",
        "branch",
        "unit",
        "floor",
  ...
  const [checkboxprint, setCheckboxprint] = useState([]);
  const checkValue = (
    id,
    company,
    branch,
        ...
        <Checkbox
          ...
          onChange={() => {
            ...
            checkValue(
              ...
              params.row.status,
              params.row.workstation,
              params.row.asset,
              params.row.assettype,
              params.row.material,
              params.row.countquantity,
              params.row.rate,
              params.row.warranty,
  ...
  const sendRequestToAddToPrintQueue = async () => {
    ...
    try {
      // const snnoValues = selectedItems.map((item) => item.snno);
  ...
  return (
    <Box>
      <Headtitle title={"Queue Label List"} />
      <PageHeading
        title="Add To Print Queue List"
        modulename="Asset"
        submodulename="Stock Label"
        mainpagename="Queue Label List"
        subpagename=""
        subsubpagename=""
      />
      <br />
      ...
      {/* ****** Table Start ****** */}
      {isUserRoleCompare?.includes("lqueuelabellist") && (
        <>
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>
                View All Print Queues
              </Typography>
            </Grid>
            <br />
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              >
                <Box>
                  {isUserRoleCompare?.includes("excelqueuelabellist") && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes("csvqueuelabellist") && (
                    <>
                      <Button
                        onClick={(e) => {
                          setIsFilterOpen(true);
                  ...
                  {isUserRoleCompare?.includes("printqueuelabellist") && (
                    <>
                  ...
                  {isUserRoleCompare?.includes("pdfqueuelabellist") && (
                    <>
                      <Button
                        sx={userStyle.buttongrp}
                        onClick={() => {
                  ...
                  {isUserRoleCompare?.includes("imagequeuelabellist") && (
                    <Button
                      sx={userStyle.buttongrp}
...
export default AddtoPrintQueue;