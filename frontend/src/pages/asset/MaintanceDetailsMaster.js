...
import MaintenanceDetailsMasterIndividual from './MaintenanceDetailsMasterIndividuallist.js';


function MaintenanceDetailsMaster() {
    ...
    const [selectedOptionsComponentEdit, setSelectedOptionsComponentEdit] = useState([]);
    let [valueComponentCatEdit, setValueComponentCatEdit] = useState([]);

    const handleComponentChangeEdit = (options) => {
        setValueComponentCatEdit(
            options.map((a, index) => {
                return a.value;
            })
        );
        setSelectedOptionsComponentEdit(options);
    };

    const customValueRendererComponentEdit = (valueComponentCat, _categoryname) => {
        return valueComponentCat?.length
            ? valueComponentCat.map(({ label }) => label)?.join(", ")
            : "Please Select Asset Material";
    };
    ...
    const [maintenanceDetailsMaster, setMaintenanceDetailsMaster] = useState({
    ...
    console.log(maintenanceDetailsMasterEdit, "maintenanceDetailsMasterEdit")
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, 'Maintenance Details Master Group.png');
                });
            });
        }
    };
    ...
    const rowDataGroup = async (id, groupid) => {
        setPageName(!pageName);
        try {
            let res = await axios.post(`${SERVICE.MAINTENANCEDETAILSMASTER_SINGLE_GROUP_DELETE}`, {
                groupids: dataForDelete
            }, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
    ...
    const [forRefresh, steForRefresh] = useState(false)
    //add function 
    const sendRequest = async () => {
        ...
        try {
            await Promise.all(
                selectedOptionsComponent.map(async (domainItem) => {
                    let subprojectscreate = await axios.post(SERVICE.MAINTENANCEDETAILSMASTER_CREATE, {
                        headers: {
                            'Authorization': `Bearer ${auth.APIToken}`
                        },
                        assetmaterial: domainItem?.assetmaterial,
                        assetmaterialcode: domainItem?.value,
                        maintenancedescriptionmaster: String(maintenanceDetailsMaster.maintenancedescriptionmaster),
                        groupid: String(uniqueId),
                        addedby: [
                            {
            ...
            setMaintenanceDetailsMaster({ ...maintenanceDetailsMaster, subcategoryname: "", maintenancedescriptionmaster: "" })
            setValueComponentCat([])
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        const isNameMatch = maintenanceDetails.some((item) =>
            maintenanceDetailsMaster.assetmaterial === item.assetmaterial &&
            valueComponentCat?.includes(item.assetmaterialcode) &&
            maintenanceDetailsMaster?.maintenancedescriptionmaster?.toLowerCase() ===
            item.maintenancedescriptionmaster?.toLowerCase()
        );

        if (maintenanceDetailsMaster.assetmaterial === "Please Select Asset Material") {
            setPopupContentMalert("Please Select Asset Material");
            setPopupSeverityMalert("info");
            handleClickOpenPopupMalert();
        }
        else if (selectedOptionsComponent?.length === 0) {
            setPopupContentMalert("Please Select Asset Material Code");
            setPopupSeverityMalert("info");
            handleClickOpenPopupMalert();
        }
        else if (maintenanceDetailsMaster.maintenancedescriptionmaster === "") {
    ...
    const handleClear = (e) => {
        e.preventDefault();
        setMaintenanceDetailsMaster({ assetmaterial: "Please Select Asset Material", maintenancedescriptionmaster: "" })
        setValueComponentCat([])
        setSelectedOptionsComponent([])
    ...
    const getviewCode = async (e) => {
        setPageName(!pageName);
        try {
            setmaintenanceDetailsMasterEdit(e)
            handleClickOpenview();
        } catch (err) { handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
    };
    // get single row to view....
    ...
    const sendEditRequest = async () => {
        ...
        try {
            let res = await axios.put(`${SERVICE.MAINTENANCEDETAILSMASTER_SINGLE}/${subprojectsid}`, {
                ...
                assetmaterialcode: [...valueComponentCatEdit],
    ...
    const editSubmit = (e) => {
        ...
        const isNameMatch = dupData.some((item) =>
            maintenanceDetailsMasterEdit.assetmaterial === item.assetmaterial &&
            maintenanceDetailsMasterEdit?.maintenancedescriptionmaster?.toLowerCase() ===
            item.maintenancedescriptionmaster?.toLowerCase()
        );
        ...
        else if (selectedOptionsComponentEdit?.length === 0) {
            setPopupContentMalert("Please Select Asset Material Code");
            setPopupSeverityMalert("info");
            handleClickOpenPopupMalert();
        }
    ...
    const fetchTasksubcategory = async () => {
        ...
        try {
            ...
            console.log(res_vendor?.data, "res_vendor?.data?.smaintenancedetailsmasterGroup")
            setmaintenanceDetails(res_vendor?.data?.smaintenancedetailsmasterGroupMatch);
    ...
    const fetchTasksubcategoryAll = async () => {
        ...
        try {
            ...
            setAllmaintenanceDetailsedit(res_meet?.data?.maintenancedetailsmaster);
        } catch (err) { handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
    }
                ...
                <Grid sx={{ display: 'flex' }}>
                    ...
                    {isUserRoleCompare?.includes("dmaintenancedetailsmaster") && (
                        <Button sx={userStyle.buttondelete} onClick={(e) => {
                            handleClickOpen();
                            setDataForDelete(params.row.groupid)
                        }}>
    ...
    return (
        <Box>
            <Headtitle title={'Maintenance Details Master'} />
            <PageHeading
                title="Maintenance Details Master"
                modulename="Asset"
                submodulename="Asset Details"
                mainpagename="Maintenance Details Master"
                subpagename=""
                subsubpagename=""
            />
            {isUserRoleCompare?.includes("amaintenancedetailsmaster")
                && (
                    <>
                        <Box sx={userStyle.dialogbox}>
                            <>
                                <Grid container spacing={2}>
                                    <Grid item xs={8}>
                                        <Typography sx={userStyle.importheadtext}>Add Maintenance Details Master </Typography>
                                    </Grid>
                                </Grid><br />
                                <Grid container spacing={2}>
                                    ...
                                    <Grid item md={3} xs={12} sm={12}>
                                        <FormControl fullWidth size="small">
                                            ...
                                            <Selects
                                                options={materialOpt}
                                                styles={colourStyles}
                                                value={{ label: maintenanceDetailsMaster.assetmaterial, value: maintenanceDetailsMaster.assetmaterial }}
                                                onChange={(e) => {
                                                    setMaintenanceDetailsMaster({
                                                    ...
                                                    Array.from(
                                                        new Set(
                                                            assetdetails
                                                                .filter(
                                                                    (t) =>
                                                                        maintenanceDetailsMaster.assetmaterial === t?.component
                                                                )
                                                                .map((t) => ({
                                    ...
                                    <Grid item md={4} xs={12} sm={12}>
                                        <FormControl fullWidth size="small">
                                            ...
                                            <TextareaAutosize
                                                ...
                                                minRows={5}
                                                value={maintenanceDetailsMaster.maintenancedescriptionmaster}
                                                onChange={(e) => {
                                                    setMaintenanceDetailsMaster({ ...maintenanceDetailsMaster, maintenancedescriptionmaster: e.target.value });
                                                }}
                                            />
                                        </FormControl>
                                    </Grid>
                                        ...
                                        <FormControl fullWidth size="small">
                                            ...
                                            <Selects
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    setSelectedOptionsComponentEdit([])
                                                }}
                                    ...
                                    <Grid item md={6} xs={12} sm={6}>
                                        <FormControl fullWidth size="small">
                                            ...
                                            <MultiSelect
                                                ...
                                                value={selectedOptionsComponentEdit}
                                                onChange={(e) => {
                                                    handleComponentChangeEdit(e);
                                                }}
            ...
            {isUserRoleCompare?.includes("lmaintenancedetailsmaster") && (
                <>
                    <Box sx={userStyle.container}>
                        { /* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>Maintenance Details Master Group List</Typography>
                        </Grid>
            ...
            <MaintenanceDetailsMasterIndividual forRefresh={forRefresh} steForRefresh={steForRefresh} />
            {/* Manage Column */}
            <Popover
                id={id}
            ...
            <ExportData
                ...
                itemsTwo={items ?? []}
                filename={"Maintenance Details Master Group"}
...
export default MaintenanceDetailsMaster;