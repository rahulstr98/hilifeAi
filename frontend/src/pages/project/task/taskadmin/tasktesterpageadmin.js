...
import { Box, Typography, Dialog, TextareaAutosize, TableHead, TableBody, TableContainer, Table, FormControlLabel, Checkbox, Divider, DialogContent, DialogActions, FormControl, Grid, Button } from "@mui/material";
...
function TaskTabs() {
  ...
  const [uiDesignData, setUiDesignData] = useState([]);
  const [developmentnData, setDevelopmentnData] = useState([]);

  const [testTasks, setTESTtasks] = useState({});
  const [testTasksuser, setTESTtasksuser] = useState([]);
  const [groupedTestData, setGroupedTestData] = useState({});
  const [checktask, setchecktask] = useState(false);
  const [todosedit, setTodosedit] = useState([]);
  const [todoseditui, setTodoseditui] = useState([]);
  ...
  const [testelopers, setDevelopers] = useState([]);
  const [testeloper, setDeveloper] = useState("");
  ...
  const handleClickOpenerrFiles = async (idval, state, title) => {
    setIndexid(idval);
    setIndexstate(state);
    setGrouptitle(title);
  ...
  const[userUsedTime, setUserUsedTime] = useState("")
  const rowData = async () => {
    try {
      let res_project = await axios.get(`${SERVICE.TASKASSIGNBOARDLIST_SINGLE}/${id}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setchecktask(true);
      let returnDevelopers = overallTasks.filter((item) => item.taskid == res_project?.data?.staskAssignBoardList.taskid && item.phase === "UI");
      let testId = returnDevelopers[0]?._id;
      if (testId) {
        let res = await axios.get(`${SERVICE.TASKASSIGNBOARDLIST_SINGLE}/${testId}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        });
        setUiDesignData(res.data.staskAssignBoardList.uidesign);
      }
      let returnDevelopersDev = overallTasks.filter((item) => item.taskid == res_project?.data?.staskAssignBoardList.taskid && item.phase === "Development");
      let testIdtest = returnDevelopersDev[0]?._id;
      if (testIdtest) {
        let res = await axios.get(`${SERVICE.TASKASSIGNBOARDLIST_SINGLE}/${testId}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
        ...
        setDevelopmentnData(res.data.staskAssignBoardList.develop);
      }
      const objtest = { title: "Testing Development" };
      const objtestUI = { title: "Testing UI" };
      let testuidata = res_project?.data?.staskAssignBoardList?.testinguidesign.map((item) => ({ ...item, ...objtestUI }));
      let testdata = res_project?.data?.staskAssignBoardList?.testing.map((item) => ({ ...item, ...objtest }));
      let data = [...testuidata, ...testdata];
      const groupedData = {};
      data.forEach((item) => {
      ...
      setTESTtasks(res_project?.data?.staskAssignBoardList);
      setTodosedit(res_project?.data?.staskAssignBoardList?.testing);
      setTodoseditui(res_project?.data?.staskAssignBoardList?.testinguidesign);
      setTESTtasksuser(data);
      setGroupedTestData(groupedData);
      const dataTime = data?.map((item, index) => ({
        ...item,
        time: item.subEstTime + " " + item.subEstType,
      }));
      let totaltasktest =[...res_project?.data?.staskAssignBoardList?.testinguidesign, ...res_project?.data?.staskAssignBoardList?.testing]
      //TOTAL USER'S USED TIME
      let totalMilliseconds = 0;
      totaltasktest.forEach(entry => {
        if(entry.state !== "running"){
      for (let i = 0; i < entry.starttime.length; i++) {
        // Rearrange the date format to "MM/DD/YYYY hh:mm:ss A"
        const startTime = new Date(entry.starttime[i].replace(/(\d{2})\/(\d{2})\/(\d{4})/, "$2/$1/$3"));
        const endTime = new Date(entry.endtime[i].replace(/(\d{2})\/(\d{2})\/(\d{4})/, "$2/$1/$3"));
      ...
      setUserUsedTime( `${String(totalHoursWithDays).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`);
      ...
      function timeStringToSeconds(timeString) {
        ...
        if (unit.toLowerCase().includes("hour")) {
          multiplier = 3600; // 1 hour = 60 minutes * 60 seconds
      ...
      const totalTimeInSeconds = dataTime.reduce((total, item) => total + timeStringToSeconds(item.time), 0);

      // Convert total time to "hh:mm:ss" format
      const totalTimeFormatted = secondsToHHMMSS(totalTimeInSeconds);
      setOverallTime(totalTimeFormatted);
    } catch (err) {handleApiError(err, setShowAlert, handleClickOpenerr);}
  };
  ...
  useEffect(() => {
    rowData();
  }, [overallTasks, todosedit, todoseditui]);

  //Assigned to testeloper checkpoints
  ...
  const handleCreateTodocheck = () => {
    if (
      testeloper &&
      testeloper.trim()?.length > 0
      //  &&
      // String(newTodoTitlecheck).trim()?.length > 0 &&
      // newpriority &&
      // newpriority.trim()?.length > 0
    ) {
      // const isDuplicate = todoscheck.some((todo) => todo.label === labelcheck);
      // if (!isDuplicate) {
      const newTodocheck = {
        tasktest: testeloper,
        checkpointsstatus: "incomplete",
        name: returnName,
        isreturn: "returned",
        priority: "High",
        subEstTime: 0,
        subEstType: "Minutes",
        state: "stopped",
        testpName: isUserRoleAccess.companyname,
        // title: "Returned",
        // returncount: (returncount[returncount.length - 1]?.returncount ? returncount[returncount.length - 1]?.returncount : 0) + 1
        // };
      };
      setTodoscheck([...todoscheck, newTodocheck]);
      setDeveloper(null);
    } else {
      // settesterrormsg(selectedOptioncheck === null && newTodoTitlecheck == "" && newpriority == "" ? "Please fill all fields" : selectedOptioncheck === null ? "Please Select name" : newpriority == "" ? "Please select priority" : "Please Enter time")
    }
  };
  ...
  const handleEditTodocheck = (index) => {
    ...
    setEditedDeveloper(todoscheck[index].tasktest);
    seteditedReturnName(todoscheck[index].name);
  };

  const handleUpdateTodocheck = () => {
    if (editedDeveloper != "" && editedReturnName != "") {
      const tasktest = editedDeveloper ? editedDeveloper : "";
      const name = editedReturnName ? editedReturnName : "";
      // if (!todoscheck.find((todo, index) => index !== editingIndexcheck && todo.label.toLowerCase() === label.toLowerCase())) {
      const newTodoscheck = [...todoscheck];
      newTodoscheck[editingIndexcheck].tasktest = tasktest;
      newTodoscheck[editingIndexcheck].name = name;
      setTodoscheck(newTodoscheck);
      ...
      const newTodoscheck = [...todoscheck];
      // newTodoscheck[editingIndexcheck].tasktest = "";
      // newTodoscheck[editingIndexcheck].returnName = "";
      setTodoscheck(newTodoscheck);
      setEditingIndexcheck(-1);
      // setEditedDeveloper("");
      // seteditedReturnName("");
  ...
  const getRowDataId = (idval) => {
    let indexdata = testTasksuser.filter((item) => item._id == idval);
  ...
  const handleUpdateTodoEdit = async (taskid) => {
    ...
    let findarrIndex = grouptitle === "Testing UI" ? todoseditui.findIndex((item) => item._id == indexid) : todosedit.findIndex((item) => item._id == indexid);

    let oldendtime = grouptitle === "Testing UI" ? todoseditui[findarrIndex].endtime : todosedit[findarrIndex].endtime;

    const newTodosedit = grouptitle === "Testing UI" ? [...todoseditui] : [...todosedit];
    ...
    try {
      if (grouptitle === "Testing UI") {
        const requests = findidArray.map((item) => {
          return axios.put(`${SERVICE.TASKASSIGNBOARDLIST_SINGLE}/${item}`, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
            testinguidesign: [...newTodosedit],
          });
        });
        setTodoseditui(newTodosedit);
        let check = [...newTodosedit, ...todosedit].filter((item) => item.taskdev === isUserRoleAccess.companyname).every((data) => data.checkpointsstatus === "completed");
        if (check === true) {
          setShowCompleteMsg("Task Successfully Completed 😎");
          handleClickOpenComplete();
        }
      } else if (grouptitle === "Testing Development") {
        const requests = findidArray.map((item) => {
          return axios.put(`${SERVICE.TASKASSIGNBOARDLIST_SINGLE}/${item}`, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
            testing: [...newTodosedit],
          });
        });
        setTodosedit(newTodosedit);
        let check = [...newTodosedit, ...todoseditui].filter((item) => item.taskdev === isUserRoleAccess.companyname).every((data) => data.checkpointsstatus === "completed");
        if (check === true) {
          setShowCompleteMsg("Task Successfully Completed 😎");
          handleClickOpenComplete();
        }
  ...
  const sendCompleRequest = async (taskid) => {
    // if (testTasksuser.every((item) => item.files.length > 0)) {
    try {
      const findidArray = overallTasks.filter((row) => row.taskid === taskid && row.phase === "UI").map((row) => row._id);
      let uiId = findidArray[0];
    
      const res = await axios.get(`${SERVICE.TASKASSIGNBOARDLIST_SINGLE}/${uiId}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
     
      const requests1 = await axios.put(`${SERVICE.TASKASSIGNBOARDLIST_SINGLE}/${uiId}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        uidesign: [...res.data.staskAssignBoardList.uidesign, ...todoscheck],
      });
  ...
  const now = new Date();
  // const initialRunningIndex = localStorage.getItem("runningIndex");

  // const [runningIndex, setRunningIndex] = useState(initialRunningIndex);
  const [runningIndex, setRunningIndex] = useState(null);

  const handleUpdateTodoEditrunning = async (idval, title, priority) => {
    try {
      ...
      if (resdev.data.taskallcheck.length === 0) {
        if (priority.toLowerCase() == "high" || priority.toLowerCase() == "veryhigh" || priority.toLowerCase() == "very high") {
          ...
          let findarrIndex = title === "Testing UI" ? todoseditui.findIndex((item) => item._id == idval) : todosedit.findIndex((item) => item._id == idval);

          let oldstarttime = title === "Testing UI" ? todoseditui[findarrIndex].starttime : todosedit[findarrIndex].starttime;

          // if (restest.data.tasks.length == 0) {
          const newTodosedit = title === "Testing UI" ? [...todoseditui] : [...todosedit];
          ...
          await sendEditRequest(newTodosedit, title);
          setRunningIndex(id);
  ...
  const handleUpdateTodoEditpaused = async (idval, title) => {
    let today = new Date();
    ...
    let oldendtime = title === "Testing UI" ? todoseditui[findarrIndex].endtime : todosedit[findarrIndex].endtime;
    ...
    await sendEditpausRequest(newTodosedit, title);
    setRunningIndex(null);
    // localStorage.setItem("runningIndex", null);
  };

  const sendEditRequest = async (todosvalue, title) => {
    try {
      if (title === "Testing UI") {
        let res = await axios.put(`${SERVICE.TASKASSIGNBOARDLIST_SINGLE}/${id}`, {
          ...
          testinguidesign: todosvalue,
        });
        setTodoseditui(todosvalue);
      } else if (title === "Testing Development") {
        let res = await axios.put(`${SERVICE.TASKASSIGNBOARDLIST_SINGLE}/${id}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const sendEditpausRequest = async (todoval, title) => {
    try {
      if (title === "Testing UI") {
        let res = await axios.put(`${SERVICE.TASKASSIGNBOARDLIST_SINGLE}/${id}`, {
          headers: {
          ...
          testinguidesign: todoval,
        });
        setTodoseditui(todoval);
      } else if (title === "Testing Development") {
        let res = await axios.put(`${SERVICE.TASKASSIGNBOARDLIST_SINGLE}/${id}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
  ...
  const calculateTotalTimes = (idval) => {
    const matchedItem = testTasksuser.find((item) => item._id == idval);
        ...
        <>
          <Box>
            ...
            <Grid item md={5} xs={5} sm={6} sx={{ whiteSpace: "pre-line", wordBreak: "break-all" }}>
                ...
                <h4>{testTasks.taskname}</h4>
              </Grid>
              <Grid item md={4} xs={4} sm={6} sx={{ whiteSpace: "pre-line", wordBreak: "break-all" }}>
                <Typography>Task ID : </Typography>
                <h4>{testTasks.taskid}</h4>
              </Grid>
              ...
              <Grid item md={1.5} xs={12} sm={6}>
                ...
                <h4 style={{ color: "#d32f2f", fontWeight: 800 }}>{userUsedTime}</h4>
              </Grid>
              <Grid item md={1.5} xs={12} sm={6}>
                <Typography>Task Time : </Typography>
                <h4 style={{ color: "#d32f2f", fontWeight: 800 }}>{overAllTime}</h4>
              </Grid>
            </Grid>
            <br />
            <Grid container spacing={2}>
              <Grid item md={3} xs={3} sm={3}>
                ...
                <h3>{testTasks.project}</h3>
              </Grid>
              <Grid item md={3} xs={3} sm={3}>
                <Typography>Subproject : </Typography>
                <h3>{testTasks.subproject}</h3>
              </Grid>
              <Grid item md={3} xs={3} sm={3}>
                <Typography>Module : </Typography>
                <h3>{testTasks.module}</h3>
              </Grid>
              <Grid item md={3} xs={3} sm={3}>
                <Typography>SubModule : </Typography>
                <h3>{testTasks.submodule}</h3>
              </Grid>
              {testTasks.mainpage && (
                <Grid item md={3} xs={3} sm={3}>
                  <Typography>Mainpage : </Typography>
                  <h3>{testTasks.mainpage}</h3>
                </Grid>
              )}
              {testTasks.subpage && (
                <Grid item md={3} xs={3} sm={3}>
                  <Typography>Subpage : </Typography>
                  <h3>{testTasks.subpage}</h3>
                </Grid>
              )}
              {testTasks.name && (
                <Grid item md={3} xs={3} sm={3}>
                  <Typography>SubSubpage : </Typography>
                  <h3>{testTasks.name}</h3>
                </Grid>
              )}
            </Grid>
          </Box>
          ...
          <TableContainer>
            <Table>
              ...
              <TableBody>
                {uiDesignData.length > 0 &&
                  uiDesignData?.map((item, index) => {
                    return (
                      <StyledTableRow key={index}>
                        <StyledTableCell sx={{ padding: "5px 10px !important" }}>{index + 1}</StyledTableCell>
                        <StyledTableCell sx={{ padding: "5px 10px !important" }}>{item.taskdev}</StyledTableCell>
                        <StyledTableCell sx={{ padding: "5px 10px !important" }}>{item.name}</StyledTableCell>
                        <StyledTableCell sx={{ padding: "5px 10px !important" }}>{item.inputvalue}</StyledTableCell>
                      </StyledTableRow>
                    );
                  })}
              </TableBody>
          ...
          <TableContainer>
            <Table>
              ...
              <TableBody>
                {developmentnData.length > 0 &&
                  developmentnData?.map((item, index) => {
                    return (
                      <StyledTableRow key={index}>
                        <StyledTableCell sx={{ padding: "5px 10px !important" }}>{index + 1}</StyledTableCell>
                        <StyledTableCell sx={{ padding: "5px 10px !important" }}>{item.taskdev}</StyledTableCell>
          ...
          <Box sx={{ padding: "12px" }}>
            {Object.keys(groupedTestData).map((title) => (
              <div key={title}>
                <h2>{title}</h2>
                <br />
                {groupedTestData[title]?.map((data, index) => {
                    ...
                    <div key={index}>
                      <Grid container sx={{ padding: "0px 10px", display: "flex", alignItems: "center" }}>
                        <Grid md={1.5} sm={12} xs={12} sx={{ whiteSpace: "pre-line", wordBreak: "break-all" }}>
                          <FormControl>
                            <FormControlLabel sx={{ color: data.isreturn === "returned" ? "red" : "inherit" }} control={<Checkbox sx={{ fontSize: "13px" }} checked={data?.files?.length > 0 ? true : false} name="" disabled />} label={" " + data.name} />
                          </FormControl>
                        </Grid>
                        ...
                        <Grid md={0.5} sm={12} xs={12}>
                          {/* <FolderIcon
                            ...
                            disabled={data.isreturn === "returned"}
                            onClick={(e) => {
                              getRowDataId(data._id);
                            }}
                          /> */}
                        </Grid>
                        <Grid md={2.2} sm={12} xs={12}>
                          <Typography>
                            {data?.files?.map((file) => (
                              <>
                        ...
                        <Grid md={1} sm={6} xs={12}>
                          <Typography style={{ background: data.state === "running" ? "#8080809e" : greaterDuration === "On Time" ? "#66c366ed" : greaterDuration === "Delay" ? "#f82c2ceb" : "", width: "max-content", borderRadius: "5px", fontSize: "12px", color: "white", padding: "0px 5px" }}>{data.state === "running" ? "On going" : data.state === "" ? "" : greaterDuration}</Typography>
                        </Grid>
                        <Grid md={0.9} sm={6} xs={12}>
                        ...
                        <Grid md={1.4} sm={6} xs={12} sx={{ whiteSpace: "pre-line", wordBreak: "break-all" }}>
                          <Typography sx={{ fontSize: "11px", fontWeight: "bold" }}>{data.taskdev}</Typography>
                        </Grid>
                      </Grid>
                      <Divider />
                      <br />
            ...
            <br />
            {/* <Typography> Return to UI Desinger </Typography>
            <Grid container spacing={2}>
              <Grid item md={4} sm={4} xs={12}>
                ...
                <FormControl fullWidth>
                  <Selects
                    ...
                    value={{ label: testeloper, value: testeloper }}
                    onChange={(e) => {
              ...
              <Grid item md={6} sm={6} xs={12}>
                <FormControl fullWidth>
                  <TextareaAutosize aria-label="minimum height" minRows={5} value={returnName} onChange={(e) => setReturnName(e.target.value)} />
                </FormControl>
              </Grid>
                  ...
                  {editingIndexcheck === index ? (
                    <Grid container spacing={1}>
                      <Grid item md={4} sm={4} xs={4}>
                        <Selects options={testelopers} value={{ label: editedDeveloper, value: editedDeveloper }} onChange={(e) => setEditedDeveloper(e.value)} />
                      </Grid>
                      <Grid item md={6} sm={6} xs={6}>
                        <FormControl fullWidth>
                          <TextareaAutosize aria-label="minimum height" minRows={5} value={editedReturnName} onChange={(e) => seteditedReturnName(e.target.value)} />
                        </FormControl>
                      </Grid>
                      <Grid item md={1} sm={1} xs={1}>
                        <Button
                          ...
                          disabled={editedReturnName == ""}
                          onClick={handleUpdateTodocheck}
                        >
                          <CheckCircleIcon
                            style={{
                              color: editedReturnName == "" ? "grey" : "#216d21",
                              fontSize: "1.5rem",
                            }}
                          />
                        </Button>
        ...
        <Dialog open={isErrorOpenFiles} onClose={handleCloseerrFiles} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="sm" fullWidth={true}>
          ...
          <DialogActions>
            <Button
              ...
              color="primary"
              onClick={() => {
                handleUpdateTodoEdit(testTasks.taskid);
              }}
            >
              Update
            </Button>
        ...
        <Dialog open={isErrorOpenRef} onClose={handleCloseRef} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="md" fullWidth={true}>
          <DialogContent>
            <Grid item md={10} xs={12} sm={12}>
              ...
              <br />
              <br />
              <Box sx={{ width: "100%", maxHeight: "max-content", minHeight: "250px", typography: "body1", boxShadow: "0px 0px 2px #808080a3", border: "1px solid #80808057" }}>
                  ...
                  <Box sx={{ borderBotton: 1, border: "divider", background: "#80808036", height: "47px" }}>
                    <TabList
                      ...
                      sx={{
                        ...
                        minHeight: "47px",
                        ".css-can5u7-MtestButtonBase-root-MtestTab-root.Mtest-selected": {
                          color: "white",
                          border: "1px solid #b5afaf",
                          background: " #3346569c",
                        },
                        ".css-1aquho2-MtestTabs-indicator": {
                          background: " none",
                        },
                      }}
                    >
