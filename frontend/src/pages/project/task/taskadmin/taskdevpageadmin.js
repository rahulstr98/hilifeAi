...
function TaskTabs() {
  ...
  const [devTasksuser, setDEVtasksuser] = useState([]);
  ...
  const rowData = async () => {
    try {
      ...
      let devId = returnDevelopers[0]?._id;
      if (devId) {
        let res = await axios.get(`${SERVICE.TASKASSIGNBOARDLIST_SINGLE}/${devId}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        });
        setUiDesignData(res.data.staskAssignBoardList.uidesign);
        //   let developersName = res.data.staskAssignBoardList.uidesign.map((item) => item.taskdev);
        //   const set = new Set(developersName);
        //   const arrayWithoutDuplicates = Array.from(set);

        //   setDevelopers(
        //     arrayWithoutDuplicates?.map((d) => ({
        //       ...d,
        //       label: d,
        //       value: d,
        //     }))
      ...
      setDEVtasks(res_project?.data?.staskAssignBoardList);
      setTodosedit(res_project?.data?.staskAssignBoardList?.develop);
      setDEVtasksuser(res_project?.data?.staskAssignBoardList?.develop);

      const dataTime = res_project?.data?.staskAssignBoardList?.develop?.map((item, index) => ({
        ...item,
      ...
      res_project?.data?.staskAssignBoardList?.develop.forEach((entry) => {
  ...
  useEffect(() => {
    rowData();
  }, [overallTasks, todosedit]);

  //Assigned to developer checkpoints
  ...
  const handleCreateTodocheck = () => {
    if (
      ...
      const newTodocheck = {
        ...
        subEstType: "Minutes",
        state: "stopped",
        devpName: isUserRoleAccess.companyname,
        // title: "Returned",
      ...
      // setdeverrormsg(selectedOptioncheck === null && newTodoTitlecheck == "" && newpriority == "" ? "Please fill all fields" : selectedOptioncheck === null ? "Please Select name" : newpriority == "" ? "Please select priority" : "Please Enter time")
    }
  };
  ...
  const handleUpdateTodocheck = () => {
    if (editedDeveloper != "" && editedReturnName != "") {
      ...
      const newTodoscheck = [...todoscheck];
      newTodoscheck[editingIndexcheck].taskdev = taskdev;
      ...
      const newTodoscheck = [...todoscheck];
      // newTodoscheck[editingIndexcheck].taskdev = "";
  ...
  const getRowDataId = (idval) => {
    let indexdata = devTasksuser.filter((item) => item._id == idval);
  ...
  const sendCompleRequest = async (taskid) => {
    // if (devTasksuser.every((item) => item.files.length > 0)) {
    try {
  ...
  const calculateTotalTimes = (idval) => {
    const matchedItem = devTasksuser.find((item) => item._id == idval);
      ...
      {!checktask ? (
        ...
        <>
          <Box>
            ...
            <Grid item md={5} xs={5} sm={6} sx={{ whiteSpace: "pre-line", wordBreak: "break-all" }}>
              ...
              {/* <Grid item md={5} xs={6} sm={5} style={{ display: "flex", justifyContent: "center", gap: "10px" }}>
                <Typography style={{ marginTop: "10px", fontFamily: "auto", fontWeight: "900" }}>{devTasks?.assignedby}</Typography>
              </Grid> */}
              ...
              {/* <Grid item md={2} xs={12} sm={12} sx={{ display: "flex", justifyContent: "end" }}>
                <Button
                  ...
                  onClick={() => {
                    sendCompleRequest(devTasks.taskid);
                  }}
                >
                  Complete
                </Button>
            ...
            <Grid container spacing={2}>
              <Grid item md={3} xs={3} sm={3}>
                ...
                <h3>{devTasks.project}</h3>
              </Grid>
              <Grid item md={3} xs={3} sm={3}>
                <Typography>Subproject : </Typography>
                <h3>{devTasks.subproject}</h3>
              </Grid>
              <Grid item md={3} xs={3} sm={3}>
                <Typography>Module : </Typography>
                <h3>{devTasks.module}</h3>
              </Grid>
              <Grid item md={3} xs={3} sm={3}>
                <Typography>SubModule : </Typography>
                <h3>{devTasks.submodule}</h3>
              </Grid>
              {devTasks.mainpage && (
                <Grid item md={3} xs={3} sm={3}>
                  <Typography>Mainpage : </Typography>
                  <h3>{devTasks.mainpage}</h3>
                </Grid>
              )}
              {devTasks.subpage && (
                <Grid item md={3} xs={3} sm={3}>
                  <Typography>Subpage : </Typography>
                  <h3>{devTasks.subpage}</h3>
                </Grid>
              )}
              {devTasks.name && (
                <Grid item md={3} xs={3} sm={3}>
          ...
          <Box sx={{ padding: "12px" }}>
            <br />
            <br />
            {devTasksuser?.length > 0 &&
              devTasksuser?.map((data, index) => {
              ...
              <Grid item md={4} sm={4} xs={12}>
                ...
                <FormControl fullWidth>
                  <Selects
                    ...
                    value={{ label: developer, value: developer }}
                    onChange={(e) => {
                  ...
                  {editingIndexcheck === index ? (
                    <Grid container spacing={1}>
                      <Grid item md={4} sm={4} xs={4}>
                        <Selects options={developers} value={{ label: editedDeveloper, value: editedDeveloper }} onChange={(e) => setEditedDeveloper(e.value)} />
                      </Grid>
        ...
        <Dialog open={isErrorOpenFiles} onClose={handleCloseerrFiles} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="sm" fullWidth={true}>
          ...
          <DialogActions>
            <Button
              ...
              color="primary"
              onClick={() => {
                handleUpdateTodoEdit(devTasks.taskid);
                // sendEditRequest();
              }}
            >
              Update
                  ...
                  <Box sx={{ borderBotton: 1, border: "divider", background: "#80808036", height: "47px" }}>
                    <TabList
                      ...
                      sx={{
                        ...
                        minHeight: "47px",
                        ".css-can5u7-MdevButtonBase-root-MdevTab-root.Mdev-selected": {
                          color: "white",
                          border: "1px solid #b5afaf",
                          background: " #3346569c",
                        },
                        ".css-1aquho2-MdevTabs-indicator": {
                          background: " none",
                        },
                      }}
                    >
