...
import {
    Box, Typography, OutlinedInput, FormControl, TableBody, FormControlLabel, Select,
...
function Pendingtask() {
    ...
    const [isDeveloperIncomplete, setIsDeveloperIncomplete] = useState([]);
    ...
    const fetchDeveloperIncomplete = async () => {


        try {
            let res = await axios.post(SERVICE.TASKHOMEDEVINCOMPLETE, {
                headers: {
            ...
            setIsDeveloperIncomplete(res?.data?.tasks)
    ...
    const addSerialNumber = () => {
        const itemsWithSerialNumber = isDeveloperIncomplete?.map((item, index) => ({ ...item, serialNumber: index + 1 }));
        setItems(itemsWithSerialNumber);
    }

    useEffect(() => {
        addSerialNumber();
    }, [isDeveloperIncomplete])
    ...
    return (
        <>
            <Box sx={userStyle.container}>
                <Box>
                    <Headtitle title={'PENDING TASK LIST'} />
                    <Typography sx={userStyle.HeaderText}>Pending Task List</Typography>
                    <Grid style={userStyle.dataTablestyle}>
                        <Box>
                            ...
                            <Select id="pageSizeSelect" value={pageSize} onChange={handlePageSizeChange} sx={{ width: "67px" }}>
                                ...
                                <MenuItem value={(isDeveloperIncomplete?.length)}>All</MenuItem>
                            </Select>
                        </Box>
                        <Box>
                        ...
                        >
                            <TableHead sx={{ fontWeight: "600" }}>
                                <StyledTableRow>
                                    ...
                                    <StyledTableCell onClick={() => handleSorting("assignedby")}><Box sx={userStyle.tableheadstyle}><Box>AssignedBy</Box> <Box sx={{ marginTop: '-6PX' }}>{renderSortingIcon("assignedby")}</Box></Box> </StyledTableCell>
            ...
            >
                <Box sx={{ width: "990px", padding: '20px 50px' }}>
                    <>
                        <Typography sx={userStyle.HeaderText}> Pending Task Info </Typography>
