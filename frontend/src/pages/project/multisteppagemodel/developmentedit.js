...
import { Box, Typography, Checkbox, Select, Divider, Dialog, MenuItem, TextareaAutosize, DialogContent, DialogActions, FormControl, Grid, OutlinedInput, Button, TextField } from "@mui/material";
...
function SubmoduleViewList({ sendDataToParentDEV, checkpagemodel, developdata, id }) {
  ...
  const [deleteDevelopment, setDeleteDevelopment] = useState();
  ...
  const [incompleteDataDevelopment, setIncompleteDataDevelopment] = useState([]);
  const [completedDataDevelop, setCompletedDataDevelop] = useState([]);
  ...
  const fetchCompleteDevelopData = async () => {
    try {
      if (checkpagemodel === "PAGEMODEL") {
        ...
        setCompletedDataDevelop(resPage?.data?.spagemodel?.develop);
      } else if (checkpagemodel === "SUBMODULE") {
        let resData = await axios.get(`${SERVICE.SUBMODULE_SINGLE}/${id}`, {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
        });
        setCompletedDataDevelop(resData?.data?.ssubmodule?.develop);
      }
  ...
  }, [developdata, isEditOpen, isDeleteOpen]);

  const filterDataFromUItoDevelop = async () => {
    try {
      if (checkpagemodel === "PAGEMODEL") {
        let resPage = await axios.get(`${SERVICE.PAGEMODEL_SINGLE}/${id}`, {
          headers: {
        ...
        const notmatcheddata = resPage?.data?.spagemodel?.uidesign?.filter((uidata) => {
          return !completedDataDevelop.some((item) => item.name === uidata.name);
        });
        setIncompleteDataDevelopment(notmatcheddata);
      } else if (checkpagemodel === "SUBMODULE") {
        let resData = await axios.get(`${SERVICE.SUBMODULE_SINGLE}/${id}`, {
          headers: {
            Authorization: `Bearer ${authToken}`,
        ...
        const notmatcheddata = resData?.data?.ssubmodule?.uidesign?.filter((uidata) => {
          return !completedDataDevelop.some((item) => item.name === uidata.name);
        });
        setIncompleteDataDevelopment(notmatcheddata);
      }
  ...
  const updateSubmit = async () => {
    const updatedvalue = incompleteDataDevelopment.find((todo) => todo._id === editincompleteid);

    if (updatedvalue) {
      updatedvalue.inputvalue = editincompletedata.inputvalue;
      updatedvalue.refCode = refCode; // Change the name property if needed
      updatedvalue.refImage = refImage; // Change the name property if needed
      updatedvalue.refDocuments = refDocuments; // Change the name property if needed
      updatedvalue.refLinks = refLinks; // Change the name property if needed
      updatedvalue.refDetails = refDetails; // Change the name property if needed
      updatedvalue.status = "complete"; // Change the name property if needed
    }
    const mergeData = [...completedDataDevelop, updatedvalue];
    ...
    try {
      ...
      await filterDataFromUItoDevelop();
      await fetchCompleteDevelopData();
      // await filterDataFromUItoDevelop();
      // await Promise.all([filterDataFromUItoDevelop(), fetchCompleteDevelopData()]);
  ...
  const updateSubmitComplete = async () => {
    try {
      const updatedTodoscheck = completedDataDevelop.map((todo) => {
  ...
  const updateNewDevelopmentData = async () => {
    try {
      ...
      if (todoscheck.length === 0) {
        ...
        if (checkpagemodel === "PAGEMODEL") {
          await axios.put(`${SERVICE.PAGEMODEL_SINGLE}/${id}`, {
            ...
            develop: [...completedDataDevelop, ...todoscheck],
            testing: [...completedDataDevelop, ...todoscheck],
            //   updatedby: [
            //     ...updateby,
            //     {
            //       name: String(username),
  ...
  const addSerialNumber = () => {
    const itemsWithSerialNumber = incompleteDataDevelopment?.map((item, index) => ({ ...item, serialNumber: index + 1 }));
    setItems(itemsWithSerialNumber);
  };

  useEffect(() => {
    addSerialNumber();
  }, [incompleteDataDevelopment]);

  //COMPLETE DATA SERIAL NUMBER ADDED
  const [itemsComplete, setItemsComplete] = useState([]);

  const addSerialNumberComplete = () => {
    const itemsWithSerialNumber = completedDataDevelop?.map((item, index) => ({ ...item, serialNumber: index + 1 }));
    setItemsComplete(itemsWithSerialNumber);
  };

  useEffect(() => {
    addSerialNumberComplete();
  }, [completedDataDevelop]);
  ...
  const getCodeComplete = async (name, id, datafetch, existingoption, inputvalue, refCode, refImage, refDocuments, refLinks, refDetails, row, idval) => {
    try {
      ...
      setEditincompletedataEditid(id);
  ...
  //get single row to edit....
  const getCodeCompleteDelete = async (id) => {
    try {
      ...
      //   },
      // });
      setDeleteDevelopment(id);
      handleClickOpen();
     } catch (err) {handleApiError(err, setShowAlert, handleClickOpenerr);}
  };

  const delDevData = async () => {
    try {
      let updateDevelop = completedDataDevelop.filter((item) => item._id !== deleteDevelopment);
      if (checkpagemodel === "PAGEMODEL") {
        await axios.put(`${SERVICE.PAGEMODEL_SINGLE}/${id}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
          develop: [...updateDevelop],
      ...
      renderCell: (params) => (
        <Grid sx={{ display: "flex" }}>
          ...
          <Button
            ...
            onClick={(e) => {
              getCodeCompleteDelete(params.row.id);
            }}
          >
  ...
  useEffect(() => {
    sendDataToParentDEV(completedDataDevelop);
  }, [completedDataDevelop, sendDataToParentDEV]);

  // useEffect(() => {
  //   sendDataToParentIncomplete(incompleteDataDevelopment);
  // }, [incompleteDataDevelopment, sendDataToParentIncomplete]);
    ...
    <Box>
      ...
      <div>
        ...
        <Box sx={{ display: "flex", justifyContent: "end" }}>
          <Button variant="contained" color="primary" onClick={updateNewDevelopmentData}>
            Add to Development
          </Button>
        </Box>
      </div>
      ...
      <Grid style={userStyle.dataTablestyle}>
        <Box>
          ...
          <Select id="pageSizeSelect" defaultValue="" value={pageSize} onChange={handlePageSizeChange} sx={{ width: "77px" }}>
            ...
            <MenuItem value={incompleteDataDevelopment?.length}>All</MenuItem>
          </Select>
        </Box>
      ...
      <br />
      <Box>
        <StyledDataGrid autoHeight rows={rowsWithCheckboxes} columns={columnDataTable.filter((column) => columnVisibility[column.field])} selectionModel={selectedRows} density="compact" hideFooter getRowClassName={getRowClassName} disableRowSelectionOnClick />
      </Box>
      <br />
      ...
      <Grid style={userStyle.dataTablestyle}>
        <Box>
          ...
          <Select id="pageSizeSelectcomplete" defaultValue="" value={pageSizeComplete} onChange={handlePageSizeChangeComplete} sx={{ width: "77px" }}>
            ...
            <MenuItem value={50}>50</MenuItem>
            <MenuItem value={100}>100</MenuItem>
            <MenuItem value={completedDataDevelop?.length}>All</MenuItem>
          </Select>
        </Box>
        <Box>
          <FormControl fullWidth size="small">
      ...
      <br />
      <Box>
        <StyledDataGrid rows={rowsWithCheckboxescomplete} columns={columnDataTableComplete.filter((column) => columnVisibility[column.field])} selectionModel={selectedRows} autoHeight={true} density="compact" hideFooter getRowClassName={getRowClassName} disableRowSelectionOnClick />
      </Box>
      <br />
      <Box style={userStyle.dataTablestyle}>
        <Box>
        ...
        <DialogContent>
          ...
          <Box sx={{ width: "100%", height: "max-content", minHeight: "250px", typography: "body1", boxShadow: "0px 0px 2px #808080a3", border: "1px solid #80808057" }}>
            <TabContext value={valueComplete}>
              ...
              <TabPanel value="2">
                <input className={classes.inputs} type="file" id="file-inputuploadcreatefirst" accept="image/*" multiple onChange={handleInputChangeComplete} />
                <label htmlFor="file-inputuploadcreatefirst">
                  <Button
                    component="span"
                    style={{
              ...
              <TabPanel value="3">
                <input className={classes.inputs} type="file" id="file-inputuploadcreatefirst" accept=".pdf, .doc, .docx, .txt, .xls, .xlsx, .zip" multiple onChange={handleInputChangedocumentComplete} />
                <label htmlFor="file-inputuploadcreatefirst">
                  <Button
                    component="span"
                    style={{
      ...
      <Dialog open={isDeleteOpen} onClose={handleCloseMod} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
        ...
        <DialogActions>
          ...
          <Button autoFocus variant="contained" color="error" onClick={(e) => delDevData(deleteDevelopment)}>
            {" "}
            OK{" "}
          </Button>
        </DialogActions>
