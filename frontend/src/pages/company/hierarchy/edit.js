...
function Hirerarchi() {
  ...
  const { isUserRoleCompare, isUserRoleAccess, isAssignBranch, allUsersData, pageName, setPageName, allTeam, buttonStyles } = useContext(UserRoleAccessContext);
  ...
  const handleSubmit = (e) => {
    ...
    console.log(filterUsername, getUsersName, userNamesFinal, 'filterUsername')
    ...
    const SameSupervisorModeEmployee = groupIndividual.length > 0 && groupIndividual.filter((data) => isDuplicateDisabled(data)
      && data.supervisorchoose.includes(hirerarchicreate.supervisorchoose)
    ...
    const SameSupervisorEmployee = groupIndividual.length > 0 && groupIndividual.filter((data) => isDuplicateDisabled(data)
      && data.supervisorchoose.includes(hirerarchicreate.supervisorchoose));
    ...
    if (hirerarchicreate.company == "Please Select Company") {
      ...
      if (!enableSameSupervisor && filterUsername?.includes(hirerarchicreate.supervisorchoose)) {
