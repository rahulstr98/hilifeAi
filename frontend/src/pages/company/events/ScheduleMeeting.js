...
import {
  ...
  FormGroup, FormControlLabel, Link, DialogTitle, TextareaAutosize,
} from "@mui/material";
...
import { reminderOption, meetingModeOption, meetingTypeOption, timeZoneOptions, } from "../../../components/Componentkeyword";
...
function ScheduleMeeting() {
  ...
  let exportColumnNames = ['Company', 'Branch', 'Team', 'Department', 'Title', 'Meeting Type', 'Meeting Category', 'Date', 'Time', 'Duration', 'Time Zone', 'Participants', 'Meeting Host', 'Reminder', 'Meeting Status'];
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Schedule Meeting"),
  ...
  const [meetingState, setMeetingState] = useState({
    branch: "Please Select Branch", department: "Please Select Department", team: "Please Select Team",
    meetingcategory: "Please Select Meeting Category", meetingtype: "Please Select Meeting Type", meetingmode: "Please Select Meeting Mode", venue: "Please Select Area",
    link: "", title: "", date: "", time: "", duration: "00:00", timezone: "Please Select Time Zone", reminder: "Please Select Reminder",
    recuringmeeting: false, repeattype: "Repeat Type", repeatevery: "00 days",
  });
  ...
  const editSubmit = (e) => {
    ...
    if (companyValueCateEdit?.length == 0) {
      ...
      setPopupContentMalert("Please Select Tean");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
  ...
  const fetchMeeting = async () => {
    ...
    try {
      ...
      setMeetingArray(final.map((item, index) => ({
        ...item,
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage.toBlob(gridRefTableImg.current)
        .then((blob) => {
          saveAs(blob, "Schedule Meeting.png");
        })
        .catch((error) => {
  ...
  const columnDataTable = [
    ...
    //   cellRenderer: (params) => "editable-cell", // Add a custom class for editable cells
    //   cellRenderer: (params) => (
    //     <div>
    //       {params.value ? (
    //         params.value
        ...
        <Grid sx={{ display: "flex" }}>
          ...
          {isUserRoleCompare?.includes("vschedulemeeting") && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
                getviewCode(params.data.tableid);
              }}
            >
          ...
          {isUserRoleCompare?.includes("ischedulemeeting") && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
                getinfoCode(params.data.tableid);
              }}
            >
          ...
          {isUserRoleCompare?.includes("vschedulemeeting") && (
            <>
              {params.data.recuringmeeting == true && (
                <Button
                  variant="contained"
                  sx={{
                    minWidth: "15px",
                  ...
                  onClick={() => {
                    backpage(
                      `/setup/schedulemeetinglog/${params.data.uniqueid}`
                    );
                  }}
                >
