...
function AttendanceIndividualStatus() {
    ...
    const getCodeClockIn = async (rowdata) => {
        ...
        const [day, month, year] = rowdata.rowformattedDate?.split('/');
        ...
        try {
            ...
            if (res_payrun?.data?.payrunlist?.length > 0) {
                ...
                if (rowdata?.clockin != "00:00:00") {
                    ...
                    result?.filter((d) => {
                        if (d.userid === rowdata.userid && d.date === moment(rowdata.date, "DD/MM/YYYY").format("DD-MM-YYYY") && d.shiftmode === rowdata.shiftMode && d.attendancestatus !== undefined) {
                            console.log(d._id)
                            setGetAttIdClockIn(d._id);
                        }
                    });
    ...
    const columnDataTableAttInd = [
        ...
        { field: "shiftmode", headerName: "ShiftMode", flex: 0, width: 150, hide: !columnVisibility.shiftmode, },
    ...
    let exportColumnNamescrt = ["Company", "Branch", "Unit", "Department", "Emp Code", "Employee Name", "Role", "Book By", "IP Address", "Date", "Shift", "ShiftMode", "ClockIn", "ClockOut",]
    let exportRowValuescrt = ["company", "branch", "unit", "department", "empcode", "username", "role", "bookby", "ipaddress", "date", "shift", "shiftmode", "clockin", "clockout",]
