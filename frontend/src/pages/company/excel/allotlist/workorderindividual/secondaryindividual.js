...
import {
    Box, Typography, OutlinedInput, Select, MenuItem, Dialog, TableBody, TableCell,
    TableRow, DialogContent, DialogActions, FormControl, Grid, Paper, Table,
    TableHead, TableContainer, Button, List, ListItem, ListItemText, Popover, TextField, IconButton
} from "@mui/material";
...
const SecondaryIndividual = () => {
    ...
    const handleCaptureImage = () => {
        ...
        html2canvas(clonedTable).then((canvas) => {
            ...
            link.href = dataURL;
            link.download = "WorkOrderIndividual_Secondary.png";
            link.click();
        });
    };
    ...
    const fetchIndividualReport = async () => {
        try {
            let res = await axios.post(SERVICE.SECONDARY_INDIVIDUALWORKORDER, {
                headers: {
    ...
    const columnDataTable = [
        ...
        {
            field: "hyperlink",
            headerName: <span style={{ fontWeight: 'bold' }}>Process Hyperlink</span>,
    ...
    const downloadExcelSecondary = async () => {
        ...
        try {
            ...
            const worksheet = workbook.addWorksheet('Secondary Individual');
            ...
            a.href = url;
            a.download = 'Workorder_Secondary_Individual_list.xlsx'; // File name
            a.click();
        } catch (error) {
    ...
    const downloadCsvSecondary = () => {
        ...
        saveAs(blob, 'Workorder_Secondary_Individual_list.csv'); // Specify the filename with .csv extension
    };
    ...
    const handleprintSecondary = useReactToPrint({
        ...
        documentTitle: 'Workorder_Secondary_Individual_list',
        pageStyle: 'print'
    });
    ...
    const downloadPdfSecondary = () => {
        ...
        doc.save("Workorder_Secondary_Individual_list.pdf");
    };
    ...
    return (
        <>
            <Headtitle title={'WorkOrder Individual Secondary '} />
            {isUserRoleCompare?.includes("lsecondaryindividualworkorderlist")
                && (
                    <>
                        <Box sx={userStyle.container}>
                            <Typography sx={userStyle.SubHeaderText}>Secondary WorkOrder List Individual</Typography>
                            <br /><br />
                            { /* ****** Header Buttons ****** */}
