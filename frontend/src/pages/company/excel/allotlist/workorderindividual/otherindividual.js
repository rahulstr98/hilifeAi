...
const Othersworkorder = () => {
    ...
    const [pageSizeOthers, setPageSizeOthers] = useState(10);
    ...
    const { isUserRoleAccess, isUserRoleCompare, isAssignBranch, allUsersData, allTeam } = useContext(UserRoleAccessContext);
    ...
    const handleCaptureImage = () => {
        ...
        html2canvas(clonedTable).then((canvas) => {
            ...
            link.href = dataURL;
            link.download = "WorkOrderIndividual_Other.png";
            link.click();
        });
    };
    ...
    const fetchDataOrderOthers = async () => {
        try {
            let res_unit = await axios.post(SERVICE.INDIVIDUAL_WORKORDER_OTHERSLIST, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
            ...
            setTableDataOthers(res_unit?.data?.finalresult);
            setIsLoader(true)
    ...
    const OnFilterSubmit = async () => {
        try {
            let res_employee = await axios.post(SERVICE.INDIVIDUAL_WORKORDER_OTHERSFILTER, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
                companyname: String(isUserRoleAccess.companyname),
            ...
            setTableDataOthers(res_employee?.data?.resulted);
            setIsLoader(true)
        } catch (err) {setIsLoader(true);handleApiError(err, setShowAlert, handleClickOpenerr);}
    }
    ...
    const searchOverTerms = searchQueryOthers.toLowerCase().split(" ");
    ...
    let overallCount = 0;
    const totalcount = filteredDataOthers && (
        filteredDataOthers.forEach((item) => {
    ...
    const rowDataTable = filteredDataOthers.map((item, index) => {
        return {
    ...
    const downloadExcelSecondary = async () => {
        ...
        try {
            ...
            if (!tableDataOthers?.length) {
                await fetchDataOrderOthers();
            }
            ...
            tableDataOthers.forEach((row, index) => {
                worksheet.addRow({
            ...
            a.href = url;
            a.download = 'Workorder_Others.xlsx'; // File name
            a.click();
        } catch (error) {
    ...
    const downloadCsvSecondary = () => {
        ...
        saveAs(blob, 'Workorder_Others.csv'); // Specify the filename with .csv extension
    };
    //  PDF
    const columns = [
    ...
    return (
        <>
            ...
            {isUserRoleCompare?.includes("lotherindividualworkorderlist")
                && (
                    <>
...
export default Othersworkorder;