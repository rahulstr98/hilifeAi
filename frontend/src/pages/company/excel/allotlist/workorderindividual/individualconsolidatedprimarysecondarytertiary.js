...
const IndividualPrimarySeondaryTertiaryAll = () => {
    const { isUserRoleAccess, isUserRoleCompare, isAssignBranch, allTeam, allUsersData } = useContext(UserRoleAccessContext);
    ...
    const handleCaptureImage = () => {
        ...
        html2canvas(clonedTable).then((canvas) => {
            ...
            link.href = dataURL;
            link.download = "Individual_Consolidated_WorkOrder(pri/sec/ter)_list.png";
            link.click();
        });
    };
    ...
    const downloadExcelSecondary = async () => {
        ...
        try {
            ...
            a.href = url;
            a.download = 'Individual_Consolidated_WorkOrder(pri/sec/ter)_list.xlsx'; // File name
            a.click();
        } catch (error) {
    ...
    const downloadCsvSecondary = () => {
        ...
        saveAs(blob, 'Individual_Consolidated_WorkOrder(pri/sec/ter)_list.csv'); // Specify the filename with .csv extension
    };
    ...
    const fetchConsWorkOrderList = async () => {
        try {
            let res_unit = await axios.post(SERVICE.CONSOLIDATED_INDIVIDUAL_LIST, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
    ...
    const OnFilterSubmit = async () => {
        try {
            let res_employee = await axios.post(SERVICE.CONSOLIDATED_INDIVIDUAL_LISTFILTER, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
                companyname: String(isUserRoleAccess.companyname),
    ...
    const searchTerms = searchQueryPriorityAll.toLowerCase().split(" ");
    ...
    const downloadPdfPriorityAll = () => {
        ...
        doc.autoTable({
            ...
            body: filteredDataPriorityAll,
        });
        doc.save("Individual_Consolidated_WorkOrder(pri/sec/ter)_list.pdf");
    };
    ...
    const handleprintPriorityAll = useReactToPrint({
        ...
        documentTitle: 'Individual_Consolidated_WorkOrder(pri/sec/ter)_list',
        pageStyle: 'print'
    });
    ...
    return (
        <>
            ...
            {isUserRoleCompare?.includes("lconsolidated(primary/secondary/tertiary)individualworkorderlist")
                && (
                    <>
                        <Box sx={userStyle.container}>
                            <Typography sx={userStyle.SubHeaderText}>Individual Consolidated WorkOrder List </Typography>
                            <br /><br />
                                ...
                                <Grid item lg={3} md={3} xs={12} sm={6}>
                                    ...
                                    <FormControl size="small" fullWidth>
                                        <Selects
                                            ...
                                            onChange={(e) => { setNewcheckBranch(e.value); fetchUnitsDrop(e.value); setNewcheckUnit("Select Unit"); handleChangeBranch(e); setAllFilters({ ...allFilters, branch: e.value, unit: "Please Select Unit", team: "Please Select Team", responsibleperson: "Please Select ResponsiblePerson" }) }}
                                        />
                                    </FormControl>
                                </Grid>
                                <Grid item lg={3} md={3} xs={12} sm={6}>
                            ...
                            <Grid style={userStyle.dataTablestyle}>
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'end' }}>
                                    <Grid >
                                        {isUserRoleCompare?.includes("csvconsolidated(primary/secondary/tertiary)individualworkorderlist")
                                            && (
                                                <>
                                                    <Button sx={userStyle.buttongrp} onClick={downloadCsvSecondary}>&ensp;<FaFileCsv />&ensp;Export to CSV&ensp;</Button>

                                                </>)}
                                        {isUserRoleCompare?.includes("excelconsolidated(primary/secondary/tertiary)individualworkorderlist")
                                            && (
                                                <>
                                                    <Button sx={userStyle.buttongrp} onClick={downloadExcelSecondary}>&ensp;<FaFileExcel />&ensp;Export to Excel&ensp;</Button>
                                                </>)}
                                        {isUserRoleCompare?.includes("printconsolidated(primary/secondary/tertiary)individualworkorderlist")
                                            && (
                                                <>
                                                    <Button sx={userStyle.buttongrp} onClick={handleprintPriorityAll}>&ensp;<FaPrint />&ensp;Print&ensp;</Button>
                                                </>)}
                                        {isUserRoleCompare?.includes("pdfconsolidated(primary/secondary/tertiary)individualworkorderlist")
                                            && (
                                                <>
                                                    <Button sx={userStyle.buttongrp} onClick={() => downloadPdfPriorityAll()}><FaFilePdf />&ensp;Export to PDF&ensp;</Button>
                                                </>)}
                                        {isUserRoleCompare?.includes("imageconsolidated(primary/secondary/tertiary)individualworkorderlist")
                                            && (
                                                <>
...
export default IndividualPrimarySeondaryTertiaryAll;