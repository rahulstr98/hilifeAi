...
function Queuewise() {
    ...
    const [queuewise, setqueuewise] = useState([]);
    ...
    // fetch count based on the queuees
    const fetchqueueTotal = async () => {
        try {
            let res_queue = await axios.get(SERVICE.QUEUEWISEREPORTEXCELUPLOAD, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
            });

            setqueuewise(res_queue?.data?.resultoutput);
            setIsLoader(true);
    ...
    const addSerialNumber = () => {
        const itemsWithSerialNumber = queuewise?.map((item, index) => ({ ...item, serialNumber: index + 1, id: index }));
    ...
    // Excel
    const fileName = "QueuewiseReport";

    //print...
    const componentRef = useRef();
    const handleprint = useReactToPrint({
        content: () => componentRef.current,
        documentTitle: "QueuewiseReport",
        pageStyle: "print",
    });
    ...
    const downloadPdf = () => {
        const doc = new jsPDF()
        autoTable(doc, { html: '#queuewisetable' })
        doc.save("QueuewiseReport.pdf");
    };
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, 'QueuewiseReport.png');
                });
            });
        }
    };
    ...
    return (
        <Box>
            <Headtitle title={'Queue Wise Report List'} />
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Queue Wise Report List</Typography>
            <br />
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("lqueuewisereport")
                && (
                    <>
                        <Box sx={userStyle.container}>
                            {/* ******************************************************EXPORT Buttons****************************************************** */}
                            <Grid item xs={8}>
                                <Typography sx={userStyle.importheadtext}>Manage Queue Wise Queue Report</Typography>
                            </Grid>
                            <Grid container sx={{ justifyContent: "center" }}>
                                <Grid>
                                    {isUserRoleCompare?.includes("excelqueuewisereport")
                                        && (
                                            <>
                                                <ExportXL csvData={filteredData.map(t => ({
                                                    "SNo": t.serialNumber, "Queue": t.queue, "Total Count": t.count, "Total Points": t.points, "Total Time": t.time
                                                }))} fileName={fileName} />

                                            </>
                                        )}
                                    {isUserRoleCompare?.includes("csvqueuewisereport")
                                        && (
                                            <>
                                    ...
                                    {isUserRoleCompare?.includes("printqueuewisereport")
                                        && (
                                            <>
                                                <Button sx={userStyle.buttongrp} onClick={handleprint}>  &ensp;  <FaPrint />  &ensp;Print&ensp;  </Button>
                                            </>
                                        )}
                                    {isUserRoleCompare?.includes("pdfqueuewisereport")
                                        && (
                                            <>
                                                <Button sx={userStyle.buttongrp} onClick={() => downloadPdf()}>  <FaFilePdf />  &ensp;Export to PDF&ensp;  </Button>
                                            </>
                                        )}
                                    {isUserRoleCompare?.includes("imagequeuewisereport")
                                        && (
                                            <>
                                    ...
                                    <Grid style={userStyle.dataTablestyle}>
                                        <Box>
                                            ...
                                            <Select id="pageSizeSelect" value={pageSize} onChange={handlePageSizeChange} sx={{ width: "77px" }}>
                                                ...
                                                <MenuItem value={(queuewise.length)}>All</MenuItem>
                                            </Select>
                                        </Box>
                                        <Box>
                                                        ...
                                                        {filteredData?.length > 0 ? (
                                                            filteredData?.map((row, index) => (
                                                                <StyledTableRow key={index} sx={{ background: selectedRows.includes(row.id) ? "#1976d22b !important" : "inherit" }}>
                                                                    ...
                                                                    <StyledTableCell sx={{ textAlign: 'left', minWidth: '150px', padding: '0px 5px !IMPORTANT' }}>{row.queue}</StyledTableCell>
                                            ...
                                            <TableFooter sx={{ backgroundColor: '#9591914f', height: '50px' }}>
                                                {filteredData && (
                                                    filteredData.forEach((item) => {
                                                        ...
                                                        const queueTime = item.time.split(":");
                                                        const hours = parseInt(queueTime[0]);
                                                        const minutes = parseInt(queueTime[1]);
                                                        const seconds = parseInt(queueTime[2]);
                                        ...
                                        {filteredData?.length > 0 ? (
                                            filteredData?.map((row, index) => (
                                                <StyledTableRow key={index} >
                                                    ...
                                                    <StyledTableCell sx={{ padding: '10px !IMPORTANT' }}>{row.queue}</StyledTableCell>
