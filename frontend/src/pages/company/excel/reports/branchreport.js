...
import { TableCell, TableRow, TableFooter, Box, Typography, TableBody, Checkbox, Paper, Grid, Table, TableHead, TableContainer, Button, FormControl, OutlinedInput, Select, MenuItem, Dialog, DialogContent, DialogActions } from "@mui/material";
...
function Branchwise() {
    ...
    // fetch count based on the branches
    const fetchBranchTotal = async () => {
        try {
            let res_branch = await axios.get(SERVICE.BRANCHWISEREPORTEXCELUPLOAD, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
            });
            setBranchwise(res_branch?.data?.resultoutput);
            setBranches(res_branch?.data?.branches);
            setIsLoader(true);
    ...
    branches.forEach((item, index) => {
        branchIndexMap[item.name] = index;
    });

    // Sort the datas array based on the branch order in the branch array
    branchwise.sort((a, b) => {
        const indexA = branchIndexMap[a.branch];
        const indexB = branchIndexMap[b.branch];
    ...
    const addSerialNumber = () => {
        const itemsWithSerialNumber = branchwise?.map((item, index) => ({ ...item, serialNumber: index + 1, id: index }));
    ...
    // Excel
    const fileName = "BranchwiseQueueReport";

    //print...
    const componentRef = useRef();
    const handleprint = useReactToPrint({
        content: () => componentRef.current,
        documentTitle: "BranchwiseQueueReport",
        pageStyle: "print",
    });

    const downloadPdf = () => {
        const doc = new jsPDF()
        autoTable(doc, { html: '#branchwisetable' })
        doc.save("BranchwiseQueueReport.pdf");
    };
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, 'BranchwiseQueueReport.png');
                });
            });
        }
    };
    ...
    return (
        <Box>
            <Headtitle title={'Branch Wise Queue Report List'} />
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Branch Wise Queue Report List</Typography>
            <br />
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("lbranchwisereport")
                && (
                    <>
                        <Box sx={userStyle.container}>
                            {/* ******************************************************EXPORT Buttons****************************************************** */}
                            <Grid item xs={8}>
                                <Typography sx={userStyle.importheadtext}>Manage Branch Wise Queue Report</Typography>
                            </Grid>
                            <br />  <br />
                            <Grid container sx={{ justifyContent: "center" }}>
                                <Grid>
                                    {isUserRoleCompare?.includes("excelbranchwisereport")
                                        && (
                                            <>
                                                <ExportXL csvData={filteredData.map(t => ({
                                                    "SNo": t.serialNumber, "Branch": t.branch, "Total Count": t.count, "Total Points": t.points, "Total Time": t.time
                                                }))} fileName={fileName} />

                                            </>
                                        )}
                                    {isUserRoleCompare?.includes("csvbranchwisereport")
                                        && (
                                            <>
                                    ...
                                    {isUserRoleCompare?.includes("pdfbranchwisereport")
                                        && (
                                            <>
                                                <Button sx={userStyle.buttongrp} onClick={() => downloadPdf()}>  <FaFilePdf />  &ensp;Export to PDF&ensp;  </Button>
                                            </>
                                        )}
                                    {isUserRoleCompare?.includes("imagebranchwisereport") && (
                                        <>
                                    ...
                                    <Grid style={userStyle.dataTablestyle}>
                                        <Box>
                                            ...
                                            <Select id="pageSizeSelect" value={pageSize} onChange={handlePageSizeChange} sx={{ width: "77px" }}>
                                                ...
                                                <MenuItem value={(branchwise.length)}>All</MenuItem>
                                            </Select>
                                        </Box>
                                        <Box>
                                                        ...
                                                        {filteredData?.length > 0 ? (
                                                            filteredData?.map((row, index) => (
                                                                <StyledTableRow key={index} sx={{ background: selectedRows.includes(row.id) ? "#1976d22b !important" : "inherit" }}>
                                                                    ...
                                                                    <StyledTableCell sx={{ textAlign: 'left', minWidth: '150px', padding: '0px 5px !IMPORTANT' }}>{row.branch}</StyledTableCell>
                                        ...
                                        {filteredData?.length > 0 ? (
                                            filteredData?.map((row, index) => (
                                                <StyledTableRow key={index} >
                                                    ...
                                                    <StyledTableCell sx={{ padding: '10px !IMPORTANT' }}>{row.branch}</StyledTableCell>
