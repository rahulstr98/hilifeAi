...
function Categorywise() {
    ...
    // fetch count based on the categoryes
    const fetchCategoryTotal = async () => {
        try {
            let res_category = await axios.get(SERVICE.CATEGORYWISEREPORTEXCELUPLOAD, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
            });
            setCategorywise(res_category?.data?.resultoutput);
            setIsLoader(true);
    ...
    const addSerialNumber = () => {
        const itemsWithSerialNumber = categorywise?.map((item, index) => ({ ...item, serialNumber: index + 1, id: index }));
    ...
    // Excel
    const fileName = "CategorywiseQueueReport";

   //print...
    const componentRef = useRef();
    const handleprint = useReactToPrint({
        content: () => componentRef.current,
        documentTitle: "CategorywiseQueueReport",
        pageStyle: "print",
    });
    ...
    const downloadPdf = () => {
        ...
        doc.save("CategorywiseQueueReport.pdf");
    };
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, 'CategorywiseQueueReport.png');
                });
            });
        }
    };
    ...
    return (
        <Box>
            <Headtitle title={'Category Wise Queue Report List'} />
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Category Wise Queue Report List</Typography>
            <br />
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("lcategorywisereport")
                && (
                    ...
                    <>
                        <Box sx={userStyle.container}>
                            {/* ******************************************************EXPORT Buttons****************************************************** */}
                            <Grid item xs={8}>
                                <Typography sx={userStyle.importheadtext}>Manage Category Wise Queue Report</Typography>
                            </Grid>
                            <Grid container sx={{ justifyContent: "center" }}>
                                <Grid>
                                    {isUserRoleCompare?.includes("excelcategorywisereport")
                                        && (
                                            <>
                                                <ExportXL csvData={filteredData.map(t => ({
                                                    "SNo": t.serialNumber, "Category": t.category, "Total Count": t.count, "Total Points": t.points, "Total Time": t.time
                                                }))} fileName={fileName} />

                                            </>
                                        )}
                                    {isUserRoleCompare?.includes("csvcategorywisereport")
                                        && (
                                            <>
                                    ...
                                    {isUserRoleCompare?.includes("printcategorywisereport")
                                        && (
                                            <>
                                                <Button sx={userStyle.buttongrp} onClick={handleprint}>  &ensp;  <FaPrint />  &ensp;Print&ensp;  </Button>
                                            </>
                                        )}
                                    {isUserRoleCompare?.includes("pdfcategorywisereport")
                                        && (
                                            <>
                                                <Button sx={userStyle.buttongrp} onClick={() => downloadPdf()}>  <FaFilePdf />  &ensp;Export to PDF&ensp;  </Button>
                                            </>
                                        )}
                                    {isUserRoleCompare?.includes("imagecategorywisereport")
                                        && (
                                            <>
                                    ...
                                    <Grid style={userStyle.dataTablestyle}>
                                        <Box>
                                            ...
                                            <Select id="pageSizeSelect" value={pageSize} onChange={handlePageSizeChange} sx={{ width: "77px" }}>
                                                ...
                                                <MenuItem value={(categorywise.length)}>All</MenuItem>
                                            </Select>
                                        </Box>
                                        <Box>
                                                        ...
                                                        {filteredData?.length > 0 ? (
                                                            filteredData?.map((row, index) => (
                                                                <StyledTableRow key={index} sx={{ background: selectedRows.includes(row.id) ? "#1976d22b !important" : "inherit" }}>
                                                                    ...
                                                                    <StyledTableCell sx={{ textAlign: 'left', minWidth: '150px', padding: '0px 5px !IMPORTANT' }}>{row.category}</StyledTableCell>
                                        ...
                                        {filteredData?.length > 0 ? (
                                            filteredData?.map((row, index) => (
                                                <StyledTableRow key={index} >
                                                    ...
                                                    <StyledTableCell sx={{ padding: '10px !IMPORTANT' }}>{row.category}</StyledTableCell>
