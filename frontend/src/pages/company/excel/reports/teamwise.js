...
import { Box, Typography, OutlinedInput, Select, MenuItem, Dialog, Checkbox, TableBody, TableCell, TableFooter, TableRow, DialogContent, DialogActions, FormControl, Grid, Paper, Table, TableHead, TableContainer, Button } from "@mui/material";
...
function Teamwise() {
    ...
    const [teamwise, setTeamwise] = useState([]);
    ...
    const fetchTeamTotal = async () => {
        try {
            let res_team = await axios.get(SERVICE.TEAMWISEREPORTEXCELUPLOAD, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
            });
            setTeamwise(res_team?.data?.resultoutput);
            setIsLoader(true);
    ...
    // Excel
    const fileName = "TeamwiseQueueReport";
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "TeamwiseQueueReport",
        pageStyle: "print",
    });
    ...
    const downloadPdf = () => {
        ...
        doc.save("TeamwiseQueueReport.pdf");
    };
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, 'TeamwiseQueueReport.png');
                });
            });
        }
    };
    ...
    teams.forEach((item, index) => {
        teamIndexMap[item.name] = index;
    });

    // Sort the datas array based on the team order in the team array
    teamwise.sort((a, b) => {
        const indexA = teamIndexMap[a.team];
        const indexB = teamIndexMap[b.team];
    ...
    const addSerialNumber = () => {
        const itemsWithSerialNumber = teamwise?.map((item, index) => ({ ...item, serialNumber: index + 1, id: index }));
    ...
    return (
        <Box>
            <Headtitle title={'Team Wise Queue Report List'} />
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.SubHeaderText}>Team Wise Queue Report List</Typography>
            <br />
            {/* ****** Table Start ****** */}
            {isUserRoleCompare.includes("lteamwisereport") && (

                <>
                    <Box sx={userStyle.container}>
                        {/* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>Manage Team Wise Queue Report</Typography>
                        </Grid>
                        <br />  <br />
                        <Grid container sx={{ justifyContent: "center" }}>
                            <Grid>
                                {isUserRoleCompare.includes("excelteamwisereport") && (
                                    <>
                                        <ExportXL csvData={filteredData.map(t => ({
                                            "SNo": t.serialNumber, "Team": t.team, "Total Count": t.count, "Total Points": t.points, "Total Time": t.time
                                        }))} fileName={fileName} />

                                    </>
                                )}
                                {isUserRoleCompare.includes("csvteamwisereport") && (
                                    <>
                                ...
                                {isUserRoleCompare.includes("printteamwisereport") && (
                                    <>
                                        <Button sx={userStyle.buttongrp} onClick={handleprint}>  &ensp;  <FaPrint />  &ensp;Print&ensp;  </Button>
                                    </>
                                )}
                                {isUserRoleCompare.includes("imageteamwisereport") && (
                                    <>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                            <Box>
                                <Grid style={userStyle.dataTablestyle}>
                                    <Box>
                                        ...
                                        <Select id="pageSizeSelect" defaultValue="" value={pageSize} onChange={handlePageSizeChange} sx={{ width: "77px" }}>
                                            ...
                                            <MenuItem value={(teamwise?.length)}>All</MenuItem>
                                        </Select>
                                    </Box>
                                    <Box>
                                                    ...
                                                    {filteredData?.length > 0 ? (
                                                        filteredData?.map((row, index) => (
                                                            <StyledTableRow key={index} sx={{ background: selectedRows.includes(row.id) ? "#1976d22b !important" : "inherit" }}>
                                                                ...
                                                                <StyledTableCell sx={{ textAlign: 'left', minWidth: '150px', padding: '0 10px !IMPORTANT' }}>{row.team}</StyledTableCell>
                                ...
                                <Box style={userStyle.dataTablestyle}>
                                    <Box>
                                        Showing  {filteredDatas.length > 0 ? ((page - 1) * pageSize) + 1 : 0}  to {Math.min(page * pageSize, filteredDatas?.length)} of {filteredDatas?.length} entries
                                    </Box>
                                    <Box>
                                    ...
                                    {filteredData?.length > 0 ? (
                                        filteredData?.map((row, index) => (
                                            <StyledTableRow key={index} >
                                                ...
                                                <StyledTableCell sx={{ padding: '10px !IMPORTANT' }}>{row.team}</StyledTableCell>
