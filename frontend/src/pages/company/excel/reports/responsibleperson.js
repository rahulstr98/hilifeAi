...
import { TableCell, TableRow, TableFooter, Box, Checkbox, Typography, TableBody, Paper, Grid, Table, TableHead, TableContainer, Button, FormControl, OutlinedInput, Select, MenuItem, Dialog, DialogContent, DialogActions } from "@mui/material";
...
function ResponsiblePerson() {
    ...
    const fetchRespersonTotal = async () => {
        try {
            let res_branch = await axios.get(SERVICE.RESPERSONWISEREPORTEXCELUPLOAD, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
            });
            setResponsiblePerson(res_branch?.data?.resultoutput);
            setIsLoader(true);
    ...
    const addSerialNumber = () => {
        const itemsWithSerialNumber = responsibleperson?.map((item, index) => ({ ...item, serialNumber: index + 1 }));
    ...
    // Excel
    const fileName = "ResponsiblePersonwiseQueueReport";
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "ResponsiblePersonwiseQueueReport",
        pageStyle: "print",
    });

    const downloadPdf = () => {
        const doc = new jsPDF()
        autoTable(doc, { html: '#responsiblepersonwisetable' })
        doc.save("ResponsiblePersonwiseQueueReport.pdf");
    };
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, 'ResponsiblePersonwiseQueueReport.png');
                });
            });
        }
    };
    ...
    return (
        <Box>
            <Headtitle title={'Responsible Person Wise Queue Report List'} />
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Responsible Person Wise Queue Report List</Typography>
            <br />
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("lresponsiblepersonwisereport")
                && (
                    <>
                        <Box sx={userStyle.container}>
                            {/* ******************************************************EXPORT Buttons****************************************************** */}
                            <Grid item xs={8}>
                                <Typography sx={userStyle.importheadtext}>Manage Responsible Person Wise Queue Report</Typography>
                            </Grid>
                            <Grid container sx={{ justifyContent: "center" }}>
                                <Grid>
                                    {isUserRoleCompare?.includes("excelresponsiblepersonwisereport")
                                        && (
                                            <>
                                                <ExportXL csvData={filteredData.map(t => ({
                                                    "SNo": t.serialNumber, "Responsible Person": t.resperson, "Total Count": t.count, "Total Points": t.points, "Total Time": t.time
                                                }))} fileName={fileName} />

                                            </>
                                        )}
                                    {isUserRoleCompare?.includes("csvresponsiblepersonwisereport")
                                        && (
                                            <>
                                    ...
                                    {isUserRoleCompare?.includes("printresponsiblepersonwisereport")
                                        && (
                                            <>
                                                <Button sx={userStyle.buttongrp} onClick={handleprint}>   &ensp;  <FaPrint />  &ensp;Print&ensp;  </Button>
                                            </>
                                        )}
                                    {isUserRoleCompare?.includes("pdfresponsiblepersonwisereport")
                                        && (
                                            <>
                                                <Button sx={userStyle.buttongrp} onClick={() => downloadPdf()}>  <FaFilePdf />  &ensp;Export to PDF&ensp;  </Button>
                                            </>
                                        )}
                                    {isUserRoleCompare?.includes("imageresponsiblepersonwisereport") && (
                                        <>
                                    ...
                                    <Grid style={userStyle.dataTablestyle}>
                                        <Box>
                                            ...
                                            <Select id="pageSizeSelect" value={pageSize} onChange={handlePageSizeChange} sx={{ width: "77px" }}>
                                                ...
                                                <MenuItem value={(responsibleperson.length)}>All</MenuItem>
                                            </Select>
                                        </Box>
                                        <Box>
                                                        ...
                                                        {filteredData?.length > 0 ? (
                                                            filteredData?.map((row, index) => (
                                                                <StyledTableRow key={index} sx={{ background: selectedRows.includes(row.id) ? "#1976d22b !important" : "inherit" }}>
                                                                    <StyledTableCell sx={{ textAlign: 'center', padding: '0 5px !IMPORTANT' }}>
                                                                        <Checkbox
                                                                            checked={selectedRows.includes(row.id)}
                                                                            onChange={() => handleCheckboxChange(row.id)}
                                                                        />
                                                                    </StyledTableCell>
                                                                    <StyledTableCell sx={{ textAlign: 'left', maxWidth: '50px', padding: '0 10px !IMPORTANT' }}>{row.serialNumber}</StyledTableCell>
                                                                    <StyledTableCell sx={{ textAlign: 'left', minWidth: '150px', padding: '0 10px !IMPORTANT' }}>{row.resperson}</StyledTableCell>
                                                                    <StyledTableCell sx={{ textAlign: 'left', padding: '0 10px !IMPORTANT' }}>{row.count}</StyledTableCell>
                                                                    <StyledTableCell sx={{ textAlign: 'left', padding: '0 10px !IMPORTANT' }}>{(row.points).toFixed(4)}</StyledTableCell>
                                                                    <StyledTableCell sx={{ textAlign: 'left', padding: '0 10px !IMPORTANT' }}>{row.time}</StyledTableCell>
                                                                </StyledTableRow>
                                        ...
                                        {filteredData?.length > 0 ? (
                                            filteredData?.map((row, index) => (
                                                <StyledTableRow key={index} >
                                                    ...
                                                    <StyledTableCell sx={{ padding: '10px !IMPORTANT' }}>{row.resperson}</StyledTableCell>
