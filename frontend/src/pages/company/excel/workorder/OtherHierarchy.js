...
const Othersworkorder = () => {
    ...
    const handleCaptureImage = () => {
        ...
        html2canvas(clonedTable).then((canvas) => {
            ...
            link.href = dataURL;
            link.download = "WorkOrder List_Others.png";
            link.click();
        });
    };
...
const TableFilter = async () => {
    try {
        let res_employee = await axios.post(SERVICE.OTHER_WORKORDER_HIERARCHY_FILTER, {
            headers: {
                'Authorization': `Bearer ${auth.APIToken}`
            },
         ...
         setTableDataOthers(res_employee?.data?.resultAccessFilter);
        } catch (err) {setIsLoader(true);handleApiError(err, setShowAlert, handleClickOpenerr);}
...
const FilterSubmitDefault = async ()=>{
    try {
        let res_employee = await axios.post(SERVICE.OTHER_WORKORDER_DEFAULT_HIERARCHY_FILTER, {
            headers: {
                'Authorization': `Bearer ${auth.APIToken}`
            },
            username : isUserRoleAccess.companyname,
...
useEffect(() => {
    ...
    return (
        <>
            <Headtitle title={'Others Hierarchy WorkOrder List'} />
            {isUserRoleCompare?.includes("lotherhierarchyworkorderlist")
                && (
                    <>
                        <Box sx={userStyle.container}>
                            <Typography sx={userStyle.SubHeaderText}>Others Hierarchy WorkOrder List</Typography>
                            <br /><br />
