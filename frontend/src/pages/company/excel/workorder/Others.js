...
const Othersworkorder = () => {
    ...
    const OnFilterSubmit = async (e) => {
        setIsLoader(false)
        try {
            let res_employee = await axios.post(SERVICE.OTHERWORKORDERALL, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
            ...
            const itemsWithSerialNumberSecondary = ans?.map((item, index) => ({ ...item, 
                serialNumber: (pageOthers - 1) * pageSizeOthers + index + 1,}));

            setTotalProjects(ans?.length > 0 ? res_employee?.data?.totalProjects : 0);
            setTotalPages(ans?.length > 0 ? res_employee?.data?.totalPages : 0);
            setPageSizeOthers((data) => { return ans?.length > 0 ? data : 10 });
            setPageOthers((data) => { return ans?.length > 0 ? data : 1 });
            setTableDataOthers(itemsWithSerialNumberSecondary);
    ...
    const filteredDatasOthers = tableDataOthers?.filter((item) => {
    ...
    const totalcount = filteredDatasOthers && (
        filteredDatasOthers.forEach((item) => {
    ...
    const rowDataTable = filteredDatasOthers.map((item, index) => {
        return {
            id: index,
    ...
    const downloadExcelSecondary = async () => {
        ...
        try {
            ...
            filteredDatasOthers.forEach((row, index) => {
                worksheet.addRow({
    ...
    const handleCaptureImage = () => {
        ...
        html2canvas(clonedTable).then((canvas) => {
            ...
            link.href = dataURL;
            link.download = "WorkOrder_List_Others.png";
            link.click();
        });
    };
    ...
    return (
        <>
            ...
            {isUserRoleCompare?.includes("lothersworkorderlist")
                && (
                    <>
                        <Box sx={userStyle.container}>
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item lg={3} md={3} xs={12} sm={6}>
                                <Button variant="contained" onClick={(e) => handleSubmit(e)} >Filter</Button>                                </Grid>
                                <Grid item lg={3} md={3} xs={12} sm={6}>
                            ...
                            {!isLoader ?
                                ...
                                <>
                                    ...
                                    <br />
                                     <Box>
                                        <Pagination
                                            page={searchQueryOthers !== "" ? 1 : pageOthers}
                                            pageSize={pageSizeOthers}
                                            totalPages={searchQueryOthers !== "" ? 1 : totalPages}
                                            onPageChange={handlePageChangeOthers}
                                            pageItemLength={filteredDatasOthers?.length}
                                            totalProjects={
                                                searchQueryOthers !== "" ? filteredDatasOthers?.length : totalProjects
                                            }
                                        />                                 
                                    </Box>
                                    {/* ****** Table End ****** */}
            ...
            <TableContainer component={Paper} sx={userStyle.printcls} >
                ...
                >
                    ...
                    <TableBody>
                        {filteredDatasOthers?.length > 0 &&
                            (filteredDatasOthers?.map((row, index) => (
                                <TableRow key={index}>
