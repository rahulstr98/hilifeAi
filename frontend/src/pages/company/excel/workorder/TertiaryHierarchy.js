...
const TertiaryHierarchyWorkOrder = () => {
    ...
    const OnFilterSubmit = async () => {
        try {
            ...
            setTableDataTertiary(res_employee.data.resulted);
            setIsLoader(true)
    ...
    const totalcount = filteredDataTertiary && (
        filteredDataTertiary.forEach((item) => {
    ...
    const rowDataTable = filteredDataTertiary.map((item, index) => {
        return {
            id: index,
    ...
    const downloadExcelSecondary = async () => {
        ...
        try {
            ...
            if (!tableDataTertiary?.length) {
                // await fetchTertiaryList();
            }
            ...
            filteredDataTertiary.forEach((row, index) => {
                worksheet.addRow({
            ...
            filteredDataTertiary.forEach((row, index) => {
                ...
                worksheet.getCell(`C${index + 2}`).value =(row.process);
            ...
            a.href = url;
            a.download = 'Tertiary_WorkOrder_Hierarchy_List.xlsx'; // File name
            a.click();
        } catch (error) {
    ...
    const downloadCsvSecondary = () => {
        ...
        const headers = ['Sno', 'Priority', 'Customer', 'Process Hyperlink', 'Count', 'Tat Expiration', 'Created',
            'Branch', 'Responsible Person', 'Category Name', 'Subcategory Name', 'Queue Name', 'S.No', 'Project Name', 'Vendor Name', 'Unit', 'Team', 'Sector', 'Points', 'Time', 'Level', 'Control'
        ];
        ...
        saveAs(blob, 'Tertiary_WorkOrder_Hierarchy_List.csv'); // Specify the filename with .csv extension
    };
    ...
    const downloadPdfTertiary = () => {
        ...
        doc.autoTable({
            ...
            body: filteredDataTertiary,
        });
        doc.save("Tertiary_WorkOrder_Hierarchy_List.pdf");
    };
    ...
    const handleprintTertiary = useReactToPrint({
        ...
        documentTitle: 'Tertiary_WorkOrder_Hierarchy_List',
        pageStyle: 'print'
    });
    ...
    const handleCaptureImage = () => {
        ...
        html2canvas(clonedTable).then((canvas) => {
            ...
            link.href = dataURL;
            link.download = "Tertiary_WorkOrder_Hierarchy_List.png";
            link.click();
        });
    };
    ...
    const TableFilter = async () => {
        setIsLoader(false)
        try {
            let res_employee = await axios.post(SERVICE.TERTIARY_CONSOLIDATED_HIERARCHY_FILTER, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
            ...
            setIsLoader(true)
            setTableDataTertiary(res_employee?.data?.resultAccessFilter);
        } catch (err) {setIsLoader(true);handleApiError(err, setShowAlert, handleClickOpenerr);}
    }
    ...
    const FilterSubmitDefault = async () => {
        try {
            let res_employee = await axios.post(SERVICE.TERTIARY_DEFAULT_HIERARCHY_FILTER, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
                username: isUserRoleAccess.companyname,
    ...
    return (
        <>
            <Headtitle title={'Tertiary WorkOrder Hierarchy List'} />
            {isUserRoleCompare?.includes("ltertiaryhierarchyworkorderlist")
                && (
                    <>
                        <Box sx={userStyle.container}>
                            <Typography sx={userStyle.SubHeaderText}>Tertiary WorkOrder Hierarchy List</Typography>
                            <br /><br />
...
s                        <Typography variant="h6" >{showAlert}</Typography>
                    </DialogContent>
                    <DialogActions>
...
export default TertiaryHierarchyWorkOrder;