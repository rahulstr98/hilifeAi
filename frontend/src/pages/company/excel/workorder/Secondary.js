...
import SecondaryConsolidated from './Secondaryconsolidated'
...
const Seondaryworkorder = () => {
    ...
    const [updatedComponent, setUpdatedComponent] = useState("Change")
    ...
    }, [pageSizeSecondary , pageSecondary])
    ...
    const OnFilterSubmit = async (e) => {
        ...
        try {
            let res_employee = await axios.post(SERVICE.SECONDARY_CONSOLIDATED_WORKORDER_FILTER, {
                ...
                page: pageSecondary,
                pageSize: pageSizeSecondary,
                value: e
            });

            const ans = res_employee?.data?.resulted?.length > 0 ? res_employee?.data?.resulted : []
            const itemsWithSerialNumberSecondary = ans?.map((item, index) => ({ ...item, serialNumber: index + 1 }));
            setTotalProjects(ans?.length > 0 ? res_employee?.data?.totalProjects : 0);
            setTotalPages(ans?.length > 0 ? res_employee?.data?.totalPages : 0);
            setPageSizeSecondary((data) => { return ans?.length > 0 ? data : 10 });
            setPageSecondary((data) => { return ans?.length > 0 ? data : 1 });
            setTableDataSecondary(itemsWithSerialNumberSecondary);
            setIsLoader(true)
    ...
    const filteredDatasSecondary = tableDataSecondary?.filter((item) => {
    ...
    let overallCount = 0;
    const totalcount = filteredDatasSecondary && (
        filteredDatasSecondary.forEach((item) => {
    ...
    const rowDataTable = filteredDatasSecondary.map((item, index) => {
        return {
            id: index,
    ...
    const downloadExcelSecondary = async () => {
        ...
        try {
            ...
            filteredDatasSecondary.forEach((row, index) => {
                worksheet.addRow({
            ...
            a.href = url;
            a.download = 'Workorder_Secondary_.xlsx'; // File name
            a.click();
        } catch (error) {
    ...
    const downloadCsvSecondary = () => {
        ...
        saveAs(blob, 'Workorder_Secondary.csv'); // Specify the filename with .csv extension
    };
    ...
    const handleCaptureImage = () => {
        if (gridRefSeco.current) {
          html2canvas(gridRefSeco.current).then((canvas) => {
            canvas.toBlob((blob) => {
              saveAs(blob, "WorkOrder_Secondary.png");
            });
          });
        }
      };
    ...
    return (
        <>
            <Headtitle title={'WorkOrder List Secondary'} />
            {isUserRoleCompare?.includes("lsecondaryworkorderlist")
                && (
                    <>
                        <Box sx={userStyle.container}>
                            ...
                            {!isLoader ?
                                ...
                                <>
                                    ...
                                    <Box>
                                        <Pagination
                                            page={searchQuerySecondary !== "" ? 1 : pageSecondary }
                                            pageSize={pageSizeSecondary}
                                            totalPages={searchQuerySecondary !== "" ? 1 : totalPages}
                                            onPageChange={handlePageChangeSecondary}
                                            pageItemLength={filteredDatasSecondary?.length}
                                            totalProjects={
                                                searchQuerySecondary !== "" ? filteredDatasSecondary?.length : totalProjects
                                            }
                                        />                                  
                                    </Box>
                                    {/* ****** Table End ****** */}
            ...
            <br />
            <SecondaryConsolidated  comp={updatedComponent}  setComp={setUpdatedComponent}/>
            <br />
            ...
            <TableContainer component={Paper} sx={userStyle.printcls} >
                ...
                >
                    ...
                    <TableBody>
                        {filteredDatasSecondary &&
                            (filteredDatasSecondary.map((row, index) => (
                                <TableRow key={index}>
