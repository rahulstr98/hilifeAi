...
const fetchUsers = async () =>{
    ...
    const downloadExcelSecondary = async () => {
        ...
        try {
            ...
            a.href = url;
            a.download = 'Consolidated_All_Hierarchy_List.xlsx'; // File name
            a.click();
        } catch (error) {
    ...
    const downloadCsvSecondary = () => {
        ...
        saveAs(blob, 'Consolidated_All_Hierarchy_List.csv'); // Specify the filename with .csv extension
    };
    ...
    const downloadPdfOverAll = () => {
        ...
        doc.save("Consolidated_All_Hierarchy_List.pdf");
    };
    ...
    const handleprintOverAll = useReactToPrint({
        ...
        documentTitle: 'Consolidated_All_Hierarchy_List',
        pageStyle: 'print'
    });
    ...
    const handleCaptureImage = () => {
        ...
        html2canvas(clonedTable).then((canvas) => {
            ...
            link.href = dataURL;
            link.download = 'Consolidated_All_Hierarchy_List.png';
            link.click();
        });
    };
    ...
    const TableFilter = async () => {
        setIsLoader(false)
        try {
            let res_employee = await axios.post(SERVICE.CONSOLIDATED_HIERARCHY_FILTER_ALL, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
    ...
    const FilterSubmitDefault = async () => {
        try {
            let res_employee = await axios.post(SERVICE.CONSOLIDATED_DEFAULT_HIERARCHY_FILTER, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
                username: isUserRoleAccess.companyname,
    ...
    return (
        <>
            <Headtitle title={'Consolidated All Hierarchy List'} />
            {isUserRoleCompare?.includes("lconsolidatedallhierarchyworkorderlist")
                && (
                    <>
                        <Box sx={userStyle.container}>
                            <Typography sx={userStyle.SubHeaderText}> Consolidated All Hierarchy List </Typography>
                            <br /><br />
