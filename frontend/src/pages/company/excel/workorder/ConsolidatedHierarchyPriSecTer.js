...
const Consolidatedwprimaryorkorder = () => {
    ...
    const { isUserRoleCompare, isUserRoleAccess, allTeam, allUsersData, isAssignBranch} = useContext(UserRoleAccessContext);
    ...
    const addSerialNumberPriorityAll = () => {
        ...
        const itemsWithSerialNumberPriorityAll = constworkorder?.map((item, index) => ({ ...item, serialNumber: index + 1 }));
    ...
    const totalcount = filteredDataPriorityAll && (
        filteredDataPriorityAll.forEach((item) => {
    ...
    const rowDataTable = filteredDataPriorityAll.map((item, index) => {
        return {
            id: index,
    ...
    const handleCaptureImage = () => {
        ...
        html2canvas(clonedTable).then((canvas) => {
            ...
            link.href = dataURL;
            link.download = "Consolidated (Primary/Secondary/Tertiary) Hierarchy WorkOrder.png";
            link.click();
        });
    };
    ...
    const downloadExcelSecondary = async () => {
        ...
        try {
            ...
            const worksheet = workbook.addWorksheet('Consolidated (Primary/Secondary/Tertiary) Hierarchy WorkOrder');
            ...
            filteredDataPriorityAll.forEach((row, index) => {
                worksheet.addRow({
            ...
            filteredDataPriorityAll.forEach((row, index) => {
                ...
                worksheet.getCell(`R${index + 2}`).value = row.points == 'Unallotted' ? 0 : parseFloat(row.points);
            ...
            a.href = url;
            a.download = 'Consolidated (Primary/Secondary/Tertiary) Hierarchy WorkOrder.xlsx'; // File name
            a.click();
        } catch (error) {
    ...
    const downloadCsvSecondary = () => {
        ...
        filteredDataPriorityAll.forEach((row, index) => {
            const rowData = [
                ...
                row.team,
                row.prioritystatus,
                row.points == 'Unallotted' ? 0 : parseFloat(row.points),
                row.time,
                row.level,
                // row.control,
            ];
        ...
        saveAs(blob, 'Consolidated (Primary/Secondary/Tertiary) Hierarchy WorkOrder.csv'); // Specify the filename with .csv extension
    };
    ...
    const downloadPdfPriorityAll = () => {
        ...
        doc.autoTable({
            ...
            body: filteredDataPriorityAll.map((item, index) => {
                return {
                    serialNumber: item.serialNumber,
                    project: item.project,
                    vendor: item.vendor,
        ...
        doc.save("Consolidated (Primary/Secondary/Tertiary) Hierarchy WorkOrder.pdf");
    };
    ...
    const handleprintPriorityAll = useReactToPrint({
        ...
        documentTitle: 'Consolidated (Primary/Secondary/Tertiary) Hierarchy WorkOrder',
        pageStyle: 'print'
    });
    ...
    const calculateDataGridHeight = () => {
        if (pageSizePriorityAll === 'All') {
            ...
            const visibleRows = Math.min(pageSizePriorityAll, filteredDataPriorityAll.length);
...
const TableFilter = async () => {
    setIsLoader(false)
    try {
        let res_employee = await axios.post(SERVICE.CONSOLIDATED_HIERARCHY_FILTER_PRISECTER, {
            headers: {
                'Authorization': `Bearer ${auth.APIToken}`
            },
        ...
        setconstworkorder(res_employee?.data?.resultAccessFilter);
        setIsLoader(true)
    } catch (err) {setIsLoader(true);handleApiError(err, setShowAlert, handleClickOpenerr);}
...
const FilterSubmitDefault = async ()=>{
    try {
        let res_employee = await axios.post(SERVICE.CONSOLIDATED_HIERARCHY_DEFAULT_PRISECTER, {
            headers: {
                'Authorization': `Bearer ${auth.APIToken}`
            },
            username : isUserRoleAccess.companyname,
...
useEffect(() => {
    ...
    return (
        <>
            ...
            {isUserRoleCompare?.includes("lconsolidated(primary/secondary/tertiary)hierarchyworkorderlist")
                && (
                    <>
                        <Box sx={userStyle.container}>
                            <Typography sx={userStyle.SubHeaderText}>Consolidated (Primary/Secondary/Tertiary) Hierarchy WorkOrder List</Typography>
                            <br /><br />
                            ...
                            <Grid style={userStyle.dataTablestyle}>
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'end' }}>
                                    <Grid >
                                        ...
                                        {isUserRoleCompare?.includes("csvconsolidated(primary/secondary/tertiary)hierarchyworkorderlist")
                                            && (
                                                <>
                                                    <Button sx={userStyle.buttongrp} onClick={downloadCsvSecondary}>&ensp;<FaFileCsv />&ensp;Export to CSV&ensp;</Button>

                                                </>)}
                                        {isUserRoleCompare?.includes("excelconsolidated(primary/secondary/tertiary)hierarchyworkorderlist")
                                            && (
                                                <>
                                                    <Button sx={userStyle.buttongrp} onClick={downloadExcelSecondary}>&ensp;<FaFileExcel />&ensp;Export to Excel&ensp;</Button>
                                                </>)}
                                        {isUserRoleCompare?.includes("printconsolidated(primary/secondary/tertiary)hierarchyworkorderlist")
                                            && (
                                                <>
                                                    <Button sx={userStyle.buttongrp} onClick={handleprintPriorityAll}>&ensp;<FaPrint />&ensp;Print&ensp;</Button>
                                                </>)}
                                        {isUserRoleCompare?.includes("pdfconsolidated(primary/secondary/tertiary)hierarchyworkorderlist")
                                            && (
                                                <>
                                                    <Button sx={userStyle.buttongrp} onClick={() => downloadPdfPriorityAll()}><FaFilePdf />&ensp;Export to PDF&ensp;</Button>
                                                </>)}
                                        {isUserRoleCompare?.includes("imageconsolidated(primary/secondary/tertiary)hierarchyworkorderlist")
                                            && (
                                                <>
...
<Button variant="contained" onClick={(e)=>HandleTablesubmit(e)}>Filter</Button>
                            </Grid>
