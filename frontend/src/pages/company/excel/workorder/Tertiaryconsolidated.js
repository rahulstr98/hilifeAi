...
function TertiaryConsolidated({ comp, setComp }) {
    ...
    const handleCaptureImage = () => {
        ...
        html2canvas(clonedTable).then((canvas) => {
            ...
            link.href = dataURL;
            link.download = "Without_Tertiary_Consolidated_WorkOrder.png";
            link.click();
        });
    };
    ...
    const rowData = async (id, name) => {
        try {
            ...
            let ans = answer.length > 0 && answer.filter((item) => item.priority != "Tertiary");
            const secPresent = answer.length > 0 && answer.filter((item) => item.priority === "Tertiary");
            if (secPresent.length > 0) {
                ...
                setShowAlert(
                    <>
                        <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: 'orange' }} />
                        <p style={{ fontSize: '20px', fontWeight: 900 }}>{"There is no Tertiary Priority Queue Allotted"}</p>
                    </>
                );
                handleClickOpenerr();
    ...
    const editSubmit = (e) => {
        e.preventDefault();
        // let answer = deleteAlloted?.todo.filter(data => data.priority);
        let ans = todoscheck.length > 0 && todoscheck.filter((item) => item.priority != "Tertiary");
    ...
    const fetchTertiaryList = async () => {
        try {
            let res = await axios.get(SERVICE.WITHOUT_TERTIARY_CONSOLIDATED, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
            });
    ...
    const sendRequestindexSelected = async (index) => {
        ...
        excelmapdataresperson.forEach(async (item, i) => {
            ...
            if (selectedRows.includes(item._id)) {
                ...
                const answer = item.todo.filter(data => data.priority === "Tertiary")
                const todoanswer = item.todo.map((data) => {
                    if (data.priority === "Tertiary") {
    ...
    const downloadPdf = () => {
        ...
        doc.save("Without_Tertiary_Consolidated_WorkOrder.pdf");
    };
    // Excel
    const fileName = "Without_Tertiary_Consolidated_WorkOrder";
    //print...
    const componentRef = useRef();
    const handleprint = useReactToPrint({
        content: () => componentRef.current,
        documentTitle: "Without_Tertiary_Consolidated_WorkOrder",
        pageStyle: "print",
    });
                ...
                <Box sx={userStyle.container}>
                    <Grid container >
                        <Grid item md={8} xs={12}>
                            <Typography sx={userStyle.SubHeaderText}>Without Tertiary Consolidated work Order </Typography>
                                                ...
                                                <StyledTableRow key={indexval} sx={{ background: selectedRows.includes(row.id) ? "#1976d22b !important" : "inherit" }}>
                                                    ...
                                                    <StyledTableCell component="th" scope="row" colSpan={1}>
                                                        <Grid sx={{ display: "flex" }}>
                                                            {isUserRoleCompare?.includes("etertiaryworkorderlist") && (
                                                                <>
                                                            ...
                                                            {isUserRoleCompare?.includes("dtertiaryworkorderlist") && (
                                                                <>
                                                                    {/* <Button
                ...
                <Box>
                    ...
                    >
                        <DialogContent sx={{ overflow: 'auto' }}>
                            <>
                                <Typography sx={userStyle.HeaderText}> Edit Without Tertiary Consolidated Work Order</Typography>
                                <br />
                                <br />
                                <br />
                                <br />
...
export default TertiaryConsolidated;