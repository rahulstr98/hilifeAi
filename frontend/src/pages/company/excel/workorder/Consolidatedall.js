...
import {
    ...
    TableContainer, Button, List, ListItem, ListItemText, Popover, TextField, IconButton
} from "@mui/material";
...
const Consolidatedallworkorder = () => {
    ...
    const { isUserRoleCompare, allTeam, allUsersData, isAssignBranch } = useContext(UserRoleAccessContext);
    ...
    const OnFilterSubmit = async (e) => {
        setIsLoader(false)
        try {
            let res_employee = await axios.post(SERVICE.CONSWORKORDER_LIST_FILTER_ALL, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
            ...
            const itemsWithSerialNumberSecondary = ans?.map((item, index) => ({
                ...item,
                serialNumber: (pageOverAll - 1) * pageSizeOverAll + index + 1,
            }));

            setTotalProjects(ans?.length > 0 ? res_employee?.data?.totalProjects : 0);
            setTotalPages(ans?.length > 0 ? res_employee?.data?.totalPages : 0);
            setPageSizeOverAll((data) => { return ans?.length > 0 ? data : 10 });
            setPageOverAll((data) => { return ans?.length > 0 ? data : 1 });
    ...
    const searchOverTerms = searchQueryOverAll.toLowerCase().split(" ");
    // Modify the filtering logic to check each term
    const filteredDatasOverAll = constworkorder?.filter((item) => {
    ...
    const totalcount = filteredDatasOverAll && (
        filteredDatasOverAll.forEach((item) => {
    ...
    const rowDataTable = filteredDatasOverAll.map((item, index) => {
        return {
            id: index,
    ...
    const downloadExcelSecondary = async () => {
        ...
        try {
            ...
            const worksheet = workbook.addWorksheet('Workorder Consolidatedall');
            ...
            filteredDatasOverAll.forEach((row, index) => {
                worksheet.addRow({
            ...
            a.href = url;
            a.download = 'Consolidated_Work_Order_All_List.xlsx'; // File name
            a.click();
        } catch (error) {
    ...
    const downloadCsvSecondary = () => {
        ...
        saveAs(blob, 'Consolidated_Work_Order_All_List.csv'); // Specify the filename with .csv extension
    };
    ...
    const downloadPdfOverAll = () => {
        ...
        doc.autoTable({
            ...
            body: filteredDatasOverAll,
        });
        doc.save("Consolidated_Work_Order_All_List.pdf");
    };
    ...
    const handleprintOverAll = useReactToPrint({
        ...
        documentTitle: 'Consolidated_Work_Order_All_List',
        pageStyle: 'print'
    });
    ...
    const handleCaptureImage = () => {
        ...
        html2canvas(clonedTable).then((canvas) => {
            ...
            link.href = dataURL;
            link.download = 'Consolidated_Work_Order_All_List.png';
            link.click();
        });
    };
    ...
    return (
        <>
            ...
            {isUserRoleCompare?.includes("lconsolidatedallworkorderlist")
                && (
                    <>
                        <Box sx={userStyle.container}>
                            ...
                            {!isLoader ?
                                ...
                                <>
                                    ...
                                    <Box>
                                        <Pagination
                                            page={searchQueryOverAll !== "" ? 1 : pageOverAll}
                                            pageSize={pageSizeOverAll}
                                            totalPages={searchQueryOverAll !== "" ? 1 : totalPages}
                                            onPageChange={handlePageChangeOverAll}
                                            pageItemLength={filteredDatasOverAll?.length}
                                            totalProjects={
                                                searchQueryOverAll !== "" ? filteredDatasOverAll?.length : totalProjects
                                            }
            ...
            <TableContainer component={Paper} sx={userStyle.printcls} >
                ...
                >
                    ...
                    <TableBody>
                        {filteredDatasOverAll &&
                            (filteredDatasOverAll.map((row, index) => (
                                <TableRow key={index}>
                                    ...
                                    <TableCell>{parseFloat(row.points)}</TableCell>
