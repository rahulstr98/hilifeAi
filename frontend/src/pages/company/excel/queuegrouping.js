...
import {
    ...
    DialogContent, DialogActions, FormControl, Grid, Table, TableHead, TableContainer, Button
...
function Queuegrouping() {

    const [queue, setQueue] = useState({
        queuename: "Please Select Queue Name", categories: ""
    });
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, 'Queue_Grouping.png');
                });
            });
        }
    };
    ...
    const getOverallBulkDeleteSection = async (e) => {
        try {
            let res = await axios.post(SERVICE.OVERALLQUEUE_GROUP_BULK_DELETE, {
                headers: {
    ...
    const fetchQueueName = async () => {
        try {
            ...
            setQueueOptions(res_queue?.data?.queues?.map((t) => ({
                ...t,
                label: t.name,
                value: t.name
            })))
    ...
    const fetchAllCategories = async () => {
        try {
            ...
            setCategoriesName(res_module?.data?.categoryexcel?.map((t) => ({
                ...t,
                label: t.name,
                value: t.name
            })))
    ...
    useEffect(() => { fetchAllCategories(); fetchQueueName() }, [])
    ...
    const customValueRendererCateEdit = (valueCate, _categories) => {
        return valueCate.length
            ? valueCate.map(({ label }) => label).join(", ")
            : "Please select Category";
    };
    ...
    const rowData = async (id, name) => {
        try {
            let res = await axios.get(`${SERVICE.QUEUEGROUP_SINGLE}/${id}`, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
            });

            setDeleteQueue(res?.data?.squeuegroup);

            getOverallDeleteSection(res?.data?.squeuegroup)
         } catch (err) {handleApiError(err, setShowAlert, handleClickOpenerr);}
    };
    ...
    const deleQueue = async () => {
        try {
            await axios.delete(`${SERVICE.QUEUEGROUP_SINGLE}/${queueid}`, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
            });
    ...
    const delProjectcheckbox = async () => {
        try {
            selectedRows?.map((item) => {
                let res = axios.delete(`${SERVICE.QUEUEGROUP_SINGLE}/${item}`, {
                    headers: {
                        'Authorization': `Bearer ${auth.APIToken}`
                    }
                });
    ...
    const getOverallEditSection = async (e) => {
        try {
            let res = await axios.post(SERVICE.OVERALLQUEUE_GROUP_EDIT, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
                oldname: e,
            ...
            setGetOverallCount(`The ${e?.queuename} is linked in 
    ...
    const getOverallDeleteSection = async (e) => {
        try {
            let res = await axios.post(SERVICE.OVERALLQUEUE_GROUP_DELETE, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
                oldname: e,
    ...
    const getOverallEditSectionUpdate = async () => {
        try {
            let res = await axios.post(SERVICE.OVERALLQUEUE_GROUPING, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
                oldname: ovProj,
    ...
    const sendRequest = async () => {
        setIsBtn(true)
        try {
            let queuecreate = await axios.post(SERVICE.QUEUEGROUP_CREATE, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
            ...
            await fetchAllQueue();
            setIsBtn(false)
            setQueue({ queuename: queue.queuename });
            setShowAlert(
                <>
                    <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: 'orange' }} />
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        let categoryNames = selectedOptionsCate.map(item => item.value)

        const isNameMatch = queues.some(item =>
            item.queuename.toLowerCase() === queue.queuename.toLowerCase() &&
            item.categories.some(data => categoryNames.includes(data))
        );


        if (queue.queuename === "Please Select Queue Name") {
            setShowAlert(
                <>
            ...
            setShowAlert(
                <>
                    ...
                    />
                    <p style={{ fontSize: "20px", fontWeight: 900 }}>
                        {"This Category Already exists in this Queue"}
                    </p>
                </>
            );
            handleClickOpenerr();
    ...
    const handleCancel = () => {
        setQueue({ queuename: "Please Select Queue Name", categories: "" });
    ...
    const getCode = async (e, name) => {
        try {
            ...
            let res = await axios.get(`${SERVICE.QUEUEGROUP_SINGLE}/${e}`, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
            });
            handleClickOpenEdit();
            setQueueId(res?.data?.squeuegroup);
            setSelectedOptionsCateEdit(res?.data?.squeuegroup?.categories);
            handleCategoryChangeEdit(res?.data?.squeuegroup?.categories?.map((a, index) => ({
                ...a,
                value: a,
                label: a
            })));
    ...
    const sendEditRequest = async () => {
        try {
            let res = await axios.put(`${SERVICE.QUEUEGROUP_SINGLE}/${projectsid}`, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
                queuename: String(queueId.queuename),
    ...
    const editSubmit = (e) => {
        ...
        let categoryNames = selectedOptionsCateEdit.map(item => item.value)

        const isNameMatch = allQueueEdit.some(item =>
            item.queuename === queueId.queuename &&
            item.categories.some(data => categoryNames.includes(data))
        );
        ...
        else if (queueId.queuename != ovProj.queuename && ovProjcount > 0) {
            setShowAlertpop(
                <>
                    <ErrorOutlineOutlinedIcon
                        sx={{ fontSize: "100px", color: "orange" }}
    ...
    const fetchAllQueue = async () => {
        try {
            ...
            setQueues(res_queue?.data?.queuegroups);
            setIsLoader(true);
    ...
    const fetchQueueAll = async () => {
        try {
            ...
            setAllQueueEdit(res_queue?.data?.queuegroups.filter(item => item._id !== queueId._id));
         } catch (err) {handleApiError(err, setShowAlert, handleClickOpenerr);}
    };

    let snos = 1;
    // Excel
    const fileName = "Queue_Grouping";

    //print...
    const componentRef = useRef();
    const handleprint = useReactToPrint({
        content: () => componentRef.current,
        documentTitle: "Queue_Grouping",
        pageStyle: "print",
    });

    //serial no for listing items 
    const addSerialNumber = () => {
        const itemsWithSerialNumber = queues?.map((item, index) => ({
            _id: item._id,
            serialNumber: index + 1 ,
            categories: item.categories?.map((item, i) => `${i + 1 + ". "}` + " " + item).toString(),
            queuename: item.queuename,
        }));
    ...
    const columnDataTable = [
        ...
        { field: "queuename", headerName: "Queue Name", flex: 0, width: 250, hide: !columnVisibility.queuename, headerClassName: "bold-header" },
        { field: "categories", headerName: "Category Name", flex: 0, width: 450, hide: !columnVisibility.categories, headerClassName: "bold-header" },
        {
            field: "actions",
            headerName: "Action",
            flex: 0,
            ...
            renderCell: (params) => (
                <Grid sx={{ display: "flex" }}>
                    {isUserRoleCompare?.includes("equeuegrouping") && (
                        <Button
                    ...
                    {isUserRoleCompare?.includes("dqueuegrouping") && (
                        <Button
                    ...
                    {isUserRoleCompare?.includes("vqueuegrouping") && (
                        <Button
                            sx={userStyle.buttonedit}
                            onClick={() => {
                    ...
                    {isUserRoleCompare?.includes("iqueuegrouping") && (
                        <Button
                            sx={userStyle.buttonedit}
                            onClick={() => {
                                handleClickOpeninfo();
    ...
    return (
        <Box>
            <Headtitle title={'QUEUE GROUPING'} />
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Queue Grouping </Typography>
            {isUserRoleCompare?.includes("aqueuegrouping") && (
                <>
                            ...
                            <Grid item md={4} xs={12} sm={6}>
                                <FormControl fullWidth size="small">
                                    ...
                                    <Selects
                                        ...
                                        type="text"
                                        placeholder="Please Select Queue Name"
                                        options={queueotions}
                                        value={{ label: queue.queuename, value: queue.queuename }}
                                        onChange={(e) => {
                                            setQueue({ ...queue, queuename: e.value });
                                        }}
                                    />
                                </FormControl>
                            </Grid>
                        ...
                        <Grid container spacing={2}>
                            <Grid item lg={1} md={2} sm={12} xs={12}>
                                ...
                                <Grid item md={12} sm={12} xs={12}>
                                    <LoadingButton sx={userStyle.buttonadd} variant="contained" onClick={handleSubmit}>SAVE</LoadingButton>
                                </Grid>
                        ...
                        <>
                            <Grid container spacing={2}>
                                <Grid item md={12} xs={12} sm={12}>
                                    <Typography sx={userStyle.HeaderText}>Edit Queue Grouping </Typography>
                                </Grid>
                            </Grid>
                            <br />
                            <Grid container spacing={2}>
                                <Grid item md={5} xs={12} sm={6}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <Selects
                                            ...
                                            type="text"
                                            placeholder="Please Select Queue Name"
                                            value={{ label: queueId.queuename, value: queueId.queuename }}
                                            options={queueotions}
                                            onChange={(e) => {
                                                setQueueId({ ...queueId, queuename: e.value });
                                            }}
                                        />
                                    </FormControl>
                                </Grid>
...
<br/><br/>
            ...
            <br />
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("lqueuegrouping") && (
                <>
                    <Box sx={userStyle.container}>
                        {/* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>Queue Grouping List</Typography>
                        </Grid>
                        ...
                        <Grid container spacing={2} style={userStyle.dataTablestyle}>
                            ...
                            <Grid item md={8} xs={12} sm={12} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                <Box >
                                    {isUserRoleCompare?.includes("excelqueuegrouping") && (
                                    ...
                                    {isUserRoleCompare?.includes("csvqueuegrouping") && (
                                    ...
                                    {isUserRoleCompare?.includes("printqueuegrouping") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("pdfqueuegrouping") && (
                                    ...
                                    {isUserRoleCompare?.includes("imagequeuegrouping") && (
                                        <>
                        ...
                        {isUserRoleCompare?.includes("bdqueuegrouping") && (
            ...
            <Box>
                ...
                >
                    <Box sx={{ width: '550px', padding: '20px 50px' }}>
                        <>
                            <Typography sx={userStyle.HeaderText}>Queue Grouping Info</Typography>
                            <br /><br />
                            <Grid container spacing={2}>
                                <Grid item md={12} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
            ...
            >
                <Box sx={{ padding: "20px 50px"}}>
                    <>
                        <Typography sx={userStyle.HeaderText}> View Queue Grouping</Typography>
                        <br /> <br />
                        <Grid container spacing={2}>
                            <Grid item md={12} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    <Typography variant="h6"> Name</Typography>
                                    <Typography>{queueId.queuename}</Typography>
                                </FormControl>
                            </Grid>
                            <Grid item md={12} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    <Typography variant="h6"> Category Name</Typography>
                                    <Typography>{queueId?.categories?.map((item, i) => `${i + 1 + ". "}` + " " + item).toString()}</Typography>
                                </FormControl>
                            </Grid>
                        </Grid>
                        <br /> <br /> <br />
