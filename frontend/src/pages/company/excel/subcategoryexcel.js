...
import {
    Box, Typography, OutlinedInput, Dialog, Select, TableRow, TableCell, MenuItem, TableBody, DialogContent, DialogActions, FormControl, Grid, Paper, Table, TableHead, TableContainer, Button
...
function SubCategoryMaster() {
    ...
    const [subcategoryid, setSubcategoryid] = useState({ name: "" });
    ...
    const getOverallDeleteSection = async (e) => {
        try {
            let res = await axios.post(SERVICE.SUBCATEGORYEXCEL_BULK_DELETE, {
                headers: {
    ...
    const rowData = async (id, name) => {
        try {
            let res = await axios.get(`${SERVICE.SUBCATEGORYEXCEL_SINGLE}/${id}`, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
            });
            setDeletemodule(res?.data?.ssubcategory);

            let restime = await axios.post(SERVICE.TIMEPOINTSSUBCATEGORYCHECK, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
                checksub: String(name)
            })
            setChecktimepoints(restime?.data?.timepoints)

            if ((restime?.data?.timepoints).length > 0) {
    ...
    const delModule = async () => {
        try {
            await axios.delete(
                `${SERVICE.SUBCATEGORYEXCEL_SINGLE}/${susbcategoriesexcelid}`, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
            });
    ...
    const delProjectcheckbox = async () => {
        try {
            const deletePromises = selectedRows?.map((item) => {
                return axios.delete(`${SERVICE.SUBCATEGORYEXCEL_SINGLE}/${item}`, {
                    headers: {
                        'Authorization': `Bearer ${auth.APIToken}`
                    }
                });
    ...
    const sendRequest = async () => {
        ...
        try {
            ...
            let modules = await axios.post(SERVICE.SUBCATEGORYEXCEL_CREATE, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        const isNameMatch = subcategories?.some(item => item?.name?.toLowerCase() === (subcategory?.name)?.toLowerCase() && item.project === selectedProject && item.categoryname === selectedCategory);
    ...
    const getCode = async (e, name) => {
        try {
            let res = await axios.get(`${SERVICE.SUBCATEGORYEXCEL_SINGLE}/${e}`, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
            });
            setSubcategoryid(res?.data?.ssubcategory);
            setSelectedProjectedit(res?.data?.ssubcategory.project)
            setSelectedCategoryedit(res?.data?.ssubcategory.categoryname)
            setOvProj(name);
    ...
    const sendEditRequest = async () => {
        try {
            ...
            let res = await axios.put(`${SERVICE.SUBCATEGORYEXCEL_SINGLE}/${categoriesid}`,
                {
                    headers: {
                        'Authorization': `Bearer ${auth.APIToken}`
                    },
    ...
    const editSubmit = (e) => {
        ...
        const isNameMatch = allModuleedit?.some(item => item?.name?.toLowerCase() === (subcategoryid?.name)?.toLowerCase() && item.project === selectedProjectedit && item.categoryname === selectedCategoryedit);
        ...
        else if (subcategoryid.name != ovProj && ovProjcount > 0) {
            setShowAlertpop(
                <>
                    <ErrorOutlineOutlinedIcon
                        sx={{ fontSize: "100px", color: "orange" }}
    ...
    const getOverallEditSection = async (e) => {
        try {
            let res = await axios.post(SERVICE.OVERALL_SUBCATEGORYEXCEL, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
                oldname: e,
       ...
       ${res?.data?.timepoints?.length > 0 ? "time and points" : ""} whether you want to do changes ..??`)
         } catch (err) {handleApiError(err, setShowAlert, handleClickOpenerr);}
    };
    ...
    const getOverallEditSectionUpdate = async () => {
        try {
            ...
            sendEditRequestOverall(res?.data?.timepoints)

         } catch (err) {handleApiError(err, setShowAlert, handleClickOpenerr);}
    };

    const sendEditRequestOverall = async (timepoints) => {
        try {
    ...
    const fetchSubCategoryexcelAll = async () => {
        try {
            ...
            setAllModuleedit(res_module?.data?.subcategoryexcel.filter(item => item._id !== subcategoryid._id));
        } catch (err) {handleApiError(err, setShowAlert, handleClickOpenerr);}
    };
    ...
    const columns = [
        ...
        { title: "Subcategory Name", field: "name" },
    ];
    ...
    }, [isEditOpen, subcategoryid, selectedProjectedit])

    useEffect(() => {
        // Filter categories based on the selected project and vendors
        const filteredCategories = categories?.filter(category =>
            category.project === selectedProject).map(category => (
                {
                    ...category,
                    label: category.name,
                    value: category.name
    ...
    const addSerialNumber = () => {
        const itemsWithSerialNumber = subcategories?.map((item, index) => ({ ...item, serialNumber: index + 1 }));
    ...
    const columnDataTable = [
        ...
        { field: "name", headerName: "SubCategory Name", flex: 0, width: 250, hide: !columnVisibility.name, headerClassName: "bold-header" },
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Subcategory </Typography>
            {isUserRoleCompare?.includes("asubcategorymaster") && (
                <Box sx={userStyle.dialogbox}>
                    <>
                        <Grid container spacing={2}>
                            <Grid item xs={8}>
                                <Typography sx={userStyle.importheadtext}>Add Subcategory</Typography>
                            </Grid>
                        </Grid>
                        <br />
                        <Grid container spacing={2}>
                                ...
                                <Grid item md={4} xs={12} sm={12}>
                                    ...
                                    <FormControl fullWidth size="small" >
                                        <Selects
                                            ...
                                            onChange={(e) => { setSelectedProjectedit(e.value); setSelectedCategoryedit("Please Select Category") }}
            ...
            {isUserRoleCompare?.includes("lsubcategorymaster") && (
                <>
                    <Box sx={userStyle.container}>
                        ...
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>Subcategory List</Typography>
                        </Grid>
                            ...
                            <Grid item md={2} xs={12} sm={12}>
                                <Box>
                                    ...
                                    <Select id="pageSizeSelect" value={pageSize}
                                        ...
                                        {/* <MenuItem value={(subcategories?.length)}>All</MenuItem> */}
                                    </Select>
                                </Box>
                            </Grid>
                        ...
                        {isUserRoleCompare?.includes("bdsubcategorymaster") && (
                ...
                >
                    ...
                    <DialogActions>
                        ...
                        <Button
                            ...
                            variant="contained"
                            color="error"
                            onClick={(e) => delModule(susbcategoriesexcelid)}
                        >
                            {" "}
                            OK{" "}
                        </Button>
                                        ...
                                        {checktimepoints?.length > 0 ? (
                                            <>
                                                <span style={{ fontWeight: '700', color: '#777' }}>
                                                    {`${deletemodule.name} `}
                                                </span>was linked in <span style={{ fontWeight: '700' }}>Timepoints </span>
                                            </>
                                        ) : ""
                                        }
                                    </Typography>
            ...
            >
                <Box sx={{ width: '550px', padding: '20px 50px' }}>
                    <>
                        <Typography sx={userStyle.HeaderText}> View Subcategory</Typography>
                        <br /><br />
                        <Grid container spacing={2}>
            ...
            >
                <Box sx={{ width: '550px', padding: '20px 50px' }}>
                    <>
                        <Typography sx={userStyle.HeaderText}>Subcategory Info </Typography>
                        <br /><br />
                        <Grid container spacing={2}>
                            <Grid item md={12} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
