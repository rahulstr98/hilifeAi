...
function EditRole() {
  ...
  const arraysEqual = (arr1, arr2) => {
    ...
    return arr1.every((item1) => arr2.some((item2) => JSON.stringify(item1) === JSON.stringify(item2))) && arr2.every((item2) => arr1.some((item1) => JSON.stringify(item1) === JSON.stringify(item2)));
  };
  ...
  const updatedRoleCheck = async () => {
    try {
      let res_vendor = await axios.post(SERVICE.UPDATE_ASSIGN_BRANCH_BASED_ON_ROLE, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        rolename: subprojid?.name,
        menuitems: menuItems
      });
  ...
  const fetchSubmit = async () => {
    ...
    try {
      ...
      console.log(res?.data?.status , "Hitted")
      if (res?.data?.status) {
        console.log("Hitted")
        updatedRoleCheck();
      }
  ...
  return (
    <Box>
      ...
      <PageHeading title="Edit Role" modulename="Setup" submodulename="Role" mainpagename="" subpagename="" subsubpagename="" />
            ...
            <>
              <Grid container spacing={2}>
                <Grid item md={8} xs={12} sm={12}>
                  <Typography sx={userStyle.importheadtext}>Edit Roles</Typography>
                </Grid>
              ...
              <Grid container sx={{ justifyContent: 'left' }} spacing={2}>
                ...
                <Grid item md={4} sm={12} xs={12}>
                  <FormControl fullWidth size="small">
                    <Typography>
                      Controls Grouping<b style={{ color: 'red' }}>*</b>
                    </Typography>
                    <MultiSelect
                ...
                <Grid item md={4} xs={12} sm={12}>
                  ...
                  <FormControl size="small" fullWidth>
                    <MultiSelect size="small" options={controls} value={selectedOptionsControls} onChange={handleControlsChange} valueRenderer={customValueRendererControls} />
                  </FormControl>
                </Grid>
              </Grid>
              <br></br>
