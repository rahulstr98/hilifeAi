...
function AllAssignedCheckList() {
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("All Completed Checklist"),
  ...
  const handleCaptureImageTwo = () => {
    if (gridRefTableImgTwo.current) {
      domtoimage.toBlob(gridRefTableImgTwo.current)
        .then((blob) => {
          saveAs(blob, "All Completed Checklist.png");
        })
        .catch((error) => {
  ...
  const addSerialNumberTwo = () => {
    ...
    setOverallItemsTwo(itemsWithSerialNumber);
  };
                ...
                <Box sx={userStyle.container}>
                  ...
                  >
                    <Typography sx={userStyle.importheadtext}>
                      Consolidated Completed Checklist
                    </Typography>
                  </Grid>
