  ...
  const fetchUsers = async () => {
    try {
      ...
      const isCurrentTimeInRange = (start, end) => {
        ...
        const startTime = moment(`${currentServerDate} ${start}`, "YYYY-MM-DD hh:mmA").subtract(
          afterAddHours,
          "hours"
        );
      ...
      const addedTimeinSecondShiftEnd = moment(`${currentServerDate} ${end2}`, "YYYY-MM-DD hh:mmA").add(
        prevAddHours,
        "hours"
      );
    ...
    const handleClockIn = async () => {
      try {
        ...
        const calculatedshiftend = await addFutureTimeToCurrentTime(
          shiftEndTime, serverTime?.currentNewDate
        );
    ...
    <>
      ...
      >
        ...
        <DialogContent>
          ...
          >
            <b>
              {`You have ${attendanceTaskStatus === "days" ? `${calculatedTime?.tasklimitcountinday}  days of` : calculatedTime?.tasklimitcount }  pending tasks. Please try to complete as soon as possible.`}
            </b>
          </DialogContentText>
        </DialogContent>
        <DialogActions>
