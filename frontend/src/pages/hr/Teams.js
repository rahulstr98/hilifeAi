...
import { Backdrop, Box, Button, Checkbox, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, Grid, IconButton, LinearProgress, List, ListItem, ListItemText, MenuItem, OutlinedInput, Popover, Select, Table, TableBody, TableHead, TextareaAutosize, TextField, Typography } from "@mui/material";
...
function Teams() {
  ...
  const { isUserRoleAccess, pageName, setPageName, isUserRoleCompare, isAssignBranch, buttonStyles } = useContext(UserRoleAccessContext);
  ...
  const fetchteams = async () => {
    ...
    } catch (err) { setIsTeams(true); handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const isNameMatch = teamsdata.some((item) =>
      item.company?.toLowerCase() === teams.company?.toLowerCase() &&
      item.unit?.toLowerCase() === selectedUnit?.toLowerCase() &&
      item.branch?.toLowerCase() === selectedBranch?.toLowerCase() &&
      item.teamname?.toLowerCase() === teams.teamname?.toLowerCase()
    );
  ...
  const editSubmit = (e) => {
    ...
    fetchteamsAll();
    const isNameMatch =
      teamsdataalledit.some((item) =>
        item.company?.toLowerCase() === teamsdatas.company?.toLowerCase() &&
        item.unit?.toLowerCase() === teamsdatas.unit?.toLowerCase() &&
        item.branch?.toLowerCase() === teamsdatas.branch?.toLowerCase() &&
        item.teamname.toLowerCase() === teamsdatas.teamname.toLowerCase()
      );
  ...
  const columnDataTable = [
    ...
    { field: "teamname", headerName: "Team Name", flex: 0, width: 130, hide: !columnVisibility.teamname, headerClassName: "bold-header" },
      ...
      renderCell: (params) => (
        <Grid sx={{ display: 'flex' }}>
          ...
          {isUserRoleCompare?.includes("dteams") && (
            <Button sx={userStyle.buttondelete} onClick={(e) => { rowData(params.row.id, params.row.teamname) }}><DeleteOutlineOutlinedIcon style={{ fontsize: 'large' }}
    ...
    <Box>
      ...
      {isUserRoleCompare?.includes("ateams") && (
        <Box sx={userStyle.dialogbox}>
          <Typography sx={userStyle.SubHeaderText}>Add Teams </Typography>
          <br /> <br />
          <>
            <Grid container spacing={2}>
              ...
              <Grid item md={3} xs={12} sm={6}>
                ...
                <FormControl size="small" fullWidth>
                  <Selects options={accessbranch?.filter(
                    (comp) =>
