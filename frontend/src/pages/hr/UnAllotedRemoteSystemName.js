...
function UnAllottedRemoteSystemName() {
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "UnAllottedRemoteSystemName.png");
        });
      });
    }
  };
  ...
  const handleCloseAllotWorkStation = () => {
    setIsOpenAllotModal(false);
  };
  const handleOpenAllotWorkStation = () => {
    setIsOpenAllotModal(true);
  };
  ...
  const allotWorkStation = async (row) => {
    try {
      ...
      setEmployeeDetails({ employeename: row.companyname, id: row?.ids });
      setloadingId("");
      ...
      // handleOpenAllotWorkStation();
    } catch (err) { setloadingId(""); handleApiError(err, setShowAlert, handleClickOpenerr); }
  };


  const updateWorkStation = async () => {
    try {
      await axios.put(`${SERVICE.USER_SINGLE_PWD}/${employeeDetails?.id}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },

        workstationinput: String(primaryWorkStationInput),
        workstationofficestatus: Boolean(true),
      });
      handleCloseAllotWorkStation();
      await fetchEmployee();
      setShowAlert(
        <>
  ...
  const fetchEmployee = async () => {
    ...
    try {
      ...
      const officeEmp = res?.data?.users?.filter(
        (item) => ((item?.workmode?.toLowerCase() === "office" || item?.workmode?.toLowerCase() === "remote") && (item?.workstationinput === "" || item?.workstationinput === undefined))
      );
      console.log(res?.data?.users[0],'res?.data?.users')
      const result = officeEmp.map((t) => {
        return {
          company: t.company,
          branch: t.branch,
...
assignbranch: accessbranch
      ...
      const officeEmp = res?.data?.users?.filter(
        (item) =>
          item?.workmode?.toLowerCase() === "office" &&
          item?.workstationofficestatus === false
      );
      const result = officeEmp.map((t) => {
        return {
          company: t.company,
  ...
  const downloadPdf = (isfilter) => {
    ...
    doc.save("UnAllottedRemoteSystemName.pdf");
  };

  // Excel
  const fileName = "UnAllottedRemoteSystemName";

  //print...
  const componentRef = useRef();
  const handleprint = useReactToPrint({
    content: () => componentRef.current,
    documentTitle: "UnAllotted Remote System Name",
    pageStyle: "print",
  });
  ...
  const columnDataTable = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("vunallottedremotesystemname") && (
            <Button
          ...
          &nbsp;
          {params.row.addremoteworkmode.length > 0 &&(
            <LoadingButton
            sx={userStyle.buttonedit}
            onClick={() => {
              allotWorkStation(params.row);
            }}
            variant="contained"
            loading={params?.row?.ids === loadingId}
          >
            Allot
          </LoadingButton>
          )}
    ...
    if (isfilter === "filtered") {
      ...
      exportToCSV(
        employeesArray.map((t, index) => ({
          ...
          Count: t.count,
          SystemName: t.systemname,
          ShortName: t.systemshortname?.slice(0, 15),
        })),
        fileName
      );
    }
  ...
  return (
    <Box>
      {/* ****** Header Content ****** */}
      <Headtitle title={"UnAllotted Remote System Name"} />
      <NotificationContainer />
      <Typography sx={userStyle.HeaderText}>
        UnAllotted Remote System Name
      </Typography>
      <br />
      {isUserRoleCompare?.includes("lunallottedremotesystemname") && (
        <>
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>
                UnAllotted Remote System Name List
              </Typography>
            </Grid>
            <br />
              ...
              >
                <Box>
                  {isUserRoleCompare?.includes(
                    "excelunallottedremotesystemname"
                  ) && (
                      <>
                        <Button
                          onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes(
                    "printunallottedremotesystemname"
                  ) && (
                      <>
                  ...
                  {isUserRoleCompare?.includes(
                    "imageunallottedremotesystemname"
                  ) && (
                      <Button
                        sx={userStyle.buttongrp}
      ...
      >
        <Box sx={{ padding: "20px 50px" }}>
          <>
            <Typography sx={userStyle.HeaderText}>
              {" "}
              View UnAllotted Remote System Name
            </Typography>
            <br /> <br />
            <Grid container spacing={2}>
              ...
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>
                    {employeesByEdit[0]?.systemshortname?.slice(0, 15)}
                  </Typography>
                </FormControl>
              </Grid>
            </Grid>
      ...
      {/* Allot Workstationt Modal */}
      <Box>
        <Dialog
          open={isOpenAllotModal}
          onClose={handleCloseAllotWorkStation}
          aria-labelledby="alert-dialog-title"
          aria-describedby="alert-dialog-description"
        >
          <DialogContent sx={{ textAlign: "center", alignItems: "center" }}>
            ...
            />
            <Typography variant="h5" sx={{ color: "red", textAlign: "center" }}>
              Are you sure? Do you want to Allot work Station{" "}
              {primaryWorkStationInput} for {employeeDetails?.employeename} ?
            </Typography>
          </DialogContent>
          <DialogActions>
            <Button
              onClick={handleCloseAllotWorkStation}
              color="error"
              variant="contained"
            >
              No
...
export default UnAllottedRemoteSystemName;