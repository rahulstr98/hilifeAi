...
function CompanyDocumentPreparationPrinted({ data, setData }) {
  ...
  const getAdjustedMargin = (selectedMargin, headImage, footImage) => {
    const base = marginValues[selectedMargin] || marginValues["normal"];
    let [top, right, bottom, left] = base;

    if (headImage) top += 48; // increase space for header image
    if (footImage) bottom += 48; // increase space for footer image
  ...
  const rowData = async (id, name) => {
    ...
    try {
      let res = await axios.get(
        `${SERVICE.SINGLE_COMPANY_DOCUMENTPREPARATION}/${id}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
    ...
    try {
      ...
      const final =
        res_freq?.data?.companydocumentPreparation?.length > 0
          ? res_freq?.data?.companydocumentPreparation?.filter((data) =>
              ["Printed", "Re-Printed"].includes(data?.printingstatus)
            )
          : [];
      const answer =
        final?.length > 0
          ? final?.map((item, index) => ({
              ...
              tocompany:
                item.tocompany === "Please Select To Company"
                  ? ""
                  : item.tocompany,
          ...
          <Box sx={userStyle.container}>
            ...
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>
                Company Documents Printed List
              </Typography>
            </Grid>
