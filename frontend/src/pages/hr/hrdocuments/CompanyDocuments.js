...
function CompanyDocuments() {
  ...
  const getAdjustedMargin = (
    ...
    return convertPxArrayToMm([
      top,
      right,
      qrcodeneed ? bottom + 20 : bottom,
      left,
    ]);
  };
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String(
        "Human Resource/HR Documents/Company Document Preparation"
      ),
  ...
  const TemplateDropdownsValue = async (comp, bran, e) => {
    ...
    try {
      ...
      if (res?.data?.templatecontrolpanel) {
        ...
        const ans =
          answer?.templatecontrolpanellog?.length > 0
            ? answer?.templatecontrolpanellog[0]
            : [];
        const toCompanyBranchOptions = ans
          ? ans?.toCompany?.map((data) => ({
              ...data,
        ...
        setGenerateData(true);
        setPopupContentMalert(
          "This company and branch is not matched with Template control panel data.Add the details in Template control panel!"
        );
        setPopupSeverityMalert("warning");
        handleClickOpenPopupMalert();
      }
  ...
  const fetchAllRaisedTickets = async () => {
    ...
    try {
      let res_queue = await axios.get(
        SERVICE.COMPANY_DOCUMENT_PREPARATION_AUTOID,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const answer = async (person, index) => {
    ...
    let newval =
      uniqueCode + "#" + templateCreationValue?.tempcode + "_" + postfixLength;
    ...
    try {
      ...
      const branchAddress = isAssignBranch?.find(
        (data) => data?.branch === documentPrepartion?.branch
      );
      ...
      let findMethod = texted
        ...
        .replaceAll(
          "$TEMPLATENAME$",
          documentPrepartion.template ? documentPrepartion.template : ""
        )
        .replaceAll(
          "$F.COMPANY$",
          toCompanyAddress ? documentPrepartion?.company : ""
        )
        .replaceAll(
          "$F.BRANCH$",
        ...
        .replaceAll(
          "$F.BRANCHADDRESS$",
          branchaddress !== "" && toCompanyAddress
            ? branchaddress?.branchaddress?.split(",").join(",\n")
            : ""
        )
        .replaceAll(
          "$T.COMPANY$",
  ...
  const handlePrintDocumentManual = () => {
    if (headerOptions === "Please Select Print Options") {
      ...
      checking
        .match(regex)
        ?.filter(
          (data) => !["$SIGNATURE$", "$FSIGNATURE$", "$RSEAL$"]?.includes(data)
        )?.length > 0
    ) {
      setPopupContentMalert(
  ...
  const downloadPdfTesdtManual = () => {
    ...
    // Create a new div element to hold the Quill content
    const pdfElement = document.createElement("div");
    pdfElement.innerHTML = checking?.replaceAll(
      "--- Page Break ---",
      `<p class="page-break-label" data-page-break="true"></p>`
    );
    ...
    const addPageNumbersAndHeadersFooters = (
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        const footerX = 5;
        const footerY =
          pageHeight * 1 - (foot === "" ? 15 : footerImgHeight - 3); // Position at the bottom
 ...
 `;
          ...
          const addPageNumbersAndHeadersFooters = (
            ...
            for (let i = 1; i <= totalPages; i++) {
              ...
              const footerX = 5; // Start from the left
              const footerY =
                pageHeight * 1 - (foot === "" ? 15 : footerImgHeight - 3);
              doc.addImage(
                foot,
                "JPEG",
                footerX,
              ...
              img.onload = () => {
                ...
                if (response?.data?.sdocumentPreparation?.qrcode) {
                  qrImg.onload = () => {
                    ...
                    // Save the PDF
                    pdf.save(
                      `${response?.data?.sdocumentPreparation?.template}.pdf`
                    );
  ...
  const downloadPdfTesdtTable = async (e) => {
    ...
    if (headerOptions === "Please Select Print Options") {
      ...
      // Create a new div element to hold the Quill content
      let response = await axios.get(
        `${SERVICE.SINGLE_COMPANY_DOCUMENTPREPARATION}/${e}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const rowData = async (id, name) => {
    ...
    try {
      let res = await axios.get(
        `${SERVICE.SINGLE_COMPANY_DELETE_DOCUMENTPREPARATION}/${id}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const delBrand = async () => {
    ...
    try {
      await axios.delete(
        `${SERVICE.SINGLE_COMPANY_DOCUMENTPREPARATION}/${brandid}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const sendRequestManual = async () => {
    ...
    try {
      let brandCreate = await axios.post(
        SERVICE.CREATE_COMPANY_DOCUMENT_PREPARATION,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      documentPrepartion.tocompany === "" ||
      documentPrepartion.tocompany === "Please Select To Company"
    ) {
      ...
      setPopupContentMalert(
        "Document with Template Name and Date already exists!"
      );
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else {
  ...
  const fetchBrandMaster = async () => {
    ...
    try {
      ...
      let res_freq = await axios.post(
        `${SERVICE.ACCESSIBLEBRANCHALL_COMPANY_DOCUMENTPREPARATION}`,
        {
      ...
      const answer =
        res_freq?.data?.companydocumentPreparation?.length > 0
          ? res_freq?.data?.companydocumentPreparation
  ...
  const rowDataTable = filteredData.map((item, index) => {
    return {
      ...
      tocompany:
        item.tocompany === "Please Select To Company" ? "" : item.tocompany,
  ...
  return (
    <Box>
      ...
      <PageHeading
        ...
        mainpagename="Company Documents"
        subpagename="Company Document Preparation"
        subsubpagename=""
      />
      <>
        {isUserRoleCompare?.includes("acompanydocumentpreparation") && (
          <Box sx={userStyle.selectcontainer}>
            <Typography>Add Company Document Preparation</Typography>
            <br /> <br />
            <>
              <Grid container spacing={2}>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      maxMenuHeight={300}
                      options={templateValues.filter((data) => {
                        return (
                          data.company === documentPrepartion.company &&
                          data.branch === documentPrepartion.branch
                        );
                      })}
                      value={{
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      value={{
                        label: documentPrepartion.tocompany,
                        value: documentPrepartion.tocompany,
                      }}
                      onChange={(e) => {
                        setDocumentPrepartion({
                          ...documentPrepartion,
          ...
          <Box sx={userStyle.container}>
            ...
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>
                List Company Document Preparation
              </Typography>
            </Grid>
              ...
              >
                <Box>
                  {isUserRoleCompare?.includes(
                    "excelcompanydocumentpreparation"
                  ) && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes(
                    "printcompanydocumentpreparation"
                  ) && (
                    <>
                  ...
                  {isUserRoleCompare?.includes(
                    "imagecompanydocumentpreparation"
                  ) && (
                    <Button
                      sx={userStyle.buttongrp}
