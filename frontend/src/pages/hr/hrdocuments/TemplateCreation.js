...
function TemplateCreation() {
  ...
  const [templateCreationEdit, setTemplateCreationEdit] = useState({
    name: "",
  });
  const [templateCreationArray, setTemplateCreationArray] = useState([]);
  const [templateCreationArrayOverall, setTemplateCreationArrayOverall] =
    useState([]);
  const {
    isUserRoleCompare,
    isUserRoleAccess,
  ...
  const customValueRendererEmpMode = (valueEmpMode, _branchs) => {
    return valueEmpMode.length
      ? valueEmpMode.map(({ label }) => label).join(", ")
  ...
  const handleBranchChange = (options) => {
    ...
    setValueBranchCat(value);
    setSelectedOptionsBranch(options);

    HeaderFooterDropdowns(templateCreation.company, value);
    setTemplateCreation({
      ...templateCreation,
      header: "Please Select Header",
      footer: "Please Select Footer",
  ...
  const [selectedTermsAndCondtionOptions, setSelectedTermsAndCondtionOptions] =
    useState([]);
  ...
  const customValueRendererTermsAndCondtion = (termsAndCondtion, _branchs) => {
    return termsAndCondtion.length
      ? termsAndCondtion.map(({ label }) => label).join(", ")
  ...
  const [
    selectedTermsAndCondtionOptionsEdit,
    setSelectedTermsAndCondtionOptionsEdit,
  ] = useState([]);
  ...
  const customValueRendererTermsAndCondtionEdit = (
    termsAndCondtion,
    _branchs
  ) => {
    return termsAndCondtion.length
  ...
  const [headerDropDowns, setHeaderDropDowns] = useState([]);
  const [footerDropDowns, setFooterDropDowns] = useState([]);
  const [headerDropDownsEdit, setHeaderDropDownsEdit] = useState([]);
  const [footerDropDownsEdit, setFooterDropDownsEdit] = useState([]);
  ...
  const HeaderFooterDropdowns = async (company, branch) => {
    setPageName(!pageName);
    try {
      let res = await axios.post(
        `${SERVICE.HEADER_FOOTER_FROM_TEMPLATECONTROLPANEL}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      const header =
        res?.data?.headerfooter?.length > 0
          ? res?.data?.headerfooter
              ?.flatMap((data) => data?.letterheadcontentheader)
              .map((data) => ({
      ...
      const footer =
        res?.data?.headerfooter?.length > 0
          ? res?.data?.headerfooter
              ?.flatMap((data) => data?.letterheadcontentfooter)
              .map((data) => ({
                label: data?.footername,
                value: data?.footername,
              }))
          : [];
      console.log(res?.data, "Template Control Panel Data's");
      setHeaderDropDowns(header);
      setFooterDropDowns(footer);
    } catch (err) {
      handleApiError(
        err,
        setPopupContentMalert,
  ...
  const HeaderFooterDropdownsEdit = async (company, branch) => {
    setPageName(!pageName);
    try {
      let res = await axios.post(
        `${SERVICE.HEADER_FOOTER_FROM_TEMPLATECONTROLPANEL}`,
      ...
      const footer =
        res?.data?.headerfooter?.length > 0
          ...
          : [];
      console.log(res?.data, "Template Control Panel Data's");
      setHeaderDropDownsEdit(header);
      setFooterDropDownsEdit(footer);
    } catch (err) {
      handleApiError(
        err,
        setPopupContentMalert,
  ...
  const sendEditRequestTemplate = async () => {
    if (docPrepData?.length > 0) {
      docPrepData?.map((data) => {
        let res = axios.put(
          `${SERVICE.SINGLE_DOCUMENTPREPARATION}/${data._id}`,
          {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
  ...
  const fetchTermsAndConditonsOptions = async () => {
    ...
    try {
      ...
      const options =
        res?.data?.termsandcondition?.length > 0
          ? res?.data?.termsandcondition?.map((data) => ({
              ...data,
              label: data?.details,
              value: data?._id,
            }))
    ...
    try {
      if (templatecreation?.length > 0) {
        let answ = templatecreation.map((d, i) => {
          ...
          let res = axios.put(
            `${SERVICE.SINGLE_DOCUMENTPREPARATION}/${d._id}`,
            {
              headers: {
                Authorization: `Bearer ${auth.APIToken}`,
              },
  ...
  const generateTemplateCodeCreate = (
    templateCreation,
    branchcode,
    templateName
  ) => {
    if (
      !templateCreation?.companycode ||
      !branchcode ||
      !templateCreation?.name
    ) {
      return ""; // return empty string if required data missing
    }

    const tempNameParts = templateName?.trim().split(/\s+/);
    const branchCode = branchcode;
    let templateCode = `${templateCreation.companycode}_${branchCode}_`;

    if (tempNameParts.length === 3) {
      templateCode += `${tempNameParts[0].slice(0, 2)}${tempNameParts[1].slice(
        0,
        2
      )}${tempNameParts[2].slice(0, 2)}`;
    } else if (tempNameParts.length === 2) {
      templateCode += `${tempNameParts[0].slice(0, 3)}${tempNameParts[1].slice(
        0,
        3
      )}`;
    } else if (tempNameParts.length === 1) {
      templateCode += `${tempNameParts[0].slice(0, 6)}`;
    } else {
      // fallback for names with more than 3 parts
      templateCode += tempNameParts
        .slice(0, 3)
        .map((part) => part.slice(0, 2))
        .join("");
    }

    return templateCode.toUpperCase(); // optional: make code uppercase
  };
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      //   if (tempName.length === 3) {
      //     temmplateCode += `${tempName[0]?.slice(0, 2)}${tempName[1]?.slice(
      //       0,
      //       2
      //     )}${tempName[2]?.slice(0, 2)}`;
      //   } else if (tempName.length === 2) {
      //     temmplateCode += `${tempName[0]?.slice(0, 3)}${tempName[1]?.slice(
      //       0,
      //       3
      //     )}`;
      //   } else if (tempName.length === 1) {
      ...
      const res = await axios.post(SERVICE.CREATE_TEMPLATECREATION, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        ...
        branch: templateCreation.branch,
        tempcode: templateCreation.tempcode,
        headvalue: headvalue,
        header: templateCreation?.header,
        footer: templateCreation?.footer,
  ...
  function generateTemplateCode(companyCode, branchCode, templateName) {
    ...
    if (tempName.length >= 3) {
      templateCode += `${tempName[0]?.slice(0, 2)}${tempName[1]?.slice(
        0,
        2
      )}${tempName[2]?.slice(0, 2)}`;
    } else if (tempName.length === 2) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const isNameMatch = templateCreationArrayOverall?.some(
      (item) =>
        item.name?.toLowerCase() === templateCreation.name?.toLowerCase() &&
        item.company?.toLowerCase() ===
          templateCreation.company?.toLowerCase() &&
        item.branch?.toLowerCase() === templateCreation.branch?.toLowerCase()
    );

    if (
      templateCreation?.tempaltemode === "Employee" &&
      valueEmpModeOptions.length === 0
    ) {
      ...
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (templateCreation?.branch === "Please Select Branch") {
      ...
      templateCreation?.header === "Please Select Header" ||
      !templateCreation?.header
    ) {
      setPopupContentMalert("Please Select Header!");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
  ...
  const getCode = async (e) => {
    ...
    try {
      ...
      HeaderFooterDropdownsEdit(
        res?.data?.stemplatecreation?.company,
        res?.data?.stemplatecreation?.branch
      );
      ...
      const termsandcond = termsAndConditons
        ?.filter((data) =>
          res?.data?.stemplatecreation?.termsAndConditons?.includes(data?._id)
        )
        .map((data) => ({
          ...data,
          label: data?.details,
      ...
      setValueTermsAndCondtionEdit(
        res?.data?.stemplatecreation?.termsAndConditons
      );
  ...
  const getOverallEditTermsandConditions = async (doc) => {
    ...
    try {
      let res = await axios.post(
        `${SERVICE.OVERALL_EDIT_TERMS_TEMPLATECREATION}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const sendEditRequest = async () => {
    ...
    try {
      let res = await axios.put(
        `${SERVICE.SINGLE_TEMPLATECREATION}/${frequencyId}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
          ...
          header: templateCreationEdit?.header,
          footer: templateCreationEdit?.footer,
  ...
  const editSubmit = (e) => {
    e.preventDefault();
    fetchBrandMasterAll();
    const isNameMatch = allTemplateCreationEdit?.some(
      (item) =>
        item.name?.toLowerCase() === templateCreationEdit.name?.toLowerCase() &&
        item.company?.toLowerCase() ===
          templateCreationEdit?.company?.toLowerCase() &&
        item.branch?.toLowerCase() ===
          templateCreationEdit?.branch?.toLowerCase()
    );

    if (
      templateCreationEdit?.tempaltemode === "" ||
      templateCreationEdit?.tempaltemode === undefined ||
      templateCreationEdit?.tempaltemode === ""
    ) {
      ...
      valueEmpModeOptionsEdit?.length === 0
    ) {
      setPopupContentMalert("Please Select Employee Mode!");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      templateCreationEdit?.company === "Please Select Company" ||
      templateCreationEdit?.company === undefined ||
      templateCreationEdit?.company === ""
    ) {
      setPopupContentMalert("Please Select Company!");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      templateCreationEdit?.branch === "Please Select Branch" ||
      templateCreationEdit?.branch === undefined ||
      templateCreationEdit?.branch === ""
    ) {
      setPopupContentMalert("Please Select Branch!");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
      ...
      templateCreationEdit.documentname === "" ||
      templateCreationEdit.documentname === undefined
    ) {
      setPopupContentMalert("Please Enter Document Name!");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      templateCreationEdit.tempcode === "" ||
      templateCreationEdit.tempcode === undefined
    ) {
      setPopupContentMalert("Please Enter Template Code!");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    ...
    // }
      ...
      templateCreationEdit?.tempaltemode === "Employee" &&
      valueTermsAndCondtionEdit?.some((data) =>
        templateCreationEdit?.termsAndConditons?.includes(data)
      )
    ) {
      setShowAlertpopTemplate(
        <>
          ...
          <p style={{ fontSize: "20px", fontWeight: 900 }}>
            {
              "Old Terms & Condtions changed,Do You want to chnange in Employee Document also.?"
            }
          </p>
        </>
      );
  ...
  const fetchBrandMasterAll = async () => {
    try {
      const answer =
        templateCreationArrayOverall?.length > 0
          ? templateCreationArrayOverall
          : [];
      setAllTemplateCreationEdit(
        answer?.filter((item) => item._id !== templateCreationEdit._id)
      );
    } catch (err) {
      handleApiError(
        err,
          ...
          <>
            ...
            <Grid container spacing={2}>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Selects
                    ...
                    options={modeoptions}
                    value={{
                      label: templateCreation.tempaltemode,
                      value: templateCreation.tempaltemode,
                    }}
                    onChange={(e) => {
                      setTemplateCreation({
                        ...templateCreation,
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Selects
                    ...
                    value={{
                      label: templateCreation.company,
                      value: templateCreation.company,
                    }}
                    onChange={(e) => {
                      setTemplateCreation({
                        ...templateCreation,
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Selects
                    ...
                    value={{
                      label: templateCreation.branch,
                      value: templateCreation.branch,
                    }}
                    onChange={(e) => {
                      setTemplateCreation({
                        ...templateCreation,
                        ...
                        header: "Please Select Header",
                        footer: "Please Select Footer",
                        tempcode: generateTemplateCodeCreate(
                          templateCreation,
                          e.branchcode,
                          templateCreation?.name
                        ),
                      });
                      HeaderFooterDropdowns(templateCreation.company, e.value);
              ...
              <Grid item md={2} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Selects
                    ...
                    value={{
                      label: templateCreation.signature,
                      value: templateCreation.signature,
                    }}
                    menuPortalTarget={document.body}
                    styles={{
                      menuPortal: (base) => ({ ...base, zIndex: 9999 }),
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Selects
                    ...
                    options={sealOptions}
                    value={{
                      label: templateCreation.seal,
                      value: templateCreation.seal,
                    }}
                    menuPortalTarget={document.body}
                    styles={{
                      menuPortal: (base) => ({ ...base, zIndex: 9999 }),
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Selects
                    ...
                    value={{
                      label: templateCreation.pagemode,
                      value: templateCreation.pagemode,
                    }}
                    menuPortalTarget={document.body}
                    styles={{
                      menuPortal: (base) => ({ ...base, zIndex: 9999 }),
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  <Typography>
                    Header <b style={{ color: "red" }}>*</b>
                  </Typography>
                  <Selects
                    maxMenuHeight={300}
                    options={headerDropDowns}
                    value={{
                      label: templateCreation.header,
                      value: templateCreation.header,
                    }}
                    menuPortalTarget={document.body}
                    styles={{
                      menuPortal: (base) => ({ ...base, zIndex: 9999 }),
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  <Typography>
                    Footer <b style={{ color: "red" }}>*</b>
                  </Typography>
                  <Selects
                    maxMenuHeight={300}
                    options={footerDropDowns}
                    value={{
                      label: templateCreation.footer,
                      value: templateCreation.footer,
                    }}
                    menuPortalTarget={document.body}
                    styles={{
                      menuPortal: (base) => ({ ...base, zIndex: 9999 }),
            ...
            <Grid container spacing={2}>
              ...
              {templateCreationEdit?.tempaltemode === "Employee" && (
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Typography>
                      {templateCreationEdit?.employeemode
                        ?.map((t, i) => `${i + 1 + ". "}` + t)
                        .join("\n")}
                    </Typography>
                  </FormControl>
              ...
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>{templateCreationEdit?.header}</Typography>
                </FormControl>
              </Grid>
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  <Typography variant="h6">Footer</Typography>
                  <Typography>{templateCreationEdit?.footer}</Typography>
                </FormControl>
              </Grid>
              <Grid item md={12} xs={12} sm={12}>
                <FormControl fullWidth size="small">
              ...
              <Grid container spacing={2}>
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      options={modeoptions}
                      value={{
                        label: templateCreationEdit.tempaltemode,
                        value: templateCreationEdit.tempaltemode,
                      }}
                      onChange={(e) => {
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      options={CompanyOptions}
                      value={{
                        label: templateCreationEdit.company,
                        value: templateCreationEdit.company,
                      }}
                      onChange={(e) => {
                        setTemplateCreationEdit({
                          ...templateCreationEdit,
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      value={{
                        label: templateCreationEdit.branch,
                        value: templateCreationEdit.branch,
                      }}
                      onChange={(e) => {
                        let companyCode = templateCreationEdit?.companycode
                          ? templateCreationEdit?.companycode
                          : templateCreationEdit.tempcode?.split("_")[0];
                        setTemplateCreationEdit({
                          ...templateCreationEdit,
                          branch: e.value,
                          branchcode: e.branchcode,
                          tempcode: generateTemplateCode(
                            companyCode,
                            e.branchcode,
                            templateCreationEdit.name
                          ),
                        });
                        HeaderFooterDropdownsEdit(
                          templateCreationEdit.company,
                    ...
                    <OutlinedInput
                      ...
                      onChange={(e) => {
                        let companyCode = templateCreationEdit?.companycode
                          ? templateCreationEdit?.companycode
                          : templateCreationEdit.tempcode?.split("_")[0];
                        let branchCode = templateCreationEdit?.branchcode
                          ? templateCreationEdit?.branchcode
                          : templateCreationEdit.tempcode?.split("_")[1];
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      options={signatureOptions}
                      value={{
                        label: templateCreationEdit.signature,
                        value: templateCreationEdit.signature,
                      }}
                      onChange={(e) => {
                        setTemplateCreationEdit({
                          ...templateCreationEdit,
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      value={{
                        label: templateCreationEdit.seal,
                        value: templateCreationEdit.seal,
                      }}
                      onChange={(e) => {
                        setTemplateCreationEdit({
                          ...templateCreationEdit,
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      options={sizenewOptions}
                      value={{
                        label: templateCreationEdit.pagemode,
                        value: templateCreationEdit.pagemode,
                      }}
                      menuPortalTarget={document.body}
                      styles={{
                        menuPortal: (base) => ({ ...base, zIndex: 9999 }),
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      maxMenuHeight={300}
                      options={headerDropDownsEdit}
                      value={{
                        label: templateCreationEdit.header,
                        value: templateCreationEdit.header,
                      }}
                      menuPortalTarget={document.body}
                      styles={{
                        menuPortal: (base) => ({ ...base, zIndex: 9999 }),
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      maxMenuHeight={300}
                      options={footerDropDownsEdit}
                      value={{
                        label: templateCreationEdit.footer,
                        value: templateCreationEdit.footer,
                      }}
                      menuPortalTarget={document.body}
                      styles={{
                        menuPortal: (base) => ({ ...base, zIndex: 9999 }),
