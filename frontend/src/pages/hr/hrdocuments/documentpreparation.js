...
function DocumentPreparation() {
  ...
  const fetchDesigDepartBasedOnDate = async (selecteddate, person) => {
    try {
      const response = await axios.post(
        SERVICE.DEPARTMENT_DESIGNATION_BASED_ON_DATE,
        {
          headers: {
  ...
  const handleHeadChange = async (options) => {
    ...
    if (!["Preview Manual", "Print Manual"]?.includes(pagePopeOpen)) {
      ...
      if (value?.length === 1 && value?.includes("With Head content")) {
        setHeader(headerFooterBase64?.headerimage);
      } else if (
        value?.length === 1 &&
        value?.includes("With Footer content")
      ) {
        setfooter(headerFooterBase64?.footerimage);
      } else if (value?.length > 1) {
        setHeader(headerFooterBase64?.headerimage);
        setfooter(headerFooterBase64?.footerimage);
      } else {
  ...
  const years = Array.from(
    new Array(10),
    (val, index) => currentYearAttStatus - index
  );
    ...
    try {
      if (otp != "") {
        let response = await axios.post(
          `${SERVICE.VERIFYTWOFA_EMPLOYEEAPPROVAL}`,
          {
            otp: String(otp),
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String(
        "Human Resource/HR Documents/Employee Document Preparation"
      ),
  ...
  const fetchDepartmentMonthsets = async (year) => {
    ...
    try {
      ...
      return await fetchProductionMonthStatus(
        employeeControlPanel,
        documentPrepartion?.productionmonth,
        year,
  ...
  const fetchProductionMonthStatus = async (person, month, year, monthset) => {
    ...
    let selectedmonthnumalter =
      Number(selectedMonthNum) <= 9
        ? `0${Number(selectedMonthNum)}`
        : selectedMonthNum;

    let selectedMonStartDate =
      selectedYear + "-" + selectedmonthnumalter + "-01";
    let dayPointsUser = prodFilter.data.answer?.filter(
      (data) => data?.name === person.value
    );
    ...
    let getdayPointsMonth = dayPointsUser
      .filter(
        (d) =>
          d.date >= findDate &&
          d.date <= (findexp ? findexp.todate : lastdateOfSelectedMonth)
      )
      .reduce((acc, current) => {
  ...
  const [availableYearsAttendance, setAvailableYearsAttendance] =
    useState(getyear);
  const [availableYearsProduction, setAvailableYearsProduction] =
    useState(getyear);
  const [availableYearsSalary, setAvailableYearsSalary] = useState(getyear);
  //  const navigate = useNavigate();
  const generateRedirectUrl = () => {
    return `${BASE_URL}/hrdocuments/templatecreation?data=${encodeURIComponent(
      "Rahul"
    )}`;
  };
  ...
  const [loadingMessageAttMonth, setLoadingMessageAttMonth] = useState(
    "Fetching Attendance Month Status...!"
  );
  const [loadingAttDate, setLoadingAttDate] = useState(false);
  const [loadingMessageAttDate, setLoadingMessageAttDate] = useState(
    "Fetching Attendance Date Status...!"
  );
  const [loadingProdDate, setLoadingProdDate] = useState(false);
  const [loadingMessageProdDate, setLoadingMessageProdDate] = useState(
    "Fetching Production Date Status...!"
  );
  const [loadingProdMonth, setLoadingProdMonth] = useState(false);
  const [loadingPreviewData, setLoadingPreviewData] = useState(false);
  const [loadingPreviewManualData, setLoadingPreviewManualData] =
    useState(false);
  const [loadingMessageProdMonth, setLoadingMessageProdMonth] = useState(
    "Fetching Production Month Status...!"
  );
  const [loadingPreviewMessage, setLoadingPreviewMessage] = useState(
    "Setting up a document for preview...!"
  );
  const [loadingPrintData, setLoadingPrintData] = useState(false);
  const [loadingPrintManualData, setLoadingPrintManualData] = useState(false);
  const [loadingPrintMessage, setLoadingPrintMessage] = useState(
    "Preparing an Document to Print...!"
  );
  const [loadingGeneratingDatas, setLoadingGeneratingDatas] = useState(false);
  const [loadingGeneratingMessages, setLoadingGeneratingMessage] = useState(
    "Generating the set of Documents...!"
  );
  const [savingDatasMessage, setSavingDatasMessage] = useState(
    "Generating the set of Documents for Saving...!"
  );
  ...
  const [noticePeriodEmpCheckPerson, setNoticePeriodEmpCheckPerson] =
    useState(false);
  const [noticePeriodEmpCheckPersonEdit, setNoticePeriodEmpCheckPersonEdit] =
    useState(false);
  const [noticePeriodEmpCheckEdit, setNoticePeriodEmpCheckEdit] =
    useState(false);
  ...
  const [isInfoOpenImagePrintManual, setInfoOpenImagePrintManual] =
    useState(false);
  ...
  const [isDeleteOpenBulkcheckbox, setIsDeleteOpenBulkcheckbox] =
    useState(false);
  ...
  const handleClickOpenBulkalert = () => {
    if (selectedRows?.length === 0) {
      ...
      const selectedData = rowDataTable?.filter((data) =>
  ...
  const [templateCreationValueEdit, setTemplateCreationValueEdit] =
    useState("");
  ...
  const TemplateDropdownsValue = async (e, control, templateCrea) => {
    ...
    try {
      ...
      if (res?.data?.templatecontrolpanel) {
        ...
        setFromEmail(headerFooterBase64?.fromemail);
        setCompanyName(headerFooterBase64);
        // if (e.headvalue?.includes("With Head content")) {
        //   setHeader(ans?.headerimage)
        // }
        // if (e.headvalue?.includes("With Footer content")) {
        //   setfooter(ans?.footerimage)
        // }
        setWaterMarkText(headerFooterBase64?.backgroundimage);
        ...
        setPopupContentMalert(
          "This Employee's company and branch is not matched with Template control panel data.Add the details in Template control panel!"
        );
        setPopupSeverityMalert("info");
        handleClickOpenPopupMalert();
      }
  ...
  const TemplateDropdownsValueManual = async (
    ...
    try {
      ...
      if (res?.data?.templatecontrolpanel) {
        ...
        //   const headerFooterBase64 = {..ans , headerimage : , footerimage : }
        setPersonId(headerFooterBase64);
      } else {
        setGenerateData(true);
        setPopupContentMalert(
  ...
  const TemplateManualDropDowns = async (
    e,
    mode,
    company,
    branch,
  ...
  const getattendancestatusmonth = (clockinstatus, clockoutstatus) => {
    let result = attStatus.filter((data, index) => {
      return (
        data?.clockinstatus === clockinstatus &&
        data?.clockoutstatus === clockoutstatus
      );
    });
    return result[0]?.name;
  };
  ...
  const fetchSalaryDetails = async (department, branch, unit, team, person) => {
    ...
    try {
      ...
      const time = await getCurrentServerTime();
      const RES_SALARYSLAB = await axios.get(SERVICE.SALARYSLAB_LIMITED, {
        headers: { Authorization: `Bearer ${auth.APIToken}` },
      });

      let res = await axios.post(
        SERVICE.USER_PAYRUNDATA_LIMITED_FINAL_SALARY_GENERATION_REVIEW,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      let employeelistnames =
        res.data.users.length > 0
          ? [...new Set(res.data.users.map((item) => item.companyname))]
          : [];
      ...
      if (res.data.users.length > 0) {
        ...
        function processEmployeeItem(item, index, data) {
          try {
            ...
            const finalSelectedMonth =
              selectedModeType == "DOJ" ? findexp.monthname : selectedMonth;
            const finalSelectedYear =
              selectedModeType == "DOJ" ? Number(findexp.year) : selectedYear;
            ...
            item.assignExpLog &&
              ...
              item.assignExpLog
                ...
                .forEach((item) => {
                  const monthYear = item.updatedate
                    ?.split("-")
                    .slice(0, 2)
                    .join("-");
            ...
            for (let i = 0; i < lastItemsForEachMonth.length; i++) {
              ...
              if (
                Number(itemYear) === finalSelectedYear &&
                Number(itemMonth) === Number(finalSelectedMonthNum)
              ) {
            ...
            let selectedmonthnumalter =
              Number(finalSelectedMonthNum) <= 9
                ? `0${Number(finalSelectedMonthNum)}`
                : finalSelectedMonthNum;

            let selectedMonStartDate =
              finalSelectedYear + "-" + selectedmonthnumalter + "-01";
              ...
              if (startDate && endDate) {
                ...
                if (days < 0) {
                  ...
                  days += new Date(
                    ...
                    0
                  ).getDate(); // Add days of the previous month
                }
            ...
            let experienceinmonthCalcVal = item.doj
              ? calculateMonthsBetweenDates(item.doj, findDate) < 0
                ? 0
            ...
            const nextMonthFirstDay = new Date(
              Number(finalSelectedYear),
              Number(finalSelectedMonthNum),
              1
            );
            ...
            let findmonthenddate = findexp
              ? findexp.todate
              : selectedMonEndDate;

            const thisMonthEndDate = findexp
              ? findexp.todate
              : selectedMonEndDate;
            const totaluserDays =
              (new Date(thisMonthEndDate) - new Date(item.doj)) /
              (1000 * 60 * 60 * 24);
            // Find the first item in the sorted array that meets the criteria
            ...
            function getTotalDaysInMonthByName(monthName, year) {
              const monthIndex =
                new Date(`${monthName} 1, ${year}`).getMonth() + 1; // Convert month name to number
              return new Date(year, monthIndex, 0).getDate();
            }

            let tond = getTotalDaysInMonthByName(
              finalSelectedMonth,
              Number(finalSelectedYear)
            );
        ...
        async function sendBatchRequestItems(batch, data) {
          try {
            const itemsWithSerialNumber = batch.emps.map(async (item, index) =>
              processEmployeeItem(item, index, data)
            );
        ...
        finalSalaryDetails = result.allResultsItems
          ...
          .map((item, index) => {
            ...
            const conveyance = item.oldconveyance
              ? Number(item.oldconveyance)
              : 0;
            const medicalallowance = item.oldmedicalallowance
              ? Number(item.oldmedicalallowance)
              : 0;
            const productionallowance = item.oldproductionallowance
              ? Number(item.oldproductionallowance)
              : 0;
            const productionallowancetwo = item.oldproductionallowancetwo
              ? Number(item.oldproductionallowancetwo)
              : 0;
            const otherallowance = item.oldotherallowance
              ? Number(item.oldotherallowance)
              : 0;
            const gross =
              basic +
              hra +
  ...
  const fetchUsersStatus = async (person, date) => {
    ...
    try {
      let res = await axios.post(
        SERVICE.USER_CLOCKIN_CLOCKOUT_STATUS_DOC_PREPARATION,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      const itemsWithSerialNumber = res?.data?.finaluser?.map((item, index) => {
        ...
        const absentItems = res?.data?.finaluser?.filter(
          (d) =>
  ...
  const fetchUsersFilter = async (
    ...
    try {
      const time = await getCurrentServerTime();
      let res_emp = await axios.post(
        SERVICE.USER_FOR_ALL_ATTENDANCE_PAGE_INDIVIDUAL_TYPE,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      let employeelistnames =
        res_emp?.data?.users.length > 0
          ? [...new Set(res_emp?.data?.users.map((item) => item.companyname))]
          : [];
      ...
      const montharray = getMonthsInRange(
        Number(getMonthIndex(choosemonth) + 1) - 1,
        Number(chooseyear)
      );
      ...
      let leaveresult = res_applyleave?.data?.applyleaves?.filter(
        (data) => data.leavetype !== "Leave Without Pay (LWP)"
      );
      let leaveresultWithoutPay = res_applyleave?.data?.applyleaves?.filter(
        (data) => data.leavetype === "Leave Without Pay (LWP)"
      );
      ...
      async function sendBatchRequest(batch) {
        try {
          ...
          let resdatawithlwp = [
            ...res_type?.data?.leavetype,
            { leavetype: "Leave Without Pay (LWP)", code: "LWP" },
          ];
          ...
          resdatawithlwp?.map((type) => {
            res_applyleave?.data?.applyleaves &&
              res_applyleave?.data?.applyleaves?.forEach((d) => {
                if (
                  type.leavetype === d.leavetype &&
                  d.tookleavecheckstatus === "Single" &&
                  d.leavestatus === "Shift"
                ) {
                ...
                if (
                  type.leavetype === d.leavetype &&
                  d.tookleavecheckstatus === "Double" &&
                  d.leavestatus === "Shift"
                ) {
                  leavestatusApproved.push(
                    "DL" + " - " + type.code + " " + d.status
                  );
                }
                if (
                  type.leavetype === d.leavetype &&
                  d.tookleavecheckstatus === "Double Day" &&
                  d.leavestatus === "Shift"
                ) {
                  leavestatusApproved.push(
                    "DDL" + " - " + type.code + " " + d.status
                  );
                }
              });
          });
          ...
          const findPreviousNonWeekOff = (items, index) => {
            for (let i = index - 1; i >= 0; i--) {
              if (
                items[i].clockinstatus !== "Week Off" &&
                items[i].clockoutstatus !== "Week Off"
              ) {
                return items[i];
              }
            }
          ...
          const changedWeekoffResult = filtered?.map((item, index) => {
            ...
            if (
              ...
              const isPrevLeave = leaveresult.some(
                (leaveItem) =>
                  prev &&
                  moment(leaveItem.date, "DD/MM/YYYY").isSame(
                    moment(prev.rowformattedDate, "DD/MM/YYYY"),
                    "day"
                  ) &&
                  leaveItem.empcode === item.empcode
              );
              const isPrevAbsent =
                prev &&
                prev.empcode === item.empcode &&
                prev.clockinstatus === "Absent" &&
                prev.clockoutstatus === "Absent" &&
                prev.clockin === "00:00:00" &&
                prev.clockout === "00:00:00";

              const isNextLeave = leaveresult.some(
                (leaveItem) =>
                  next &&
                  moment(leaveItem.date, "DD/MM/YYYY").isSame(
                    moment(next.rowformattedDate, "DD/MM/YYYY"),
                    "day"
                  ) &&
                  leaveItem.empcode === item.empcode
              );
              const isNextAbsent =
                next &&
                next.empcode === item.empcode &&
                next.clockinstatus === "Absent" &&
                next.clockoutstatus === "Absent" &&
                next.clockin === "00:00:00" &&
                next.clockout === "00:00:00";

              const isPrevLeaveWithoutPay = leaveresultWithoutPay.some(
                (leaveItem) =>
                  prev &&
                  moment(leaveItem.date, "DD/MM/YYYY").isSame(
                    moment(prev.rowformattedDate, "DD/MM/YYYY"),
              ...
              const isNextLeaveWithoutPay = leaveresultWithoutPay.some(
                (leaveItem) =>
                  next &&
                  moment(leaveItem.date, "DD/MM/YYYY").isSame(
                    moment(next.rowformattedDate, "DD/MM/YYYY"),
              ...
              if (isPrevLeave && isNextLeave) {
                updatedClockInStatus = updatedClockOutStatus =
          ...
          Object.keys(empGrouped).forEach((empcode) => {
            const records = empGrouped[empcode].sort(
              (a, b) =>
                moment(a.rowformattedDate, "DD/MM/YYYY") -
                moment(b.rowformattedDate, "DD/MM/YYYY")
            );
                ...
                if (streak.length > attendanceCriteriaData?.longabsentcount) {
                  streak.forEach((day) => {
                    ...
                    if (clockinstatus === "Absent") {
                      ...
                      clockinstatus = `${counterIn++}Long Leave ${rearr?.filter(
                        (d) => d === clockinstatus
                      )}`;
                    }
                    ...
                    if (clockoutstatus === "Absent") {
                      ...
                      clockoutstatus = `${counterOut++}Long Leave ${rearr?.filter(
                        (d) => d === clockoutstatus
                      )}`;
                    }
            ...
            departmentDateSet.forEach((dateRange) => {
              ...
              let recordsInDateRange = attendanceRecords.filter((record) => {
                ...
                return (
                  ...
                  formattedDate >= new Date(fromdate) &&
                  formattedDate <= new Date(todate)
                );
              });

              let processedRecords = recordsInDateRange.map((item) => {
                let formattedDate = new Date(item.finalDate);
                let reasonDate = item.reasondate
                  ? new Date(item.reasondate)
                  : null;
          ...
          itemsWithSerialNumber.forEach((item, index, array) => {
            if (item.shift === "Week Off") {
              ...
              const isLopRelevant = (entry) =>
                entry &&
                (entry.lopcalculation === "Yes - Full Day" ||
                  entry.lopcalculation === "Yes - Double Day");
              ...
              const isPrevLop =
                isLopRelevant(previousItem) && !isPreviousManualPaidFull;
              const isNextLop =
                isLopRelevant(nextItem) && !isNextManualPaidFull;
              ...
              item.daystatus =
                item.attendanceautostatus || getattendancestatus(item);
              item.appliedthrough = getAttModeAppliedThr(
                item.attendanceautostatus || getattendancestatus(item)
              );
              item.lop = getAttModeLop(
                item.attendanceautostatus || getattendancestatus(item)
              );
              item.loptype = getAttModeLopType(
                item.attendanceautostatus || getattendancestatus(item)
              );
              ...
              item.modetarget = getAttModeTarget(
                item.attendanceautostatus || getattendancestatus(item)
              );
              item.paidpresentbefore = getAttModePaidPresent(
                item.attendanceautostatus || getattendancestatus(item)
              );
              item.paidleavetype = getAttModePaidPresentType(
                item.attendanceautostatus || getattendancestatus(item)
              );
              ...
              item.isweekoff = getIsWeekoff(
                item.attendanceautostatus || getattendancestatus(item)
              );
              item.isholiday = getIsHoliday(
                item.attendanceautostatus || getattendancestatus(item)
              );
            }
            if (
              attStatusOption.includes(item.daystatus) &&
              item.clockin === "00:00:00" &&
              item.clockout === "00:00:00" &&
              item.appliedthrough === "Manual" &&
              item.paidpresent === "Yes - Full Day"
            ) {
              const previousItem = array[index - 1];
              const nextItem = array[index + 1];

              const hasRelevantStatus = (entry) =>
                entry &&
                (weekOption.includes(entry.clockinstatus) ||
                  weekOption.includes(entry.clockoutstatus)) &&
                entry.shift === "Week Off";
              ...
              if (hasRelevantStatus(previousItem)) {
                if (!weekOption.includes(previousItem.clockinstatus)) {
                  ...
                  previousItem.attendanceauto =
                    getattendancestatus(previousItem);
                  previousItem.daystatus = previousItem.attendanceautostatus
                    ? previousItem.attendanceautostatus
                    : getattendancestatus(previousItem);
                  previousItem.appliedthrough = getAttModeAppliedThr(
                    previousItem.attendanceautostatus
                      ? previousItem.attendanceautostatus
                      : getattendancestatus(previousItem)
                  );
                  previousItem.lop = getAttModeLop(
                    previousItem.attendanceautostatus
                      ? previousItem.attendanceautostatus
                      : getattendancestatus(previousItem)
                  );
                  previousItem.loptype = getAttModeLopType(
                    previousItem.attendanceautostatus
                      ? previousItem.attendanceautostatus
                      : getattendancestatus(previousItem)
                  );
                  ...
                  previousItem.modetarget = getAttModeTarget(
                    previousItem.attendanceautostatus
                      ? previousItem.attendanceautostatus
                      : getattendancestatus(previousItem)
                  );
                  previousItem.paidpresentbefore = getAttModePaidPresent(
                    previousItem.attendanceautostatus
                      ? previousItem.attendanceautostatus
                      : getattendancestatus(previousItem)
                  );
                  previousItem.paidleavetype = getAttModePaidPresentType(
                    previousItem.attendanceautostatus
                      ? previousItem.attendanceautostatus
                      : getattendancestatus(previousItem)
                  );
                  ...
                  previousItem.isweekoff = getIsWeekoff(
                    previousItem.attendanceautostatus
                      ? previousItem.attendanceautostatus
                      : getattendancestatus(previousItem)
                  );
                  previousItem.isholiday = getIsHoliday(
                    previousItem.attendanceautostatus
                      ? previousItem.attendanceautostatus
                      : getattendancestatus(previousItem)
                  );
              ...
              if (hasRelevantStatus(nextItem)) {
                if (!weekOption.includes(nextItem.clockinstatus)) {
                  ...
                  nextItem.daystatus = nextItem.attendanceautostatus
                    ? nextItem.attendanceautostatus
                    : getattendancestatus(nextItem);
                  nextItem.appliedthrough = getAttModeAppliedThr(
                    nextItem.attendanceautostatus
                      ? nextItem.attendanceautostatus
                      : getattendancestatus(nextItem)
                  );
                  nextItem.lop = getAttModeLop(
                    nextItem.attendanceautostatus
                      ? nextItem.attendanceautostatus
                      : getattendancestatus(nextItem)
                  );
                  nextItem.loptype = getAttModeLopType(
                    nextItem.attendanceautostatus
                      ? nextItem.attendanceautostatus
                      : getattendancestatus(nextItem)
                  );
                  ...
                  nextItem.modetarget = getAttModeTarget(
                    nextItem.attendanceautostatus
                      ? nextItem.attendanceautostatus
                      : getattendancestatus(nextItem)
                  );
                  nextItem.paidpresentbefore = getAttModePaidPresent(
                    nextItem.attendanceautostatus
                      ? nextItem.attendanceautostatus
                      : getattendancestatus(nextItem)
                  );
                  nextItem.paidleavetype = getAttModePaidPresentType(
                    nextItem.attendanceautostatus
                      ? nextItem.attendanceautostatus
                      : getattendancestatus(nextItem)
                  );
                  ...
                  nextItem.isweekoff = getIsWeekoff(
                    nextItem.attendanceautostatus
                      ? nextItem.attendanceautostatus
                      : getattendancestatus(nextItem)
                  );
                  nextItem.isholiday = getIsHoliday(
                    nextItem.attendanceautostatus
                      ? nextItem.attendanceautostatus
                      : getattendancestatus(nextItem)
                  );
          ...
          let fianlResult = itemsWithSerialNumber.filter(
            (data) =>
      ...
      const resultdataEnd = results.allResults?.forEach((item) => {
        const leaveOnDateApproved = leaveresult?.find(
          (d) => d.date === item.rowformattedDate && d.empcode === item.empcode
        );
        const permissionOnDateApproved = permissionresult?.find(
          (d) =>
            d.date === item.finalDate &&
            d.employeeid === item.empcode &&
            d.compensationstatus === ""
        );
        ...
        const existingEntryIndex = finalresult.findIndex(
          (entry) => entry.empcode === item.empcode
        );
        ...
        if (existingEntryIndex !== -1) {
          ...
          if (
            ...
            item.attendanceauto === undefined &&
            item.daystatus === undefined
          ) {
          ...
          if (permissionOnDateApproved) {
            const requestMinutes = Number(
              permissionOnDateApproved.requesthours
            );
            ...
            if (matchedData) {
              finalresult[existingEntryIndex].permissionhours += Number(
                matchedData.permissionhours
              );
            }
          }
          ...
          if (leaveOnDateApproved) {
            const workingHours =
              Number(item.totalshifthours) -
              (Number(item.totalbreakhours) + Number());
            ...
            if (leaveOnDateApproved?.shiftcount === "0.5") {
              finalresult[existingEntryIndex].clslhours +=
                Number(workingHours) / 2;
            }
          }

          if (item.lop === "Yes") {
            // Find the previous non-weekoff record for the same employee
            const previousValid = [...results.allResults]
              .reverse()
              .find(
                (prev) =>
                  prev.empcode === item.empcode &&
                  prev.rowformattedDate < item.rowformattedDate &&
                  prev.isweekoff !== "Yes" &&
                  prev.shift !== "Not Allotted" &&
                  prev.shiftMode === "Main Shift"
              );

            if (previousValid) {
              const preWorkingHours =
                Number(previousValid.totalshifthours) -
                Number(previousValid.totalbreakhours);
              if (item.lopcalculation === "Yes - Half Day") {
                finalresult[existingEntryIndex].absenthours +=
                  Number(preWorkingHours) / 2;
              }
              if (item.lopcalculation === "Yes - Full Day") {
                finalresult[existingEntryIndex].absenthours +=
                  Number(preWorkingHours);
              }
            } else {
              const workingHours =
                Number(item.totalshifthours) - Number(item.totalbreakhours);
          ...
          if (item.isweekoff === "Yes") {
            ...
            if (previousValid) {
              const preWorkingHours =
                Number(previousValid.totalshifthours) -
                Number(previousValid.totalbreakhours);
              finalresult[existingEntryIndex].weekoffhours +=
                Number(preWorkingHours);
            } else {
              const workingHours =
                Number(item.totalshifthours) - Number(item.totalbreakhours);
          ...
          if (item.isholiday === "Yes") {
            const workingHours =
              Number(item.totalshifthours) - Number(item.totalbreakhours);
            finalresult[existingEntryIndex].holidayhours +=
              Number(workingHours);
          }

          if (
            item.shift !== "Week Off" &&
            item.shift !== "Not Allotted" &&
            ...
            if (item.clockin !== "00:00:00") {
              ...
              const differenceLate = getMinutesDifferenceForClockIn(
                item.clockin,
                shiftStartTime
              );
              ...
              const matchedData = attendanceCriteriaData.lateclockintodo
                ...
                ?.filter(
                  (todo) => Number(todo.lateclockinminutes) <= lateMinutes
                )
                ?.reduce((nearest, current) => {
                  const currentMinutes = Number(current.lateclockinminutes);
                  const nearestMinutes = nearest
                    ? Number(nearest.lateclockinminutes)
                    : -1;
                  return currentMinutes > nearestMinutes ? current : nearest;
                }, null);
              // console.log(matchedData, 'matched Late')
              if (matchedData) {
                finalresult[existingEntryIndex].totallateclockin += Number(
                  matchedData.lateclockinhours
                );
              }
            }
          }
          ...
          if (
            ...
            if (item.clockout !== "00:00:00") {
              ...
              const differenceEarly = getMinutesDifferenceForClockOut(
                item.clockout,
                shiftEndTime
              );
              ...
              const matchedData = attendanceCriteriaData.earlyclockouttodo
                ...
                ?.filter(
                  (todo) => Number(todo.earlyclockoutminutes) <= earlyMinutes
                )
                ?.reduce((nearest, current) => {
                  const currentMinutes = Number(current.earlyclockoutminutes);
                  const nearestMinutes = nearest
                    ? Number(nearest.earlyclockoutminutes)
                    : -1;
                  return currentMinutes > nearestMinutes ? current : nearest;
                }, null);
              // console.log(matchedData, 'matched early')
              if (matchedData) {
                finalresult[existingEntryIndex].totalearlyclockout += Number(
                  matchedData.earlyclockouthours
                );
              }
            }
          }
          ...
          const newItem = {
            ...
            doublehalflopcount:
              item.lopcalculation === "Double Half Day" ? 0.5 : 0,
      ...
      let resultdata = finalresult?.map((item, index) => {
        const finalPaidPresentDays =
          Number(item.paidpresentday) -
          (Number(item.weekoff) +
            Number(item.holidayCount) +
            Number(item.leaveCount) +
            Number(item.doublelopcount) +
            Number(item.doublehalflopcount));
        const workingHours =
          Number(item.totalshifthours) -
          (Number(item.totalbreakhours) +
            Number(item.weekoffhours) +
            Number(item.holidayhours));
        const missedHours =
          Number(item.totallateclockin) + Number(item.totalearlyclockout);
        const presentHours =
          Number(workingHours) -
          (Number(missedHours) +
            Number(item.absenthours) +
            Number(item.permissionhours));
        const paidHours =
          Number(presentHours) +
          Number(item.weekoffhours) +
          Number(item.clslhours) +
          Number(item.holidayhours);
  ...
  const fetchProductionDateStatus = async (person, date) => {
    ...
    try {
      ...
      const answer =
        res_applyleave?.data?.daypointsupload?.length > 0
          ? res_applyleave?.data?.daypointsupload[0]
          : "";
  ...
  const handleEmployeeChange = (options) => {
    const uniqueEntries = options?.filter(
      (item, index, self) =>
        index ===
        self.findIndex(
    ...
    if (uniqueEntries?.length > 1) {
      ...
      setPopupContentMalert(
        "Employee's have different company and branch.Please Select Users With same company and same branch!"
      );
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
  ...
  const fetchSignatureSealPlacement = async (document, name) => {
    console.log(document, "937");
    try {
      const base64 = await convertFileUrlToBase64(
        `${BASE_URL}/templatecontrolpanel/${document?.name}`
      );
      if (name === "Seal") {
        setSealPlacement(base64);
      }
  ...
  const IdentifyUserCodeEdit = async (e) => {
    ...
    try {
      ...
      const value =
        res?.data?.documentPreparation +
        e?.team?.slice(0, 3) +
        "#" +
        templateCreationValueEdit +
        "_" +
        cateCode;
  ...
  const CheckNoticePeriod = async (e) => {
    ...
    try {
      ...
      const answer = res?.data?.noticeperiodapply?.some(
        (data) => data.empname === e && data.exitstatus == true
      );
      const answerPerson = res?.data?.noticeperiodapply?.some(
        (data) => data.empname === e
      );
  ...
  const CheckNoticePeriodMulti = async (e) => {
    ...
    try {
      ...
      const answer = res?.data?.noticeperiodapply?.some(
        (data) => e?.includes(data.empname) && data.exitstatus == true
      );
      const answerPerson = res?.data?.noticeperiodapply?.some((data) =>
        e?.includes(data.empname)
      );
      setNoticePeriodEmpCheck(answer);
      setNoticePeriodEmpCheckPerson(answerPerson);
    } catch (err) {
  ...
  const fetchTeamNames = async (e, mode) => {
    ...
    try {
      let res_type = await axios.post(
        SERVICE.USERNAMES_EMP_DOCUMENT_DEPARTMENT,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const fetchAllEmployee = async (e) => {
    ...
    try {
      ...
      let usersEmployeemode =
        res_module?.data?.userteamgroup?.length > 0
  ...
  let Allcodedata = `${BASE_URL}/document/documentpreparation/${encryptString(
    documentPrepartion.person
  )}/${personId ? personId?._id : ""}/${encryptString(
    documentPrepartion?.issuingauthority
  )}/${DateFormat}/${isUserRoleAccess?._id}`;

  let AllcodedataEdit = `${BASE_URL}/document/documentpreparation/${encryptString(
    documentPreparationEdit.person
  )}/${companyNameEdit?._id}/${encryptString(
  ...
  const answerDefine = async (person, index) => {
    ...
    let employeename =
      employeeMode === "Manual" ? "" : person ? person : employeeValue;
    if (employeename) {
      departmentDesigDateBased = await fetchDesigDepartBasedOnDate(
        documentPrepartion?.manualdate,
        employeename
      );
    }
    ...
    let prefixLength =
      Number(constAuotId[constAuotId?.length - 1]) +
      (employeeControlPanel && index ? index + 1 : 1);
    ...
    setPageName(!pageName);
    try {
      const [res, res_emp, res_emp_break, userDetails, companynameSettings] =
        await Promise.all([
          axios.post(
            SERVICE.EMPLOYEE_TEMPLATECREATION,
            {
      ...
      if (AttendanceNeed) {
        const result =
          documentPrepartion.attendancesort === "Date"
            ? getMonthYear(documentPrepartion.attendancedate)
            : {};
        const attMonthCal =
          documentPrepartion.attendancesort === "Date"
            ? result?.month
            : documentPrepartion.attendancemonth;
        const attYearCal =
          documentPrepartion.attendancesort === "Date"
            ? result?.year
            : documentPrepartion.attendanceyear;
        const resultAttendance = await fetchUsersFilter(
          employee,
          attMonthCal,
          attYearCal,
          documentPrepartion.attendancesort
        );
        attendanceDetails =
      ...
      let format = res?.data?.templatecreation?.find(
        (data) =>
          ...
          data?.name === documentPrepartion?.template?.split("--")[0]
      );
      ...
      let employeeBreak = res_emp_break?.data?.shifts.find(
        (data) => data?.name === employee?.shifttiming
      );
      ...
      if (employeeMode === "Manual") {
        let findMethod = texted
          ...
          .replaceAll(
            "$V.BRANCHADDRESS$",
            branchAddressTextVertical ? branchAddressTextVertical : ""
          )
        ...
        let findMethod = texted
          ...
          .replaceAll(
            ...
            departmentDesigDateBased
              ? departmentDesigDateBased?.presentdesignation
              : ""
          )
          .replaceAll(
            "$RECENT_DEPARTMENT$",
            departmentDesigDateBased
              ? departmentDesigDateBased?.presentdepartment
              : ""
          )
          .replaceAll(
            "$YEARMONTH_DEPARTMENT$",
            departmentDesigDateBased
              ? departmentDesigDateBased?.datedepartment
              : ""
          )
          .replaceAll(
            "$YEARMONTH_DESIGNATION$",
            departmentDesigDateBased
              ? departmentDesigDateBased?.datedesignation
              : ""
          )
          .replaceAll(
            "$DOJ_DEPARTMENT$",
            departmentDesigDateBased
              ? departmentDesigDateBased?.dojdepartment
              : ""
          )
          .replaceAll(
            "$DOJ_DESIGNATION$",
            departmentDesigDateBased
              ? departmentDesigDateBased?.dojdesignation
              : ""
          )
          .replaceAll(
            "$NOTICEPERIOD_DAYS$",
            departmentDesigDateBased
              ? departmentDesigDateBased?.noticeperiodDays
              : ""
          )
          .replaceAll(
            "$PROBATION_PERIOD$",
            departmentDesigDateBased
              ? departmentDesigDateBased?.probationPeriod
              : ""
          )
          .replaceAll(
            "$USER_NOTICE_PERIOD_APPLIED$",
            departmentDesigDateBased
              ? departmentDesigDateBased?.userNoticeperiodApplied
              : ""
          )
          .replaceAll(
            "$USER_NOTICE_PERIOD_REQUEST$",
            departmentDesigDateBased
              ? departmentDesigDateBased?.userNoticeperiodRequest
              : ""
          )
          .replaceAll(
            "$USER_NOTICE_PERIOD_ENDDATE$",
            departmentDesigDateBased
              ? departmentDesigDateBased?.userNoticeperiodEnd
              : ""
          )
          .replaceAll(
            "$USER_NOTICE_PERIOD_STATUS$",
            departmentDesigDateBased
              ? departmentDesigDateBased?.noticePeriodStatus
              : ""
          )
          .replaceAll(
            "$GROSS$",
            salaryKeyValues?.gross ? salaryKeyValues?.gross : "0.00"
          )
          .replaceAll(
            "$BASIC$",
            salaryKeyValues?.basic ? salaryKeyValues?.basic : "0.00"
          )
          .replaceAll(
            "$CONVEYANCE$",
            salaryKeyValues?.conveyance ? salaryKeyValues?.conveyance : "0.00"
          )
          .replaceAll(
            "$MA$",
            salaryKeyValues?.medicalallowance
              ? salaryKeyValues?.medicalallowance
              : "0.00"
          )
          .replaceAll(
            "$PRODALLOWANCE1$",
            salaryKeyValues?.productionallowance
              ? salaryKeyValues?.productionallowance
              : "0.00"
          )
          .replaceAll(
            "$PRODALLOWANCE2$",
            salaryKeyValues?.productionallowancetwo
              ? salaryKeyValues?.productionallowancetwo
              : "0.00"
          )
          .replaceAll(
            "$OTHERALLOW$",
            salaryKeyValues?.otherallowance
              ? salaryKeyValues?.otherallowance
              : "0.00"
          )
          .replaceAll(
            "$HRA$",
            salaryKeyValues?.hra ? salaryKeyValues?.hra : "0.00"
          )
          .replaceAll(
            "$ANNUALGROSSCTC$",
            salaryKeyValues?.annualgrossctc
              ? salaryKeyValues?.annualgrossctc
              : "0.00"
          )
          ...
          .replaceAll(
            "$SALUTATION$",
            employee?.prefix ? employee?.prefix : "Mr/Ms"
          )
          ...
          .replaceAll(
            "$P:NUMBER$",
            employee?.contactpersonal ? employee?.contactpersonal : ""
          )
          ...
          .replaceAll(
            "$DOJSALARYCOMPONENT$",
            SalaryNeed && selectedModeType === "DOJ" ? htmlImg : ""
          )
          .replaceAll(
            "$MONTHYEARSALARYCOMPONENT$",
            SalaryNeed && selectedModeType !== "DOJ" ? htmlImg : ""
          )
          .replaceAll("$C:TIME$", new Date(NewDatetime).toLocaleTimeString())
          .replaceAll(
          ...
          .replaceAll(
            "$WORKSTATION:NAME$",
            employee?.workstation ? employee?.workstation : ""
          )
          .replaceAll(
            "$WORKSTATION:COUNT$",
            employee?.workstation ? employee?.workstation?.length : ""
          )
          .replaceAll(
            "$SYSTEM:COUNT$",
            employee?.employeecount ? employee?.employeecount : ""
          )
          .replaceAll("$UNIID$", newval ? newval : "")
          .replaceAll(
            "$ATTENDANCEDATE$",
            documentPrepartion?.attendancesort === "Date" && AttendanceNeed
              ? attendanceDetails
              : ""
          )
          .replaceAll(
            "$ATTENDANCEMONTH$",
            documentPrepartion?.attendancesort === "Month" && AttendanceNeed
              ? attendanceDetails
              : ""
          )
          .replaceAll(
            "$PRODUCTIONDATEPOINT$",
            ProductionNeed && ProductionDetails ? ProductionDetails?.point : ""
          )
          .replaceAll(
            "$PRODUCTIONDATETARGET$",
            ProductionNeed && ProductionDetails ? ProductionDetails?.target : ""
          )
          .replaceAll(
            "$PRODUCTIONMONTHTARGET$",
            productionMonthStatus &&
              documentPrepartion?.productionsort === "Month"
              ? productionMonthStatus?.target
              : ""
          )
          .replaceAll(
    ...
    `
        ...
        answer.push({
          ...
          branch: employee?.branch
            ...
            : String(documentPrepartion.branch),
          unit: employee?.unit
            ? String(employee?.unit)
            : String(documentPrepartion.unit),
          team: employee?.team
            ? String(employee?.team)
            : String(documentPrepartion.team),
          ...
          topcontent: signatureContent?.topcontent
            ? signatureContent?.topcontent
            : "",
          bottomcontent: signatureContent?.bottomcontent
            ? signatureContent?.bottomcontent
            : "",
  ...
  const handlePrintDocumentManual = () => {
    if (
      ...
      setPopupContentMalert(
        "Fill All the Fields Which starts From $ and Ends with $"
      );
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (generateData) {
  ...
  const downloadPdfTesdt = (index) => {
    if (
      !noticePeriodEmpCheck &&
      noticePeriodEmpCheckPerson &&
      documentPrepartion.employeemode !== "Notice Period"
    ) {
      setPopupContentMalert(
        "This Employee is not eligibile to receieve any kind of documents!"
      );
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else {
      ...
      pdfElement.innerHTML = checkingArray[index]?.data.replaceAll(
        "--- Page Break ---",
      ...
      const addPageNumbersAndHeadersFooters = (
        ...
        for (let i = 1; i <= totalPages; i++) {
          ...
          if (
            ...
            // Seal on left
            if (
              checkingArray[index]?.signature ||
              checkingArray[index]?.seal ||
              ...
              if (i === totalPages) {
                ...
                yPos = checkingArray[index]?.qrcodeNeed
              ...
              if (checkingArray[index]?.signature) {
                if (
                  checkingArray[index]?.signaturetype === "For Seal" &&
                  checkingArray[index]?.topcontent
                ) {
                  doc.setFontSize(8);
          ...
          if (checkingArray[index]?.pagenumberneed === "All Pages") {
            ...
            checkingArray[index]?.pagenumberneed === "End Page" &&
            i === totalPages
          ) {
            const textY = footerY - 3;
          ...
          if (
            checkingArray[index]?.qrcodeNeed &&
            checkingArray[index]?.qrcodevalue === "End Page"
          ) {
          ...
          if (
            checkingArray[index]?.qrcodeNeed &&
            checkingArray[index]?.qrcodevalue === "All Pages"
          ) {
            // Add QR code and statement only on the last page
            if (i === totalPages) {
              // Add QR code in the left corner
        ...
        html2pdf()
          ...
          .then((pdf) => {
            ...
            img.onload = (e) => {
              ...
              qrImg.src = checkingArray[index]?.qrcodeNeed
      ...
      const addPageNumbersAndHeadersFooters = (
        ...
        for (let i = 1; i <= totalPages; i++) {
          ...
          if (documentPrepartion?.pagenumberneed === "All Pages") {
            ...
            documentPrepartion?.pagenumberneed === "End Page" &&
            i === totalPages
          ) {
            const textY = footerY - 3;
            doc.text(`End of the document`, pageWidth / 2, textY, {
          ...
          if (
            ...
            if (
              ...
              if (i === totalPages) {
                // ✅ Use available space from bottom instead of rect.height
                yPos = documentPrepartion?.qrcodeNeed
                  ? pageHeight - footerGap - userSigHeight - 30
                  : pageHeight - footerGap - userSigHeight; // stays near bottom
              } else {
                yPos = contentEndY + rowYOffset;
          ...
          img.onload = () => {
            ...
            if (qrCodeNeed) {
              qrImg.onload = () => {
                ...
                // Save the PDF
                pdf.save(
                  `${documentPrepartion.template}_${documentPrepartion.person}.pdf`
                );
                setLoadingPrintManualData(false);
                setButtonLoading(false);
                handleCloseInfoImagePrint();
  ...
  const handlePreviewDocument = (index) => {
    ...
    margin: 20px 0;
            ...
            const pageHeightMM =
              pdfDimensions[1] - (adjustedMargin[0] + adjustedMargin[2]);
            const totalPagesNeeded = Math.ceil(contentHeightMM / pageHeightMM);
            const lastPageUsedHeight =
              contentHeightMM % pageHeightMM || pageHeightMM;
    ...
    margin: 20px 0;
            ...
            const addPageNumbersAndHeadersFooters = (
              ...
              for (let i = 1; i <= totalPages; i++) {
                ...
                if (
                  qrCodeNeed &&
                  documentPrepartion?.qrcodevalue === "End Page"
                ) {
                  if (i === totalPages) {
                    const qrCodeWidth = 25;
                    const qrCodeHeight = 25;
                ...
                if (
                  qrCodeNeed &&
                  documentPrepartion?.qrcodevalue === "All Pages"
                ) {
                  if (i === totalPages) {
                    const qrCodeWidth = 25;
                    const qrCodeHeight = 25;
      ...
      await Promise.all(
        selectedRows?.map(async (item) => {
          ...
          let response = await axios.get(
            `${SERVICE.SINGLE_DOCUMENTPREPARATION}/${item}`,
            {
              headers: {
                Authorization: `Bearer ${auth.APIToken}`,
              },
    ...
    try {
      let brandCreate = await axios.post(SERVICE.CREATE_DOCUMENT_PREPARATION, {
        ...
        topcontent: signatureContent?.topcontent
          ...
          : "",
        bottomcontent: signatureContent.bottomcontent
          ? String(signatureContent.bottomcontent)
          : "",
        usersignature: signatureContent.usersignature
          ? String(signatureContent.usersignature)
          : "",
        qrcodevalue: String(documentPrepartion.qrcodevalue),
        ...
        person:
          documentPrepartion.person === "Please Select Person"
            ? ""
            : documentPrepartion.person,
        proption: String(documentPrepartion.proption),
        employeedoj: "",
        ...
        pagesize: pageSizePdf,
        head:
          documentPrepartion?.documentneed === "Employee Approval" ? head : "",
        foot:
          documentPrepartion?.documentneed === "Employee Approval" ? foot : "",
        qrCodeNeed: qrCodeNeed,
        sign: documentPrepartion.signature,
        sealing: documentPrepartion.seal,
        termsAndConditons: templateCreationValue?.termsAndConditons,
  ...
  let userRoles = isUserRoleAccess?.role?.map((data) =>
    data?.toUpperCase().replace(/[^A-Z0-9]/g, "")
  );

  //submit option for saving
  const handleSubmit = (e) => {
    e.preventDefault();
    const [first, second, third] = moment(new Date(serverTime))
      .format("DD-MM-YYYY hh:mm a")
      ?.split(" ");
    ...
    if (selectedBranch?.length === 0) {
      ...
      documentPrepartion.template === "" ||
      documentPrepartion.template === "Please Select Template Name"
    ) {
      setPopupContentMalert("Please Select Template Name!");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      documentPrepartion.employeemode === "" ||
      documentPrepartion.employeemode === "Please Select Employee Mode"
    ) {
      ...
      employeeMode !== "Manual" &&
      allBranchValue === false &&
      (documentPrepartion.department === "" ||
      ...
      employeeMode !== "Manual" &&
      DocumentNeed &&
      selectedEmployeeValues?.length < 1
    ) {
      setPopupContentMalert("Please Select Person!");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
      ...
      AttendanceNeed &&
      documentPrepartion.attendancesort === "Please Select Attendance Sort"
    ) {
      ...
      ProductionNeed &&
      documentPrepartion.productionsort === "Please Select Production Sort"
    ) {
      ...
      AttendanceNeed &&
      documentPrepartion.attendancesort === "Date" &&
      documentPrepartion?.attendancedate === ""
    ) {
      ...
      documentPrepartion?.attendancemonth === "Please Select Attendance Month"
    ) {
      ...
      AttendanceNeed &&
      documentPrepartion.attendancesort === "Month" &&
      documentPrepartion?.attendanceyear === "Please Select Attendance Year"
    ) {
      ...
      ProductionNeed &&
      documentPrepartion.productionsort === "Month" &&
      documentPrepartion?.productionmonth === "Please Select Production Month"
    ) {
      ...
      ProductionNeed &&
      documentPrepartion.productionsort === "Month" &&
      documentPrepartion?.productionyear === "Please Select Production Year"
    ) {
      ...
      documentPrepartion.issuingauthority === "" ||
      documentPrepartion.issuingauthority === "Please Select Issuing Authority"
    ) {
      ...
      (documentPrepartion.signature === "" ||
        documentPrepartion.signature === "Please Select Signature")
    ) {
      ...
      signatureContent?.seal !== "None" &&
      (documentPrepartion.seal === "" ||
        documentPrepartion.seal === "Please Select Seal")
    ) {
      ...
      setPopupContentMalert(
        "Document with Person Name and Template already exists!"
      );
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      ...
      setPopupContentMalert(
        "Document with Person Name and Template already exists in Todo!"
      );
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      documentPrepartion?.documentneed === "Employee Approval" &&
      documentPrepartion?.printoptions === "Please Select Print Options"
    ) {
      setPopupContentMalert("Please Select Print Options!");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
  ...
  const generatePDFs = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < totalFiles; i++) {
        ...
        setCurrentFile(
          `${checkingArray[i]?.documentname} - ${checkingArray[i]?.empname}`
        );
  ...
  const processInBatches = async (array, batchSize) => {
    ...
    for (let i = 0; i < array.length; i += batchSize) {
      ...
      const batchResults = await Promise.all(
        batch.map((data, index) => downloadPdfTesdtCheckTrue(i + index))
      );
  ...
  const getViewFile = async (id) => {
    let response = await axios.get(
      `${SERVICE.SINGLE_DOCUMENTPREPARATION}/${id}`,
      {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
            ...
            img.onload = () => {
              ...
              qrImg.onload = () => {
                ...
                reader.onloadend = async () => {
                  setLoadingMessage(
                    "Document is converting to Email format..."
                  );
  ...
  const getCode = async (e, pagename) => {
    ...
    try {
      ...
      if (res?.data?.templatecontrolpanel) {
        ...
        setDataTableId(e?.id);
        const qrInfoDetails =
          headerFooterBase64?.qrInfo?.length > 0 ? ans?.qrInfo : [];
        setQrCodeInfoDetails(
          qrInfoDetails?.map(
            (data, index) =>
              `${index + 1}. ${data?.details
  ...
  const downloadPdfTesdtTable = async (e) => {
    if (headerOptions === "Please Select Print Options") {
      ...
      await getUpdatePrintingStatus(
        response.data.sdocumentPreparation?._id,
        response.data.sdocumentPreparation?.updatedby
      );
      const pdfElement = document.createElement("div");
      pdfElement.innerHTML =
  ...
  const rowDataTable = filteredData.map((item, index) => {
    return {
      ...
      department:
        item.department === "Please Select Department" ? "" : item.department,
  ...
  return (
    <Box>
      ...
      <PageHeading
        ...
        subpagename="Employee Document Preparation"
        subsubpagename=""
      />
      {/* ****** Header Content ****** */}
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      value={{
                        label: documentPrepartion.template,
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      value={{
                        label: documentPrepartion.employeemode,
                        value: documentPrepartion.employeemode,
                      }}
                      onChange={(e) => {
                        setDocumentPrepartion({
                          ...documentPrepartion,
                        ...
                        <FormControlLabel
                          control={
                            <Checkbox
                              ...
                              disabled={DocumentNeed}
                              checked={AttendanceNeed}
                              onChange={() => setAttendanceNeed((val) => !val)}
                              color="primary"
                            />
                          }
                          // sx={{marginTop: 1}}
                        ...
                        <FormControlLabel
                          control={
                            <Checkbox
                              ...
                              disabled={DocumentNeed}
                              checked={ProductionNeed}
                              onChange={() => setProductionNeed((val) => !val)}
                              color="primary"
                            />
                          }
                          // sx={{marginTop: 1}}
                        ...
                        <FormControlLabel
                          control={
                            <Checkbox
                              ...
                              disabled={DocumentNeed}
                              checked={SalaryNeed}
                              onChange={() => setSalaryNeed((val) => !val)}
                              color="primary"
                            />
                          }
                          // sx={{marginTop: 1}}
                    ...
                    <Grid item md={2} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <Selects
                          ...
                          value={{
                            label: documentPrepartion.department,
                            value: documentPrepartion.department,
                          }}
                          onChange={(e) => {
                            setDocumentPrepartion({
                              ...documentPrepartion,
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <Selects
                          ...
                          value={{
                            label: documentPrepartion?.attendancesort,
                            value: documentPrepartion?.attendancesort,
                          }}
                          onChange={(e) => {
                            setDocumentPrepartion({
                              ...documentPrepartion,
                          ...
                          <Box>
                            <FormControl fullWidth size="small">
                              ...
                              <Selects
                                ...
                                options={months}
                                value={{
                                  label: documentPrepartion?.attendancemonth,
                                  value: documentPrepartion?.attendancemonth,
                                }}
                                onChange={handleMonthChangeAttendance}
                              />
                            </FormControl>
                          </Box>
                        </Grid>
                        <Grid item md={3} sm={12} xs={12}>
                          <FormControl fullWidth size="small">
                            ...
                            <Selects
                              maxMenuHeight={200}
                              styles={colourStyles}
                              options={availableYearsAttendance}
                              value={{
                                label: documentPrepartion?.attendanceyear,
                                value: documentPrepartion?.attendanceyear,
                              }}
                              onChange={handleYearChangeAttendance}
                            />
                          </FormControl>
                        </Grid>
                      </>
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <Selects
                          ...
                          value={{
                            label: documentPrepartion?.productionsort,
                            value: documentPrepartion?.productionsort,
                          }}
                          onChange={(e) => {
                            setDocumentPrepartion({
                              ...documentPrepartion,
                            ...
                            <Box>
                              <FormControl fullWidth size="small">
                                ...
                                <Selects
                                  ...
                                  options={months}
                                  value={{
                                    label: documentPrepartion?.productionmonth,
                                    value: documentPrepartion?.productionmonth,
                                  }}
                          ...
                          <Grid item md={3} sm={12} xs={12}>
                            <FormControl fullWidth size="small">
                              ...
                              <Selects
                                maxMenuHeight={200}
                                styles={colourStyles}
                                options={availableYearsProduction}
                                value={{
                                  label: documentPrepartion?.productionyear,
                                  value: documentPrepartion?.productionyear,
                                }}
                      ...
                      <>
                        <Grid item md={1.5} xs={12} sm={6}>
                          <FormControl fullWidth size="small">
                            ...
                            <Selects
                              options={availableYearsSalary}
                              value={{
                                label: selectedYear,
                                value: selectedYear,
                              }}
                              onChange={handleYearChangeSalary}
                            />
                          </FormControl>
                        </Grid>
                        <Grid item md={1.5} xs={12} sm={6}>
                          <FormControl fullWidth size="small">
                            ...
                            <Selects
                              options={
                                selectedYear === "Select Year" ? [] : months
                              }
                              value={{
                                label: selectmonthname,
                                value: selectmonthname,
                              }}
                              onChange={handleMonthChangeSalary}
                            />
                          </FormControl>
                        </Grid>
                      </>
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <Selects
                          ...
                          value={{
                            label: documentPrepartion.signature,
                            value: documentPrepartion.signature,
                          }}
                          onChange={(e) => {
                            setDocumentPrepartion({
                              ...documentPrepartion,
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <Selects
                          ...
                          value={{
                            label: documentPrepartion.seal,
                            value: documentPrepartion.seal,
                          }}
                          onChange={(e) => {
                            setDocumentPrepartion({
                              ...documentPrepartion,
                            ...
                            fetchSignatureSealPlacement(e?.document, "Seal");
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      value={{
                        label: documentPrepartion.signatureneed,
                        value: documentPrepartion.signatureneed,
                      }}
                      onChange={(e) => {
                        setDocumentPrepartion({
                          ...documentPrepartion,
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      value={{
                        label: documentPrepartion.pagenumberneed,
                        value: documentPrepartion.pagenumberneed,
                      }}
                      onChange={(e) => {
                        setDocumentPrepartion({
                          ...documentPrepartion,
                  ...
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <Selects
                        ...
                        value={{
                          label: documentPrepartion.qrcodevalue,
                          value: documentPrepartion.qrcodevalue,
                        }}
                        onChange={(e) => {
                          setDocumentPrepartion({
                            ...documentPrepartion,
        ...
        <>
          <Box sx={userStyle.container}>
            ...
            <Button
              variant="contained"
              color="error"
              onClick={handleClickOpenBulkalert}
            >
              Bulk Print
            </Button>
            <br />
        ...
        >
          <DialogContent sx={{ textAlign: "center", alignItems: "center" }}>
            <Typography variant="h6">
              Once Check the Document by clicking Preview button while
              Saving/Printing the Document whether it's perfectly alligned
            </Typography>
          </DialogContent>
          <DialogActions>
      ...
      <Box>
        ...
        >
          ...
          <DialogActions>
            <Button
              onClick={handleCloseInfoImageManual}
              sx={buttonStyles.btncancel}
            >
              Cancel
            </Button>
      ...
      <Box>
        ...
        >
          ...
          <DialogActions>
            <Button
              onClick={handleCloseInfoImagePrint}
              sx={buttonStyles.btncancel}
            >
              Cancel
            </Button>
      ...
      <Box>
        ...
        >
          ...
          <DialogActions>
            <Button
              onClick={handleCloseInfoImagePrintManual}
              sx={buttonStyles.btncancel}
            >
              Cancel
            </Button>
      ...
      <Box>
        ...
        >
          ...
          <DialogActions>
            <Button
              onClick={handleCloseManualCheck}
              sx={buttonStyles.btncancel}
            >
              Cancel
            </Button>
            ...
            <Grid container spacing={2}>
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  <Typography variant="h6">Date</Typography>
                  <Typography>
                    {moment(documentPreparationEdit.date).format("DD-MM-YYYY")}
                  </Typography>
                </FormControl>
              </Grid>
              <Grid item md={4} xs={12} sm={12}>
              ...
              {documentPreparationEdit?.sealing !== "Document" &&
                documentPreparationEdit?.sealing !== "" &&
                documentPreparationEdit?.sealing !== "Please Select Seal" && (
                  <Grid item md={4} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
              ...
              {documentPreparationEdit.sign !== "Document" &&
                documentPreparationEdit.sign !== "" &&
                documentPreparationEdit.sign !== "Please Select Signature" && (
                  <Grid item md={4} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
              ...
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>
                    {documentPreparationEdit.documentneed}
                  </Typography>
                </FormControl>
              </Grid>
      ...
      >
        ...
        <DialogActions>
          <Button
            ...
            autoFocus
            variant="contained"
            onClick={handleCloseBulkModalert}
          >
            {" "}
            OK{" "}
          </Button>
      ...
      <Box>
        <Dialog
          open={isDeleteOpenBulkcheckbox}
          onClose={handleCloseBulkModcheckbox}
          aria-labelledby="alert-dialog-title"
          aria-describedby="alert-dialog-description"
        >
          <DialogContent
          ...
          <DialogActions>
            <Button
              onClick={handleCloseBulkModcheckbox}
              sx={buttonStyles.btncancel}
            >
              Cancel
            </Button>
      ...
      >
        ...
        >
          ...
          <Table>
            <TableHead
              sx={{ background: "#f5f5f6", border: "1px solid #c3c3c982" }}
            >
              <TableCell
                sx={{
                  ...tableStyles,
            ...
            <TableBody sx={{ border: "1px solid #dbdbdf", fontSize: "16px" }}>
              <TableRow>
                <StyledTableCell sx={tableStyles}>
                  {"Basic Salary"}
                </StyledTableCell>
                <StyledTableCell sx={tableStylesVales}>
                  {" "}
                  ₹ {viewData.basic || "0.00"}
                </StyledTableCell>
              </TableRow>
              ...
              <TableRow>
                ...
                <StyledTableCell sx={tableStylesVales}>
                  {" "}
                  ₹ {viewData.medicalallowance || "0.00"}
                </StyledTableCell>
              </TableRow>
              <TableRow>
                <StyledTableCell sx={tableStyles}>
                ...
                <StyledTableCell sx={tableStylesVales}>
                  {" "}
                  ₹ {viewData.productionallowance || "0.00"}
                </StyledTableCell>
              </TableRow>
              <TableRow>
                <StyledTableCell sx={tableStyles}>
      ...
      >
        <DialogTitle style={progressDialogStyles.dialogTitle}>
          📄 Checking Documents for Page Mode...
        </DialogTitle>
        <DialogContent>
          <p style={progressDialogStyles.checkingText}>
            Checking:{" "}
            <span style={progressDialogStyles.highlightText}>
              {currentFile}
            </span>
          </p>
          ...
          <p style={progressDialogStyles.percentageText}>
      ...
      <Loader
        loading={loadingPreviewManualData}
      ...
      <Loader
        loading={loadingGeneratingDatas}
        message={loadingGeneratingMessages}
      />
