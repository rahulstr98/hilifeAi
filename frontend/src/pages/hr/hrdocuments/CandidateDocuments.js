...
function CandidateDocuments() {
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String(
        "Human Resource/HR Documents/Candidate Document Preparation"
      ),
  ...
  const TemplateDropdowns = async (branch, company) => {
    ...
    try {
      ...
      const candidateDocuments =
        res.data.templatecreation?.length > 0
          ? res?.data?.templatecreation
          : [];
      setTemplateValues(
  ...
  const fetchDesignation = async (company, branch) => {
    try {
      ...
      const designations =
        res?.data?.designation?.length > 0 ? res?.data?.designation : [];
  ...
  const fetchRounds = async (designation) => {
    try {
      ...
      const roundOrder = res?.data?.rounds?.round
        ? res.data.rounds.round
        : // ?.slice(0, -1)
          [];
      setRoundNames(
        roundOrder?.map((data) => ({
  ...
  const fetchEmployeeNames = async (rounds) => {
    ...
    try {
      ...
      const employeeNames = res?.data?.employeenames
        ? res?.data?.employeenames
        : [];
      setEmployeeNameOptions(
        employeeNames?.map((data) => ({
          ...data,
          label: data.fullname,
          value: data.fullname,
        }))
      ...
      for (const batch of batches) {
        const batchRequests = batch.map(async (data) => {
          await axios.post(SERVICE.CREATE_CANDIDATE_DOCUMENT_PREPARATION, {
            ...
            head:
              data.documentneed === "Candidate Approval" ? data?.header : "",
            foot:
              data.documentneed === "Candidate Approval" ? data?.footer : "",
  ...
  )}/${isUserRoleAccess ? isUserRoleAccess?._id : ""}/${encryptString(
  ...
  const fetchAllRaisedTickets = async () => {
    setPageName(!pageName);
    try {
      let res_queue = await axios.get(
        SERVICE.CANDIDATE_DOCUMENT_PREPARATION_AUTOID,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      await Promise.all(
        selectedRows?.map(async (item) => {
          ...
          let response = await axios.get(
            `${SERVICE.SINGLE_CANDIDATE_DOCUMENTPREPARATION}/${item}`,
            {
              headers: {
                Authorization: `Bearer ${auth.APIToken}`,
              },
          ...
          await getUpdatePrintingStatus(
            item,
            response?.data?.scandidateDocumentPreparation?.updatedby
          );

          setLoadingGeneratingMessage("Printing the set the Documents..!");
          const pdfElement = document.createElement("div");
          pdfElement.innerHTML =
            response?.data?.scandidateDocumentPreparation?.document.replaceAll(
              "--- Page Break ---",
    ...
    background-color: #f9f9f9;
          ...
          const addPageNumbersAndHeadersFooters = (
            ...
            for (let i = 1; i <= totalPages; i++) {
              ...
              if (
                response?.data?.scandidateDocumentPreparation
                  ?.pagenumberneed === "All Pages"
              ) {
          ...
          const adjustedMargin = getAdjustedMargin(
            response.data.scandidateDocumentPreparation?.marginQuill,
            hasHeaderImage,
            hasFooterImage
          );
          const pdfDimensions = getPageDimensionsTable(
            response.data.scandidateDocumentPreparation?.pagesizeQuill,
            response.data.scandidateDocumentPreparation?.orientationQuill
          ); // as before
          ...
          html2pdf()
            ...
            .then((pdf) => {
              ...
              const img = new Image();
              img.src =
                response?.data?.scandidateDocumentPreparation?.watermark;
              img.onload = () => {
                ...
                qrImg.src =
                  response?.data?.scandidateDocumentPreparation?.qrcode; // QR code image URL
                qrImg.onload = () => {
                  ...
                  // Save the PDF
                  pdf.save(
                    `${response?.data?.scandidateDocumentPreparation?.template}_${response?.data?.scandidateDocumentPreparation?.person}.pdf`
                  );
...
.__se__t-shadow {
              ...
              for (let i = 1; i <= totalPages; i++) {
                ...
                const footerY =
                  ...
                  (checkingArray[index]?.footer === ""
                    ? 15
                    : footerImgHeight - 3) -
                  (checkingArray[index]?.footer ? 6 : -6);
  ...
  const downloadPdfTesdtTable = async (e) => {
    ...
    if (headerOptions === "Please Select Print Options") {
      ...
      // Create a new div element to hold the Quill content
      let response = await axios.get(
        `${SERVICE.SINGLE_CANDIDATE_DOCUMENTPREPARATION}/${e}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      await getUpdatePrintingStatus(
        response.data.scandidateDocumentPreparation?._id,
        response.data.scandidateDocumentPreparation?.updatedby
      );
      const pdfElement = document.createElement("div");
      pdfElement.innerHTML =
    ...
    background-color: #f9f9f9;
      ...
      const addPageNumbersAndHeadersFooters = (
        ...
        for (let i = 1; i <= totalPages; i++) {
          ...
          if (
            response?.data?.scandidateDocumentPreparation?.pagenumberneed ===
            "All Pages"
          ) {
            const textY = footerY - 3;
            doc.text(`Page ${i} of ${totalPages}`, pageWidth / 2, textY, {
...
.__se__t-shadow {
    ...
    background-color: #f9f9f9;
      ...
      const addPageNumbersAndHeadersFooters = (
        ...
        for (let i = 1; i <= totalPages; i++) {
          ...
          const footerY =
            pageHeight * 1 -
            (checkingArray[index]?.footer === "" ? 15 : footerImgHeight - 3) -
            (checkingArray[index]?.footer ? 6 : -6);
  ...
  const delBrand = async () => {
    ...
    try {
      await axios.delete(
        `${SERVICE.SINGLE_CANDIDATE_DOCUMENTPREPARATION}/${brandid}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const handleSubmit = (e) => {
    ...
    const isNameMatchInside = checkingArray?.some((item) =>
      selectedEmployeeValues?.includes(item.empname)
    );
    if (
      documentPrepartion.company === "" ||
      documentPrepartion.template === "Please Select Company"
    ) {
      ...
      documentPrepartion.designation === "" ||
      documentPrepartion.designation === "Please Select Designation"
    ) {
  ...
  const answerDefine = async (personDetails, index) => {
    ...
    const person = selectedEmployee?.find(
      (data) => data?.value === personDetails
    );
    console.log(person, "person");
    if (!person || !person?.salarytable?.length) {
      setPopupContentMalert(
        "No person selected or salary details are missing."
      );
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
      return;
    ...
    const personSalary = person
      ? person?.salarytable[person?.salarytable?.length - 1]
      : "";
    const ImageComponent = await renderFilePreviewMulterUploaded(
      personSalary?.filename
    );
    const formattedDOB = person?.dateofbirth
    ...
    try {
      const [
        ...
        axios.post(
          SERVICE.CANDIDATE_TEMPLATE_CREATION_FILTER,
          {
      ...
      let caddress = `<br>${!person?.pdoorno ? "" : person?.pdoorno + ", "}${
        !person?.pstreet ? "" : person?.pstreet + ", "
      }${!person?.parea ? "" : person?.parea + ", "}
            <br>${!person?.plandmark ? "" : person?.plandmark + ", "}${
        !person?.ptaluk ? "" : person?.ptaluk + ", "
      }${!person?.ppost ? "" : person?.ppost + ", "}
            <br>${!person?.pcity ? "" : person?.pcity + ", "}${
        !person?.pstate ? "" : person?.pstate + ", "
      }${!person?.pcountry ? "" : person?.pcountry + ", "}
            ${!person?.ppincode ? "" : "- " + person?.ppincode}`;
      console.log(
        companyTitleName,
        branchAddressTextHorizontal,
        branchAddressTextVertical
      );
      let findMethod = texted
        ...
        .replaceAll(
          "$C:AADHAR$",
          person?.adharnumber ? person?.adharnumber : ""
        )
        ...
        .replaceAll(
          "$GROSS$",
          personSalary?.grossmonthsalary ? personSalary?.grossmonthsalary : "0"
        )
        .replaceAll(
          "$ANNUALGROSSCTC$",
          personSalary?.annualgrossctc ? personSalary?.annualgrossctc : "0"
        )
        .replaceAll("$BASIC$", personSalary?.basic ? personSalary?.basic : "0")
        .replaceAll(
          "$CONVEYANCE$",
          personSalary?.conveyance ? personSalary?.conveyance : "0"
        )
        .replaceAll("$HRA$", personSalary?.hra ? personSalary?.hra : "0")
        .replaceAll(
          "$MA$",
          personSalary?.medicalallowance ? personSalary?.medicalallowance : "0"
        )
        .replaceAll(
          "$PRODALLOWANCE1$",
          personSalary?.productionallowance
            ? personSalary?.productionallowance
            : "0"
        )
        .replaceAll(
          "$PRODALLOWANCE2$",
          personSalary?.productionallowance2
            ? personSalary?.productionallowance2
            : "0"
        )
        .replaceAll(
          "$OTHERALLOW$",
          personSalary?.otherallowance ? personSalary?.otherallowance : "0"
        )
        ...
        .replaceAll(
          "$SALARYCOMPONENT$",
          personSalary?.filename ? ImageComponent : ""
        )
        ...
        .replaceAll(
          ...
          signatureContent?.seal === "For Seal"
            ? `
                      <h4 style="color:#53177e;">${
                        signatureContent?.topcontent
                      }</h4><br/>
              ${
                signature
                  ? `<img src="${signature}" alt="Signature" style="postion:absolute; z-index:-1; width: 200px; height: 30px;"" /> <br/>`
                  : ""
              }
                  <h4 style="color:#53177e;">${
          ...
          userESignature
            ? `
                  <span style="display: inline-flex; flex-direction: column; align-items: center; text-align: center;">
                      <img src="${userESignature}" alt="Signature" style="${
                pageSizePdf === "A3"
                  ? "width: 200px !important; height: 30px !important;"
                  : "width: 130px !important; height: 25px !important;"
              }"/>
                  </span>
              `
            : ""
      ...
      answer.push({
        ...
        headertext: documentPrepartion?.header,
        footertext: templateCreationValue?.footer,
  ...
  const fetchBrandMaster = async () => {
    ...
    try {
      ...
      let res_freq = await axios.post(
        `${SERVICE.ACCESSIBLEBRANCHALL_CANDIDATE_DOCUMENTPREPARATION}`,
        {
          assignbranch: accessbranchs,
        },
        {
      ...
      const answer =
        res_freq?.data?.candidatedocumentPreparation?.length > 0
          ? res_freq?.data?.candidatedocumentPreparation?.map(
              (item, index) => ({
                ...item,
                serialNumber: index + 1,
                roundname: item.rounds
                  ?.map((t, i) => `${i + 1 + ". "}` + t)
                  .join("\n"),
  ...
  const fetchCandidateDocuments = async () => {
    ...
    try {
      ...
      let res_freq = await axios.get(
        `${SERVICE.CANDIDATE_DOCUMENT_DUPLICATE_CHECKING}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const fetchEmailForUser = async (
    ...
    if (headerOptions === "Please Select Print Options") {
      ...
      let findMethodEmail = textedEmail
        .replaceAll(
          "$TEMPLATENAME$",
          response.data.scandidateDocumentPreparation?.template
            ? response.data.scandidateDocumentPreparation?.template
            : ""
        )
        .replaceAll(
          "$REFERENCEID$",
          response.data.scandidateDocumentPreparation?.templateno
            ? response.data.scandidateDocumentPreparation?.templateno
            : ""
        )
        .replaceAll(
          "$CANDIDATENAME$",
          response.data.scandidateDocumentPreparation?.person
            ? response.data.scandidateDocumentPreparation?.person
            : ""
        )
        .replaceAll(
      ...
      return new Promise((resolve, reject) => {
        ...
        html2pdf()
          ...
          .then(async (pdf) => {
              ...
              qrImg.onload = () => {
                ...
                reader.onloadend = async () => {
                  ...
                  let res_module = await axios.post(
                    SERVICE.CANDIDATE_DOCUMENT_PREPARATION_MAIL,
                    {
                      ...
                      letter:
                        response?.data?.scandidateDocumentPreparation?.template,
                      email:
                        response?.data?.scandidateDocumentPreparation?.email,
            ...
            if (
              response?.data?.scandidateDocumentPreparation?.mail === "Send"
            ) {
              let res = await axios.put(
                `${SERVICE.SINGLE_CANDIDATE_DOCUMENTPREPARATION}/${e}`,
                {
...
const backGroundCondition = ans?.letterheadbodycontent?.find(
  ...
  const rowDataTable = filteredData.map((item, index) => {
    return {
      ...
      designation:
        item.designation === "Please Select Designation"
          ? ""
          : item.designation,
  ...
  return (
    <Box>
      ...
      <PageHeading
        ...
        mainpagename="Candidate Documents"
        subpagename="Candidate Document Preparation"
        subsubpagename=""
      />
      <>
        {isUserRoleCompare?.includes("acandidatedocumentpreparation") && (
          <Box sx={userStyle.selectcontainer}>
            <Typography>Add Candidate Document Preparation</Typography>
            <br /> <br />
            <>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      value={{
                        label: documentPrepartion.designation,
                        value: documentPrepartion.designation,
                      }}
                      onChange={(e) => {
                        setDocumentPrepartion({
                          ...documentPrepartion,
                ...
                {signatureStatus === "With" && (
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <Selects
                        maxMenuHeight={300}
                        options={companyName?.documentsignature?.map(
                          (data) => ({
                            ...data,
                        ...
                        onChange={(e) => {
                          ...
                          fetchSignatureSealPlacement(e?.document, "Signature");
          ...
          <Box sx={userStyle.container}>
            ...
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>
                List Candidate Document Preparation
              </Typography>
            </Grid>
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              >
                <Box>
                  {isUserRoleCompare?.includes(
                    "excelcandidatedocumentpreparation"
                  ) && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes(
                    "csvcandidatedocumentpreparation"
                  ) && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes(
                    "printcandidatedocumentpreparation"
                  ) && (
                    <>
                  ...
                  {isUserRoleCompare?.includes(
                    "pdfcandidatedocumentpreparation"
                  ) && (
                    // <>
                  ...
                  {isUserRoleCompare?.includes(
                    "imagecandidatedocumentpreparation"
                  ) && (
                    <Button
                      sx={userStyle.buttongrp}
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>
                    {documentPreparationEdit.rounds
                      ?.map((t, i) => `${i + 1 + ". "}` + t)
                      .join("\n")}
                  </Typography>
                </FormControl>
