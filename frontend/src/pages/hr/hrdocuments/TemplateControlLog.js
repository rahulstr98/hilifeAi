...
function TemplateControlLog() {
  ...
  const delSource = async (e) => {
    try {
      if (deleteSource) {
        await axios.delete(
          `${SERVICE.TEMPLATECONTROLPANEL_SINGLEDELETE}/${userid}/${deleteSource}`,
          {
            headers: {
  ...
  const getviewCode = async (e, logid) => {
    try {
      ...
      const lastupdatedata =
        res?.data?.stemplatecontrolpanel?.templatecontrolpanellog?.filter(
          (item) => item._id === logid
        );
      ...
      setdocumentFilesDOcumentBodyContentView(
        lastupdatedata[0]?.letterheadbodycontent
      );
      setdocumentFilesDOcumentFrontHeaderView(
        lastupdatedata[0]?.idcardfrontheader
      );
      setdocumentFilesDOcumentFrontFooterView(
        lastupdatedata[0]?.idcardfrontfooter
      );
      setdocumentFilesDOcumentBackHeaderView(
        lastupdatedata[0]?.idcardbackheader
      );
      setdocumentFilesDOcumentBackFooterView(
        lastupdatedata[0]?.idcardbackfooter
      );
  ...
  const fetchEmployee = async () => {
    try {
      let res_employee = await axios.get(
        `${SERVICE.TEMPLATECONTROLPANEL_SINGLE}/${userid}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      let data_emp =
        res_employee?.data?.stemplatecontrolpanel?.templatecontrolpanellog;
  ...
  const addSerialNumber = () => {
    const itemsWithSerialNumber = employees?.map((item, index) => ({
      ...item,
      serialNumber: index + 1,
      datetime: `${moment(item.createdAt).format("DD-MM-YYYY")} / ${moment(
        item.createdAt
      ).format("HH:mm:ss")}`,
    }));
  ...
  return (
    <Box>
      ...
      <Typography sx={userStyle.HeaderText}>
        Template Control Panel Log List
      </Typography>
      <br />
        ...
        <>
          <Box sx={userStyle.container}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={2}>
                <Link
                  to="/settings/templatecontrolpanel"
                  style={{
                    textDecoration: "none",
                    color: "white",
                    float: "right",
