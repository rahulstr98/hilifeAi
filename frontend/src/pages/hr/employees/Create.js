...
function MultistepForm() {
  ...
  const { isUserRoleAccess, isUserRoleCompare, allUsersData, isAssignBranch, buttonStyles, workStationSystemName } = useContext(UserRoleAccessContext);
  // for attendance mode
  ...
  const handleCommitUserBiometric = async (e) => {
    ...
    try {
      ...
      console.log(err, 'Error in Employee Create Biometric');
      // handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  ...
  const SalaryFixFilter = async () => {
    ...
    try {
      ...
      console.log(res?.data?.result, 'salary fix filter');
      setIsArea(false);
    } catch (err) {
      setIsArea(false);
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
  ...
  // console.log(reportingtonames,"reportingtonames")
  ...
  console.log(roleDatas, 'Role Datas');
  useEffect(() => {
    getRolesDatas();
  }, [roles]);
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      let employees_data = await axios.post(SERVICE.USER_CREATE, {
        ...
        ccity: !employee?.samesprmnt ? selectedCityc?.name || '' : selectedCityp?.name || '',
                    ...
                    <Grid item md={4} sm={12} xs={12}>
                      <FormControl size="small" fullWidth>
                        ...
                        <Selects
                          ...
                          label="Please Select Work Station Primary"
                          value={{
