...
function Assignedrole() {
  ...
  const getCode = async (e) => {
    ...
    try {
      ...
      let singleUser = res?.data?.suser;
      let findAccess = isAssignBranch?.find((data) => data?.company === singleUser?.company && data?.branch === singleUser?.branch && data?.unit === singleUser?.unit) || {};
      setEmpupdateform({
        ...singleUser,
        companycode: findAccess?.companycode || '',
        branchcode: findAccess?.branchcode || '',
        branchemail: findAccess?.branchemail || '',
        branchaddress: findAccess?.branchaddress || '',
        branchstate: findAccess?.branchstate || '',
        branchcity: findAccess?.branchcity || '',
        branchcountry: findAccess?.branchcountry || '',
        branchpincode: findAccess?.branchpincode || '',
        unitcode: findAccess?.unitcode || '',
      });
  ...
  const sendRequestt = async () => {
    ...
    try {
      ...
      let newRoles = selectedNewRoles?.filter((data) => !selectedOldRoles?.includes(data?.role));
  ...
  const fetchRoleNamesAndUrl = async (modulename, rolename) => {
    ...
    try {
      ...
      if (matchedDatas) {
        let allurls = [...matchedDatas?.modulenamesurl, ...matchedDatas?.submodulenamesurl, ...matchedDatas?.mainpagenamesurl, ...matchedDatas?.subpagenamesurl, ...matchedDatas?.subsubpagenamesurl];
        let newObj = {
          role: rolename,
  ...
  const rowDataRemove = (i, val) => {
    setSelectedRoles(selectedRoles.filter((value, item) => item !== i));
    setSelectedNewRoles(selectedNewRoles.filter((value, index) => value?.role?.trim()?.toLowerCase() !== val?.trim()?.toLowerCase()));
  };
        ...
        <Grid sx={{ display: 'flex' }}>
          {isUserRoleCompare?.includes('eassignedroleupdate') && (
            <Button
              ...
              disabled={isEditOpen}
            >
              {isEditOpen ? <CircularProgress size={20} color="inherit" /> :
