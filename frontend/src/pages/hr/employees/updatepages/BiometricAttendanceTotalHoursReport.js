...
function BiometricAttendanceTotalHoursReport() {
    ...
    const sendRequest = async () => {
        ...
        try {
            const response = await axios.post(SERVICE.BIOMETRIC_USERS_ATTENDANCE_TOTALHOURS_REPORT_CHECK, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
  ...
  const columns = [
    ...
    { title: "Break Hours (hh:mm:ss)", field: "break" },
];
const downloadPdf = () => {
    const doc = new jsPDF();
    ...
    doc.save("Biometric Attendance Total Hours Report.pdf");
};
