...
function BiometricPairedDevicesGrouping() {
  ...
  const [VisitorInSwitchFirst, setVisitorInSwitchFirst] = useState(false);
  const [VisitorInOutSwitchFirst, setVisitorInOutSwitchFirst] = useState(false);
  const [VisitorOutSwitchFirst, setVisitorOutSwitchFirst] = useState(false);
  const [VisitorInSwitchFirstEdit, setVisitorInSwitchFirstEdit] =
    useState(false);
  const [VisitorInOutSwitchFirstEdit, setVisitorInOutSwitchFirstEdit] =
    useState(false);
  const [VisitorOutSwitchFirstEdit, setVisitorOutSwitchFirstEdit] =
    useState(false);
  ...
  const [AttendanceInOutSwitchSecond, setAttendanceInOutSwitchSecond] =
    useState(false);
  const [VisitorInSwitchSecond, setVisitorInSwitchSecond] = useState(false);
  const [VisitorInOutSwitchSecond, setVisitorInOutSwitchSecond] =
    useState(false);
  const [VisitorOutSwitchSecondEdit, setVisitorOutSwitchSecondEdit] =
    useState(false);
  const [VisitorInSwitchSecondEdit, setVisitorInSwitchSecondEdit] =
    useState(false);
  const [VisitorInOutSwitchSecondEdit, setVisitorInOutSwitchSecondEdit] =
    useState(false);
  const [VisitorOutSwitchSecond, setVisitorOutSwitchSecond] = useState(false);
  ...
  const isDisabledVisitor = (switchName) => {
    // If no switch is ON, nothing is disabled
    if (
    ...
    // FIRST group rules
    if (VisitorInSwitchFirst) {
      return !["VisitorInSwitchFirst", "VisitorOutSwitchSecond"].includes(
        switchName
      );
    }
    if (VisitorOutSwitchFirst) {
      return !["VisitorOutSwitchFirst", "VisitorInSwitchSecond"].includes(
        switchName
      );
    }
    if (VisitorInOutSwitchFirst) {
      return switchName !== "VisitorInOutSwitchFirst";
    }

    // SECOND group rules
    if (VisitorInSwitchSecond) {
      return !["VisitorInSwitchSecond", "VisitorOutSwitchFirst"].includes(
        switchName
      );
    }
    if (VisitorOutSwitchSecond) {
      return !["VisitorOutSwitchSecond", "VisitorInSwitchFirst"].includes(
        switchName
      );
    }
    if (VisitorInOutSwitchSecond) {
      return switchName !== "VisitorInOutSwitchSecond";
    }

    return false;
  };
  const isDisabledVisitorEdit = (switchName) => {
    // If no switch is ON, nothing is disabled
    if (
    ...
    // FIRST group rules
    if (VisitorInSwitchFirstEdit) {
      return ![
        "VisitorInSwitchFirstEdit",
        "VisitorOutSwitchSecondEdit",
      ].includes(switchName);
    }
    if (VisitorOutSwitchFirstEdit) {
      return ![
        "VisitorOutSwitchFirstEdit",
        "VisitorInSwitchSecondEdit",
      ].includes(switchName);
    }
    if (VisitorInOutSwitchFirstEdit) {
      return switchName !== "VisitorInOutSwitchFirstEdit";
    }

    // SECOND group rules
    if (VisitorInSwitchSecondEdit) {
      return ![
        "VisitorInSwitchSecondEdit",
        "VisitorOutSwitchFirstEdit",
      ].includes(switchName);
    }
    if (VisitorOutSwitchSecondEdit) {
      return ![
        "VisitorOutSwitchSecondEdit",
        "VisitorInSwitchFirstEdit",
      ].includes(switchName);
    }
    if (VisitorInOutSwitchSecondEdit) {
      return switchName !== "VisitorInOutSwitchSecondEdit";
    }
  ...
  const fetchUserGroupingArray = async () => {
    ...
    try {
      const response = await axios.get(
        SERVICE.ALL_BIOMETRIC_PAIRED_DEVICE_GROUPING,
        {
          headers: {
      ...
      const answer =
        response?.data?.biometricpaireddevicesgrouping?.length > 0
          ? response?.data?.biometricpaireddevicesgrouping?.map(
              (item, index) => ({
                id: item._id,
  ...
  const [selectedPairedDeviceOptions, setSelectedPairedDeviceOptions] =
    useState([]);
  ...
  const customValueRendererPairedDevice = (
    valuePairedDevices,
    _categoryname
  ) => {
  ...
  const [selectedPairedDeviceOptionsEdit, setSelectedPairedDeviceOptionsEdit] =
    useState([]);
  ...
  const customValueRendererPairedDeviceEdit = (
    valuePairedDevices,
    _categoryname
  ) => {
    return valuePairedDevices?.length
  ...
  const rowData = async (id, name) => {
    ...
    try {
      let res = await axios.get(
        `${SERVICE.SINGLE_BIOMETRIC_PAIRED_DEVICE_GROUPING}/${id}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const delSource = async () => {
    ...
    try {
      const deletePromises = await axios.delete(
        `${SERVICE.SINGLE_BIOMETRIC_PAIRED_DEVICE_GROUPING}/${Sourcesid}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const delSourcecheckbox = async () => {
    ...
    try {
      const deletePromises = selectedRows?.map((item) => {
        return axios.delete(
          `${SERVICE.SINGLE_BIOMETRIC_PAIRED_DEVICE_GROUPING}/${item}`,
          {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
  ...
  const sendRequest = async () => {
    ...
    try {
      axios.post(SERVICE.CREATE_BIOMETRIC_PAIRED_DEVICE_GROUPING, {
        ...
        isVisitorOne: Boolean(pairedDeviceGroupingCreate.isVisitorOne),
        isVisitorTwo: Boolean(pairedDeviceGroupingCreate.isVisitorTwo),
        ...
        visitorinone: VisitorInSwitchFirst,
        visitoroutone: VisitorOutSwitchFirst,
        visitorinoutone: VisitorInOutSwitchFirst,
        ...
        visitorintwo: VisitorInSwitchSecond,
        visitorouttwo: VisitorOutSwitchSecond,
        visitorinouttwo: VisitorInOutSwitchSecond,
  ...
  const handleSubmit = async (e) => {
    ...
    const isNameMatch = pairingDevicesArray.some(
      (item) =>
        ...
        item.paireddeviceone?.toLowerCase() === pairedDeviceGroupingCreate.paireddeviceone?.toLowerCase()
    );
    ...
    if (
      pairedDeviceGroupingCreate.company === "Please Select Company" ||
      !pairedDeviceGroupingCreate.company
    ) {
      setPopupContentMalert("Please Select Company");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      pairedDeviceGroupingCreate.branch === "Please Select Branch" ||
      !pairedDeviceGroupingCreate.branch
    ) {
      setPopupContentMalert("Please Select Branch");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      pairedDeviceGroupingCreate.unit === "Please Select Unit" ||
      !pairedDeviceGroupingCreate.unit
    ) {
      setPopupContentMalert("Please Select Unit");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      pairedDeviceGroupingCreate.floor === "Please Select Floor" ||
      !pairedDeviceGroupingCreate.floor
    ) {
  ...
  const getCode = async (e, name) => {
    ...
    try {
      let res = await axios.get(
        `${SERVICE.SINGLE_BIOMETRIC_PAIRED_DEVICE_GROUPING}/${e}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      setPairedDevicesDuplicateEdit(
        pairingDevicesArray?.filter((item) => item?._id !== e)
      );
      ...
      const {
        ...
        minTwo,
        secTwo,
      } = res?.data?.sbiometricpaireddevicesgrouping;
      ...
      setVisitorInSwitchFirstEdit(visitorinone);
      setVisitorOutSwitchFirstEdit(visitoroutone);
      setVisitorInOutSwitchFirstEdit(visitorinoutone);
      setVisitorInSwitchSecondEdit(visitorintwo);
      setVisitorOutSwitchSecondEdit(visitorouttwo);
      setVisitorInOutSwitchSecondEdit(visitorinouttwo);
      ...
      fetchDeviceNamesBasedOnAreaEdit(
        res?.data?.sbiometricpaireddevicesgrouping,
        res?.data?.sbiometricpaireddevicesgrouping?.area
      );
      setSelectedPairedDeviceOptionsEdit(
        res?.data?.sbiometricpaireddevicesgrouping?.pairdevices?.map(
          (data) => ({
            label: data,
            value: data,
          })
      ...
      setValuePairedDevicesEdit(
        res?.data?.sbiometricpaireddevicesgrouping?.pairdevices
      );
      handleClickOpenEdit();
    } catch (err) {
      handleApiError(
  ...
  const sendEditRequest = async () => {
    ...
    try {
      let res = await axios.put(
        `${SERVICE.SINGLE_BIOMETRIC_PAIRED_DEVICE_GROUPING}/${subprojectsid}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
          ...
          isVisitorOne: String(pairedDeviceGroupingEdit.isVisitorOne),
          isVisitorTwo: String(pairedDeviceGroupingEdit.isVisitorTwo),
          pairedstatus: pairedDeviceGroupingEdit.pairedstatus,
          visitorinone: VisitorInSwitchFirstEdit,
          visitoroutone: VisitorOutSwitchFirstEdit,
          visitorinoutone: VisitorInOutSwitchFirstEdit,
          visitorintwo: VisitorInSwitchSecondEdit,
          visitorouttwo: VisitorOutSwitchSecondEdit,
          visitorinouttwo: VisitorInOutSwitchSecondEdit,
  ...
  const editSubmit = async (e) => {
    ...
    const isNameMatch = pairedDevicesDuplicateEdit?.some(
      (item) =>
        ...
        item.paireddeviceone?.toLowerCase() === pairedDeviceGroupingEdit.paireddeviceone?.toLowerCase() 
    );
    ...
    if (
      pairedDeviceGroupingEdit.company === "Please Select Company" ||
      !pairedDeviceGroupingEdit.company
    ) {
      setPopupContentMalert("Please Select Company");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      pairedDeviceGroupingEdit.branch === "Please Select Branch" ||
      !pairedDeviceGroupingEdit.branch
    ) {
      setPopupContentMalert("Please Select Branch");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      pairedDeviceGroupingEdit.unit === "Please Select Unit" ||
      !pairedDeviceGroupingEdit.unit
    ) {
      setPopupContentMalert("Please Select Unit");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
    } else if (
      pairedDeviceGroupingEdit.floor === "Please Select Floor" ||
      !pairedDeviceGroupingEdit.floor
    ) {
      setPopupContentMalert("Please Select Floor");
      setPopupSeverityMalert("warning");
      handleClickOpenPopupMalert();
  ...
  const columnDataTable = [
    ...
    {
      field: "paireddeviceone",
      headerName: "Pair Device 1",
      flex: 0,
      width: 250,
      hide: !columnVisibility.paireddeviceone,
      headerClassName: "bold-header",
    },
    {
      field: "paireddevicetwo",
      headerName: "Pair Device 2",
      flex: 0,
      width: 250,
      hide: !columnVisibility.paireddevicetwo,
      headerClassName: "bold-header",
    },
    {
      field: "actions",
  ...
  return (
    <Box>
      ...
      <PageHeading
        ...
        subpagename="Biometric Paired Devices Grouping"
        subsubpagename=""
      />
              ...
              <Grid container spacing={2}>
                <Grid item xs={8}>
                  <Typography sx={userStyle.importheadtext}>
                    Add Biometric Paired Devices Grouping
                  </Typography>
                </Grid>
              </Grid>
              <br />
                        ...
                        options={accessbranch
                          ?.filter(
                            (comp) =>
                              comp.company ===
                                pairedDeviceGroupingCreate?.company &&
                              comp.branch === pairedDeviceGroupingCreate?.branch
                          )
                          .map((data) => ({
                            label: data.unit,
                            value: data.unit,
                    ...
                    <FormControl fullWidth size="small">
                      ...
                      <Selects
                        options={allfloor
                          ?.filter(
                            (u) =>
                              pairedDeviceGroupingCreate?.branch === u.branch
                          )
                          .map((u) => ({
                            ...u,
                            label: u.name,
                        ...
                        options={filteredAreas
                          .filter(
                            (item) =>
                              pairedDeviceGroupingCreate?.floor ===
                                item.floor &&
                              pairedDeviceGroupingCreate?.branch === item.branch
                          )
                          ...
                          <FormControlLabel
                            ...
                            control={
                              <Switch
                                ...
                                onChange={(e) =>
                                  setAttendanceInOutSwitchSecond(
                                    e.target.checked
                                  )
                                }
                                disabled={isDisabled(
                ...
                <Grid container spacing={2}>
                  <Grid item md={12} xs={12} sm={12}>
                    <Typography sx={userStyle.HeaderText}>
                      Edit Biometric Paired Devices Grouping
                    </Typography>
                  </Grid>
                </Grid>
                <br />
                          ...
                          options={accessbranch
                            ?.filter(
                              (comp) =>
                                comp.company ===
                                  pairedDeviceGroupingEdit?.company &&
                                comp.branch === pairedDeviceGroupingEdit?.branch
                            )
                            .map((data) => ({
                              label: data.unit,
                              value: data.unit,
                      ...
                      <FormControl fullWidth size="small">
                        ...
                        <Selects
                          options={allfloor
                            ?.filter(
                              (u) =>
                                pairedDeviceGroupingEdit?.branch === u.branch
                            )
                            .map((u) => ({
                              ...u,
                              label: u.name,
                          ...
                          options={filteredAreas
                            .filter(
                              (item) =>
                                pairedDeviceGroupingEdit?.floor ===
                                  item.floor &&
                                pairedDeviceGroupingEdit?.branch === item.branch
                            )
                            .flatMap((item) => item.area)
                            .map((location) => ({
                              label: location,
    ...
    hh
                          ...
                          <Grid item lg={6} md={6} sm={6} xs={12}>
                            ...
                            <Grid container spacing={1}>
                              ...
                              <Grid item xs={4}>
                                ...
                                onChange={(e) => {
                                  setTimeIntHourMinSecSecondEdit((prev) => ({
                                    ...prev,
                                    hour: e.target.value,
                                  }));
                                }}
      ...
      {isUserRoleCompare?.includes("lbiometricpaireddevicesgrouping") && (
        <>
          <Box sx={userStyle.container}>
            ...
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>
                Biometric Paired Devices Grouping List
              </Typography>
            </Grid>
            <br />
              ...
              >
                <Box>
                  {isUserRoleCompare?.includes(
                    "excelbiometricpaireddevicesgrouping"
                  ) && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes(
                    "csvbiometricpaireddevicesgrouping"
                  ) && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes(
                    "printbiometricpaireddevicesgrouping"
                  ) && (
                    <>
                  ...
                  {isUserRoleCompare?.includes(
                    "pdfbiometricpaireddevicesgrouping"
                  ) && (
                    <>
                      <Button
                        sx={userStyle.buttongrp}
                  ...
                  {isUserRoleCompare?.includes(
                    "imagebiometricpaireddevicesgrouping"
                  ) && (
                    <Button
                      sx={userStyle.buttongrp}
            ...
            &ensp;
            {isUserRoleCompare?.includes(
              "bdbiometricpaireddevicesgrouping"
            ) && (
              <Button
      ...
      >
        <Box sx={{ padding: "20px 50px" }}>
          <>
            <Typography sx={userStyle.HeaderText}>
              {" "}
              View Biometric Paired Devices Grouping
            </Typography>
            <br /> <br />
            <Grid container spacing={2}>
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>
                    {sourceEdit?.attendanceinone?.toString()}
                  </Typography>
                </FormControl>
              </Grid>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>
                    {sourceEdit?.attendanceoutone?.toString()}
                  </Typography>
                </FormControl>
              </Grid>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>
                    {sourceEdit?.attendanceinoutone?.toString()}
                  </Typography>
                </FormControl>
              </Grid>
              <Grid item md={3} xs={12} sm={12}>
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>
                    {sourceEdit?.exitinoutone?.toString()}
                  </Typography>
                </FormControl>
              </Grid>
              <Grid item md={3} xs={12} sm={12}>
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>
                    {sourceEdit?.timeintervalOne?.toString()}
                  </Typography>
                </FormControl>
              </Grid>
              {sourceEdit?.timeintervalOne && (
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    <Typography variant="h6">Duration</Typography>
                    <Typography>{`${sourceEdit?.hourOne}:${sourceEdit?.minOne}:${sourceEdit?.secOne}`}</Typography>
                  </FormControl>
                </Grid>
              )}
                ...
                <>
                  ...
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      <Typography variant="h6">Attendance In</Typography>
                      <Typography>
                        {sourceEdit?.attendanceintwo?.toString()}
                      </Typography>
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      <Typography variant="h6">Attendance Out</Typography>
                      <Typography>
                        {sourceEdit?.attendanceouttwo?.toString()}
                      </Typography>
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      <Typography variant="h6">Attendance InOut</Typography>
                      <Typography>
                        {sourceEdit?.attendanceinouttwo?.toString()}
                      </Typography>
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={12}>
                  ...
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      <Typography variant="h6">Exit Out</Typography>
                      <Typography>
                        {sourceEdit?.exitouttwo?.toString()}
                      </Typography>
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      <Typography variant="h6">Exit InOut</Typography>
                      <Typography>
                        {sourceEdit?.exitinouttwo?.toString()}
                      </Typography>
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={12}>
                  ...
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      <Typography variant="h6">Time Interval</Typography>
                      <Typography>
                        {sourceEdit?.timeintervalTwo?.toString()}
                      </Typography>
                    </FormControl>
                  </Grid>
                  {sourceEdit?.timeintervalTwo && (
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        <Typography variant="h6">Duration</Typography>
                        <Typography>{`${sourceEdit?.hourTwo}:${sourceEdit?.minTwo}:${sourceEdit?.secTwo}`}</Typography>
                      </FormControl>
                    </Grid>
                  )}
                </>
      ...
      <InfoPopup
        ...
        heading="Biometric Paired Devices Grouping Info"
        addedby={addedby}
        updateby={updateby}
      />
