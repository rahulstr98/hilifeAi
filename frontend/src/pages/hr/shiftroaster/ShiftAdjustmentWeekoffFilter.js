...
function ShiftAdjustmentListWeekoffFilter() {
    ...
    const [shiftRoasterUserBulkEdit, setShiftRoasterUserBulkEdit] = useState({
        ...
        selectedColumnShiftMode: '',
        selectedColumnShiftTime: '',
        selectedColumnDate: '', weekdate: '',
        adjfirstshiftmode: "", adjustmenttype: "WeekOff Adjustment", adjchangeshift: "Choose Shift",
        adjchangeshiftime: "", adjchangereason: "", adjweekdate: 'Choose Date', adjweekoffdate: 'Choose Date', adjdate: "",
    })
    ...
    const [filterUser, setFilterUser] = useState({ mode: "Please Select Mode", shiftgrptype: 'Select Day/Night', shiftfilter: 'Select Shift', shiftmodefilter: 'Week Off', fromdate: today, todate: today });
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Shift Adjustment Weekoff Filter"),
    ...
    // console.log(isUserRoleAccess?.role?.includes("Manager"), 'accessbranch')
            ...
            getAllResults().then(async (results) => {
                ...
                const itemsWithSerialNumber = filteredItems?.flatMap((item, index) => {
                    ...
                    const days = daysArray.map((column, index) => {
                        ...
                        const uniqueEntriesShiftType = {};
                        item.boardingLog?.forEach(entry => {
                            const key = entry.startdate;
                            if (!(key in uniqueEntriesShiftType)) {
                                uniqueEntriesShiftType[key] = entry;
                            }
                        });
                        const uniqueShiftTypeBoardingLog = Object.values(uniqueEntriesShiftType);

                        const relevantShiftTypeBoardingLogEntry = uniqueShiftTypeBoardingLog
                            .filter(log => log.startdate <= finalDate)
                            .sort((a, b) => new Date(b.startdate) - new Date(a.startdate))[0];
                        ...
                        return {
                            ...
                            shifttype: (relevantShiftTypeBoardingLogEntry && relevantShiftTypeBoardingLogEntry.shifttype),
                ...
                const finalFilter = result?.filter((data) => {
                    ...
                    return data.shifttype === "Standard" && filterUser.fromdate === data.finalDate && data.adjstatus !== 'Approved' && filterUser.shiftmodefilter === data.shiftlabel;
    ...
    const handleFilterSubmit = (e) => {
        ...
        else if (filterUser.shiftmodefilter === '') {
    ...
    const sendBulkRequest = async () => {
        ...
        try {
            ...
            await axios.post(SERVICE.USER_SHIFTALLOT_BULK_UPDATE_FOR_WEEKOFFADJUSTMENT_OPT, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                ...
                weekdate: moment(shiftRoasterUserBulkEdit.weekdate).format('DD/MM/YYYY'),
            });
            await fetchUsersShiftFilter();
    ...
    const handleBulkSubmit = () => {
        if (shiftRoasterUserBulkEdit.weekdate === "") {
            setPopupContentMalert("Please Select Week Date");
            setPopupSeverityMalert("warning");
            handleClickOpenPopupMalert();
        } else if (shiftRoasterUserBulkEdit.weekdate === filterUser.fromdate) {
            setPopupContentMalert("Week Date and Week Off Date's are same. Please Select another Week Date");
            setPopupSeverityMalert("warning");
            handleClickOpenPopupMalert();
        } else {
            sendBulkRequest();
    ...
    // userdataarray [
    //     {company: 'comp', branch: 'bran', unit: 'unit', companyname: 'AAA', weekiff: [], _id: Object('0000'),
    //         shiftallot: [{
    //             company: 'comp', branch: 'bran', unit: 'unit', companyname: 'AAA', weekiff: [], adjstatus: 'Approved',  adjdate: '02/05/2024',...,
    //         }]
    //     },
    //     {company: 'comp', branch: 'bran', unit: 'unit', companyname: 'BBB', weekiff: [], _id: Object('0000'),
    //         shiftallot: [{
    //             company: 'comp', branch: 'bran', unit: 'unit', companyname: 'BBB', weekiff: [], adjstatus: 'Approved', adjdate: '29/09/2024',...,
    //         },
    //         {
    //             company: 'comp', branch: 'bran', unit: 'unit', companyname: 'BBB', weekiff: [], adjstatus: 'Approved',  adjdate: '03/11/2024',...,
    //         }]
    //     },        
    // ]
    ...
    // Excel Set Table
    const fileNameAdj = "Shift Adjustment Weekoff Filter";
        ...
        if (isfilter === "filtered") {
            data = resultdataShiftFilter.map((row, index) => {
                return [
                    ...
                    row.branch,
                    row.unit,
                    `${row.depstatus === undefined ? '' : row.depstatus} ${row.adjstatus === undefined ? '' : row.adjstatus} ${row.shiftlabel === undefined ? '' : row.shiftlabel}`,
                ];
            });
    ...
    const handleprintAdj = useReactToPrint({
        ...
        documentTitle: "Shift Adjustment Weekoff Filter",
        pageStyle: "print",
    });
    ...
    const downloadPdfAdj = (isfilter) => {
        ...
        doc.save("Shift Adjustment Weekoff Filter.pdf");
    };
    ...
    const handleCaptureImageShiftFilter = () => {
        if (gridRefImageShiftFilter.current) {
            domtoimage.toBlob(gridRefImageShiftFilter.current)
                .then((blob) => {
                    saveAs(blob, "Shift Adjustment Weekoff Filter.png");
                })
                .catch((error) => {
    ...
    return (
        <Box>
            <Headtitle title={"SHIFT ADJUSTMENT WEEKOFF FILTER"} />
            {/* ****** Header Content ****** */}
            <PageHeading
                ...
                subpagename="Shift Adjustment Weekoff Filter"
                subsubpagename=""
            />
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("lshiftadjustmentweekofffilter") && (
                <>
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item md={3} sm={12} xs={12}>
                                ...
                                <FormControl fullWidth size="small">
                                    <Selects
                                        size="small"
                                        options={shiftmodefilterOptions}
                                        styles={colourStyles}
                                        value={{ label: filterUser.shiftmodefilter, value: filterUser.shiftmodefilter }}
                                        onChange={(e) => { setFilterUser({ ...filterUser, shiftmodefilter: e.value }); }}
                                    />
                                </FormControl>
                            </Grid>
                    ...
                    <Box sx={userStyle.container}>
                        ...
                        <Grid container style={userStyle.dataTablestyle}>
                            ...
                            <Grid item md={8} xs={12} sm={12} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                                <Box>
                                    {isUserRoleCompare?.includes("excelshiftadjustmentweekofffilter") && (
                                        <>
                                            <Button onClick={(e) => {
                                    ...
                                    {isUserRoleCompare?.includes("csvshiftadjustmentweekofffilter") && (
                                        <>
                                            <Button onClick={(e) => {
                                                setIsFilterOpenAdj(true)
                                                setFormatAdj("csv")
                                    ...
                                    {isUserRoleCompare?.includes("printshiftadjustmentweekofffilter") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("pdfshiftadjustmentweekofffilter") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("imageshiftadjustmentweekofffilter") && (
                                        <>
                        ...
                        <Button sx={userStyle.buttongrp} onClick={handleOpenManageColumnsShiftFilter}> Manage Columns  </Button>&ensp;
                ...
                <Box sx={{ padding: "20px 50px" }}>
                    <>
                        ...
                        <Grid container spacing={1}>
                            ...
                            <Grid item md={12} xs={12} sm={12} >
                                {shiftRoasterUserBulkEdit.adjustmenttype == "WeekOff Adjustment" ? (
                                    <Grid container spacing={2}>
                                        <Grid item md={4} xs={12} sm={12} >
                                            <Typography sx={{ fontSize: '14px', marginTop: '10px' }}>Week Date<b style={{ color: "red" }}>*</b></Typography>
                                            <FormControl fullWidth >
                                                <TextField type="date" size="small" value={shiftRoasterUserBulkEdit.weekdate}
                                                    onChange={(e) => { setShiftRoasterUserBulkEdit({ ...shiftRoasterUserBulkEdit, weekdate: e.target.value }); }}
                                                />
                                            </FormControl>
                                        </Grid>
                                        <Grid item md={4} sm={12} xs={12}>
                                            <Typography sx={{ fontSize: '14px', marginTop: '10px' }}>WeekOff Date<b style={{ color: "red" }}>*</b></Typography>
                                            <FormControl fullWidth >
                                                <FormControl fullWidth size="small">
...
export default ShiftAdjustmentListWeekoffFilter;