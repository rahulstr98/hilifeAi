...
function Location() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const isNameMatch = locations.some(
      (item) => item.name.toLowerCase() === location.name.toLowerCase()
    );
    const isCodeMatch = locations.some(
      (item) => item.code.toLowerCase() === location.code.toLowerCase()
    );
    ...
    try {
      ...
      const performUploads = async () => {
        ...
        try {
          ...
          if (locatid.name?.toLowerCase() !== oldLocationName?.toLowerCase()) {
            await axios.put(
  ...
  const editSubmit = (e) => {
    e.preventDefault();
    const isNameMatch = locationsalledit.some(
      (item) => item.name.toLowerCase() === locatid.name.toLowerCase()
    );
    const isCodeMatch = locationsalledit.some(
      (item) => item.code.toLowerCase() === locatid.code.toLowerCase()
    );
  ...
  const addSerialNumber = () => {
    const itemsWithSerialNumber = locations?.map((item, index) => ({
      ...item,
      serialNumber: index + 1,
    }));
      ...
      <Box>
        ...
        >
          <Box sx={userStyle.container}>
            <>
              <Grid container spacing={2}>
                <Typography sx={userStyle.SubHeadtext}>
                  Edit Location
                </Typography>
              </Grid>
              <br />
                ...
                <Grid item md={6} sm={12} xs={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput
                      ...
                      value={locatid.code}
                      onChange={(e) => {
                        setLocatid({ ...locatid, code: e.target.value });
                      }}
                    />
                  </FormControl>
                </Grid>
                <Grid item md={6} sm={12} xs={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput
                      ...
                      value={locatid.name}
                      onChange={(e) => {
                        setLocatid({ ...locatid, name: e.target.value });
                      }}
                    />
                  </FormControl>
                </Grid>
