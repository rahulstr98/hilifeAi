...
function ShiftDetails() {
  let exportColumnNames = ['Name', 'FromTime', 'ToTime', 'Payhours', 'shifthours', 'Breakhours', 'Workinghours', 'Shiftallowance'];
  let exportRowValues = ['name', 'from', 'to', 'payhours', 'shifthours', 'breakhours', 'workinghours', 'isallowance'];
  ...
  const calculateWorkingHours = (breakmin) => {
    ...
    const [payHoursHours, payHoursMinutes] = shift.payhours.split(':').map(Number);
    const [breakHoursHours, breakHoursMinutes] = shift.breakhours.split(':').map(Number);

    // Subtracting break hours from pay hours
    let totalHours = payHoursHours - breakHoursHours;
    let totalMinutes = payHoursMinutes - Number(breakmin === 'Mins' ? '00' : breakmin);
    ...
    const formattedTotalShiftHours = `${totalHours < 10 ? '0' : ''}${totalHours}:${totalMinutes < 10 ? '0' : ''}${totalMinutes}`;

    setShift({ ...shift, workinghours: formattedTotalShiftHours });
  };
  const calculateWorkingHoursEdit = (breakmin) => {
    // Splitting hours and minutes
    const [payHoursHours, payHoursMinutes] = shiftsid.payhours.split(':').map(Number);
    const [breakHoursHours, breakHoursMinutes] = shiftsid.breakhours.split(':').map(Number);
  ...
  const calculateShiftHours = async (reference, inputValue) => {
    if (reference === 'fromhourvalue') {
      ...
      const totalFromMinutes = parseInt(shift.frommin === '' ? 0 : shift.frommin, 10);
      ...
      let minutesDifference = parseInt(shift.tomin === '' ? 0 : shift.tomin, 10) - parseInt(inputValue, 10);
      ...
      const minutesDifference = parseInt(tomin === '' ? 0 : tomin, 10) - parseInt(frommin === '' ? 0 : frommin, 10);
      const minfinal = minutesDifference / 60;
      ...
      setShift({
        ...shift,
        [reference === 'fromhourvalue' ? 'fromhour' : reference === 'fromminutesvalue' ? 'frommin' : reference === 'fromtimevalue' ? 'fromtime' : reference === 'tohourvalue' ? 'tohour' : reference === 'tominutesvalue' ? 'tomin' : 'totime']: inputValue,
      ...
      const minutesDifference = parseInt(shift.tomin === '' ? 0 : shift.tomin, 10) - parseInt(shift.frommin === '' ? 0 : shift.frommin, 10);
      const minfinal = minutesDifference / 60;
      ...
      if (shift.totime === 'AM' && shift.fromtime === 'PM' && hoursDifference < 0) {
        hoursDifference += 24; // Add 24 hours to handle the next day scenario
      }

      // Calculate the minutes difference
      let minutesDifference = parseInt(inputValue, 10) + fromHour24 * 60 - (toHour24 * 60 + parseInt(shift.tomin === '' ? 0 : shift.tomin, 10));
      ...
      // Calculate total shift hours
      const totalShiftHours = String(hoursDifference).length === 2 ? hoursDifference + ':' + hoursFinal : '0' + hoursDifference + ':' + hoursFinal;
      ...
      let minutesDifference =
        ...
        (fromHour24 * 60 + parseInt(shift.frommin === '' ? 0 : shift.frommin, 10)); // Convert to minutes
      ...
      const totalShiftHours = `${totalHours < 10 ? '0' + totalHours : totalHours}:${totalMinutes < 10 ? '0' + totalMinutes : totalMinutes}`;
  ...
  const calculateShiftHoursEdit = async (reference, inputValue) => {
    if (reference === 'fromhourvalue') {
      ...
      // Calculate the total minutes from the 'frommin' and 'tomin' values
      const totalFromMinutes = parseInt(shiftsid.frommin === '' ? 0 : shiftsid.frommin, 10);
      const totalToMinutes = parseInt(shiftsid.tomin === '' ? 0 : shiftsid.tomin, 10);
      ...
      // Convert selected times to 24-hour format
      let fromHour24 = parseInt(shiftsid.fromhour === '' ? 0 : shiftsid.fromhour, 10);
      if (shiftsid.fromtime === 'PM' && fromHour24 !== 12) {
        fromHour24 += 12;
      } else if (shiftsid.fromtime === 'AM' && fromHour24 === 12) {
        fromHour24 = 0;
      ...
      // Calculate the minutes difference
      let minutesDifference = parseInt(shiftsid.tomin === '' ? 0 : shiftsid.tomin, 10) - parseInt(inputValue, 10);
      ...
      const minutesDifference = parseInt(shiftsid.tomin === '' ? 0 : shiftsid.tomin, 10) - parseInt(shiftsid.frommin === '' ? 0 : shiftsid.frommin, 10);
      const minfinal = minutesDifference / 60;
      ...
      // If the "to" time is in the AM and "from" time is in the PM, adjust the hours difference
      if (shiftsid.totime === 'AM' && shiftsid.fromtime === 'PM' && hoursDifference < 0) {
        hoursDifference += 24; // Add 24 hours to handle the next day scenario
      }

      // Calculate the minutes difference
      let minutesDifference = parseInt(inputValue, 10) + fromHour24 * 60 - (toHour24 * 60 + parseInt(shiftsid.tomin === '' ? 0 : shiftsid.tomin, 10));
      ...
      let minutesDifference =
        ...
        (fromHour24 * 60 + parseInt(shiftsid.frommin === '' ? 0 : shiftsid.frommin, 10)); // Convert to minutes
  ...
  const delShiftcheckbox = async () => {
    ...
    try {
      const [resuser, resusernew] = await Promise.all([
        axios.post(
          SERVICE.USERSHIFTCHECKBULK,
          { checkshifttouser: selectedNames },
          {
            headers: { Authorization: `Bearer ${auth.APIToken}` },
          }
        ),
        axios.post(
          SERVICE.GETALLSHIFTGROUPSBULK,
          { shifts: selectedNames, selectedrows: selectedRows },
          {
            headers: { Authorization: `Bearer ${auth.APIToken}` },
          }
        ),
      ...
      if (usersExist) {
        ...
        let updatedSelectedRows = selectedRows.filter((selectedId) => !finalDatas.includes(selectedId));
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const isNameMatch = shifts.some((item) => item.name.toLowerCase() === shift.name.toLowerCase());
  ...
  const getCode = async (e, name, action, used) => {
    setPageName(!pageName);
    try {
      ...
      const [shiftHours, shiftMinutes] = res?.data?.sshift.shifthours.split(':');
      const [payHours, payMinutes] = res?.data?.sshift.payhours.split(':');
      const [breakHours, breakMminutes] = res?.data?.sshift.breakhours.split(':');
  ...
  const editSubmit = (e) => {
    e.preventDefault();
    fetchShiftAll();
    const isNameMatch = shiftsalledit.some((item) => item.name.toLowerCase() === shiftsid.name.toLowerCase());
    ...
    if (shiftsid.name === '') {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (shiftsid.payhours === '00:00' || shiftsid.payhours.includes('Mins')) {
      setPopupContentMalert('Please Select Pay Hours');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
  ...
  const fetchShiftBreakHours = async () => {
    setPageName(!pageName);
    try {
      let response = await axios.get(SERVICE.ALL_SHIFT_BREAK_HOURS, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
...

    {
      ...
      renderCell: (params) => (
        <Grid sx={{ display: 'flex' }}>
          {isUserRoleCompare?.includes('eshift') && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
                getCode(params.row.id, params.row.name, 'edit', params?.row?.hasresults);
              }}
            >
          ...
          {isUserRoleCompare?.includes('vshift') && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
                getCode(params.row.id, params.row.name, 'view', params?.row?.hasresults);
              }}
            >
  ...
  return (
    <Box>
      ...
      <PageHeading title="Shift" modulename="Human Resources" submodulename="HR" mainpagename="HR Setup" subpagename="Shift" subsubpagename="" />
      {isUserRoleCompare?.includes('ashift') && (
        <Box sx={userStyle.selectcontainer}>
          <>
            <Grid container spacing={2}>
              <Grid item sm={12} md={12} xs={12}>
                <Typography sx={userStyle.importheadtext}>Add Shift</Typography>
              </Grid>
            </Grid>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6} md={4} lg={4}>
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput id="component-outlined" type="text" value={shift.name} />
                </FormControl>
              </Grid>
            </Grid>
            <Grid container spacing={2}>
              ...
              <Grid item lg={2} md={3} sm={6} xs={12}>
                ...
                <FormControl fullWidth size="small">
                  <OutlinedInput id="component-outlined" type="text" placeholder="HH:MM" value={shift.shifthours} />
                </FormControl>
              </Grid>
              <Grid item lg={2} md={3} sm={6} xs={12}>
                <Typography>
              ...
              <Grid item lg={2} md={3} sm={6} xs={12}>
                ...
                <FormControl fullWidth size="small">
                  <OutlinedInput id="component-outlined" placeholder="HH:MM" value={shift.workinghours} />
                </FormControl>
              </Grid>
              <Grid item lg={2} md={3} sm={6} xs={12}>
                <Typography>
            ...
            <Grid container spacing={2}>
              <Grid item md={2.5} xs={12} sm={6}>
                <>
                  <Button variant="contained" color="primary" sx={{ display: 'flex', ...buttonStyles.buttonsubmit }} onClick={handleSubmit}>
                    Submit
                  </Button>
                </>
              </Grid>
        ...
        >
          <Box sx={userStyle.dialogbox}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={8}>
                  <Typography sx={userStyle.HeaderText}>Edit Shift   {linked && (
                    <Typography
                      component="span"
                      sx={{ fontSize: "0.85rem", color: "red", ml: 1 }}
                    >
                      (This shift is linked to other data. All fields are disabled, except the
                      shift allowance.)
                    </Typography>
                  )}</Typography>
                </Grid>
              ...
              <Grid container spacing={1}>
                      ...
                      <FormControl size="small" fullWidth>
                        <Select
                          ...
                          onChange={(e) => {
                            ...
                            calculateShiftHoursEdit('fromhourvalue', e.target.value);
                          }}
                      ...
                      <FormControl size="small" fullWidth>
                        <Select
                          ...
                          onChange={(e) => {
                            ...
                            calculateShiftHoursEdit('fromminutesvalue', e.target.value);
                          }}
                        >
                          <MenuItem value={'00'}>00</MenuItem>
                          <MenuItem value={'01'}>01</MenuItem>
                      ...
                      <FormControl size="small" fullWidth>
                        <Select
                          ...
                          onChange={(e) => {
                            ...
                            calculateShiftHoursEdit('fromtimevalue', e.target.value);
                          }}
                        >
                          <MenuItem value={'AM'}>AM</MenuItem>
                          <MenuItem value={'PM'}>PM</MenuItem>
                      ...
                      <FormControl size="small" fullWidth>
                        <Select
                          ...
                          onChange={(e) => {
                            ...
                            calculateShiftHoursEdit('tohourvalue', e.target.value);
                          }}
                        >
                          <MenuItem value={'00'}>00</MenuItem>
                          <MenuItem value={'01'}>01</MenuItem>
                      ...
                      <FormControl size="small" fullWidth>
                        <Select
                          ...
                          onChange={(e) => {
                            ...
                            calculateShiftHoursEdit('tominutesvalue', e.target.value);
                          }}
                        >
                          <MenuItem value={'00'}>00</MenuItem>
                          <MenuItem value={'01'}>01</MenuItem>
                      ...
                      <FormControl size="small" fullWidth>
                        <Select
                          ...
                          onChange={(e) => {
                            ...
                            calculateShiftHoursEdit('totimevalue', e.target.value);
                          }}
                        >
                          <MenuItem value={'AM'}>AM</MenuItem>
                          <MenuItem value={'PM'}>PM</MenuItem>
                ...
                <Grid item xs={12} sm={6} md={4} lg={4}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput id="component-outlined" type="text" value={shiftsid.name} readOnly={linked} />
                  </FormControl>
                </Grid>
              </Grid>
              <Grid container spacing={1}>
                ...
                <Grid item lg={4} md={4} sm={6} xs={12}>
                  ...
                  <FormControl fullWidth size="small">
                    <OutlinedInput id="component-outlined" value={shiftsid.shifthours} readOnly={linked} />
                  </FormControl>
                </Grid>
                <Grid item lg={4} md={4} sm={6} xs={12}>
                  <Typography>
                ...
                <Grid item lg={4} md={4} sm={6} xs={12}>
                  ...
                  <FormControl fullWidth size="small">
                    <OutlinedInput id="component-outlined" value={shiftsid.workinghours} readOnly={linked} />
                  </FormControl>
                </Grid>
                <Grid item lg={4} md={4} sm={6} xs={12}>
                  <Typography>
      ...
      <Box>
        ...
        <Dialog open={isDeleteOpen} onClose={handleCloseMod} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
          ...
          <DialogActions>
            ...
            <Button autoFocus variant="contained" color="error" onClick={(e) => delShift(shiftsid)}>
              {' '}
              OK{' '}
            </Button>
          </DialogActions>
              ...
              <Dialog open={isCheckOpen} onClose={handleCloseCheck} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                ...
                >
                  ...
                  <Typography variant="h6" sx={{ color: 'black', textAlign: 'center' }}>
                    <>
                      <span style={{ fontWeight: '700', color: '#777' }}>{`${deleteshift.name} `}</span>
                      was linked in <span style={{ fontWeight: '700' }}>User or Shift Grouping</span>{' '}
                    </>
                  </Typography>
                </DialogContent>
                <DialogActions>
        ...
        <Box>
          <>
            <Box>
              {/* ALERT DIALOG */}
              <Dialog open={isCheckOpenNew} onClose={handleCloseCheckNew} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                <DialogContent
                  sx={{
                    width: '350px',
                    textAlign: 'center',
                ...
                >
                  ...
                  <Typography variant="h6" sx={{ color: 'black', textAlign: 'center' }}>
                    <>
                      <span style={{ fontWeight: '700', color: '#777' }}>{`Some of the Shifts `}</span>
                      were linked to <span style={{ fontWeight: '700' }}>Users or Shift Grouping</span>{' '}
                    </>
                  </Typography>
                </DialogContent>
                <DialogActions>
                  <Button onClick={handleCloseCheckNew} autoFocus variant="contained" color="error">
                    {' '}
                    OK{' '}
                  </Button>
                </DialogActions>
              ...
              {filteredData &&
                filteredData.map((row, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{row.fromhour + ':' + row.frommin + '' + row.fromtime}</TableCell>
                    <TableCell>{row.tohour + ':' + row.tomin + '' + row.totime}</TableCell>
      ...
      <Dialog open={openview} onClose={handleClickOpenview} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
        <Box sx={{ width: '550px', padding: '20px 50px' }}>
          <>
            <Typography sx={userStyle.HeaderText}>View Shift Details</Typography>
            <Grid container spacing={2}>
      ...
      <Box>
        <Dialog open={isDeleteOpencheckbox} onClose={handleCloseModcheckbox} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
          ...
          <DialogActions>
            ...
            <Button autoFocus variant="contained" color="error" onClick={(e) => delShiftcheckbox(e)}>
              {' '}
              OK{' '}
            </Button>
          </DialogActions>
