...
function PendingReleaveExitList() {
  ...
  const fetchHandler = async () => {
    ...
    try {
      ...
      setEmployees(
        res?.data?.configuredUsers?.map((item) => {
          let foundData = resNew?.data?.users?.find(
            (itemNew) => itemNew?.empname === item?.companyname
          );
          // foundData.approvenoticereq = moment(foundData?.approvenoticereq).format('DD-MM-YYYY');
          return {
            ...item,
            approvenoticereq: foundData?.approvenoticereq
              ? moment(foundData?.approvenoticereq).format("DD-MM-YYYY")
              : moment(item?.approvenoticereq || new Date()).format(
                  "DD-MM-YYYY"
                ), // Spread foundData only if it exists
          };
        })
      );
    ...
    const itemsWithSerialNumber = datas?.map((item, index) => {
      ...
      return {
        ...
        doj: item?.doj
          ...
          : "",
        // approvenoticereq: item?.approvenoticereq ? moment(item.approvenoticereq, 'YYYY-MM-DD').format('DD-MM-YYYY') : '',
  ...
  return (
    <Box>
      ...
      <PageHeading
        ...
        subsubpagename="Pending Releave Exit List"
      />
      <br />
                        ...
                        <FormControlLabel
                          control={
                            <Checkbox
                              ...
                              checked={higherDesignation}
                              disabled={sameDesignation}
                              onChange={() => {
