...
// const { generatePassword ,validatePassword,attachFilesToObjects } = require("../../../components/passwordGenerator");
...
function InternDraftEdit() {
  ...
  const { isUserRoleCompare, isUserRoleAccess, buttonStyles, alldesignation, allUsersData, workStationSystemName } = useContext(UserRoleAccessContext);
  ...
  const fetchHandlerEdit = async () => {
    try {
      ...
      let designationGrpName = alldesignation?.find((data) => response?.data?.sdraft?.designationlog[0]?.designation === data?.name)?.group;
      ...
      setValueCate(response?.data?.sdraft?.boardingLog[0]?.weekoff ? response?.data?.sdraft?.boardingLog[0]?.weekoff : []);
      setOldData({
        ...oldData,
  ...
  const handleBranchChange = (event) => {
    ...
    var newEmpCode = branchCode[0].code.substring(0, 2) + employee?.empcode.substring(2);
