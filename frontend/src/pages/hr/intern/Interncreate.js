...
function Interncreate() {
  ...
  const fetchUserDefaultDatas = async () => {
    ...
    try {
      ...
      setEmployee((prev) => ({
        ...
        caddresstype: migrateData?.caddesstype ? migrateData?.caddesstype : '',
        cpersonalprefix: migrateData?.cpersonalprefix ? migrateData?.cpersonalprefix : '',
        cresourcename: migrateData?.creferencename ? migrateData?.creferencename : '',
        clandmarkandpositionalprefix: migrateData?.clandmarkpositionprefix ? migrateData?.clandmarkpositionprefix : '',
        cgpscoordination: migrateData?.cgpscoordinate ? migrateData?.cgpscoordinate : '',
        clandmark: migrateData?.clandmarkname ? migrateData?.clandmarkname : '',
        cpincode: migrateData?.cpincode ? migrateData?.cpincode : '',
        ccountry: migrateData?.ccountry ? migrateData?.ccountry : '',
        cstate: migrateData?.cstate ? migrateData?.cstate : '',
        ccity: migrateData?.ccity ? migrateData?.ccity : '',
        cdoorno: migrateData?.chouseflatnumber ? migrateData?.chouseflatnumber : '',
        cstreet: migrateData?.cstreetroadname ? migrateData?.cstreetroadname : '',
        carea: migrateData?.clocalityareaname ? migrateData?.clocalityareaname : '',
        samesprmnt: migrateData?.samesprmnt ? migrateData?.samesprmnt : false,

        pgenerateviapincode: migrateData?.pgenerateviapincode ? Boolean(migrateData?.pgenerateviapincode) : false,
        pvillageorcity: migrateData?.pvillageorcity ? migrateData?.pvillageorcity : '',
        pdistrict: migrateData?.pdistrict ? migrateData?.pdistrict : '',
        cgenerateviapincode: migrateData?.cgenerateviapincode ? Boolean(migrateData?.cgenerateviapincode) : false,
        cvillageorcity: migrateData?.cvillageorcity ? migrateData?.cvillageorcity : '',
        cdistrict: migrateData?.cdistrict ? migrateData?.cdistrict : '',
        ...
        dob: migrateData?.dateofbirth !== '' ? moment(migrateData?.dateofbirth).format('YYYY-MM-DD') : '',
        age: migrateData?.dateofbirth !== '' ? calculateAge(migrateData?.dateofbirth) : 0,
        ...
        panstatus: migrateData?.pannumber ? 'Have PAN' : 'Yet to Apply',
      ...
      setSalaryTableData({
        salaryfixed: migrateData?.salaryfixed || false,
        salarystatus: migrateData?.salarystatus || 'With Salary',
        expectedsalary: migrateData?.expectedsalary || '',
        basic: migrateData?.basic || 0,
        hra: migrateData?.hra || 0,
        conveyance: migrateData?.conveyance || 0,
        medicalallowance: migrateData?.medicalallowance || 0,
        productionallowance: migrateData?.productionallowance || 0,
        otherallowance: migrateData?.otherallowance || 0,
        performanceincentive: migrateData?.performanceincentive || 0,
        shiftallowance: migrateData?.shiftallowance || 0,
        grossmonthsalary: migrateData?.grossmonthsalary || 0,
        annualgrossctc: migrateData?.annualgrossctc || 0,
      });
      setSalaryTableDataManual({
        salaryfixed: migrateData?.salaryfixed || false,
        salarystatus: migrateData?.salarystatus || 'With Salary',
        expectedsalary: migrateData?.expectedsalary || '',
        basic: migrateData?.basic || 0,
      ...
      //permananet addresss
      if (migrateData?.pgenerateviapincode && migrateData?.ppincode !== '') {
        const result = await getPincodeDetails(migrateData?.ppincode);
      ...
      if (migrateData?.cgenerateviapincode && migrateData?.cpincode !== '') {
        const result = await getPincodeDetails(migrateData?.cpincode);
        if (result?.status === 'Success' && result?.data?.length > 0) {
      ...
      const country = Country.getAllCountries().find((country) => country?.name === migrateData.ccountry);
      const state = State.getStatesOfCountry(country?.isoCode).find((state) => state?.name === migrateData.cstate);
      const city = City.getCitiesOfState(state?.countryCode, state?.isoCode).find((city) => city?.name === migrateData.ccity);

      // Find the corresponding Country, State, and City objects
      const countryp = Country.getAllCountries().find((country) => country?.name === migrateData.pcountry);
      const statep = State.getStatesOfCountry(countryp?.isoCode).find((state) => state?.name === migrateData.pstate);
      const cityp = City.getCitiesOfState(statep?.countryCode, statep?.isoCode).find((city) => city?.name === migrateData.pcity);
      ...
      migeduall = migrateData?.educationdetails?.map((data, index) => {
        return {
          ...
          cgpa: data?.percentage || 0,
        };
      });
      const edurawData = migeduall || [];
      const updatedEduData = await attachFilesToObjects(edurawData, SERVICE.VIEW_RECRUITMENT_FILES);
      ...
      const eduWorkData = migworkll || [];
      const updatedWorkData = await attachFilesToObjects(eduWorkData, SERVICE.VIEW_RECRUITMENT_FILES);
      setWorkhistTodo(updatedWorkData);
      let desig = await fetchCandidatedocumentdropdowns();

      if (!migrateData?.candidatedatafile || migrateData?.candidatedatafile?.length === 0) {
        await handleCandidateUploadSingleAll(desig);
      } else {
        let fileDatas = migrateData?.candidatedatafile
        ...
        const updatedFilesData = await attachFilesToObjects(eduFilesData, SERVICE.VIEW_RECRUITMENT_FILES);
  ...
  const handleSalaryfix = (process, updatedate, doj, assignExpMode, assignExpvalue, assignEndExpvalue, assignEndExpDate, assignEndTarvalue, assignEndTarDate, department) => {
    ...
    // console.log(findSalDetailsTarlogs, "findSalDetailsTarlogs")
  ...
  const [educationalOverallLink, setEducationalOverallLink] = useState({
    internal: '',
    external: '',
  });
  const [experienceOverallLink, setExperienceOverallLink] = useState({
    internal: '',
    external: '',
  });
  const [educationalGroupId, setEducationalGroupId] = useState('');
  const [experienceGroupId, setExperienceGroupId] = useState('');
  const handleSubmittodo = (e) => {
    ...
    if (employee?.categoryedu == 'Please Select Category' || employee?.subcategoryedu == 'Please Select Sub Category' || employee?.specialization == 'Please Select Specialization' || institution == '' || passedyear == '' || cgpa == '') {
      ...
      const updatedEducationTodo = eduTodo.map((item) => ({
        ...item,
  ...
  const handleSubmitWorkSubmit = (e) => {
    ...
    if (Object.keys(errorstodo)?.length === 0) {
      ...
      const updatedExperienceTodo = workhistTodo.map((item) => ({
        ...item,
        groupid: finalGroupId,
      }));
  ...
  const handleBranchChange = (event) => {
    ...
    setAccessible({
      ...
      branchemail: event?.email?.toString(','),
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      //         branchemail: data?.branchemail?.toString(','),
      ...
      if (migrateData && from === 'interviewRounds') {
        let formData = new FormData();

        if (salarytable.length > 0) {
          salarytable.forEach((item, index) => {
            formData.append(`salarytablefile`, item.file); // Append the file
            formData.append(`salarystatus[${index}]`, item?.salarystatus || ''); // Append unique ID
            ...
            formData.append(`otherallowance[${index}]`, item?.otherallowance || 0);
            formData.append(`performanceincentive[${index}]`, item?.performanceincentive || 0);
          });
        }
        ...
        const jsonData = {
          ...
          directonboardingdetails: {
            ...
            onboardby: isUserRoleAccess?.companyname,
            onboardat: String(new Date()),
          },
        };

        formData.append('jsonData', JSON.stringify(jsonData));

        let res_queue = await axios.put(`${SERVICE.CANDIDATE_UPDATE_MULTER}/${migrateData.id}`, formData, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        });
      }
      if (migrateData && from === 'visitor') {
        const formData = new FormData();
        if (salarytable.length > 0) {
          salarytable.forEach((item, index) => {
            formData.append(`salarytablefile`, item.file); // Append the file
        ...
        let addVendorDetails = await axios.put(`${SERVICE.SINGLE_VISITORS}/${migrateData.id}`, formData, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        });
  ...
  const SendDraftRequest = async () => {
    ...
    try {
      ...
      // Clean and validate `primaryWork`
      let primaryWork = ['please select primary workstation', 'select primary workstation', '', undefined, 'please select primary work station', 'select primary work station', 'primary workstation', 'primary work station', null].includes((primaryWorkStation || '').toLowerCase())
        ? null
        : primaryWorkStation;
      ...
      const employeeDocuments = await uploadEmployeeDocuments({
        empcode: String(employee?.wordcheck === false ? newEmpCode : employee?.empcode),
        commonid: String(employees_draft?.data?.user?._id),
        companyname: String(companycaps),
        type: 'Internship',
        files: validTodos, // assuming it's already [{ file, data, name, remark }]
        eduTodo: eduTodo,
  ...
  const nextStepLog = () => {
    ...
    //   newErrorsLog.password = <Typography style={{ color: 'red' }}>Password must be atleast 8 characters</Typography>;
    //   missingFieldstwo.push('Password');
    // }
  ...
  const handleAccessibleBranchTodo = () => {
    let newObject = {
      ...
      branchemail: accessible?.branchemail?.toString(','),
