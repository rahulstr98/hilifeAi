...
function Floor() {
  ...
  const rowData = async (id, name) => {
    ...
    try {
      ...
      setCheckUser(
        resuser?.data?.users +
        resuser?.data?.manpower +
        resFloor?.data?.workstation +
        resFloor?.data?.locationgrouping +
        resFloor?.data?.areagrouping
      );

      if (
        resuser?.data?.users.length > 0 ||
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const isNameMatchtest = floors.some(
      (item) =>
        item.name.toLowerCase() === floor.name.toLowerCase() &&
        item.branch === floor.branch
    );

    const isCodeMatch = floors.some(
      (item) =>
        item.code.toLowerCase() === floor.code.toLowerCase() &&
        item.branch === floor.branch
    );
    ...
    try {
      ...
      const performUploads = async () => {
        ...
        try {
          ...
          if (floorsid.name?.toLowerCase() !== oldLocationName?.toLowerCase()) {
            await axios.put(
  ...
  const editSubmit = (e) => {
    e.preventDefault();
    fetchFloorAll();
    const isNameMatchedit = floorsalledit.some(
      (item) =>
        item.name.toLowerCase() === floorsid.name.toLowerCase() &&
        item.branch === floorsid.branch
    );

    const isCodeMatch = floorsalledit.some(
      (item) =>
        item.code.toLowerCase() === floorsid.code.toLowerCase() &&
        item.branch === floorsid.branch
    );
  ...
  const getOverallEditSection = async (e) => {
    ...
    try {
      ...
      setGetOverallCount(`The ${e} is linked in
         ...
         ${res?.data?.manpower?.length > 0 ? "Man Power" : ""
  ...
  const fetchAllFloor = async () => {
    ...
    try {
      ...
      let ans = res_floor?.data?.floors?.filter((item) =>
        accessbranch.some((branch) => branch.branch === item.branch)
      );
      setFloors(ans);
      setIsFloor(true);
    } catch (err) {
  ...
  const fetchFloorAll = async () => {
    ...
    try {
      ...
      setFloorsalledit(
        res_floor?.data?.floors.filter((item) => item._id !== floorsid?._id)
      );
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  ...
  const addSerialNumber = () => {
    const itemsWithSerialNumber = floors?.map((item, index) => ({
      ...item,
      serialNumber: index + 1,
    }));
