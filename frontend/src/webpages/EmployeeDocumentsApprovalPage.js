...
function EmployeeDocumentsApprovalPage() {
  ...
  const [openDialogTermsConditions, setOpenDialogTermsConditions] =
    useState(false);
  ...
  const allChecked =
    ...
    termsAndConditions.every(
      (condition) => checkedConditions[condition.details]
    );
  ...
  const TemplateDropdownsValue = async () => {
    ...
    try {
      ...
      setDocumentData(res);
      TermsAndConditionsFetching(
        res?.data?.sdocumentPreparation?.termsAndConditons
      );
      if (res?.data?.sdocumentPreparation?.person) {
        ...
        if (tempControl?.data?.templatecontrolpanel) {
          const ans = tempControl?.data?.templatecontrolpanel
            ? tempControl?.data?.templatecontrolpanel?.templatecontrolpanellog[
                tempControl?.data?.templatecontrolpanel?.templatecontrolpanellog
                  ?.length - 1
              ]
            : "";
        ...
        const usersData =
          response?.data?.users?.loginUserStatus?.length > 0
            ? response?.data?.users?.loginUserStatus?.filter(
        ...
        const userESignature = userDetails?.data?.semployeesignature
          ...
          : "";
        const userMail = userDetails?.data?.tempControlPanel
          ? userDetails?.data?.semployeesignature?.tempControlPanel
          : "";
  ...
  const TermsAndConditionsFetching = async (selectedids) => {
    try {
      ...
      setTermsAndConditions(
        res?.data?.termsandcondition?.filter((data) =>
          selectedids?.includes(data?._id)
        ) ?? []
      );
    } catch (err) {
      console.log(err, "39");
  ...
  const generatePdfPreview = async (response, imageView) => {
    ...
    setSignature(
      response?.data?.sdocumentPreparation?.document?.replaceAll(
        ...
        imageView
          ? `
          <span style="display: inline-flex; flex-direction: column; align-items: center; text-align: center;">
        <img src="${imageView}" alt="Signature" style="${
    ...
    const getPrintPageCount = (htmlString) => {
      ...
      const pagesNeeded = Math.ceil(
        (contentHeight + watermarkHeight) / pageHeight
      );
  ...
  const verifyValidation = async () => {
    try {
      if (dob != "" || mobile != "") {
        let response = await axios.post(
          `${SERVICE.VERIFYTWOFA_EMPLOYEEAPPROVAL_VALIDATION}`,
          {
  ...
  const downloadPdfTesdtTable = async (datadetails, response, documentTemp) => {
    ...
    pdfElement.innerHTML = documentTemp.replaceAll(
      "--- Page Break ---",
      `<p class="page-break-label" data-page-break="true"></p>`
    );
          ...
          img.onload = () => {
            ...
            if (response.data.sdocumentPreparation?.qrcode !== "") {
              ...
              qrImg.onload = () => {
                ...
                formData.append(
                  "documentname",
                  response.data.sdocumentPreparation?.documentname
                );
                formData.append(
                  "username",
                  response.data.sdocumentPreparation?.person
                ...
                axios
                  .post(SERVICE.EMPLOYEE_APPROVAL_FORMDATA, formData, {
                    headers: {
                  ...
                  .then((response) => {
                    if (response.status === 200 || response.status === 201) {
                      ...
                      return axios.put(
                        `${SERVICE.SINGLE_DOCUMENTPREPARATION}/${datadetails?._id}`,
                        {
                          approval: "approved",
                      ...
                      throw new Error(
                        "PDF upload failed with unexpected status code: " +
                          response.status
                      );
                    }
                  })
                  .then(() => {
                    console.log(
                      "PDF uploaded and document updated successfully"
                    );
                    setTimeout(() => {
        ...
        {documentData && documentData?.data?.sdocumentPreparation ? (
          <Box>
            ...
            >
                  ...
                  {termsAndConditions?.map((condition) => (
                    <FormControlLabel
                      ...
                      control={
                        <Checkbox
                          checked={
                            checkedConditions[condition.details] || false
                          }
                          onChange={(event) =>
                            handleCheckboxChange(event, condition)
                          }
                          name={condition.details}
                          disabled={
                            condition.viewmore &&
                            condition.description &&
                            !acceptedConditions[condition.details]
                          }
                        />
                      }
                      label={
              ...
              >
                ⚠️ Property of TTS Business Services. Unauthorized sharing is
                strictly prohibited.
              </Typography>
            </Container>
            <Dialog
            ...
            >
              <DialogTitle>
                <Stack direction="row" alignItems="center" spacing={1}>
                  ...
                  <Typography
                    variant="h6"
                    sx={{ fontWeight: "bold", color: "#FAC921" }}
                  >
                    Are you sure?
                  </Typography>
                </Stack>
          ...
          <div class="progress" style={{ zIndex: 20 }}>
            <div class="bar"></div>
          </div>
        )}
      </div>
