...
function PaySlipDocumentPreparationPage() {
    // Error Popup model
    ...
    const genby = useParams()?.genby;
    const gendate = useParams()?.gendate;
    const tempid = useParams()?.tempid;

    const TemplateDropdownsValue = async () => {
        try {
            let res = await axios.get(`${SERVICE.TEMPLATECONTROLPANEL_SINGLE}/${tempid}`, {
            });
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        >
                            {`For ${decryptString(month)}-${decryptString(year)}, ${decryptString(name)}-${code} received an payslip from ${decryptString(genby)}.
                        The document was generated at  ${gendate?.replaceAll("_" , " ")} and delivered promptly by our team to ensure timely 
                        communication and collaboration.We are committed to providing timely updates and necessary information.
`}
                        </p>
...
export default PaySlipDocumentPreparationPage;