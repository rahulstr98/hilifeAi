...
import { Dialog, TableContainer, DialogContent, DialogActions, Box, Table, Paper, Typography, Grid, Button, TableCell, TableRow, TableBody, TableHead } from '@mui/material';
...
function Visitorinformation() {
  ...
  useEffect(() => {
    const generateQRCode = async () => {
      try {
        const url = await QRCode.toDataURL(`${BASE_URL}/Visitorsinformationregister/${company}/${branch}`);
        setQrCodeUrl(url);
      } catch (err) {
  ...
  const logOut = async () => {
    try {
      ...
      localStorage.clear();
      window.location.href = `/Visitorsinformationregister/${company}/${branch}`;
    } catch (err) {
      const messages = err?.response?.data?.message;
      if (messages) {
        setShowAlert(
        ...
        <Grid container spacing={2}>
          ...
          >
            <Box
              ...
              sx={{
                height: 450,
                width: 450,
                maxHeight: { xs: 100, sm: 100, md: 450 },
          ...
          >
            ...
            <Box>
              ...
              <Dialog open={openviewed} onClose={handleCloseviewed} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="lg" fullWidth={true}>
                <GlobalStyles
                  styles={{
                    '@import': "url('https://fonts.googleapis.com/css2?family=League+Spartan:wght@100..900&display=swap')",
                    body: {
