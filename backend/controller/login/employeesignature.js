...
// get All Employeesignature => /api/employeesignature
exports.getAllPreEmployeesignature = catchAsyncErrors(
  async (req, res, next) => {
    let empsignatures;
...
exports.getSingleEmployeesignature = catchAsyncErrors(
  async (req, res, next) => {
...
exports.getSingleEmployeeSignatureByCommonid = catchAsyncErrors(
  async (req, res, next) => {
    // const { commonid } = req.body;
    const { commonsignatureid } = req.body;
    const semployeesignature = await Employeesignature.findOne(
      { commonsignatureid },
      { profileimage: 1, signatureimage: 1 }
    );
...
exports.getIndividualUserESignature = catchAsyncErrors(
  async (req, res, next) => {
    let semployeesignature, tempControlPanel, userimage;

    try {
      const { companyname } = req.body;
      const userDetails = await User.findOne(
        { companyname },
        { _id: 1, company: 1, branch: 1 }
      ).lean();
      if (userDetails) {
        tempControlPanel = await TemplatecontrolpanelModel.findOne(
          { company: userDetails?.company, branch: userDetails?.branch },
          { _id: 1, fromemail: 1 }
        ).lean();
        semployeesignature = await Employeesignature.findOne(
          { commonsignatureid: userDetails?._id?.toString() },
          { profileimage: 1, signatureimage: 1 }
        );
        userimage = await EmployeeDocuments.findOne(
          { commonid: userDetails?._id?.toString() },
          { profileimage: 1 }
        );
      }
...
exports.getSingleEmployeeSignatureByCommonidWithAll = catchAsyncErrors(
  async (req, res, next) => {
    const { commonsignatureid } = req.body;
    const semployeesignature = await Employeesignature.findOne({
      commonsignatureid,
...
exports.updateEmployeesignature = catchAsyncErrors(async (req, res, next) => {
  ...
  const updateemployeesignature = await Employeesignature.findByIdAndUpdate(
    id,
    req.body
  );
...
exports.getAllEmployeesignatureProfile = catchAsyncErrors(
  async (req, res, next) => {
    let empsignatures;
...
exports.getSingleEmployeeSignatureByCommonidWithAllnew = catchAsyncErrors(
  async (req, res, next) => {
    const { commonsignatureid } = req.body;
    const semployeesignature = await Employeesignature.findOne(
      { commonsignatureid },
      { profileimage: 1, files: 1, signatureimage: 1 }
    );
    if (!semployeesignature) {
      return res.status(200).json({});
    }
...
//employee signatures
exports.getAllEmployeeSignaturesforidcard = catchAsyncErrors(
  async (req, res, next) => {
    let empsignatures;
