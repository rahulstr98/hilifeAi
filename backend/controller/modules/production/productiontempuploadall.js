...
const getShiftForDate = (column, matchingItem, matchingItemAllot, isWeekOffWithAdjustment, isWeekOffWithManual, boardingLog, isWeekOff, matchingDoubleShiftItem, department, overAllDepartment) => {
  // const selectedDateIndex = createdUserDates.findIndex(dateObj => dateObj.formattedDate === column.formattedDate);
  // console.log(column, matchingItem, matchingItemAllot, isWeekOffWithAdjustment, isWeekOffWithManual, boardingLog, isWeekOff, matchingDoubleShiftItem, department);
  // if (selectedDateIndex === -1) {
  //     return !isWeekOff ? actualShiftTiming : "Week Off";
  // }
...
exports.productionTempUploadOverAllFetchLimitedNew = catchAsyncErrors(async (req, res, next) => {
  ...
  let dateoneafter = new Date(req.body.fromdate);
    ...
    //	console.log(newDateOneMinusTwo, 'newDateOneMinusTwo')
    ...
    const query = {
   	fromdate:{$in:[req.body.fromdate,newDateOneMinus,newDateOnePlus,newDateOneMinusThree,newDateOneMinusTwo ]}, 
      checkunique: { $in: req.body.checkunique }, // using RegExp for partial match
    };

    productionuploadtemp = await ProductionTempUploadAll.find(query, { checkunique: 1, flagcount: 1, uniqueid: 1, _id: 0 });
  } catch (err) {
    console.log(err);
  }
...
exports.getAllProductionTempUploadAllFilenamesonly = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    matchedData = await ProductionTempUploadAll.find({fromdate:req.body.fromdate, uniqueid: req.body.id }, { dateval: 1, filename: 1, uniqueid: 1, filenamenew: 1, createdAt: 1 });
...
exports.productionTempCheckMismatchPresentFilter = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const unitRates = await Unitrate.find({}, { category: 1, project: 1, flagcount: 1, subcategory: 1, mrate: 1, trate: 1 }).lean();
...
exports.getProductionUploadDatasByIdTemp = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const unitRates = await Unitrate.find({}, { category: 1, project: 1, flagcount: 1, subcategory: 1, mrate: 1, flagstatus: 1, trate: 1 }).lean();
    ...
    productionupload = productionunmatched.reduce((acc, obj) => {
      ...
      if (
        (Number(finalunitrate) != Number(mratevalcheck) ||
          (Number(mflagalcheck) > 1 && Number(mflagalcheck) != Number(finalflagcount)) ||
          (Number(mrateval) == Number(finalunitrate) && Number(finalflagcount) != Number(mflagalcheck) && mismatchmodeval.includes("Flag Mismatched"))) &&
        ...
        acc.push({
          ...
          formatteddatetime: obj.formatteddate,
          section: obj.section,
          mrate: mrateval,
...
exports.getAllproductiontempFilter = catchAsyncErrors(async (req, res, next) => {
  let productionupload = [],  mergedData, producionIndividual;
  let finaluser = [];
  let mergedDataall = [];
  try {
...
const filteredLogs = logs.filter(log => {
  return (
    new Date(log.date) <= new Date(req.body.date) &&
    (!log.enddate || new Date(log.enddate) >= new Date(req.body.date))
  );
});
...
exports.getAllProductionTempReportFilter = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
   ...
   let users = users1
    ...
    let allData =  mode.includes('Production') && mode.includes('Manual Production') ? [...producionIndividual, ...productionupload] : 
    mode.includes('Production') ? productionupload : producionIndividual;
  ...
  try {
    ...
    const [producionIndividualZeroCount, productionUploadZeroCount] = await Promise.all([
      ProducionIndividual.countDocuments({
        ...
        $or: [{ fromdate: req.body.date }, { fromdate: newDateOnePlus }],
        unallothide: { $ne: 'true' },
        updatedunitrate: { $exists: false },
        status: 'Approved',
      }),
...
exports.getProductionSingleDayUserTemp = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    // const uploadfromdate = `${fromDateonly}T${fromTime}`;
    // const uploadtodate = `${toDateonly}T${toTime}`;
    const uploadfromdate = `${fromDateonly}T${fromTime.split('.000Z')[0]}Z`;
    const uploadtodate = `${toDateonly}T${toTime.split('.000Z')[0]}Z`;
    ...
    productionupload = await ProductionTempUploadAll.find(query, { filenameupdated: 1, formatteddatetime: 1, unitid: 1, user: 1, category: 1, updatedunitrate: 1, flagcount: 1, updatedflag: 1, unitrate: 1, vendor: 1 }).lean();
    ...
    productionuploadManual = await ProducionIndividual.find(query1, { filename: 1, time: 1, fromdate: 1, unitid: 1, mode: 1, user: 1, category: 1, updatedunitrate: 1, updatedflag: 1, flagcount: 1, lateentrystatus: 1, vendor: 1 }).lean();
  } catch (err) {
    console.log(err.message);
  }
  // if (!productionupload) {
...
exports.productionDayCategoryIdFilterTemp = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    if (mode == "Manual") {
      productionupload = await ProducionIndividual.find(queryManual, { fromdate: 1, time: 1, mode: 1, vendor: 1, user: 1, unitid: 1, filename: 1, category: 1, flagcount: 1, lateentrystatus: 1, updatedunitrate: 1, updatedflag: 1 });
    } else {
      productionupload = await ProductionTempUploadAll.find(query, { formatteddatetime: 1, vendor: 1, user: 1, unitid: 1, filenameupdated: 1, category: 1, flagcount: 1, unitrate: 1, updatedunitrate: 1, updatedflag: 1 });
    }

    console.log(productionupload.length, "sdfd");
  } catch (err) {
    console.log(err.message);
  }
  // if (!productionupload) {
...
exports.undoFieldNameBulkTemp = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    console.log(ids.length, "ids count2");
...
exports.getAllProductionTempUploadAllFilenamesonlyBulkDownload = catchAsyncErrors(async (req, res, next) => {
  let productiontempuploadall, matchedData;
  try {
    matchedData = await ProductionTempUploadAll.find({ uniqueid: req.body.id }, { filenamenew: 1 });
    ...
    console.log(productiontempuploadall.length, "hgfgh");
  } catch (err) {
    console.log(err.message);
  }
  if (!productiontempuploadall) {
...
exports.productionMismatchStatusDateFilterTemp = catchAsyncErrors(async (req, res, next) => {
  let mismatchCount = 0;
  ...
  try {
    ...
    prodday = await ProductionDayTemp.countDocuments({ date: fromdate });
...
exports.productionTempFlagUpdateBulkInUpload = catchAsyncErrors(async (req, res, next) => {
  const datas = req.body.toupdateFlagDatas;
  console.log(datas[0], 'dataslenthtempall');
  const BATCH_SIZE = 2000;

  // Simplified reduce to directly return the result
  const uniqueCounts = datas.reduce((acc, { uniqueid }) => {
    if (!acc[uniqueid]) acc[uniqueid] = { uniqueid, count: 0 };
    acc[uniqueid].count++;
    return acc;
  }, {});
  ...
  const processBatches = async () => {
    ...
    for (let i = 0; i < datas.length; i += BATCH_SIZE) {
      const batchIds = datas.slice(i, i + BATCH_SIZE);

      // Extract the uniqueKeys for the batch
      const uniqueKeys = batchIds.map(({ uniqueKey }) => uniqueKey);

      // Add a deleteMany operation for the batch
      promises.push(
        ProductionTempUploadAll.deleteMany({
          fromdate: req.body.fromdate,
          checkunique: { $in: uniqueKeys },
        })
      );
    }
  ...
  const overallCountUpdate = await ProductionTemp.find({ fromdate: req.body.fromdate }, { overallcount: 1, uniqueid: 1 });
  // console.log(overallCountUpdate,'overallCountUpdate')
  const bulkOverallCountUpdate = overallCountUpdate
    .map((item) => {
      const calculatedCount = result.find((d) => d.uniqueid == item.uniqueid);
      //  ? (result.find(d => d.uniqueid === item.uniqueid).count - Number(item.overallcount))
      if (calculatedCount) {
        return {
          uniqueid: item.uniqueid,
          overallcount: String(Math.abs(calculatedCount.count - Number(item.overallcount))),
        };
      }
    })
    .filter((d) => d != null);
  // console.log(bulkOverallCountUpdate,'bulkOverallCountUpdate')
  if (bulkOverallCountUpdate.length > 0) {
    const bulkOps = bulkOverallCountUpdate.map((update) => ({
      updateOne: {
        filter: { uniqueid: update.uniqueid },
        update: {
          $set: {
    ...
    // Perform bulk write operation
    try{
    await ProductionTemp.bulkWrite(bulkOps);
    }catch(err){
      console.log(err)
    }
  }
...
exports.getSingleDateDataforprodDayTemp = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    let users1 = usersAll.map((item) => {
      ...
      // if (loginallot.length > 0) {
      //   const filteredLoginData = loginallot
      //     .map((user) => {
      //       // Filter logs up to the given date
      //       const filteredLogs = user.loginallotlog.filter((log) => {
      //         return new Date(log.date) <= new Date(date);
      //       });

      //       // Group logs by date and pick the most recent one
      //       const logsByDate = filteredLogs.reduce((acc, log) => {
      //         if (!acc[log.date] || acc[log.date].time < log.time) {
      //           acc[log.date] = log; // Keep the most recent log for this date
      //         }
      //         return acc;
      //       }, {});

      //       // Convert grouped logs back to an array
      //       const recentLogs = Object.values(logsByDate);

      //       return recentLogs.length > 0 ? { projectvendor: user.projectvendor, loginallotlog: recentLogs } : null;
      //     })
      //     .filter((user) => user !== null)
      //     .flatMap((item) => item.loginallotlog.map((d) => ({ ...d, projectvendor: item.projectvendor })));

      //   userids = filteredLoginData.filter((d) => d.empname === item.companyname && new Date(d.date) <= new Date(date)).map((d) => ({ userid: d.userid, projectvendor: d.projectvendor }));
      // }
    ...
    const allData = [];
    // console.log(users, 'userlenghPRODAYS');
  } catch (err) {
...
exports.getSingleDateDataforprodDayTempAllBatch = catchAsyncErrors(async (req, res, next) => {
  let productionupload, depMonthSet;
  let finaluser = [];
