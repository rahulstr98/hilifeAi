...
const ProducionIndividual = require('../../../model/modules/production/productionindividual');// get All ClientUserID Name => /api/clientuserids
...
exports.getAllDayPointsUploadLimitedTemp = catchAsyncErrors(async (req, res, next) => {
  let daypointsupload;
  try {
    daypointsupload = await DayPointsUploadTemp.find({}, { filename: 1, date: 1, type: 1, addedby:1 }).sort({ date: -1 });
  } catch (err) {
...
exports.getEmployeeProductionLastThreeMonthsTemp = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    let productionDayid = await ProductionDayTemp.find({ date: { $gte: currMonthFromdate, $lte: currMonthTodate } }, { uniqueid: 1, _id: 0 });
    let LastProductionDayDate = await ProductionDayTemp.findOne({}, { date: 1, _id: 0 }).sort({ date: -1 });
    ...
    const [productionsAll, productionsManualAll, productiondaylistsAll, daypointsupload] = await Promise.all([
      ProductionTempUploadAll.find(prodquery, { filename: 1, dateval: 1, user: 1 }).lean(),
      ProducionIndividual.find(prodqueryManual, { filename: 1, fromdate: 1, time: 1, user: 1, mode: 1 }).lean(),
      ProductionDayListTemp.find({ uniqueid: { $in: productionUniqIds }, empname: empname }, {}),
      DayPointsUploadTemp.find({ date: { $gte: result.fromdate, $lte: result.todate } }, { uploaddata: 1, date: 1 }),
    ]);
    productiondaylists = productiondaylistsAll;
    productions = productionsAll;
    productionsManual = productionsManualAll;
	console.log(productionUniqIds,empname,'productiondaylistsAll')
    return res.status(200).json({
      // count: products.length,
      daypointsupload,
      finalFromToDate,
...
exports.getEmployeeProductionDayListLastThreeMonthsTemp = catchAsyncErrors(async (req, res, next) => {
  let daypointsupload = [],
    productiondaylists,
    productions,
    productionsManual;
  try {
    ...
    const [
      ...
      // ProducionIndividual.find(prodqueryManual, { filename: 1, fromdate: 1, time: 1, user: 1, mode: 1 }).lean(),
      ProductionDayListTemp.find({ uniqueid: { $in: productionUniqIds }, empname: empname }, {}),
      DayPointsUploadTemp.aggregate(pipeline),
    ]);
    productiondaylists = productiondaylistsAll;
    // productions = productionsAll;
    // productionsManual = productionsManualAll;
...
exports.getCheckDaypointIsCreatedTemp = catchAsyncErrors(async (req, res, next) => {
  let count;
  try {
    count = await DayPointsUploadTemp.countDocuments({ date: req.body.date });
    let count1 = await ProductionDayTemp.countDocuments({ date: req.body.date, filestatus:"Final" });
...
exports.productionDayPointTempLast = catchAsyncErrors(async (req, res, next) => {
  let productiontemp;
  try {
    ...
    productiontemp = await DayPointsUploadTemp.countDocuments({ date: newDateOneMinus });
  } catch (err) {
    return next(new ErrorHandler('Data not found!', 404));
  }
  return res.status(200).json({
...
exports.dayPointTempDeleteByDate = catchAsyncErrors(async (req, res, next) => {
  // Delete documents with matching unid
  let result = await DayPointsUploadTemp.deleteMany({ date: req.body.date });
...
exports.getAllProductionsByUserForCurrMonthViewTemp = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    if (mode === 'Production') {
      productiondaylists = await ProductionTempUploadAll.find(
        { dateobjformatdate: { $gte: uploadfromdate, $lte: uploadtodate }, filenameupdated: filename, user: user },
        {
          project: 1,
          category: 1,
          filenameupdated: 1,
    ...
    if (mode === 'Manual') {
      productiondaylistsManual = await ProducionIndividual.find(
        { fromdate: { $gte: fromDateonly, $lte: toDateonly }, filenameupdated: filename, user: user, status: 'Approved' },
        {
          project: 1,
          category: 1,
          filename: 1,
