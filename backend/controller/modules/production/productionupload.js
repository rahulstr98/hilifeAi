...
exports.productionUploadOverAllFetchLimitedNew = catchAsyncErrors(async (req, res, next) => {
  ...
  console.log(date,'date')
   let dateoneafter = new Date(date);
    ...
    const query = {
    	formatteddate:{$in:[date,newDateOneMinus,newDateOnePlus,newDateOneMinusTwo,newDateOneMinusThree ]},
...
exports.getAllProductionUploadFilterUsers = catchAsyncErrors(async (req, res, next) => {
...
// /unallot
exports.getAllProductionUnAllotFilter2old = catchAsyncErrors(async (req, res, next) => {
...
exports.getAllProductionUnAllotFilter = catchAsyncErrors(async (req, res, next) => {
  let productionupload1 = [], mergedData, mergedDataall, producionIndividual;
  ...
  try {
    const [producionIndividual, productionupload] = await Promise.all([
      ProducionIndividual.find(
        ...
        { vendor: 1, category: 1, mode: 1, filename: 1, section: 1, flagcount: 1,unallotcategory:1, unallotsubcategory:1, unitrate: 1, fromdate: 1,unallothide: 1, }
      ),
    ...
    productionupload = productionunmatched.reduce((acc, obj) => {
      ...
      if (
        (Number(finalunitrate) != Number(mratevalcheck) ||
          (Number(mflagalcheck) > 1 && Number(mflagalcheck) != Number(finalflagcount) && mismatchmodeval.includes("Flag Mismatched")) ||
...
exports.bulkProductionOrgUpdateCategorySubcategory = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    let prodManual = updates.filter((item) => item.mode === "Manual");
    let prodUploads = updates.filter((item) => item.mode === "Production");
    console.log(prodManual,'prodManual')
    ...
    if (prodManual.length > 0) {
      ...
      // Perform bulk write operation
     await ProducionIndividual.bulkWrite(bulkOps);
      console.log(await ProducionIndividual.bulkWrite(bulkOps))
    }
    if (prodUploads.length > 0) {
      const bulkOps = prodUploads.map((update) => ({
        updateOne: {
...
exports.productionUploadCheckZeroMismatchPresent = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const zeroCount = (producionIndividualZeroCount ? 1 : 0) + (productionUploadZeroCount ? 1 : 0);
...
exports.getSingleDateDataforprodDay = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
      ...
      userShifts?.flatMap((item, index) => {
        ...
        const matchingAssignShiftItem = filteredRowData?.find(item => item?._doc?.adjdate === date.formattedDate && item?._doc?.adjstatus === 'Approved' && item?._doc?.adjustmenttype === 'Assign Shift');
          ...
          const row = {
            ...
            empcode: item.empcode,
            username: item.username,
            shifttiming: getShiftForDateProdDay(date, matchingItem, matchingItemAllot, isWeekOffWithAdjustment, isWeekOffWithManual, filterBoardingLog, isWeekOff, matchingDoubleShiftItem, item.department, depMonthSet, matchingRemovedItem,matchingAssignShiftItem, item.companyname),
            date: date.formattedDate,
            shiftmode: date.shiftMode,
            shiftsts: findShiftTimingsts(getShiftForDateProdDay(date, matchingItem, matchingItemAllot, isWeekOffWithAdjustment, isWeekOffWithManual, filterBoardingLog, isWeekOff, matchingDoubleShiftItem, item.department, depMonthSet, matchingRemovedItem,matchingAssignShiftItem)),
          };
    ...
    const allData = [];
    //  console.log(users.length > 1 ? users.find(item => item.companyname === "ABINAYA.KARUNANITHI").userShiftTimings : [], 'userlenghPRODAYS');
  } catch (err) {
    console.log(err, 'prodday');
    const errorMessage = err.message === "Cannot read properties of undefined (reading 'shifttiming')" ? 'shifttiming' : 'Records not found';
...
// console.log(filteredLogs,'filteredLogs');
      ...
      productionupload = allData
        .map((upload) => {
          ...
          new Date(dateTime) >= new Date(user.userShiftTimings.shiftFromTime) && new Date(dateTime) <= new Date(user.userShiftTimings.shiftEndTime))
...
exports.getAllProductionUploadOriginalSummaryReport = catchAsyncErrors(async (req, res, next) => {
  try {
    const { projectvendor, filename, category, fromdate, todate, page, pageSize } = req.body;
    ...
    console.log(category.length,filename.length,"length")

    const pipeline = [
      // Match Stage: Efficient Filtering with Date and Indexes
      {
        $match: {
      ...
      // Group Stage: Aggregate Required Fields
      {
        $group: {
          _id: {
            filenameupdated: "$filenameupdated",
       ...
       //   uploaddate: {$first: {$concat: ["$fromdate", " to ", "$todate"] }},
      ...
      // Sorting Stage: Order Data by Filename
      { $sort: { filenameupdated: 1 } },

    ];

    // Execute the Aggregation Pipeline with allowDiskUse
...
exports.productionUploadRawdataFilterForNewrate = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const productionupload = await ProductionUpload.aggregate([
