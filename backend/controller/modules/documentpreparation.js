...
exports.getDepartmentDesignationBasedOnDate = catchAsyncErrors(
  async (req, res, next) => {
    let finalresult;
    ...
    try {
      ...
      const userNoticePeriod = await Noticeperiodapply.findOne(
        { empname: usernames, empcode: empcode },
        {
          noticedate: 1,
          empname: 1,
          approvenoticereq: 1,
      ...
      const result = {
        ...
        noticeperiodDays: userDesignation
          ? `${userDesignation?.noticeperiodto} - ${userDesignation?.noticeperiodfrom}`
          : "",
        probationPeriod:
          userDesignation && userDesignation?.isprobation
            ? `${userDesignation?.probationto} - ${userDesignation?.probationfrom}`
            : "",
        userNoticeperiodApplied:
          userNoticePeriod && userNoticePeriod?.noticedate
            ? moment(userNoticePeriod?.noticedate).format("DD-MM-YYYY")
            : "",
        userNoticeperiodRequest:
          userNoticePeriod && userNoticePeriod?.requestdate
            ? moment(userNoticePeriod?.requestdate).format("DD-MM-YYYY")
            : "",
        userNoticeperiodEnd:
          userNoticePeriod && userNoticePeriod?.approvenoticereq
            ? moment(userNoticePeriod?.approvenoticereq).format("DD-MM-YYYY")
            : "",
        noticePeriodStatus:
          userNoticePeriod && userNoticePeriod?.status
            ? userNoticePeriod?.status
            : "",
        usernames: usernames ?? "",
      };
...
exports.getUserslistforEmpDocumentPrintedList = catchAsyncErrors(
  async (req, res, next) => {
    try {
      ...
      if (
        ...
        resonablestatus &&
        resonablestatus !== "Current List" &&
        resonablestatus !== "Notice Period"
      ) {
      ...
      if (
        ["Candidate to Intern", "Visitor to Intern"]?.includes(resonablestatus)
      ) {
        query.workmode = "Internship";
        const DbName =
          resonablestatus === "Candidate to Intern" ? Candidate : Visitors;
      ...
      if (resonablestatus === "Notice Period") {
        userteamgroup = userteamgroup.filter((data) =>
          noticePeriodNames.includes(data.companyname)
        );
      }
...
exports.getUserslistforEmpDocument = catchAsyncErrors(
  async (req, res, next) => {
    try {
      let { resonablestatus } = req.body;
      let userteamgroup = [];
...
exports.getUserListBasedDepartment = catchAsyncErrors(
  async (req, res, next) => {
...
exports.getUserApprovalPendingDocuments = catchAsyncErrors(
  async (req, res, next) => {
    let documentPreparation;
...
exports.getApprovalEmployeesTemplate = catchAsyncErrors(
  async (req, res, next) => {
    let templates;
    const { company, branch } = req?.body;
    try {
      templates = await TemplateCreation.find(
        {
          company: { $in: company },
          branch: { $in: branch },
...
exports.getUserFindProfileImage = catchAsyncErrors(async (req, res, next) => {
  ...
  const { companyname } = req?.body;
  try {
    image = await EmployeeDocuments.findOne(
...
// Approval documents
exports.getEmployeeApprovalFormDatas = catchAsyncErrors(
  async (req, res, next) => {
    upload.single("pdfFile")(req, res, async (err) => {
      ...
      try {
        ...
        if (newDocumentUpload) {
          let documentUser = await EmployeeDocuments.findOne({
            companyname: document?.username,
          });
          let userDetails = await User.findOne(
            { companyname: document?.username },
            { _id: 1, companyname: 1, empcode: 1 }
          );
          if (documentUser) {
            const documentExists = documentUser?.files?.some(
              (file) => file?.documentid === newDocumentUpload?.documentid
            );
            if (!documentExists) {
              const documentUpd = await DocumentPreparation?.findByIdAndUpdate(
                document?.id,
                { approvedfilename: req?.file?.filename }
              );
...
exports.getHierarchyApprovalEmployeesTemplate = catchAsyncErrors(
  async (req, res, next) => {
    let resultArray,
      user,
      result1,
    ...
    try {
      ...
      if (approvalStatus?.includes("notyetsent")) {
        const filteredStatuses = approvalStatus.filter(
      ...
      result = await DocumentPreparation.find(query, {
        date: 1,
        template: 1,
        printoptions: 1,
        manualdate: 1,
          ...
          ? result
              .map((item1) => {
                const matchingItem2 = ans1D.find((item2) =>
                  item2.employeename.includes(item1.person)
                );
      ...
      //overall Teams List
      myallTotalNames = DataAccessMode
        ? uniqueNames
        : [...hierarchyMap, ...branches];
      ...
      resultAccessFilter =
        overallRestrictList?.length > 0
          ? resultAccessFilterHierarchy?.filter((data) =>
              overallRestrictList?.includes(data?.person)
            )
          : [];
      uniqueDataresult = resultedTeam?.filter(
        (item, index, self) =>
          index === self.findIndex((obj) => obj._id === item._id)
      );
      uniqueData = resultAccessFilter?.filter(
        (item, index, self) =>
          index === self.findIndex((obj) => obj._id === item._id)
      );
    } catch (err) {
...
exports.getApprovalEmployeesDocumentsPreparations = catchAsyncErrors(
  async (req, res, next) => {
    let documentPreparation;
    ...
    try {
      ...
      if (approval?.includes("notyetsent")) {
        const filteredStatuses = approval.filter(
          (status) => status !== "notyetsent"
        );
        filterQuery =
          filteredStatuses.length > 0
      ...
      documentPreparation = await DocumentPreparation.find(
        { ...filterQuery, documentneed: "Employee Approval" },
        {
          date: 1,
          template: 1,
          printoptions: 1,
...
exports.getAllVerifytwofaEmployeeApproval = catchAsyncErrors(
  async (req, res, next) => {
    ...
    const user = await User.findOne(
      { companyname: companyname },
      { companyname: 1, loginUserStatus: 1 }
    ).lean();
    if (!user) {
    ...
    let foundData = user?.loginUserStatus?.find(
...
exports.getAllVerifytwofaEmployeeApprovalValidation = catchAsyncErrors(
  async (req, res, next) => {
    try {
...
exports.getAccessibleBranchAllDocumentPreparation = catchAsyncErrors(
  async (req, res, next) => {
...
exports.getAccessibleBranchAllDocumentPreparationOverall = catchAsyncErrors(
  async (req, res, next) => {
    let documentPreparation, overalldocuments;
    try {
      const { assignbranch, printed } = req.body;
...
exports.getAllEmployeeDocumentsPreparationPrintedStatusList = catchAsyncErrors(
  async (req, res, next) => {
    let documentPreparation, overalldocuments;
    try {
      const {
      ...
      const cleaned = Object.fromEntries(
        Object.entries(req.body).filter(
          ([key, value]) =>
            ["template", "status", "employeemode"]?.includes(key) &&
      ...
      // console.log(query, "query")
      documentPreparation = await DocumentPreparation.find(query, {
        date: 1,
        template: 1,
        printoptions: 1,
        manualdate: 1,
...
// User LoginDetails
exports.getFilterdocumentUserLogin = catchAsyncErrors(
  async (req, res, next) => {
    let user;
    try {
...
exports.getLastAutoIdCompanyDocumentPrep = catchAsyncErrors(
  async (req, res, next) => {
    let documentPreparation;
    try {
      documentPreparation = await DocumentPreparation.aggregate([
...
exports.getDocumentPreparationCodes = catchAsyncErrors(
  async (req, res, next) => {
    let documentPreparation;
    try {
...
exports.getCompanyDocumentPreparationCodes = catchAsyncErrors(
  async (req, res, next) => {
    let documentPreparation;
    try {
      const company = await Company.findOne({ name: req.body.company }).lean();
      const branch = await Branch.findOne({ name: req.body.branch }).lean();
      documentPreparation =
        company?.code?.slice(0, 3) + branch?.code?.slice(0, 3);
    } catch (err) {
      return next(new ErrorHandler("Records not found!", 404));
    }
...
exports.getSingleDocumentPreparation = catchAsyncErrors(
  async (req, res, next) => {
...
exports.updateDocumentPreparation = catchAsyncErrors(async (req, res, next) => {
  const id = req.params.id;
  let udocumentPreparation = await DocumentPreparation.findByIdAndUpdate(
    id,
    req.body
  );
