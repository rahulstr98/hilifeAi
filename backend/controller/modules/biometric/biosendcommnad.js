...
const {
  getUserParticularDeviceInfoStatus,
} = require("./biometriconlinestatus");
...
exports.getSendCommand = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (deviceCommand !== undefined && [DeviceName]?.includes(DeviceName)) {
      if (deviceCommand === "4") {
        ...
        if (executionSuccess) {
          ...
          if (completedCommands.length > 1) {
            const idsToDelete = completedCommands
              .slice(1)
              .map((cmd) => cmd._id);
...
exports.getDeviceInfoCommand = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    const existingUser = await Biogetdeviceinfo.findOne({
      cloudIDC: req?.body?.cloudIDC,
    });
...
let templateNo = 0;
exports.getBioDownloadUserTemplate = catchAsyncErrors(
  async (req, res, next) => {
    try {
      const newData = null;
...
exports.getUploadUserTemplateInfo = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    let userTemplateInfo = await Biouploaduserinfo.findOne({
      cloudIDC: userInfo?.cloudIDC,
      biometricUserIDC: userInfo.biometricUserIDC,
    });

    if (userInfo?.staffNameC) {
      userInfo.staffNameC = Buffer.from(userInfo?.staffNameC, "base64")
        .toString("utf-8")
        .replace(/[^A-Za-z]/g, "");
    ...
    const checkOne = await Biouploaduserinfo.findOneAndUpdate(
      {
        ...
        $or: [
          ...
          {
            $and: [{ isFaceEnrolledC: "Yes" }, { downloadedFaceTemplateN: 0 }],
          },
        ],
      },
      { $set: { datastatus: "Sent", updatedAt: new Date() } },
      { addedby: 0, updatedby: 0, _id: 0 }
    ).sort({ createdAt: -1 });
      ...
      if (incrementField) {
        ...
        if (existingUploadUserInfo) {
          ...
          if (templateTypeC === "FINGER") {
            ...
            const newValue =
              existingUploadUserInfo.downloadedFingerTemplateN + incrementValue;
            if (newValue > existingUploadUserInfo.fingerCountN) {
              incrementValue = Math.max(
                0,
                existingUploadUserInfo.fingerCountN -
                  existingUploadUserInfo.downloadedFingerTemplateN
              );
            }
          }
      ...
      const answer = await Biouploaduserinfo.aggregate([
        ...
        // 3️⃣ Flatten device info
        {
          $addFields: {
            ...
            biometriccommonname: {
              $arrayElemAt: ["$deviceDetails.biometriccommonname", 0],
            },
          },
        },

        // 4️⃣ Match status (Matched vs Unmatched)
        {
          $addFields: {
            matchStatus: {
              $cond: [
        ...
        // 5️⃣ Biometric status (independent)
        {
          $addFields: {
            biometricStatus: {
              $cond: [
...
const { unmatched = [] } = answer[0] || {};

const unmatchedAssigned = unmatched.filter(
  u => u.biometricStatus === "Assigned"
);

const unmatchedNotAssigned = unmatched.filter(
  u => u.biometricStatus === "Not-Assigned"
);

const unmatchedDisabled = unmatched.filter(
  u => u.biometricStatus === "Disabled"
);
      ...
      users =
        status === "Assigned"
          ? unmatchedAssigned
          : status === "Not-Assigned"
          ? unmatchedNotAssigned
          : status === "Disable"
          ? unmatchedDisabled
          : [ ...unmatchedAssigned, ...unmatchedNotAssigned, ...unmatchedDisabled];
    } else {
      const answer = await Biouploaduserinfo.aggregate([
        {
        ...
        },
        {
          $group: {
            ...
            notAssigned: {
              $push: {
                $cond: {
                  if: { $eq: ["$statusreport", "Not-Assigned"] },
                  then: "$$ROOT",
                  else: "$$REMOVE",
                },
              },
...
exports.getUserDataIndCheck = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const userData = await User.findOne(
      { company, branch, unit, team, companyname, username },
      { _id: 0, username: 1 }
    );
    if (userData) {
      let userNameDetails = userData?.username;
      // let userNameDetails = "rahulstr" ;
      biometricData = await Biouploaduserinfo.find({
        staffNameC: userNameDetails,
      });
    }
    return res.status(200).json({
      success: true,
...
exports.getUserDetailsEditUnmatchedData = catchAsyncErrors(
  async (req, res, next) => {
    const id = req.params.id;
    try {
      let biometricUnmatchedData = await Biouploaduserinfo.findByIdAndUpdate(
        id,
        req?.body
      );
      return res.status(200).json({
