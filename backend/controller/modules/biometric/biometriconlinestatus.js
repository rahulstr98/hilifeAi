...
exports.getBiometricDevicesOfflineHistory = catchAsyncErrors(
  async (req, res, next) => {
    let biometricdevicehistory;
    let { company, branch, unit, devices, fromdate, todate } = req?.body;
    try {
      const result = await BiometricOfflineHistory.find({
        cloudIDC: { $in: devices },
        date: { $gte: fromdate, $lte: todate },
      }).populate(
        "deviceNameID",
        "company branch unit floor area biometriccommonname"
      );

      return res.status(200).json({
        biometricdevicehistory: result,
      });
    } catch (err) {
      console.log(err, "err");
      return next(new ErrorHandler("Records not found!", 500));
...
exports.getParticularDeviceOnlineStatus = catchAsyncErrors(
  async (req, res, next) => {
    let deviceonlinestatus;
    try {
...
exports.addOnlineStatus = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    const existingUser = await Biometriconlinestatus.findOne({
