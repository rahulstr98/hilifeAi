...
exports.getHeaderFooterFromTemplateControlPanel = catchAsyncErrors(
  async (req, res, next) => {
    let headerfooter = [];
    const { company, branch } = req?.body;
    // console.log(req?.body, "template");
    try {
      headerfooter = await TemplatecontrolpanelModel.find(
        {
          company,
          branch,
        },
        { letterheadcontentfooter: 1, letterheadcontentheader: 1 }
      ).lean();
    } catch (err) {
      return next(new ErrorHandler("Records not found!", 404));
    }
...
exports.getEmployeeAllTemplate = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    templatecreation = await TemplateCreation.find(
      { $or: branchFilter, tempaltemode: "Employee" },
      {}
    ).lean();
  } catch (err) {
    return next(new ErrorHandler("Records not found!", 404));
...
exports.getCandidateFilteredTemplate = catchAsyncErrors(
  async (req, res, next) => {
    let templatecreation;
    try {
      ...
      templatecreation = await TemplateCreation.findOne(
        {
          $or: branchFilter,
          tempaltemode: "Candidate",
          company: company,
          branch: branch,
          name: name,
...
exports.getCompanyAllTemplateFilter = catchAsyncErrors(
  async (req, res, next) => {
    let templatecreation;
...
exports.getCandidateTemplateDocuments = catchAsyncErrors(
  async (req, res, next) => {
    let templatecreation;
    const { company, branch } = req?.body;
    try {
...
// Candidate Documents from Template Creation
exports.getCandidateTemplateDocumentsAssignBranch = catchAsyncErrors(
  async (req, res, next) => {
    let templatecreation;
    try {
      const { assignbranch } = req.body;
      ...
      templatecreation = await TemplateCreation.find(
        { $or: branchFilter, tempaltemode: "Candidate" },
        {}
      ).lean();
    } catch (err) {
      console.log(err, "errr");
...
exports.getOverallEditTemplate = catchAsyncErrors(async (req, res, next) => {
  let templatecreation;
  try {
    templatecreation = await DocumentPreparation.find(
...
exports.getOverallEditTemplatedelete = catchAsyncErrors(
  async (req, res, next) => {
    let result, documnentprep, taskcate, count;
    let id = req.body.id;
    try {
      taskcate = await TemplateCreation.find().lean();
      const answer = taskcate?.filter((data) =>
        id?.includes(data._id?.toString())
      );

      documnentprep = await DocumentPreparation.find().lean();
      const unmatchedSubCate = answer
        .filter((answers) =>
          documnentprep.some((sub) => sub.template === answers.name)
        )
...
exports.updateTemplate = catchAsyncErrors(async (req, res, next) => {
  const id = req.params.id;
  let utemplatecreation = await TemplateCreation.findByIdAndUpdate(
    id,
    req.body
  );
