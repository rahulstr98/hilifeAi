...
exports.getDesignationControl = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    designationgroups = await Designationgroup.find({
...
exports.getAllUserWiseFilter = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
      ...
      if (key !== "headers") {
        ...
        if (value !== "" && value !== "All") {
          if (key === "designationgroup") {
            desigFinal = designation.filter((data) =>
              value.includes(data.group)
            );
    ...
    console.log(
      result?.map((data) => `${data?.resonablestatus}-${data?.companyname}`),
      query,
      "query"
    );
    anwerFinal = desig
      ? result.filter(
          (data) =>
            desig.includes(data.designation) &&
            ![
              "Not Joined",
              "Postponed",
              "Rejected",
...
exports.getHierarchyGroupListInidividual = catchAsyncErrors(
  async (req, res, next) => {
    ...
    try {
      ...
      hirerarchi?.forEach((item1) => {
        ...
        const matchingItem = allUsersData.find((item2) =>
          item1.employeename.includes(item2.companyname)
        );
      ...
      let ans = resulted.forEach((item1) => {
        // Find the corresponding item in the second array
        const matchingItem = alldesignation.find(
          (item2) => item1.designation === item2.name
        );
...
exports.getAllUserWiseFilteredit = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    anwerFinal =
      desig && desig.length > 0
        ? result.filter((data) => desig.includes(data.designation))
        : result;
...
exports.checkHierarchyAddNewEmp = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    console.log(
      ...
      team,
      unit,
      designationGrp.group,
      "company, department, designation, branch, team, unit"
    );
    ...
    const condition1 = hirerarchi.filter((item) => {
      if (
        ...
        item.department === "" &&
        item.access === "all" &&
        (item.designationgroup === designationGrp.group ||
          item.designationgroup === "All")
      ) {
        return item;
      }
    });
...
exports.checkHierarchyEditEmpDetails = catchAsyncErrors(
  async (req, res, next) => {
    let hirerarchi,
      hierarchyupdate,
      resultString,
...
exports.getAllHierarchyTeamAndDesignation = catchAsyncErrors(
  async (req, res, next) => {
    ...
    try {
      ...
      designation =
        type === "Designation" && newname !== "All"
          ? await Designation.find({ name: newname })
          : [];
      hierarchyold = hierarchy.filter((data) =>
        data.employeename?.includes(username)
      );
      hierarchyoldsupervisor = hierarchy.filter((data) =>
        data.supervisorchoose?.includes(username)
      );

      hierarchyfindchange =
        type === "Team"
          ? hierarchy.filter(
              (data) => data.team === newname && data.level === "Primary"
            )
          : type === "Designation"
          ? hierarchy.filter(
              (data) =>
                data?.designationgroup ===
                  (designation?.length > 0 ? designation[0]?.group : "") &&
                data.level === "Primary"
            )
          : [];
    } catch (err) {
...
exports.checkHierarchyEditEmpDetailsDesignation = catchAsyncErrors(
  async (req, res, next) => {
    let hirerarchiall,
      allCondataold,
      allCondata,
    ...
    try {
      ...
      designationGrp = designations.find((data) => designation == data.name);
      olddesignationGrp = designations.find(
        (data) => olddesignation == data.name
      );
      ...
      allCondataolddata = hirerarchiall.filter((item) => {
        if (
          ...
          item.department === "" &&
          item.access === "all" &&
          item.employeename.includes(companyname) &&
          (item.designationgroup === olddesignationGrp.group ||
            item.designationgroup === "All")
        ) {
          return item;
        }
      ...
      let allCondataoldval =
        allCondataold && allCondataold.length > 0 ? allCondataold : [];
      let con1value = condition1 && condition1.length > 0 ? condition1 : [];
      let con2value = condition2 && condition2.length > 0 ? condition2 : [];
      let con3value = condition3 && condition3.length > 0 ? condition3 : [];
      let con4value = condition4 && condition4.length > 0 ? condition4 : [];
...
exports.getNotAssignHierarchyData = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const altered = removedDuplicatesTeam.map((item) => ({
      branch: item.branch,
      teamname: "All",
    ...
    notassignlistteam = overallTeam.flatMap((item) =>
      Priorities.map((priority) => ({
        team: item.teamname,
        branch: item.branch,
        company: branchall?.find((br) => br.name === item.branch)?.company,
        unit: item.unit,
        department: item.department,
        level: priority,
      }))
    ...
    notassignlist = notassignlistteam.flatMap((item) =>
      designationGrp.map((grp) => ({
        team: item.team,
        branch: item.branch,
        ...
        unit: item.unit,
        level: item.level,
        designationgroup: grp.name,
      }))
    );

    overallNotassign = notassignlist.flatMap((item) =>
      controls.map((ctrl) => ({
        team: item.team,
        branch: item.branch,
        company: item.company,
    ...
    filtered = overallNotassign.filter(
      (obj1) =>
        !removedDuplicates.some(
          (obj2) =>
            `${obj2.designationgroup}-${obj2.department}-${obj2.company}-${obj2.branch}-${obj2.unit}-${obj2.team}-${obj2.level}` ===
            `${obj1.designationgroup}-${
              obj2.department === "" ? "" : obj1.department
            }-${obj1.company}-${obj2.branch === "" ? "" : obj1.branch}-${
              obj2.unit === "" ? "" : obj1.unit
            }-${obj2.team === "" ? "" : obj1.team}-${obj1.level}`
        )
    );
  } catch (err) {
    return next(new ErrorHandler("Records not found!", 404));
...
exports.getAllHierarchyListSalary = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    if (
      ...
      resulted = result.filter((data) =>
        hierarchyMap.includes(data.companyname)
      );
    }
    ...
    if (
      ...
      resultedTeam = result.filter((data) =>
    ...
    if (
        ...
        const filteredData = hierarchySecond.filter((item) =>
          item.supervisorchoose.some(
            (supervisor) =>
              currentSupervisors.includes(supervisor) &&
              (req.body.sector == "all"
                ? ["Primary", "Secondary", "Tertiary"].includes(item.level)
                : req.body.sector == item.level) &&
              !processedSupervisors.has(supervisor)
          )
        );
      ...
      filteredoverall = result.filter((data) =>
        answerDeoverall.includes(data.companyname)
      );
    }
    ...
    let finalsupervisor = DataAccessMode
      ? uniqueNames
      : req.body.hierachy == "myhierarchy"
    ...
    let overallRestrictList = DataAccessMode
      ? restrictListTeam
      : req.body.hierachy === "myhierarchy"
      ? restrictList
      : req.body.hierachy === "allhierarchy"
    ...
    const resultAccessFilterHierarchy = DataAccessMode
      ? finalResultTask
      : req.body.hierachy === "myhierarchy"
      ? resulted
      : req.body.hierachy === "allhierarchy"
    ...
    resultAccessFilter =
      overallRestrictList?.length > 0
        ? resultAccessFilterHierarchy?.filter((data) =>
            overallRestrictList?.includes(data?.companyname)
          )
        : [];
  } catch (err) {
    // console.log(err, "err");
...
function getAllSupervisors(hierarchy, criteria) {
  ...
  let matches = hierarchy.filter(
    (item) =>
      ...
      (item.unit === unit || item.unit === "All") &&
      (item.team === team || item.team === "All") &&
      (!item.department || // if department not present in row
        item.department === "" || // or empty string
        item.department === department || // or exact match
        item.department === "All") // allow All
  );

  // Step 2 → Scoring (higher score for more exact matches)
  const score = (item) => {
    let s = 0;
    if (item.company === company) s++;
    if (item.branch === branch) s++;
    if (item.unit === unit) s++;
    if (item.team === team) s++;
    if (item.department && item.department === department) s++;
    return s;
  };

  matches.sort((a, b) => score(b) - score(a));

  // Step 3 → Return supervisor list
  return matches.flatMap((m) => m.supervisorchoose);
}

exports.getAllHierarchyReportingTo = catchAsyncErrors(
  async (req, res, next) => {
    let result;
    try {
      ...
      // Base query (applies to both cases)
      let query = {
        resonablestatus: activeStatusFilter,
      };

      // If companyNamesResult exists → use company filter
      if (companyNamesResult?.length > 0) {
        query.companyname = { $in: companyNamesResult };
      } else {
        // Otherwise allow only Manager + Super Admin
        query.role = { $in: ["Manager", "Super Admin"] };
      }
...
exports.getAllUserReportingToChange = catchAsyncErrors(
  async (req, res, next) => {
    ...
    try {
      ...
      result = await User.find(
        { reportingto: oldname },
        { _id: 1, companyname: 1, username: 1 }
      ).lean();
...
exports.getUserBasedHierarchyRestriction = catchAsyncErrors(
  async (req, res, next) => {
    let result;
    try {
...
exports.getAllHierarchyDesignationLogRelations = catchAsyncErrors(
  async (req, res, next) => {
    let newdata, olddata, olddataEmp;
    let { designation, desiggroup, user } = req.body;
    try {
      const CompanyStatus =
        req.body.company === "none" ? user.company : req.body.company;
      const BranchStatus =
        req.body.branch === "none" ? user.branch : req.body.branch;
      ...
      const DepartmentStatus =
        req.body.department === "none" ? user.department : req.body.department;
      console.log(
        user?.boardingLog?.length,
        desiggroup,
        BranchStatus,
        UnitStatus,
        TeamStatus,
...
exports.getAllReportingToDesignationUserHierarchyRelation = catchAsyncErrors(
  async (req, res, next) => {
    let newdata;
    let { designation, desiggroup, user } = req.body;
    try {
...
exports.getAllHierarchyProcessTeamRelations = catchAsyncErrors(
  async (req, res, next) => {
...
exports.getNotAssignHierarchyDataBackend = catchAsyncErrors(
  async (req, res, next) => {
    let teamsdetails,
      desiglog,
      designation,
      ...
      overallNotassign,
      controls,
      combinedResultsArrayDepartment;
    ...
    try {
      ...
      teamsdetails = await Teams.find(filterQuery, {
        company: 1,
        branch: 1,
        unit: 1,
        teamname: 1,
      ...
      desiglog = await DesignationLog.find(
        {},
      ...
      if (!teamsdetails || !desiglog || !designation) {
        return next(
          new ErrorHandler(
            "Teams details, Designation log, or Designation Master not found",
            404
          )
        );
      }
      ...
      // Combine the data
      const combinedResults = teamsdetails
        .map((team) => {
          // Find the designation(s) that match the department of the current team
          const matchingDesignations = desiglog.filter(
            (log) => log.department === team.department
          );
          ...
          return matchingDesignations.map((desig) => {
            ...
            const matchingGroup = designation.find(
              (desigMaster) => desigMaster.name === desig.designation
            );
      ...
      overallNotassign = combinedResults.flatMap((item) => {
        const matchingItems = controls.filter(
          (data) => data.designationgroupname === item.group
        );
      ...
      combinedResultsArray = overallNotassign.filter(
        (doc1) =>
          !ans1.some((doc2) => {
            if (doc2?.department === "All") {
              return (
                doc1?.designationgroup === doc2?.designationgroup &&
                doc1?.control === doc2?.control &&
                doc1?.level === doc2?.level
              );
            } else if (doc2?.department !== "All" && doc2?.department !== "") {
              return (
                doc1?.designationgroup === doc2?.designationgroup &&
                doc1?.department === doc2?.department &&
                doc1?.control === doc2?.control &&
                doc1?.level === doc2?.level
              );
...
exports.getAllReportingToUserHierarchyRelation = catchAsyncErrors(
  async (req, res, next) => {
    let newdata, olddata, supData;
...
exports.getAllHierarchyActionEmployeesList = catchAsyncErrors(
  async (req, res, next) => {
    const { user, oldname } = req.body;

    try {
      // 1) Delete where supervisor contains user AND employee contains oldname
      const condition1 = await Hirerarchi.deleteMany({
        supervisorchoose: { $in: [user] },
        employeename: { $in: [oldname] },
      });

      // 2) Delete where supervisor does NOT contain user AND employee contains oldname
      const condition2 = await Hirerarchi.deleteMany({
        supervisorchoose: { $nin: [user] },
        employeename: { $in: [oldname] },
      });

      // 3) Delete where supervisor contains oldname AND employee contains user
      const condition3 = await Hirerarchi.deleteMany({
        supervisorchoose: { $in: [oldname] },
        employeename: { $in: [user] },
      });

      // 4) Update supervisor from oldname → user for all remaining
      // Step 1: find rows to update
      // Step 1: Find rows where old supervisor needs to be replaced
      const rowsToUpdate = await Hirerarchi.find({
        supervisorchoose: oldname, // ["AROKIYAM"]
        employeename: { $ne: [user] }, // not ["JOHN"]
      }).lean();

      for (const row of rowsToUpdate) {
        // Step 2: Replace old supervisor with new supervisor
        const updated = await Hirerarchi.findOneAndUpdate(
          { _id: row._id },
          { $set: { supervisorchoose: [user] } }, // since it's a single-item array
          { new: true }
        );

        // Step 3: Check if another row already exists with EXACT same:
        // company, branch, unit, team, employeename[0], supervisorchoose[0]
        const duplicate = await Hirerarchi.findOne({
          _id: { $ne: updated._id }, // exclude updated row
          company: updated.company,
          branch: updated.branch,
          unit: updated.unit,
          team: updated.team,
          employeename: updated.employeename, // array match
          supervisorchoose: updated.supervisorchoose,
        }).lean();

        // Step 4: Delete duplicate if exists
        if (duplicate) {
          await Hirerarchi.deleteOne({ _id: duplicate._id });
...
exports.getAllHierarchyBasedEmployeeFind = catchAsyncErrors(
  async (req, res, next) => {
    let hierarchydata;
